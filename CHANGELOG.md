...
- On Windows, a program committed to a repository can no longer run in place of one Cline launches. Windows resolves a program named without a path by looking in the working directory before `PATH`, and Cline uses the open repository as that working directory, so an `rg.exe`, `git.exe`, or `powershell.exe` checked into a repository ran as you the moment the workspace opened, before you approved anything. Cline now turns that working-directory step off for its own processes and for the processes they start. If you use cmd.exe as your terminal profile, commands Cline runs there now need `.\` to launch a program from the current folder, which is how PowerShell has always behaved.
