...
- `Room`, `Participant` (kind `human | agent`), `HumanRole`, `AgentRole`, and `StageState` types exist and are parseable at boundaries (zod or the repo's existing schema pattern). Room / conversation shapes leave room for `addressSet` on sends and stage `sharer: human | agent` (filled by DRV-ADDRESS / DRV-SHARE without a schema rewrite).
...
- [ ] Define `Room`, `Participant`, roles, and `StageState` types with schema parsing.
...
- [ ] Define the `DriveEvent` discriminated union with version field and the five track families.
  - Owner package: `@cline/shared`
...
- [ ] Wire exports into both package indexes and rebuild.
  - Owner package: `@cline/shared`
...
- [ ] Add a privacy assertion test that no event schema accepts an `audio` or raw-transcript payload field.
  - Owner package: `@cline/shared`
