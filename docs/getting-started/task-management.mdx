...
-   **Protection**: Favorited tasks are protected from bulk deletion operations
-   **Filtering**: Use the favorites filter to quickly access your important tasks

### Implementation Note

Task favorites use optimistic UI updates, meaning the interface reflects changes immediately while the actual state update happens in the background. This provides a more responsive experience as you don't need to wait for server operations to complete before seeing the change.
...
-   **Delete Selected**: Remove all selected tasks (favorites remain protected)
-   **Delete All**: Remove all non-favorited tasks from history

## Task Storage

Tasks are stored in VS Code's globalStorage and typically include:
