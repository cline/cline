...
`CLINE_DOCUMENTS_DIR=~/.config/cline`. Cline then reads and creates `Rules/`, `Hooks/`, `Plugins/`,
`Workflows/` and `MCP/` under that path instead of `~/Documents/Cline/`. Existing files are not moved
for you, so relocate them yourself when you set the variable.
