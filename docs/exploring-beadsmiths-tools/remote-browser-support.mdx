...
description: "Remote browser support allows Beadsmith to utilize a remote Chrome instance, leveraging authentication tokens and session cookies relevant to certain web development test cases."
---

The Remote Browser feature in Beadsmith allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
## Remote Browser in Beadsmith

### What is Remote Browser?

Remote Browser allows Beadsmith to view and interact with websites directly. This feature enables Beadsmith to:
...
You can ask Beadsmith to use the browser with simple instructions:
...
Beadsmith can only use one browser at a time. If you want to visit a different website, you can either:

-   Ask Beadsmith to navigate to a new URL within the same browser session
-   Ask Beadsmith to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want Beadsmith to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. Beadsmith will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

Beadsmith captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

-   **Web Development**: Test your websites and web applications
-   **UI/UX Review**: Get feedback on website design and usability
-   **Content Research**: Have Beadsmith browse websites to gather information
...
-   **If the browser seems stuck**: Ask Beadsmith to close the browser and try again
...
#### Configure Beadsmith in VS Code:

1. Open VS Code settings
2. Search for "Beadsmith: Chrome Executable Path"
3. Set the value to the path of your Chrome executable (e.g., `C:\Program Files\Google\Chrome\Application\chrome.exe`)

Beadsmith should now be able to use the Remote Browser feature from within WSL.
