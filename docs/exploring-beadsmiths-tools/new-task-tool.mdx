...
Beadsmith includes a powerful internal tool, `new_task`, designed to help manage workflow continuity and context preservation, especially during complex or long-running tasks. This tool, combined with Beadsmith's awareness of its own context window usage and the flexibility of `.beadsmithrules`, enables sophisticated strategies for breaking down work and ensuring seamless transitions between task sessions.
...
1. **The `new_task` Tool:**
    - **Function:** Allows Beadsmith, upon user approval, to end the current task session and immediately start a new one.
    - **Context Preloading:** Crucially, Beadsmith can **preload** this new task session with specific context provided within the tool's `<context>` block. This context can be anything Beadsmith or a `.beadsmithrules` file defines – summaries, code snippets, next steps, project state, etc.
2. **Context Window Awareness:**
    - **Tracking:** Beadsmith internally tracks the percentage of its available context window currently being used during a task.
    - **Visibility:** This information is visible in the `environment_details` provided to Beadsmith in its prompt.

#### Using the `/newtask` Slash Command

As a quick alternative to Beadsmith suggesting the `newtask` tool or defining complex rules, you can directly initiate the process using a Slash Command.

-   **How:** Simply type `/newtask` in the chat input field.
-   **Action:** Beadsmith will propose creating a new task, typically suggesting context based on the current session (similar to its default behavior when using the tool). You will still get the `ask_followup_question` prompt to confirm and potentially modify the context before the new task is created.
-   **Benefit:** Provides a fast, user-initiated way to leverage the `new_task` functionality for branching explorations or managing long sessions without waiting for Beadsmith to suggest it.
...
#### Default Behavior (Without `.beadsmithrules`)

By default, without specific `.beadsmithrules` dictating its behavior:

-   **Tool Availability:** The `new_task` tool exists, and Beadsmith _can_ choose to use it.
-   **Context Awareness:** Beadsmith _is_ aware of its context usage percentage.
-   **No Automatic Trigger:** Beadsmith **will not** automatically initiate a task handoff _solely_ based on context usage reaching a specific percentage (like 50%). The decision to suggest using `new_task` comes from the AI model's reasoning based on the overall task progress and prompt instructions.
-   **Basic Context Preloading:** If `new_task` is used without specific rules defining the `<context>` block structure, Beadsmith will attempt to preload relevant information based on its current understanding (e.g., a basic summary of progress and next steps), but this may be less comprehensive than a rule-driven approach.

#### The Power of `.beadsmithrules`: Enabling Custom Workflows

While the core capabilities exist by default, the true power, automation, and customization emerge when you combine `new_task` and context awareness with custom workflows defined in `.beadsmithrules`. This allows you to precisely control _when_ and _how_ Beadsmith manages context and task continuity.

Key benefits of using `.beadsmithrules` with `new_task`:
...
-   **Intelligent Breakpoints:** Instruct Beadsmith via rules to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.beadsmithrules` to have Beadsmith automatically create a new task via `new_task` upon completing each subtask, preloading the context for the _next_ subtask.
-   **Custom Context Packaging:** Mandate the exact structure and content of the `<context>` block in `.beadsmithrules` for highly detailed and consistent handoffs (see example below).
...
A common workflow, **driven by specific `.beadsmithrules` like the example below**, involves these steps:

1. **Trigger Identification (Rule-Based):** Beadsmith monitors for handoff points defined in the rules (e.g., context usage > 50%, task completion).
2. **User Confirmation:** Beadsmith uses `ask_followup_question` to propose creating a new task, often showing the intended context defined by the rules.
...
3. **User Control:** You can approve, deny, or ask Beadsmith to modify the context before the new task is created.
4. **Context Packaging (`new_task` Tool):** If approved, Beadsmith uses `new_task`, packaging the context according to the structure mandated by the `.beadsmithrules`.
...
The effectiveness of rule-driven handoffs depends heavily on how `.beadsmithrules` define the `<context>` block. A comprehensive structure often includes:
...
The flexibility of `new_task` combined with `.beadsmithrules` opens up many possibilities:
...
-   **Intelligent Breakpoints:** Instruct Beadsmith to find logical stopping points (e.g., after completing a function or test) _after_ a context threshold is passed, ensuring cleaner handoffs.
-   **Structured Task Decomposition:** Use Plan Mode to define subtasks, then use `.beadsmithrules` to have Beadsmith automatically create a new task via `new_task` upon completing each subtask.
...
Experimenting with `.beadsmithrules` is encouraged to discover workflows that best suit your needs!

#### Example `.beadsmithrules`: Task Handoff Strategy Guide

Below is an example `.beadsmithrules` file focused specifically on using `new_task` for context window management. **Remember, this is just one specific strategy; the core `new_task` tool can be used differently with other custom rules.**
...
-   **User Approval:** You always have control, approving the handoff and having the chance to modify the context Beadsmith proposes to carry forward.
-   **Flexibility:** The core `new_task` tool is a flexible building block. Experiment with `.beadsmithrules` to create workflows that best suit your needs, whether for strict context management, task decomposition, or other creative uses.
```
````
