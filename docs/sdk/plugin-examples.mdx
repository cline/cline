...
| [`clineignore-read-files-guard.ts`](https://github.com/cline/cline/blob/main/sdk/examples/plugins/clineignore-read-files-guard.ts) | Runtime hook policy that blocks file reads, edits, and shell access to paths listed in a workspace `.clineignore` file. |
...
To specify the protected files in a dedicated `.clineignore` file instead of keying off `.gitignore`, use `clineignore-read-files-guard.ts`. It applies the same `beforeTool` skip to `read_files`, `editor`, `apply_patch`, and additionally `run_commands` (a conservative token check that catches shell access like `cat .env`), and it works even outside a git repository:
...
See [.clineignore](/customization/clineignore) for the full walkthrough.
