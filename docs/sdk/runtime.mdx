...
- `Agent` / `AgentRuntime` from `@clinebot/agents`: browser-safe, stateless, in-process execution. You provide tools, persistence, and lifecycle management.
- `ClineCore` from `@clinebot/core`: Node runtime with sessions, persistence, built-in tools, approvals, automation, and hub support.
