...
Use the `clite plugin install` command to install plugins from npm, git repositories, or local paths.

```sh
clite plugin install <source> [options]
```

The shorthand `clite plugin i` works as well.

### Install from npm

```sh
clite plugin install --npm @scope/plugin-name
```
...
```sh
clite plugin install ./path/to/plugin.ts     # Single file
clite plugin install /absolute/path/to/dir   # Directory with package.json
```
...
clite plugin install ./my-custom-plugin

# Install an npm package as a plugin
clite plugin install --npm @cline/plugin-sql

# Install from a GitHub repo
clite plugin install --git https://github.com/my-org/my-plugin.git

# Force reinstall an existing plugin
clite plugin install --force --npm @scope/plugin-name
