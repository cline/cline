...
For `Agent` / `AgentRuntime`, define tools as `AgentTool` objects. For core/extensions, use `createTool(...)` from `@cline/shared`, `@cline/agents`, or `@cline/core` when you want the legacy/core `Tool` shape.
