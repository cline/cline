...
Workflows live alongside [Cline Rules](/3.17/features/cline-rules). Creating one is straightforward:
...
-   Leverage Cline's [built-in tools](/3.17/exploring-clines-tools/cline-tools-guide) like `ask_followup_question`, `read_file`, `search_files`, and `new_task`
-   Use command-line tools you already have installed like `gh` or `docker`
-   Reference external [MCP tool calls](/3.17/mcp/mcp-overview) like Slack or Whatsapp
