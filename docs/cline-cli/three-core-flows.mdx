...
Use the `-s` flag to override settings for a specific task. You can specify multiple settings in two ways:

```bash
# Comma-separated settings
cline -s aws-region=us-west-2,mode=act "Deploy the application"

# Multiple -s flags
cline task new -s aws-region=us-west-2 -s mode=act "Deploy the application"
```

Both formats work with `cline "prompt"` and `cline task new`.
