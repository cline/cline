...
```bash
# Phase 1: Quick summarization (cheap model)
gh issue view $(gh issue list -L 1 | awk '{print $1}') \
    | cline -y --config ~/.cline-haiku "summarize this issue in 2-3 sentences"

# Phase 2: Deep planning (expensive model with thinking)
# ... (summary from phase 1 as context)
echo "Issue summary: $SUMMARY" | cline -y --thinking \
    "create a detailed implementation plan with edge cases"

# Phase 3: Execution (mid-tier model)
# ... (plan from phase 2)
cline -y --config ~/.cline-sonnet \
