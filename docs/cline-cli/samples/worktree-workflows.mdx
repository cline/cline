...
Create dedicated instances for long-running worktree sessions:

```bash
# Create instances for each worktree
cline instance new --default  # For worktree-a
INSTANCE_A=$(cline instance list | grep localhost | awk '{print $2}')

cline instance new            # For worktree-b
INSTANCE_B=$(cline instance list | grep localhost | tail -1 | awk '{print $2}')

# Run tasks on specific instances
cline --cwd ~/worktree-a task new -y "long-running task"
cline --address $INSTANCE_B --cwd ~/worktree-b task new -y "another task"
```

This keeps task history and state separate per worktree.
