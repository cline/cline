...
INSTANT TASK OPTIONS
       ...
       --config <path>
              Path to Cline configuration directory.  Allows switching between
              different provider configurations without changing global
              settings.  Useful for multi-model workflows where different tasks
              use different models.  Example: cline --config ~/.cline-haiku
              "summarize this quickly"

       --thinking
              Enable extended thinking with 1024 token budget.  Activates deep
              reasoning mode for complex planning tasks.  Best used when Cline
              needs to analyze multiple approaches before implementation.
              Example: cline --thinking "create a detailed implementation plan"

       -c, --cwd <path>
              Working directory for the task.  Run Cline in a specific
              directory without changing your current location.  Essential for
              worktree workflows where multiple Cline instances operate in
              different folders simultaneously.  Example: cline --cwd
              ~/worktree-a -y "refactor authentication code"
