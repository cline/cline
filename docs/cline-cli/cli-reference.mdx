...
INSTANT TASK OPTIONS
       ...
       -s, --setting setting value
              Override a setting for this task.  Multiple settings can be
              specified in two ways:

              • Comma-separated: -s aws-region=us-west-2,mode=act

              • Multiple flags: -s aws-region=us-west-2 -s mode=act
...
COMMANDS
   ...
   Task Management
       ...
       cline t n prompt [options]
              ...
              -s, --setting setting value
                     Set task-specific settings.  Supports multiple settings
                     via comma-separated values (-s key1=val1,key2=val2) or
                     multiple flags (-s key1=val1 -s key2=val2).
