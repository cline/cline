...
```bash
--address string         Cline Core gRPC address (default "127.0.0.1:50052")
...
cline task new "Create a feature" --address 127.0.0.1:51052
```

### File and Image Attachments

Attach files and images to your tasks using the `-f` and `-i` flags:
...
# Attach images
cline task new "Fix this UI bug" -i screenshot.png

# Combine files and images
cline task new "Update docs" -f README.md -i diagram.png
```

### Mode Selection

Set the initial mode for your task (plan or act):

```bash
# Start in plan mode (default)
cline task new "Build a feature"

# Start in act mode
cline task new "Build a feature" --mode act

# Explicitly specify plan mode
cline task new "Design architecture" -m plan
```

### Task Settings

Pass custom settings to your task using key=value format:

```bash
# Single setting
cline task new "Deploy app" -s aws-region=us-west-2

# Multiple settings
cline task new "Run tests" -s aws-region=us-west-2 --s mode=act
```

### Workspace Directories

Specify workspace directories for your task:

```bash
# Single directory
cline task new "Build project" -w /path/to/project

# Multiple directories
cline task new "Refactor code" -w /path/to/frontend -w /path/to/backend
```

### YOLO Mode

Enable YOLO (non-interactive) mode to auto-approve all actions:

```bash
# Enable YOLO mode
cline task new "Quick fix" --yolo

# Short form
cline task new "Quick fix" -y

# Combine with other flags
cline task new "Deploy" -y -m act -w /path/to/project
```

### Output Formats

The CLI supports three formats for task output:
...
cline task follow -o json

# Plain text format
cline task follow -o plain
```
...
# Output shows:
# - ADDRESS: Instance address (e.g., 127.0.0.1:51051)
...
cline instance use 127.0.0.1:51052
```
...
cline instance kill 127.0.0.1:51052
...
### Logs Command Overview

```
List and manage log files created by Cline instances.
...
Available Commands:
  ...
  path        Print the logs directory path
...
**Command Help:**
```
List all log files created by Cline instances with details about their size, 
creation time, and associated instance.

Usage:
  cline logs list [flags]
...
**Command Help:**
```
Delete old log files to free up disk space. By default, removes log files 
older than 7 days.

Usage:
  cline logs clean [flags]
...
**Command Help:**
```
Print the directory path where Cline stores log files. Useful for manual 
inspection or scripting.

Usage:
  cline logs path [flags]
...
# Open logs directory in file browser (macOS)
open "$(cline logs path)"

# Navigate to logs directory
cd "$(cline logs path)"

# List log files with system tools
ls -lh "$(cline logs path)"
```
...
**Command Help:**
```
Create a new Cline task with the specified prompt. If no Cline instance exists at the specified address, a new one will be started automatically.
...
cline task new "Task for specific instance" --address 127.0.0.1:51052
...
# Specify instance
cline task send "Message" --address 127.0.0.1:51052
```
...
cline task follow --address 127.0.0.1:51052
```
...
# Specify instance
cline task view --address 127.0.0.1:51052
```
...
cline task cancel --address 127.0.0.1:51052
```
...
cline task list --address 127.0.0.1:51052
```
...
cline task resume abc123 --address 127.0.0.1:51052
```
...
cline task restore 1234567890 --address 127.0.0.1:51052
```
