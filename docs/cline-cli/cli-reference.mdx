...
cline task --help     # Show task-specific options
...
When you run `cline` without arguments, it launches an interactive welcome prompt with a rich terminal UI. You can type your task, view conversation history, and interact with Cline in real-time.
...
Run `cline "prompt"` or `cline task "prompt"` to immediately start a task. If stdin is a TTY, you'll see the interactive UI. If stdin is piped or output is redirected, the CLI automatically switches to plain text mode.
...
Activated automatically when stdin is piped, output is redirected, or `--json`/`--yolo` flags are used. Outputs clean text without the Ink UI, suitable for scripting and CI/CD pipelines.
...
| `-y`, `--yolo` | Enable yolo/yes mode (auto-approve all actions, output in plain mode, exit automatically when complete) |
...
```bash
# Basic task
cline task "Add error handling to utils.ts"

# Plan mode
cline task -p "Design a REST API for user management"

# Auto-approve mode for CI/CD
cline task -y "Run tests and fix any failures"

# With specific model
cline task -m claude-sonnet-4-5-20250929 "Refactor this function"

# Include images
cline task -i screenshot.png "Fix the UI based on this image"

# Verbose output with extended thinking
cline task -v --thinking "Analyze this codebase architecture"
```
...
Display global and workspace state.
...
Launch interactive authentication wizard, or use quick setup flags.
...
# Yolo mode for automated workflows (auto-approves all actions)
...
Override the default configuration directory. When set, Cline stores all data in this directory instead of `~/.cline/data/`.
...
<Columns cols={2}>
  ...
  <Card title="Model Orchestration" icon="brain" href="/cline-cli/samples/model-orchestration">
    Use multiple AI models strategically to optimize costs and quality
  </Card>
  
  <Card title="ACP Editor Integrations" icon="code" href="/cline-cli/acp-editor-integrations">
    Run Cline as an ACP agent in JetBrains, Neovim, Zed, and more
  </Card>
  
  <Card title="Hooks" icon="webhook" href="/features/hooks">
    Inject custom logic into Cline's workflow at key moments
  </Card>
</Columns>
...
**Full documentation:** [https://docs.cline.bot](https://docs.cline.bot)
