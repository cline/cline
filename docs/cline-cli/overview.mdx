...
**Hooks integration**
- Run [hooks](/features/hooks/index) in CLI tasks to validate operations and enforce policies
- Enable hooks per-task with `-s hooks_enabled=true` or globally via `cline config set hooks-enabled=true`
- Combine hooks with headless automation for consistent enforcement across all environments
