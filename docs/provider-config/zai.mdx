...
3.  **Select Entrypoint:** Choose your appropriate entrypoint:
    -   **International Standard** - Standard API for international users (`api.z.ai`)
    -   **China Standard** - Standard API for China mainland users (`open.bigmodel.cn`)
    -   **International Coding Plan** - Coding Plan API for international users (`api.z.ai/coding`)
    -   **China Coding Plan** - Coding Plan API for China mainland users (`open.bigmodel.cn/coding`)
...
1. **Subscribe:** Go to [https://z.ai/subscribe](https://z.ai/subscribe) and choose your plan (or [https://open.bigmodel.cn/](https://open.bigmodel.cn/) for China users).
...
3. **Configure in Cline:**
   - Open Cline settings and select "Z AI" as your provider
   - Select the appropriate **Coding Plan entrypoint**:
     - "International Coding Plan" for international users
     - "China Coding Plan" for China users
   - Paste your API key into the "Z AI API Key" field
...
**Important:** Make sure to select the correct entrypoint type. Coding Plan subscriptions require the Coding Plan entrypoints, while standard API usage requires the Standard entrypoints.
...
### Regional Optimization and Entrypoints

#### API Endpoints
Z AI provides four distinct API entrypoints to serve different regions and subscription types:

- **International Standard:** `https://api.z.ai/api/paas/v4` - Standard pay-as-you-go API for international users
- **China Standard:** `https://open.bigmodel.cn/api/paas/v4` - Standard pay-as-you-go API for China users
- **International Coding Plan:** `https://api.z.ai/api/coding/paas/v4` - For international users with GLM Coding Plan subscription (Lite/Pro)
- **China Coding Plan:** `https://open.bigmodel.cn/api/coding/paas/v4` - For China users with GLM Coding Plan subscription (Lite/Pro)

#### Choosing the Right Entrypoint
1. **Subscription Type:** Choose "Coding Plan" entrypoints if you have subscribed to a GLM Coding Plan (Lite/Pro), or "Standard" entrypoints for pay-as-you-go API usage
2. **Location-based:** Select "International" or "China" based on your geographic location for optimal performance and compliance
3. **Model Availability:** The region (International vs China) determines available models and pricing, automatically filtered for compatibility
...
-   **Entrypoint Selection:** Choose the correct entrypoint based on your subscription type and location. Use "Coding Plan" entrypoints if you have subscribed to a GLM Coding Plan (Lite/Pro), otherwise use "Standard" entrypoints for pay-as-you-go API usage.
-   **Coding Plan Subscriptions:** Coding Plan entrypoints are exclusively for users who have subscribed to GLM Coding Plan (Lite/Pro). Standard API keys will not work with Coding Plan entrypoints.
