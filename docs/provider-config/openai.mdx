---
title: "OpenAI"
description: "Learn how to configure and use official OpenAI models with Beadsmith."
---

Beadsmith supports accessing models directly through the official OpenAI API.
...
3.  **Create a Key:** Click on "Create new secret key". It's good practice to give your key a descriptive name (e.g., "Beadsmith API Key").
...
Beadsmith is compatible with a variety of OpenAI models, including common choices from OpenAI's featured/frontier lists:
...
1.  **Open Beadsmith Settings:** Click the settings gear icon (⚙️) in the Beadsmith panel.
...
-   **Azure OpenAI Service:** If you are looking to use the Azure OpenAI service, please note that specific documentation for Azure OpenAI with Beadsmith may be found separately, or you might need to configure it as an OpenAI-compatible endpoint if such functionality is supported by Beadsmith for custom configurations.
