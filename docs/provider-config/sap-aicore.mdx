...
8.  **Configure Orchestration Mode:** If you have an `extended` service plan, the "Orchestration Mode" checkbox will be automatically appeared.
9.  **Select Model:** Choose your desired model from the "Model" dropdown.

### Orchestration Mode vs Native API

**Orchestration Mode:**
- **Requires Extended Plan:** Only available with SAP AI Core `extended` service plan
- **Simplified usage:** provides access to all available models without requiring individual deployments.

**Native API Mode:**
- **Available on all plans:** Works with any SAP AI Core service plan
- **Manual deployments:** Requires manual deployment and management of each model you want to use

### Tips and Notes

- In case your service plan is `extended`, keep Orchestration Mode enabled unless you have specific requirements that necessitate direct AI Core API access or need features not supported by the orchestration mode.

- In case your Orchestration Mode is disabled:
