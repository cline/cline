...
description: "Learn how to configure Beadsmith with various AI model providers that offer OpenAI-compatible APIs."
---

Beadsmith supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This allows you to use models from providers _other than_ OpenAI, while still utilizing a familiar API interface. This includes providers such as:
...
The key to using an OpenAI-compatible provider with Beadsmith is to configure these main settings:
...
You'll find these settings in the Beadsmith settings panel (click the ⚙️ icon):
...
While the "OpenAI Compatible" provider type allows connecting to various endpoints, if you are connecting directly to the official OpenAI API (or an endpoint that mirrors it exactly), Beadsmith recognizes the following model IDs based on the `openAiNativeModels` definition in its source code:
...
### v0 (Vercel SDK) in Beadsmith:

-   For developers working with v0, their [AI SDK documentation](https://vercel.com/docs/v0/beadsmith) provides valuable insights and examples for integrating various models, many of which are OpenAI-compatible. This can be a helpful resource for understanding how to structure calls and manage configurations when using Beadsmith with services deployed on or integrated with Vercel.

-   v0 can be used in Beadsmith with the OpenAI Compatible provider.
...
By using an OpenAI-compatible provider, you can leverage the flexibility of Beadsmith with a wider array of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
