...
Tuning Engines exposes an OpenAI-compatible inference endpoint that can be used with Cline's **OpenAI Compatible** provider.

1. In Cline settings, select **OpenAI Compatible** as the API provider.
2. Set **Base URL** to `https://api.tuningengines.com/v1`.
3. Paste your Tuning Engines inference key, for example `sk-te-...`.
