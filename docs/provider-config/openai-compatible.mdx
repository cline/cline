...
Tuning Engines uses Cline's **OpenAI Compatible** provider. See [Tuning Engines](/provider-config/tuning-engines) for a dedicated setup walkthrough.

1. In Cline settings, select **OpenAI Compatible** as the API provider.
2. Set **Base URL** to `https://api.tuningengines.com/v1`.
3. Paste your Tuning Engines inference key into the **API Key** field, for example `sk-te-...`.
4. Set **Model ID** to a model alias that is enabled for your Tuning Engines key. You can find available aliases in the [Tuning Engines model catalog](https://app.tuningengines.com/inference/catalog).
