...
[Token Market](https://www.tokensmarket.ai/) provides OpenAI-compatible access to models from multiple providers through a unified API endpoint.

#### Quickstart

1. Create a workspace API key by following the [Token Market API Key guide](https://www.tokensmarket.ai/docs/api-examples/api-key).
2. In Cline settings, select **OpenAI Compatible** as the API Provider.
3. Set **Base URL** to `https://api.tokensmarket.ai/v1`. Do not append `/chat/completions`.
4. Paste your Token Market API key.
5. Enter a currently available model ID from the [Token Market Model Market](https://www.tokensmarket.ai/models).
6. Click **Verify** to test and save the connection.

For provider-specific notes and the current Cline interface, see the [Token Market Cline guide](https://www.tokensmarket.ai/docs/scenarios/cline).
