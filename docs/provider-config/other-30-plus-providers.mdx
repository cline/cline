...
### ai&
AiAnd (ai&) offers sovereign inference for open-weight models served from Japan through an OpenAI-compatible API.

**Website:** [https://aiand.com](https://aiand.com)

#### Basic Setup

1. Create/sign in to the [ai& Console](https://console.aiand.com/).
2. Generate an API key from the dashboard.
3. In Cline, choose **ai&** and paste the key.
...
### Avian
Avian is an OpenAI-compatible inference provider offering fast, affordable LLM access.

**Website:** [https://avian.io](https://avian.io)

#### Basic Setup

1. Sign up or sign in at [avian.io](https://avian.io) and generate an API key.
2. In Cline, choose **OpenAI Compatible** from the API Provider dropdown.
3. Set the base URL to `https://api.avian.io/v1`, paste your API key, and enter a model ID (e.g., `deepseek/deepseek-v3.2`).

See the [Avian models page](https://avian.io/models) for the current model list with pricing and capabilities. Not every model supports every optional feature (e.g., tool calling, reasoning).
...
### CoreWeave
CoreWeave provides serverless inference for open-weight models through an OpenAI-compatible API.

**Documentation:** [CoreWeave Serverless Inference](https://docs.wandb.ai/inference/)

#### Basic Setup

1. Follow the [account and API key prerequisites](https://docs.wandb.ai/inference/prerequisites). The service currently uses a W&B account and API key.
2. In Cline, select **CoreWeave** and paste your API key. Older releases may list the provider as **Weights & Biases** or **W&B**.
3. Choose a model from the [current model catalog](https://docs.wandb.ai/inference/models). Model availability and capabilities vary.

For manual configuration, select **OpenAI Compatible**, set the base URL to `https://api.inference.wandb.ai/v1`, enter your W&B API key, and use an exact model ID from the catalog. See the [Cline integration guide](https://docs.wandb.ai/inference/tutorials/integration-cline) for further setup instructions.
...
### MindsHub
MindsHub is an OpenAI/Anthropic-compatible inference gateway: one API key and one bill reach its whole catalog (Claude, GPT, Gemini, Kimi, DeepSeek, and more) through the OpenAI Chat Completions wire format, with the same catalog also reachable from other tools via OpenAI Responses or Anthropic Messages.

**Website:** [https://mindshub.ai](https://mindshub.ai) · **Docs:** [https://docs.mindshub.ai/inference/](https://docs.mindshub.ai/inference/)

#### Basic Setup

1. Create an account at [console.mindshub.ai](https://console.mindshub.ai) and generate an API key.
2. In Cline, select **MindsHub** and paste the key.
3. Pick a model alias from the dropdown (`sonnet`, `opus`, `kimi`, `deepseek`, `gpt-codex`, etc.) — see the [model catalog](https://docs.mindshub.ai/inference/models) for the full list.
