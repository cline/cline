...
### Yolo-Auto
Yolo-Auto is a flat-rate, OpenAI-compatible gateway aimed at coding agents. It bills a fixed monthly rate instead of per-token charges.

**Website:** [https://yolo-auto.com/](https://yolo-auto.com/)

#### Basic Setup

1. Create a Yolo-Auto account and generate an API key.
2. In Cline, select **Yolo-Auto** and paste the key (or set `YOLO_AUTO_API_KEY`).
3. Choose a model from the model picker.

The model list is fetched live from the Yolo-Auto endpoint (`GET /v1/models`), so it always reflects what your key can call; you do not need to update Cline when new models are added. The stable default model ID is `yolo`. For manual configuration, select **OpenAI Compatible**, set the base URL to `https://yolo-auto.com/v1`, and enter your key.
