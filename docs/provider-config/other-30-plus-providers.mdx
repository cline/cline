...
### Gonka Broker
Gonka Broker is an OpenAI-compatible API for open-source models served on the decentralized Gonka GPU network, with card payment and rates locked at top-up.

**Website:** [https://gonkabroker.com/](https://gonkabroker.com/)

#### Basic Setup

1. Sign up at [app.gonkabroker.com](https://app.gonkabroker.com/) and create an API key.
2. In Cline, choose **OpenAI Compatible** from the API Provider dropdown.
3. Set the base URL to `https://proxy.gonkabroker.com/v1`, paste your API key, and enter a model ID (e.g., `zai-org/GLM-5.3-Flash`).

The lineup changes as the network brings models online; see [gonkabroker.com/models](https://gonkabroker.com/models) or the unauthenticated `/v1/models` endpoint for the current list with pricing.
