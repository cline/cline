...
### xAI (Grok)
xAI provides Grok models through API keys and eligible SuperGrok or X Premium subscriptions.

**Website:** [https://x.ai/](https://x.ai/)

#### Subscription Setup

Subscription OAuth is currently available through Cline CLI and SDK hosts that use the shared Core authentication APIs.

1. Run `cline auth xai-subscription` or choose **xAI Subscription** during CLI setup.
2. Open the displayed verification URL and enter the device code.
3. After sign-in, choose **Grok Build 0.1**.

The OAuth flow uses your subscription entitlement and does not require an xAI API key. Availability and usage limits are determined by xAI and your subscription plan.

#### API Key Setup

1. Sign in to xAI platform.
2. Generate an API key.
3. In Cline, choose **xAI** and paste the key.
