...
description: "Set up AWS Bedrock with Beadsmith using IAM Access Key and Secret Key credentials. Best for enterprise environments with established IAM policies."
---
...
2.  Use the JSON editor to add the following policy document:
    ...
    ```
3.  Name the policy (e.g., `BeadsmithBedrockInvokeAccess`) and attach it to your IAM user or role.
...
By following these steps, your enterprise team can securely integrate AWS Bedrock with the Beadsmith VS Code extension to accelerate development:
...
3. **Configure Beadsmith in VS Code:** Install and set up Beadsmith with your AWS credentials and choose an appropriate model.
