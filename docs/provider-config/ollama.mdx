...
Beadsmith supports running models locally using Ollama. This approach offers privacy, offline access, and potentially reduced costs. It requires some initial setup and a sufficiently powerful computer. Because of the present state of consumer hardware, it's not recommended to use Ollama with Beadsmith as performance will likely be poor for average hardware configurations.
...
3.  **Configure the Model's Context Window:**
    By default, Ollama models often use a context window of 2048 tokens, which can be insufficient for many Beadsmith requests. A minimum of 12,000 tokens is advisable for decent results, with 32,000 tokens being ideal. To adjust this, you'll modify the model's parameters and save it as a new version.
...
4.  **Configure Beadsmith:**
    -   Open the Beadsmith sidebar (usually indicated by the Beadsmith icon).
    ...
    -   (Optional) Configure the Model context size in Beadsmith's Advanced settings. This helps Beadsmith manage its context window effectively with your customized Ollama model.
...
-   **Offline Capability:** After downloading a model, you can use Beadsmith with that model even without an internet connection.
-   **Token Usage Tracking:** Beadsmith tracks token usage for models accessed via Ollama, allowing you to monitor consumption.
