...
New models drop constantly, so this guide focuses on what's working well with Beadsmith right now. We'll keep it updated as the landscape shifts.

<Callout type="tip">
**New to model selection?** Start with [Module 2 of Beadsmith's Learning Path](https://beadsmith.bot/learn) for a comprehensive guide to choosing and configuring models.
</Callout>

## What is an AI Model?

Think of an AI model as the "brain" that powers Beadsmith. When you ask Beadsmith to write code, fix bugs, or refactor your project, it's the model that actually understands your request and generates the response.
...
- **You choose which model Beadsmith uses** like picking between different experts for different tasks
- **Models are accessed via API providers** - companies like Anthropic, OpenAI, and OpenRouter host these models

**Why it matters:** The model you choose directly impacts Beadsmith's capabilities, response quality, speed, and cost. A premium model might handle complex refactoring beautifully but cost more, while a budget model works great for routine tasks at a fraction of the price.

## How to Select a Model in Beadsmith

Follow these 5 simple steps to get Beadsmith up and running with your preferred AI model:

### Step 1: Open Beadsmith Settings

First, you need to access Beadsmith's configuration panel.

**Two ways to open settings:**
- **Quick method**: Click the **gear icon (⚙️)** in the top-right corner of Beadsmith's chat interface
- **Command palette**: Press **Cmd/Ctrl + Shift + P** → type "Beadsmith: Open Settings"

<Frame>
  <img src="https://storage.googleapis.com/beadsmith_public_images/step1-config.png" alt="Beadsmith Settings Panel" />
</Frame>
...
<Frame>
  <img src="https://storage.googleapis.com/beadsmith_public_images/step2-provider.png" alt="Beadsmith Settings Panel" />
</Frame>
...
|----------|----------|-------|
| **Beadsmith** | Easiest setup | No API keys needed, access to multiple models including stealth models |
...
<Info>
**Recommended for beginners:** Start with **Beadsmith** as your provider - no API key management needed, instant access to multiple models, and occasional free inferencing through partner providers.
</Info>
...
#### If you selected **Beadsmith** as your provider:

- **No API key needed!** Simply sign in with your Beadsmith account
- Click the **Sign In** button when prompted
- You'll be redirected to [app.beadsmith.bot](https://app.beadsmith.bot) to authenticate
...
4. **Paste your key** in the **"API Key"** field in Beadsmith settings

5. **Save automatically** - Your key is stored securely in your editor's secrets storage

<Frame>
  <img src="https://storage.googleapis.com/beadsmith_public_images/step3-API.png" alt="Beadsmith API Selection" />
</Frame>
...
<Frame>
  <img src="https://storage.googleapis.com/beadsmith_public_images/step4-model.png" alt="Beadsmith Model Selection" />
</Frame>
...
### Step 5: Start Using Beadsmith

**Congratulations! You're all set up.** Here's how to start coding with Beadsmith:

1. **Type your request** in the Beadsmith chat box
...
Check [OpenRouter's Beadsmith usage stats](https://openrouter.ai/apps?url=https%3A%2F%2Fbeadsmith.bot%2F) to see real usage patterns from the community.
