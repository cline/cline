...
### 🛠️ Tool Calling (GPT-OSS)

- Cline now advertises its built-in tools to LM Studio’s OpenAI-compatible endpoint. Models that support tool/function calling (for example, GPT-OSS) can invoke tools directly.
- Tool calls are streamed as OpenAI-style deltas and automatically transformed into Cline’s tool blocks, so they execute just like with other providers.
