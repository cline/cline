...
description: "Learn about the two primary transport mechanisms for communication between Beadsmith and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Beadsmith and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Beadsmith) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as Beadsmith
...
-   Single-client scenarios (one Beadsmith instance per server)
...
1. The client (Beadsmith) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your Beadsmith instance
...
STDIO servers run locally on the same machine as Beadsmith, which has several important implications:
...
-   **Execution**: Starts and stops with Beadsmith (child process lifecycle)
...
-   Start when needed by Beadsmith
-   Not require network configuration
-   Need to be installed alongside Beadsmith or via a package manager
...
## Configuring Transports in Beadsmith

For detailed information on configuring STDIO and SSE transports in Beadsmith, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).
