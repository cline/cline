...
Utilizing MCP servers will increase your token usage. Beadsmith offers the ability to restrict or disable MCP server functionality as desired.

1. Click the "MCP Servers" icon in the top navigation bar of the Beadsmith extension.
2. Select the "Configure" tab, and then Click the "Advanced MCP Settings" link at the bottom of that pane.
3. Beadsmith will open a new settings window. find `Beadsmith>Mcp:Mode` and make your selection from the dropdown menu.

<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/MCP-settings-edit%20(1).png"
...
<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/MCP-settings-individual.png"
...
Settings for all installed MCP servers are located in the `beadsmith_mcp_settings.json` file:

1. Click the MCP Servers icon at the top navigation bar of the Beadsmith pane.
...
_Example of MCP Server config in Beadsmith (STDIO Transport)_
...
After configuring an MCP server, Beadsmith will automatically detect available tools and resources. To use them:

1. Type your request in Beadsmith's conversation window
2. Beadsmith will identify when an MCP tool can help with your task
