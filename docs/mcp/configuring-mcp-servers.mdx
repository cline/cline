...
- **type**: Transport type (`stdio`, `sse`, or `streamableHttp`).
- **url**: Server endpoint URL (required for `sse` and `streamableHttp`)
...
- **headers**: HTTP headers for authentication (optional for remote transports)
- **alwaysAllow**: Array of tool names to auto-approve (optional)
- **disabled**: Whether the server is disabled (optional, defaults to false)
...
Used for remote servers that require flexible HTTP communication with optional streaming:

- Combines HTTP requests with optional Server-Sent Events streaming
- Supports both direct JSON responses and SSE streaming
- Provides session management and connection resumption
- Includes built-in security options
- Can handle server-initiated notifications and requests

For more in-depth information about how Streamable HTTP transport works, see [MCP Transport Mechanisms](/mcp/mcp-transport-mechanisms).
