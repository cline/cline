...
description: "This protocol is designed to streamline the development process of building MCP servers with Beadsmith."
---

> **Build and share your MCP servers with the world.** Once you've created a great MCP server, submit it to the [Beadsmith MCP Marketplace](https://github.com/beadsmith/mcp-marketplace) to make it discoverable and one-click installable by thousands of developers.

## What Are MCP Servers?

Model Context Protocol (MCP) servers extend AI assistants like Beadsmith by giving them the ability to:
...
The heart of effective MCP server development is following a structured protocol. This protocol is implemented through a `.beadsmithrules` file that lives at the **root** of your MCP working directory (/Users/your-name/Documents/Beadsmith/MCP).

### Using `.beadsmithrules` Files

A `.beadsmithrules` file is a special configuration that Beadsmith reads automatically when working in the directory where it's placed. These files:

-   Configure Beadsmith's behavior and enforce best practices
-   Switch Beadsmith into a specialized MCP development mode
...
Here's the complete MCP Server Development Protocol that should be placed in your `.beadsmithrules` file:
...
When this `.beadsmithrules` file is present in your working directory, Beadsmith will:
...
### 1. Create a `.beadsmithrules` file (IMPORTANT)

First, add a `.beadsmithrules` file to the root of your MCP working directory using the protocol above. This file configures Beadsmith to use the MCP development protocol when working in this folder.

### 2. Start a Chat with a Clear Description

Begin your Beadsmith chat by clearly describing what you want to build. Be specific about:
...
Beadsmith will automatically start in PLAN MODE, guiding you through the planning process:
...
One of the most effective ways to help Beadsmith build your MCP server is to share official API documentation right at the start:
...
Providing comprehensive API details (endpoints, authentication, data structures) significantly improves Beadsmith's ability to implement an effective MCP server.

## Understanding the Two Modes

### PLAN MODE

In this collaborative phase, you work with Beadsmith to design your MCP server:
...
Once planning is complete, Beadsmith helps implement the server:
...
<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/planning-phase.gif"
...
<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/building-mcp-plugin.gif"
...
Resources let your MCP servers expose data to Beadsmith without executing code. They're perfect for providing context like files, API responses, or database records that Beadsmith can reference during conversations.
...
Resources make your MCP servers more context-aware, allowing Beadsmith to access specific information without requiring you to copy/paste. For more information, refer to the [official documentation](https://modelcontextprotocol.io/docs/concepts/resources).
