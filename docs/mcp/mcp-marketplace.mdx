---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance Beadsmith's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance Beadsmith's capabilities. They enable Beadsmith to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In Beadsmith, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), Beadsmith will guide you through:
...
-   Beadsmith will show confirmation when installation is complete
-   Check the server status in Beadsmith's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Beadsmith will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Beadsmith's system prompt
-   Simply ask Beadsmith to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Beadsmith in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   The server code is cloned/installed to `/Users/<username>/Documents/Beadsmith/MCP/`
...
-   This file is located at: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/beadsmith_mcp_settings.json`
-   Environment variables (like API keys) are securely stored
-   The server path is registered

### 3. Server Launch

-   Beadsmith detects the configuration change
-   Beadsmith launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with Beadsmith

-   Your server's capabilities are added to Beadsmith's system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   Beadsmith can now use these capabilities when prompted by the user
...
1. Open the MCP settings file: `/Users/<username>/Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/beadsmith_mcp_settings.json`
...
-   Try removing and reinstalling the server (remove from both the `beadsmith_mcp_settings.json` file and the `/Users/<username>/Documents/Beadsmith/MCP/` directory)
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Beadsmith is seeing

## MCP Server Rules

Beadsmith is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.beadsmithrules` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making Beadsmith's tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with Beadsmith.
...
-   **Contextual Tool Selection**: Beadsmith selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", Beadsmith detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.beadsmithrules` file or to your custom instructions to make Beadsmith's MCP server selection more intuitive and context-aware.
