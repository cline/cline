---
title: "MCP Overview"
description: "Learn about Model Context Protocol (MCP) servers, their capabilities, and how Beadsmith can help build and use them. MCP standardizes how applications provide context to LLMs, acting like a USB-C port for AI applications."
---
...
<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/mcp-diagram.png"
...
Beadsmith does not come with any pre-installed MCP servers. You'll need to find and install them separately.
...
-   **Beadsmith Marketplace:** Install one from Beadsmith's [MCP Marketplace](/mcp/mcp-marketplace)
-   **Ask Beadsmith:** You can ask Beadsmith to help you find or create MCP servers
...
## Integration with Beadsmith

Beadsmith simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct Beadsmith in natural language to build an MCP server by describing its functionalities. Beadsmith will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** Beadsmith can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** Beadsmith handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** Beadsmith helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** Beadsmith seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** Beadsmith can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, Beadsmith could use a GitHub server to get data and a Notion server to create a formatted report.
