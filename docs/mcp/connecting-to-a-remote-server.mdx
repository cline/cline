...
description: "The Model Context Protocol (MCP) allows Cline to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers using SSE or Streamable HTTP transports through the MCP Servers interface."
---
...
For Streamable HTTP servers, you can explicitly specify the transport type:
...
- **type**: Transport type (`sse` or `streamableHttp`)
