...
description: "Complete guide to resolving terminal integration issues in Beadsmith"
---

This guide helps you resolve terminal integration issues in Beadsmith. Terminal integration is crucial for Beadsmith to execute commands and read their output, enabling it to understand errors, test results, and command responses.
...
```mermaid
graph TD
    A[Terminal Issue] --> B{Can Beadsmith execute commands?}
    B -->|No| C[Shell Integration Unavailable]
    B -->|Yes| D{Can Beadsmith see the output?}
...
-   Commands execute but Beadsmith can't read output
-   Terminal works fine manually but not with Beadsmith
...
-   Beadsmith states in chat: "[Command is running but producing no output]"
-   Commands complete but Beadsmith doesn't see results
...
-   Recommend AI to use file output instead
    1. Tell Beadsmith in chat or Beadsmith rules, to use `command > output.txt` before reading the file/s

<Tip>
	This family of issues is only partially solved in the latest Beadsmith versions, so if you still face this, create a GitHub issue
...
-   Docker builds never complete in Beadsmith
-   Progress bars consume thousands of tokens
-   The Beadsmith button "Proceed while running" doesn't work properly in chat

<Tip>
	This family of issues has been solved in latest Beadsmith versions but if you still face any issues, then create a GitHub issue
	for this.
</Tip>

## Terminal Settings Explained

Access these in Beadsmith by clicking the settings icon, and navigating to the "Terminal Settings" section:

### Default Terminal Profile

-   **What it does**: Selects which shell Beadsmith uses for commands
...
-   **What it does**: How long Beadsmith waits for the terminal to be ready
-   **Default**: 4 seconds
-   **When to increase**:
...
-   **What it does**: Limits how many lines Beadsmith reads from terminal output
-   **Default**: 500 lines
-   **When to adjust**:
...
3. Select "WSL Bash" as terminal profile in Beadsmith

#### Path Issues

Windows path problems:

1. Use forward slashes in Beadsmith: `C:/Users/...`
...
1. Install Beadsmith on the remote machine, not locally
...
1. Install Beadsmith in the container
...
3. Choose "Trace"
4. Check Output panel → "Beadsmith" for terminal logs
...
### Why does Beadsmith create so many terminals?

When shell integration fails, Beadsmith can't reuse terminals safely (they might be running long processes). Enable shell integration or adjust the terminal reuse setting.

### Can I use my custom shell (nushell, xonsh, etc.)?

Beadsmith officially supports bash, zsh, fish, and PowerShell. Custom shells may work but aren't guaranteed. Use bash as a fallback.
...
Working integration shows command output in Beadsmith's chat. Failed integration shows "Shell Integration Unavailable" or "[Command is running but producing no output]".
...
2. **Report the Issue**:
    - Use `/reportbug` in Beadsmith github issues
