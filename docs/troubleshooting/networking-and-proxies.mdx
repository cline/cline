...
description: "Configure Beadsmith to work behind firewalls and proxies"
---

If you're working behind a corporate proxy or firewall, you'll need to configure
proxy settings for Beadsmith to connect to AI providers. The configuration varies
depending on which version of Beadsmith you're using.
...
is needed for Beadsmith itself.

## CLI

The Beadsmith CLI uses standard HTTP proxy environment variables. Configure these before running `beadsmith` commands.
...
Beadsmith CLI only supports HTTP proxies. It does not support SOCKS proxies,
...
After configuring the proxy, test that Beadsmith can connect to your AI provider:

1. Open the Beadsmith panel
...
Beadsmith in JetBrains only supports HTTP proxies. It does not support SOCKS
proxies, proxy autoconfiguration (PAC) scripts, or HTTP proxies which require
authentication beyond a basic username and password.

Beadsmith does not pick up changed proxy settings dynamically. After changing proxy
settings, restart the IDE for Beadsmith to use the new settings.
...
3. Use curl to verify the certificate works, for example, `curl -x proxy.corp.example:8080 --cacert /path/to/ca-cert.pem -o - -vv https://api.beadsmith.bot/`
4. Consider disabling `http.proxyStrictSSL` in VSCode (not recommended for production)

### Testing Proxy Configuration

If you encounter problems with Beadsmith networking, first verify your proxy
...
Next, check ~/.beadsmith/beadsmith-core-service.log (CLI, JetBrains) for log messages
