...
import path. They are intentionally exposed through the command palette and
extension host command only. Future work should replace the QuickPick-only UI
with a manifest-backed gallery panel that supports preview, search, filters,
trust labels, license/citation display, and import readiness.
