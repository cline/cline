...
description: "Automated GitHub issue analysis using Beadsmith CLI to identify root causes."
---

# GitHub Root Cause Analysis

Automated GitHub issue analysis using Beadsmith CLI. This script uses Beadsmith's autonomous AI capabilities to fetch, analyze, and identify root causes of GitHub issues, outputting clean, parseable results that can be easily integrated into your development workflows.

<Note>
**New to Beadsmith CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.beadsmith.bot/beadsmith-cli/installation) and authenticated with `beadsmith auth`. If you haven't set up Beadsmith CLI yet, please start there first.
</Note>

<Frame>
  <img src="https://storage.googleapis.com/beadsmith_public_images/cli-rca.gif" alt="CLI Root Cause Analysis Demo" width="600" />
</Frame>
...
- **Beadsmith CLI** installed and authenticated ([Installation Guide](https://docs.beadsmith.bot/beadsmith-cli/installation))
- **At least one AI model provider** configured (e.g., OpenRouter, Anthropic, OpenAI)
- **Basic familiarity** with Beadsmith CLI commands
...
```bash
curl -O https://raw.githubusercontent.com/beadsmith/beadsmith/main/src/samples/cli/github-issue-rca/analyze-issue.sh
```
...
### Using Specific Beadsmith Instance

Target a particular Beadsmith instance by address:
...
<Warning>
This is useful when:
- Running multiple Beadsmith instances
- Using a remote Beadsmith server
- Testing with specific configurations
</Warning>

<Note>
The script will automatically handle everything: fetching the issue, analyzing it with Beadsmith, and displaying the results. The analysis typically takes 30-60 seconds depending on the issue complexity.
</Note>
...
- Supports optional Beadsmith instance address
...
```bash
# Ask Beadsmith for his analysis, showing only the summary
beadsmith -y "$PROMPT: $ISSUE_URL" --mode act $ADDRESS -F json | \
    sed -n '/^{/,$p' | \
    jq -r 'select(.say == "completion_result") | .text' | \
    sed 's/\\n/\n/g'
...
This sample demonstrates how to build an autonomous GitHub issue analysis tool using Beadsmith CLI:

1. **Building autonomous CLI tools** using Beadsmith's capabilities
2. **Parsing structured JSON output** from Beadsmith CLI
...
- [CLI Installation Guide](https://docs.beadsmith.bot/beadsmith-cli/installation)
- [CLI Reference Documentation](https://docs.beadsmith.bot/beadsmith-cli/cli-reference)
- [Three Core Flows](https://docs.beadsmith.bot/beadsmith-cli/three-core-flows)
