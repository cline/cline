...
Some first-party plugins ship with the Cline CLI and are available out of the box — no install required. The [goal plugin](https://github.com/cline/plugins/tree/main/plugins/goal) is bundled today, so `/goal` works in the interactive TUI on a fresh install.

Bundled plugins behave like regular plugins with two differences:

- They can't be uninstalled — `cline plugin uninstall` refuses, and the CLI re-seeds them on startup. They appear with a `built-in` label in `cline config` (Plugins tab).
- They can still be disabled: open `cline config`, go to the Plugins tab, and toggle the plugin off. The disabled state persists across CLI updates.

If you previously installed the same plugin yourself (for example `cline plugin install goal`), the CLI keeps your installed copy and skips seeding the bundled one.
...
~/.cline/
  plugins/                     # Global plugins
    _bundled/                  # Bundled plugins seeded by the CLI
