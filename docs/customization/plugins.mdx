...
The `clite plugin install` command installs plugins from three source types:
...
<Tabs>
  <Tab title="Git Repository">
    ...
    ```bash
    clite plugin install https://github.com/owner/repo.git@v1.2.0
    clite plugin install https://github.com/owner/repo.git@main
    ```
  </Tab>
  <Tab title="npm Package">
    ```bash
    clite plugin install npm:@scope/my-plugin
  ...
  <Tab title="Local Path">
    ...
    clite plugin install /absolute/path/to/plugin.ts
    ```
...
After installation, confirm the plugin is loaded by running `clite config` and checking the plugin tab.
