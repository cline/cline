...
description: "Understand how Rules, Skills, Workflows, Hooks, and .clineignore fit together in Cline customization."
---
...
Cline offers four primary systems plus a file-based compatibility path: Rules, Skills, Hooks, `.clineignore`, and Workflows. Each serves a different purpose and activates at different times.
...
| **[Workflows](/customization/workflows)** | File-based slash-command compatibility | Invoked with `/workflow.md` | Existing `/file.md` automations you still need while migrating to skills |
...
3. **Workflows** can still provide the explicit `/release.md` sequence while you migrate that automation toward skills
...
**Start with project storage.** Most customizations belong in your project's directory because they're tied to that specific codebase. Team coding standards, deployment skills, workflows, and architectural constraints all live with the code they describe. This also means your customizations travel with the repository, so collaborators get them automatically and changes can be reviewed in pull requests.
