...
description: "Understand how Rules, Skills, Plugins, Hooks, and .clineignore work together to customize Cline."
---

Out of the box, Cline is a general-purpose AI assistant. Customizations transform it into an expert on your codebase, your team's conventions, and your development process. Instead of repeating the same instructions every task, you define them once and Cline follows them automatically.

Cline offers five systems for this: Rules, Skills, Plugins, Hooks, and .clineignore. Each serves a different purpose and activates at different times.
...
| **[Plugins](/customization/plugins)** | Extend agent capabilities at runtime | Active when loaded | Custom tools, commands, integrations, lifecycle extensions |
...
**[Plugins](/customization/plugins)** extend what agents can do at runtime by adding capabilities like tools, commands, and lifecycle behavior. Use plugins when you need deeper integration or custom runtime behavior beyond prompt-level instructions.
...
3. **Plugins** add custom runtime capabilities such as internal deployment tools or release-specific commands
...
These systems support global and/or project-specific configurations:

| System | Global Location | Project Location |
|--------|-----------------|------------------|
| Rules | `~/Documents/Cline/Rules/` | `.clinerules/` |
| Skills | `~/.cline/skills/` | `.cline/skills/` |
| Plugins | See [SDK Plugins](/sdk/plugins) | See [SDK Plugins](/sdk/plugins) |
| Hooks | `~/Documents/Cline/Hooks/` | `.clinerules/hooks/` |
| .clineignore | N/A | `.clineignore` |

### When to Use Each

**Start with project storage.** Most customizations belong in your project's directory because they're tied to that specific codebase. Team coding standards, deployment steps, and architectural constraints all live with the code they describe. This also means your customizations travel with the repository, so collaborators get them automatically and changes can be reviewed in pull requests.

**Use global storage for personal preferences.** If you find yourself adding the same customization to every project, move it to global storage. Your preferred communication style, personal productivity patterns, and tools you use everywhere belong here. Global customizations apply to all projects but stay out of version control, so they won't affect your teammates.
