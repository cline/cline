...
<Note>
On Windows, each hook type supports either filename style: `HookName` or `HookName.ps1` (for example, `PreToolUse` or `PreToolUse.ps1`). If both files exist for the same hook type, Cline uses the extensionless `HookName` file.
</Note>
