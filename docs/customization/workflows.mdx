...
description: "Use Markdown workflow files when you need file-based slash-command compatibility while migrating to skills."
---

Workflows are a compatibility feature for file-based slash commands like `/deploy.md`. If you're starting fresh, use [Skills](/customization/skills) instead. Skills are the preferred product direction for reusable guidance because they load on demand, support bundled resources cleanly, and align with the rest of the SDK migration.

<Warning>
For new reusable automations, create a [skill](/customization/skills) instead of a workflow. Keep workflows only when you still rely on existing `/name.md` commands or need a compatibility bridge while migrating file-based commands created for earlier Cline versions.
</Warning>
...
<Tip>
**Prefer skills for new reusable guidance.** After finishing something you'll need to repeat, ask Cline whether it should become a skill or a workflow. Choose a workflow only if you specifically want a `/file.md` command for backward compatibility.
</Tip>
