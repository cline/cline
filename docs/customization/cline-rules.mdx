...
|-----------|----------|-------------|
| Cline Rules | `.clinerules/`, `.cline/rules/` | Supported workspace rule directories |
...
**Workspace rules** go in `.clinerules/` or `.cline/rules/` at your project root. Both layouts are supported by VS Code, Desktop, and the CLI. Use these for team standards, project-specific constraints, and anything you want to share with collaborators via version control.
...
The example uses `.clinerules/`; you can put the same rule files in `.cline/rules/` instead. Both directories are searched when present, so you do not need to copy rules into both locations. The VS Code Rules panel still creates new workspace rules in `.clinerules/`.

Cline processes rule files inside either directory, combining them into a unified set of rules. Numeric prefixes (like `01-coding.md`) help organize files but are optional.
...
<Note>
Cline also searches `~/.cline/rules` and `~/Cline/Rules` for global rules. On Windows, it checks `Documents\Cline\Rules` under the OneDrive directories configured by `OneDrive`, `OneDriveConsumer`, and `OneDriveCommercial`.
</Note>
