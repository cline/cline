...
**Context management**: Rules count toward your context window. Keep them focused and use [conditional rules](/features/cline-rules/conditional-rules) to load only what's relevant.
...
<Note>
Cline supports the [AGENTS.md](https://agents.md/) standard as a fallback. If no `.clinerules` file or folder exists, Cline reads `AGENTS.md` from your workspace root. This lets you use the same rules across multiple AI coding tools.
</Note>
