...
For an introduction to Beadsmith Rules, see the [Overview](/features/beadsmith-rules/overview).
...
Conditional rules use YAML frontmatter at the top of your rule files. When Beadsmith processes a request, it gathers context from your current work (open files, visible tabs, mentioned paths, edited files), evaluates each rule's conditions, and activates matching rules.
...
Add YAML frontmatter to the top of any rule file in your `.beadsmithrules/` directory:
...
**Invalid YAML**: If frontmatter can't be parsed, Beadsmith fails open: the rule activates with raw content visible to help debugging.

## What Counts as "Current Context"

Beadsmith evaluates rules based on:
...
4. **Edited files**: Files Beadsmith has created, modified, or deleted during the task
5. **Pending operations**: Files Beadsmith is about to edit

Conditional rules can activate on your first message, when relevant files are open, or mid-task when Beadsmith starts working with matching files.
...
- [Beadsmith Rules Overview](/features/beadsmith-rules/overview) - Complete rules system guide
...
- [Understanding Context Management](/prompting/understanding-context-management) - How Beadsmith manages context window
