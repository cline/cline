...
# coding standards and best practices into the AI context. This helps Beadsmith understand
...
# Return the context to guide Beadsmith's behavior
if [[ -n "$context" ]]; then
...
# users about system resource issues that may be slowing down Beadsmith's operations.
#
...
if (( execution_time > 3000 )); then
  ...
  mkdir -p "$HOME/.beadsmith_logs"
  
  # Log the slow operation
  echo "$(date -Iseconds): SLOW OPERATION - $tool_name took ${execution_time}ms" >> "$HOME/.beadsmith_logs/performance.log"
...
reports_dir="$HOME/.beadsmith_reports"
...
cat > "$report_file" << EOF
# Beadsmith Task Completion Report
...
activity_log="$HOME/.beadsmith_user_activity/$(date +%Y-%m-%d).log"
...
# CI/CD systems. Enables seamless integration of Beadsmith operations into enterprise workflows.
#
...
if [[ -n "$triggered_workflows" ]]; then
  ...
  if [[ -n "$slack_webhook" ]]; then
    slack_payload=$(jq -n \
      ...
      '{
        text: ("🔧 Beadsmith modified `" + $file + "` in " + $workspace),
        color: "good",
        fields: [{
...
mkdir -p .beadsmithrules/hooks

# Create multiple hooks
touch .beadsmithrules/hooks/PreToolUse
touch .beadsmithrules/hooks/PostToolUse
touch .beadsmithrules/hooks/TaskStart

# Make them executable
chmod +x .beadsmithrules/hooks/*
```
...
export CI_WEBHOOK_URL="https://your-ci-system.com/hooks/beadsmith"
```
