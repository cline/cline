...
description: "Complete API reference for all Beadsmith hook types, JSON schemas, and field documentation"
---
...
Beadsmith provides multiple hook types that let you tap into different stages of the AI workflow. They're organized into categories based on their trigger points and use cases.
...
Each hook receives base fields in addition to its specific data: `beadsmithVersion`, `hookName`, `timestamp`, `taskId`, `workspaceRoots`, `userId`.
...
Triggered immediately before Beadsmith uses any tool (see the [Beadsmith Tools Reference Guide](/beadsmith-tools) for all available tools). Use it to block invalid operations, validate parameters, and enforce project policies before changes happen.

**Input Fields:**
```json
{
  "beadsmithVersion": "string",
  "hookName": "PreToolUse",
  "timestamp": "string",
  "taskId": "string",
...
Triggered immediately after Beadsmith uses any tool (see the [Beadsmith Tools Reference Guide](/beadsmith-tools) for all available tools). Use it to learn from results, track performance metrics, and build project knowledge based on operations performed.
...
These hooks monitor and enhance user communication with Beadsmith. Use them to validate input, inject context, and track interaction patterns.
...
Triggered once at the beginning of a new task. Use it to detect project type, initialize tracking, and inject initial context that shapes how Beadsmith approaches the work.
...
Triggered when Beadsmith finishes its work and successfully executes the `attempt_completion` tool to finalize the task output. Use it to track completion metrics, generate reports, log task outcomes, and trigger completion workflows.
...
# Log completion
completion_log="$HOME/.beadsmith_completions/$(date +%Y-%m-%d).log"
...
These hooks monitor internal Beadsmith operations and system-level events. Use them to track context usage, log system behavior, and analyze performance patterns.
...
Beadsmith will parse only the final JSON object from stdout.
...
**Important:** Even when a hook fails (non-zero exit, invalid JSON, timeout), Beadsmith continues with the task. Only `"cancel": true` stops execution.
...
Beadsmith searches for hooks in this order:
1. Project-specific: `.beadsmithrules/hooks/` in workspace root
2. User-global: `~/Documents/Beadsmith/Hooks/`
