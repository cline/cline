...
Runs when a task completes successfully. Use it to track completion metrics, generate reports, log task outcomes, and trigger completion workflows.
...
If your hook script:
- Exits with non-zero status: The action is blocked
- Outputs invalid JSON: The action is blocked  
- Times out: The action is blocked
- Outputs no JSON: The action proceeds
...
Hooks run with the same permissions as VS Code and have access to:
...
- **Keep hooks fast**: Aim for sub-100ms execution time
- **Avoid blocking operations**: Network calls, heavy computations
- **Use async patterns**: For expensive operations, trigger background processes
- **Cache when possible**: Store results to avoid re-computation
