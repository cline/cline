...
#
# Overview: A comprehensive hook template with built-in debugging capabilities that can
# be toggled via environment variables. Includes error handling, detailed logging, and
# proper input validation - perfect as a starting point for production hooks.
#
# Demonstrates: Environment-based debug toggling, stderr logging patterns, error trapping,
# JSON validation, graceful error handling, and structured debug output.
