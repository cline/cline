...
<Frame>
  <img src="/assets/hooks/hooks-interface-with-dropdown.png" alt="Hooks management interface showing Global Hooks and workspace-specific hooks with dropdown menu" />
</Frame>
...
<Frame>
  <img src="/assets/hooks/hooks-empty-state.png" alt="Empty hooks interface showing New hook... dropdowns for both Global Hooks and workspace-specific hooks before any hooks are created" />
</Frame>
...
```bash
#!/usr/bin/env bash
# Hook Input Inspection Script
#
# Overview: A minimal starter script that demonstrates the fundamental mechanics of hooks:
# reading JSON input from stdin and using jq to inspect the data structure. Perfect for
# understanding how hook data flows before building more complex validation logic.
#
# Demonstrates: Basic input reading with stdin, jq usage for JSON parsing and inspection,
# and the foundation pattern that all hooks build upon.
