...
description: "Inject custom logic into Beadsmith's workflow to validate operations, monitor tool usage, and shape AI decisions"
---

Hooks let you inject custom logic into Beadsmith's workflow at key moments. Think of them as automated checkpoints where you can validate operations before they execute, monitor tool usage as it happens, and shape how Beadsmith makes decisions.
...
<Frame>
	<img src="https://storage.googleapis.com/beadsmith_public_images/hooks.gif" alt="Hooks in action" />
</Frame>
...
Setting up hooks in Beadsmith is user-friendly with the built-in hooks management interface. Here's how to get started:
...
1. Open Beadsmith (ensure hooks are enabled in settings)
...
To develop and refine your hook, you'll need to trigger it multiple times during testing. Each hook type is triggered by different events in Beadsmith's workflow. For example:

- **TaskStart** hooks trigger when you start a new task
- **PreToolUse** hooks trigger before Beadsmith executes tools like file editing
- **PostToolUse** hooks trigger after tool execution completes
- **UserPromptSubmit** hooks trigger when you submit a message to Beadsmith
...
The key is combining hooks with external tools. A hook can be the glue between Beadsmith's workflow and the rest of your development ecosystem.
...
Hooks are also available in the [Beadsmith CLI](/beadsmith-cli/overview). You can enable or disable hooks when running tasks from the command line:
...
Hooks complement other Beadsmith features:

- [Beadsmith Rules](/features/beadsmith-rules) define high-level guidance that hooks can enforce
...
- [Beadsmith CLI](/beadsmith-cli/overview) enables hooks in terminal-based and automated workflows
