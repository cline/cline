...
Once you've created a hook through the interface, here's how to understand and modify its code:

<Tip>
New to hooks? Start with the **TaskStart** hook  it runs once when a task begins and is the simplest way to experiment with hook development.
</Tip>

### Anatomy of a Hook

Hooks are executable scripts that follow a simple pattern:

1. **Read JSON input** from stdin containing event details
2. **Process the data** and decide whether to allow, block, or modify
3. **Output JSON** to stdout with your decision
...
echo "$input" | jq '.'  # See what data is available

# Make your decision
echo '{"cancel": false}'
```

### Your First Hook: Hello World

Here's the simplest possible hook  it just logs that it ran and allows the operation:

```bash
#!/usr/bin/env bash
# Save this as .clinerules/hooks/TaskStart

input=$(cat)
echo "Hook triggered!" >&2  # Log to stderr
...
Let's create a hook that prevents creating JavaScript files in TypeScript projects:

```bash
#!/usr/bin/env bash
# Save this as .clinerules/hooks/PreToolUse
...
if [[ "$tool_name" == "write_to_file" ]]; then
  if [[ "$file_path" == *.js ]] && [[ -f "tsconfig.json" ]]; then
    echo '{"cancel": true, "errorMessage": "Use .ts extension in TypeScript projects"}'
    exit 0
  fi
fi
...
Your hook must output valid JSON with these fields:

| Field | Type | Description |
|-------|------|-------------|
| `cancel` | boolean | `true` to block the operation, `false` to allow |
| `contextModification` | string | (Optional) Text to inject into the conversation |
| `errorMessage` | string | (Optional) Message shown when blocking |

### File Locations and Naming

- **Project hooks**: `.clinerules/hooks/` (committed to your repo)
- **Global hooks**: `~/Documents/Cline/Hooks/` (applies to all projects)
- **File names**: Must exactly match hook type (e.g., `PreToolUse`, `TaskStart`)

<Note>
Make your hook executable: `chmod +x .clinerules/hooks/PreToolUse`
</Note>
...
- **[Hook Reference](/features/hooks/hook-reference)**: Complete API reference for all hook types, JSON schemas, and field documentation.
- **[Samples](/features/hooks/samples)**: Practical examples and complete working scripts for common use cases.
