...
Beadsmith works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, Beadsmith can read files, write code, and run commands across all of them.

<Frame>
  <video
    src="https://storage.googleapis.com/beadsmith_public_images/multiworkspace.mp4"
    autoPlay
    muted
    loop
    playsInline
...
- **Beadsmith rules** only work in the primary workspace folder
...
Beadsmith can complete tasks that span multiple projects or repositories:
...
Beadsmith supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and Beadsmith tracks them independently.
...
When you open separate Git repositories in one workspace, Beadsmith treats each as an independent project with its own version control.

### What Beadsmith Tracks Per Repository

For each workspace folder, Beadsmith detects:
...
This means Beadsmith understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While Beadsmith detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [Beadsmith rules](/features/beadsmith-rules), [workflows](/features/slash-commands/workflows/index), and [Git-related features](/features/at-mentions/git-mentions) like `@git` mentions.
</Note>
...
Beadsmith understands natural references to your workspaces:
...
- Beadsmith needs to resolve ambiguity
...
Each service has its own repository. Beadsmith can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, Beadsmith can fix both consumers.
...
Mix of repositories and plain folders. Beadsmith adapts to each folder's configuration.
...
[Beadsmith rules](/features/beadsmith-rules) (`.beadsmithrules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/Beadsmith/Rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/features/checkpoints) are disabled in multi-root workspace mode. Beadsmith displays a warning when this happens.
...
4. **Consider the primary workspace** for Beadsmith rules placement
...
- Use [Plan mode](/features/plan-and-act) to let Beadsmith understand structure first
- Add a `.beadsmithignore` file to reduce noise, speed up scanning, and keep Beadsmith focused on source code:
...
For more patterns and gotchas, see the [.beadsmithignore File Guide](/prompting/prompt-engineering-guide#beadsmithignore-file-guide).
