...
<Warning>
Multi-root workspaces are currently experimental. Two limitations apply:
...
```
fullstack.code-workspace       # Workspace file (not a repo)
...
Multi-root workspace support is experimental. Two features have limitations:
...
- Exclude large generated folders (`node_modules`, `dist`) from workspace search scope
