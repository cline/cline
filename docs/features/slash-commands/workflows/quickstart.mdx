...
description: "A step-by-step guide to creating your first Beadsmith workflow."
---
...
<Steps>
  <Step title="Create the Workflow File">
    ...
    1.  In the root of your project, create a new folder named `.beadsmithrules`.
    2.  Inside `.beadsmithrules`, create another folder named `workflows`.
  ...
  <Step title="Run the Workflow">
    ...
    1.  Open the Beadsmith chat panel.
    2.  Type `/pr-review.md` followed by the PR number (e.g., `/pr-review.md 42`) and press Enter.
    3.  Beadsmith will fetch the PR details, analyze the code, and present you with its findings before submitting the review.

    <Tip>
      As Beadsmith executes commands (like `gh pr view`), it may show you the output and pause. You will need to click the **Proceed While Running** button to allow Beadsmith to analyze the content and continue with the workflow.
    </Tip>
  </Step>
</Steps>
...
*   **Refactoring Code:** Guide Beadsmith through a complex refactoring process step-by-step.

Explore Beadsmith's capabilities and your own development processes to find repetitive tasks that can be turned into efficient workflows.
