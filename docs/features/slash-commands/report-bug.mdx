...
`/reportbug` is an absolute lifesaver when you hit a weird issue with Beadsmith. Instead of having to remember all the details GitHub wants for a bug report, this command turns Beadsmith into your personal bug reporting assistant.
...
-   Beadsmith will guide you through all the details we need:
...
Last week I hit a weird bug where Beadsmith kept timing out when reading large files. Instead of trying to remember all the GitHub template fields, I just typed `/reportbug` and Beadsmith guided me through the whole process.

It asked me about what I was trying to do, what happened instead, and the exact steps that led to the issue. The best part was that it automatically included my OS version, Beadsmith version, and all the technical details our devs would need.
