...
`/explain-changes` is a slash command that generates AI-powered explanations for any git diff. Unlike the [Explain Changes button](/features/explain-changes) which explains changes from a completed task, this command lets you explain changes between any two git references - commits, branches, tags, PRs, staged changes, or your working directory.
	...
	<video
		src="https://storage.googleapis.com/beadsmith_public_images/slash-code-explain.mp4"
		autoPlay
		loop
		muted
		playsInline
...
Type `/explain-changes` in the chat input. Beadsmith will:
...
When you use `/explain-changes`, Beadsmith:
...
Beadsmith will examine HEAD and compare it to HEAD~1, explaining all the changes in that commit.
...
Beadsmith compares HEAD to your working directory, explaining all modified files.
...
Beadsmith examines only the changes you've staged with `git add`.
...
Beadsmith will find the commit and explain what it changed.
...
Beadsmith compares the endpoints and explains all changes between them.
...
Beadsmith uses the GitHub CLI to fetch PR details and explain the changes.
...
Beadsmith filters the diff to show only relevant changes.
...
Beadsmith compares the tag to HEAD and explains all subsequent changes.
...
Beadsmith explains all the changes that were merged.
...
Beadsmith examines stash@{0} and explains its contents.
...
Click the title area of any comment thread to move that conversation into Beadsmith's main chat. This is useful when:

- You want Beadsmith to make additional changes
- The discussion reveals something that needs more investigation
- You want to continue working with full Beadsmith capabilities
...
1. **Be specific**: Instead of just `/explain-changes`, tell Beadsmith what you want explained. "Explain the authentication changes in PR #42" gives better context than just "explain PR #42".
...
3. **Chain with other commands**: Use `/explain-changes` after investigating an issue to understand potential fixes, then continue with Beadsmith to implement improvements.
