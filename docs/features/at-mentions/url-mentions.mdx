...
URL mentions let you bring web content directly into your conversation with Beadsmith. Just type `@` followed by any URL, and Beadsmith can see the content of that webpage without you having to copy and paste anything.

When you type `@` in chat followed by a URL (like `@https://example.com`), Beadsmith will fetch the content of that webpage and include it in the context. This works for documentation pages, GitHub issues, Stack Overflow questions, or any other web content you want to reference.
...
This gives Beadsmith the complete documentation page, so it can see all the authentication requirements, endpoints, parameters, and examples. Beadsmith can then provide more accurate and comprehensive help based on the official documentation.
...
This shows Beadsmith the complete GitHub issue, including the description, comments, and any code snippets or screenshots. Beadsmith can then help you implement a solution that directly addresses the reported issue.

Next time you're working with external documentation or online resources, try using URL mentions instead of copying and pasting content. You'll get more accurate help because Beadsmith can see the complete context of the webpage, including formatting, code examples, and surrounding information.

## How It Works Under the Hood

When you use a URL mention in your message, here's what happens behind the scenes:

1. When you send your message, Beadsmith detects the `@http://...` or `@https://...` pattern in your text
