...
Folder mentions let you bring entire directories into your conversation with Beadsmith. Just type `@/` followed by a folder path ending with a slash, and Beadsmith gets access to the folder structure and its contents.

When you type `@/` in chat, Beadsmith shows your workspace files and folders. Navigate to the folder you want, make sure to include the trailing slash, and Beadsmith will see the folder's structure and contents.
...
Beadsmith can then see all the files in the auth directory, their contents, and how they relate to each other. This gives it the full context to explain complex interactions between multiple files.

Folder mentions are also perfect for getting help with project organization. When I'm unsure if my project structure makes sense, I'll ask Beadsmith to review it:
...
Next time you're working with multiple related files, try using folder mentions instead of referencing each file individually. You'll get more comprehensive help because Beadsmith can see the bigger picture of how everything fits together.

## How It Works Under the Hood

When you use a folder mention in your message, here's what happens behind the scenes:

1. When you send your message, Beadsmith detects the `@/path/to/folder/` pattern (with trailing slash) in your text
