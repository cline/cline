...
You can make rules activate only when working with specific files by adding YAML frontmatter:
...
This rule will only load when you're working with files matching those patterns. See [Conditional Rules](/features/conditional-rules) for the complete guide on path-based rule activation.
