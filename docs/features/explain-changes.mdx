...
Explain Changes is an AI-powered code review feature that adds inline explanations to your code changes. When Beadsmith makes modifications to your codebase, you can click a button to get streaming, contextual explanations that appear directly in VS Code's diff view.
...
<Frame>
	<video
		...
		muted
		playsInline
		src="https://storage.googleapis.com/beadsmith_public_images/explain-code-button.mp4"
	/>
</Frame>
...
## How It Works
<Note>
Explain Changes requires **[Checkpoints](/features/checkpoints)** to be enabled. Make sure to enable checkpoints in your Beadsmith settings before using this feature.
</Note>

After Beadsmith completes a task that involves file changes, you'll see an "Explain Changes" button alongside the "View Changes" button in the completion message. Clicking this button:
...
If a conversation in a comment thread becomes complex or you want to continue working on that code, click the title area of the comment thread to move the entire conversation into Beadsmith's main chat input. This lets you:

- Continue the discussion with full Beadsmith capabilities
- Have Beadsmith make additional changes based on the discussion
...
When you're new to a codebase or working with unfamiliar patterns, Explain Changes helps you understand not just what Beadsmith did, but why. The explanations cover:
...
4. **Review before committing**: Use Explain Changes as a final check before committing changes to ensure you understand everything Beadsmith did.
