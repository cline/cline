...
title: "Skills"
sidebarTitle: "Skills"
description: "Extend Beadsmith with reusable, on-demand instruction sets for specialized tasks"
---

Skills are modular instruction sets that extend Beadsmith's capabilities for specific tasks. Each skill packages detailed guidance, workflows, and optional resources that Beadsmith loads only when relevant to your request.

Unlike rules (which are always active), skills load on-demand. You can install dozens of skills without affecting context or performance because Beadsmith only sees the skill name and description until it's actually needed.
...
- **At startup**: Beadsmith sees only a brief description of each skill
- **When triggered**: Beadsmith loads the full instructions for that specific skill
- **As needed**: Skills can bundle additional files that Beadsmith reads only when referenced
...
Detailed instructions for Beadsmith to follow when this skill is activated.
...
- `description`: Tells Beadsmith when to use this skill (max 1024 characters)

The description is critical because it's how Beadsmith decides whether to activate a skill. Be specific about what the skill does and when it should be used.
...
- **macOS/Linux:** `~/.beadsmith/skills/`
- **Windows:** `C:\Users\USERNAME\.beadsmith\skills\`

**Project Skills** apply only to the current workspace:
- `.beadsmith/skills/` (recommended)
...
## How Beadsmith Uses Skills

When you send a message, Beadsmith sees a list of available skills with their descriptions. If your request matches a skill's description, Beadsmith activates it using the `use_skill` tool, which loads the full instructions.
...
Asking "deploy this to AWS" would trigger Beadsmith to activate the skill, load its detailed instructions, and follow them to complete your request.
...
Skills can include additional files that Beadsmith accesses only when needed:
...
Beadsmith reads these files using `read_file` when the instructions reference them. Scripts can be executed directly, with only the output entering the context (not the script code itself).
...
|---------|---------|-------------|
| **Rules** | Define how Beadsmith should behave | Always (or contextually) |
...
**Skills** are expertise that Beadsmith activates automatically when relevant (like data analysis knowledge when you're working with CSV files).
...
- [Beadsmith Rules](/features/beadsmith-rules) for always-active project guidance
