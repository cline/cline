...
Speed up your workflow by accessing Beadsmith's AI assistance without taking your hands off the keyboard.

<Tip>
**The One Shortcut You Need:** `Ctrl+'` (Windows/Linux) or `Cmd+'` (macOS)

This context-aware shortcut handles your most common needs:
- **With text selected:** Adds code to Beadsmith chat
...
Beadsmith has minimal default shortcuts by design, so they won't conflict with your existing VSCode setup:
...
4. **Get help** → Ask Beadsmith to interpret errors or output

<Info>
**Pro Tip:** Assign `Alt+T` to the `beadsmith.addTerminalOutputToChat` command for quick terminal output capture. Without a shortcut, you can still right-click in the terminal and select "Add to Beadsmith" - but the keyboard approach is much faster for frequent debugging workflows.
</Info>

## Customizing Shortcuts

Want to assign shortcuts to more Beadsmith commands? Here's how:
...
These commands help you navigate and manage Beadsmith tasks:

| Command ID | Description | Suggested Shortcut |
| ---------- | ----------- | ------------------ |
| `beadsmith.plusButtonClicked` | Start a new task | `Ctrl+Shift+N` / `Cmd+Shift+N` |
| `beadsmith.historyButtonClicked` | Open task history | `Ctrl+Shift+H` / `Cmd+Shift+H` |
| `claude-dev.SidebarProvider.focus` | Open Beadsmith sidebar | `Ctrl+Shift+L` / `Cmd+Shift+L` |

**Note:** `claude-dev` prefix is for historical reasons - it works with Beadsmith.
...
| Command ID | Description | Suggested Shortcut |
| ---------- | ----------- | ------------------ |
| `beadsmith.addToChat` | Add selected code to chat | `Ctrl+'` / `Cmd+'` ⭐ (default) |
| `beadsmith.focusChatInput` | Focus chat input | `Ctrl+'` / `Cmd+'` ⭐ (default) |
| `beadsmith.explainCode` | Explain selected code | `Ctrl+Shift+E` / `Cmd+Shift+E` |
| `beadsmith.improveCode` | Suggest code improvements | `Ctrl+Shift+I` / `Cmd+Shift+I` |
...
Connect Beadsmith with your terminal:

| Command ID | Description | Suggested Shortcut |
| ---------- | ----------- | ------------------ |
| `beadsmith.addTerminalOutputToChat` | Add terminal output to Beadsmith | `Alt+T` |
...
| Command ID | Description | Suggested Shortcut |
| ---------- | ----------- | ------------------ |
| `beadsmith.generateGitCommitMessage` | Generate commit message | `Ctrl+Shift+G` / `Cmd+Shift+G` |
| `beadsmith.abortGitCommitMessage` | Stop generation | `Ctrl+Shift+Esc` / `Cmd+Shift+Esc` |

</Accordion>

<Accordion title="Settings & Configuration Commands (Advanced)">

These commands open Beadsmith's configuration panels. Most users access these via the sidebar buttons, but keyboard shortcuts can be useful for:
...
| Command ID | Description | Suggested Shortcut |
| ---------- | ----------- | ------------------ |
| `beadsmith.settingsButtonClicked` | Open Beadsmith settings | `Ctrl+Alt+,` / `Cmd+Opt+,` |
| `beadsmith.mcpButtonClicked` | Open MCP servers config | `Ctrl+Alt+M` / `Cmd+Opt+M` |
| `beadsmith.accountButtonClicked` | Open account settings | `Ctrl+Alt+A` / `Cmd+Opt+A` |
| `beadsmith.openWalkthrough` | Open walkthrough guide | (not recommended) |
...
## What About "Fix with Beadsmith"?

<Warning>
**You CAN'T assign a keyboard shortcut to "Fix with Beadsmith"**
...
- Or select code with errors and use `Ctrl+'` / `Cmd+'` to ask Beadsmith to fix them
- Or right-click and select "Add to Beadsmith"
</Warning>
...
- **Create patterns:** Group related commands (e.g., all Beadsmith shortcuts use `Ctrl+Shift+...`)
