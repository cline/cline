...
Beadsmith's Git integration brings AI assistance directly to your version control workflow. Generate commit messages without leaving your editor.
...
<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/generate-commit-message-with-beadsmith.png"
		alt="Generate Commit Message with Beadsmith"
	/>
</Frame>

1. Make your changes and stage them in Git
2. Click the robot icon in the Source Control view or run the "Generate Commit Message with Beadsmith" command
3. Beadsmith analyzes your changes and generates a descriptive commit message
...
When you use Beadsmith's commit message generation feature, here's what happens behind the scenes:

1. Beadsmith retrieves the current Git diff using `getWorkingState()`
...
This process uses your current Beadsmith API configuration, so the quality of the generated messages matches your chosen AI model.
...
1. When you trigger the command:
    - Beadsmith gets the current diff
...
Next time you're struggling to write a good commit message, try using Beadsmith's commit message generation. You'll save time and improve your version control workflow with AI assistance right where you need it.
