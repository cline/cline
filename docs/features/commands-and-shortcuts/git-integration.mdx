...
<img src="https://storage.googleapis.com/cline_public_images/docs/assets/git-integration.png" alt="Git Integration" />
...
1. When you trigger the command:
    - Cline uses `getWorkingState()` to get the current diff
    - It sends this to the AI with specific instructions for commit message formatting
    - It uses `extractCommitMessage()` to parse the AI's response
    - It accesses the Git extension API to set the commit message

This seamless integration with Git makes it easy to generate high-quality commit messages without disrupting your workflow.
