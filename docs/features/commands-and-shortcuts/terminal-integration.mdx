...
Beadsmith's terminal integration lets you bring your terminal output directly into your conversations with Beadsmith. Instead of copying and pasting error messages or command results, you can send them to Beadsmith with a simple right-click in the terminal.

<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/terminal-integration.png"
...
When you're working in the VSCode terminal and see output you want to discuss with Beadsmith:

1. Right-click in the terminal
2. Select "Add to Beadsmith" from the context menu
3. The terminal output is immediately sent to the Beadsmith chat panel
...
Instead of switching context to the Beadsmith chat panel and typing a description of the problem, you can send the terminal output directly to Beadsmith with just a couple of clicks.

Alternatively, you can use the [`@terminal`](/features/at-mentions/terminal-mentions) mention to send the full terminal output to Beadsmith.
...
When you use the right-click terminal integration, Beadsmith:
...
This gives Beadsmith the full context it needs to provide accurate help with terminal-related issues.

## Behind the Scenes

The terminal integration uses a clever technique to capture terminal output:

1. When you trigger the integration, Beadsmith:
...
-   **Use terminal integration for error messages**: When you encounter an error in the terminal, sending it to Beadsmith often results in faster resolution than trying to describe the error.
...
-   **Combine terminal outputs with file mentions**: After sending terminal output to Beadsmith, you can enhance your question by mentioning relevant files using the @ mentions feature.
...
Next time you're staring at a cryptic error message in your terminal, try using Beadsmith's terminal integration instead of copying and pasting. You'll get more accurate help because Beadsmith can see the complete terminal context with proper formatting.
...
<Tip>
	**Quick Fix**: Most terminal issues can be resolved by switching to bash in the Beadsmith settings and increasing the shell
