...
When your conversation approaches the model's context window limit, Beadsmith automatically summarizes it to free up space and keep working. 

<Frame>
	<img
		src="https://storage.googleapis.com/beadsmith_public_images/docs/assets/condensing.png"
...
Beadsmith monitors token usage during your conversation. When you're getting close to the limit, he:
...
Previously, Beadsmith would truncate older messages when hitting context limits. This meant losing important context from earlier in the conversation.
...
1. Beadsmith uses a [summarization prompt](https://github.com/beadsmith/beadsmith/blob/main/src/core/prompts/contextManagement.ts) to request a summary of the conversation.

2. Once the summary is generated, Beadsmith replaces the conversation history with a [continuation prompt](https://github.com/beadsmith/beadsmith/blob/main/src/core/prompts/contextManagement.ts#L69) that asks Beadsmith to keep working and provides the summary as context.

Different models have different context window thresholds for when auto-summarization kicks in. You can see how thresholds are determined in [context-window-utils.ts](https://github.com/beadsmith/beadsmith/blob/main/src/core/context/context-management/context-window-utils.ts). 
...
<Note>
When using other models, Beadsmith automatically falls back to the standard rule-based context truncation method, even if Auto Compact is enabled in settings.
</Note>
