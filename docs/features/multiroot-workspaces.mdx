...
Multiroot workspaces let Cline work seamlessly across multiple project folders open in VSCode at the same time. Instead of treating everything as one big workspace, Cline understands each folder as a separate workspace and can target specific ones when reading, writing, or searching files.

<Frame>
	<img src="[PLACEHOLDER: Screenshot showing VSCode with multiple workspace folders in the sidebar and Cline working across them]" alt="Multiroot workspace support in Cline" />
</Frame>
...
<Frame>
	<img src="[PLACEHOLDER: Screenshot showing file search results with workspace annotations like 'frontend:/src/config.json' and 'backend:/src/config.json']" alt="File search with workspace annotations" />
</Frame>
...
If you're working with files that exist in multiple workspaces, be explicit about which one you want:
...
- Keep related projects together in the same VSCode workspace
...
<AccordionGroup>
  <Accordion title="Workspace names not showing in autocomplete">
    ...
    - Check your VSCode sidebar to see your workspace names
  ...
  <Accordion title="Cline can't find files in my workspace">
    ...
    - Verify the workspace is actually open in VSCode (check the sidebar)
    ...
    - Restart VSCode if you just added a new workspace folder
  </Accordion>
  ...
  <Accordion title="Auto-approve not working as expected">
    ...
    - Consider using separate VSCode windows for projects with different security requirements
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Workspace Detection">
    When you open VSCode with multiple workspace folders, Cline:
    - Detects all workspace roots through VSCode's workspace API
