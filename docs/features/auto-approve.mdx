...
- **Notifications**: If enabled, Cline sends OS-level notifications when approval is required, and when an auto-approved terminal command has been running for 30 seconds. Notifications may use abbreviated text instead of full command details for safety and privacy, and behavior can vary by OS.
...
| Enable notifications | Notifies you when Cline needs attention, with abbreviated details on some platforms |
