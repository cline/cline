---
title: "Disable Terminal Pagers During Beadsmith Sessions"
description: "Make CLI output non-interactive when Beadsmith runs commands by detecting the CLINE_ACTIVE environment variable and disabling pagers like less."
---

Many CLI tools (like Git) use a pager such as `less` for interactive, scrollable output. When Beadsmith runs commands in your terminal, that interactivity gets in the way — the pager can pause on the first page and block progress. You can configure your shell so that when a terminal is spawned by Beadsmith, pagers are disabled and output streams through normally.

## How it works

Beadsmith sets an environment variable for terminals it opens to run commands:

- `CLINE_ACTIVE` — non-empty when the shell is running under Beadsmith

You can detect this variable in your shell startup file and adjust environment variables or aliases only for Beadsmith-run sessions. This keeps your normal interactive terminals unchanged.
...
```bash
# Disable pagers when the terminal is launched by Beadsmith
...
- Open a task in Beadsmith that runs terminal commands and check:
...
- You can also override paging via shell aliases scoped to Beadsmith sessions using options rather than env vars:
...
- If you prefer environment variables, many CLIs also respect a generic or tool-specific pager variable:
  ...
  - Man pages: `MANPAGER=cat` (not typically needed for Beadsmith-driven commands)
