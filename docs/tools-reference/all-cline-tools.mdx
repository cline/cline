...
<Info>
Most tools below are available for Cline to use autonomously during tasks. A few are **slash-command or system-triggered** — these are activated by you typing a command (like `/smol`) or automatically by the system, not chosen by the model on its own.
</Info>
