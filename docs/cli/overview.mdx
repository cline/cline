...
<Tip>
Ready to get started? Check out the [installation guide](/cli/installation) to install Cline CLI and run your first task.
</Tip>
...
<Columns cols={2}>
  ...
  <Card title="Headless Mode" icon="robot" href="/cli/three-core-flows">
    **For automation & CI/CD.** Run `cline --autoapprove true "task"` to let Cline work autonomously — no interaction needed. Pipe input/output, get JSON results, and chain commands in scripts and pipelines.
  </Card>
</Columns>
...
- **Slash commands** with `/` - Quick access to `/settings`, `/history`, `/models`, and skill commands
...
**When it activates:** Using `--autoapprove true`, the `--json` flag, piping input/output, or when stdin is not a TTY.

```bash
# Headless with auto-approval
cline --autoapprove true "Run tests and fix any failures"
...
git diff | cline --autoapprove true "explain these changes" | cline --autoapprove true "write a commit message"
```

Key features:

- **No visual interface** - Clean text or JSON output suitable for scripting
- **Automatic execution** - With `--autoapprove true`, Cline approves all actions and runs autonomously
...
<Warning>
Headless mode with `--autoapprove true` gives Cline full autonomy. Run on a clean git branch so you can easily revert changes if needed.
</Warning>
...
| `cline --autoapprove true "task"` | Headless | auto-approval mode |
...
During setup, authenticate with `cline auth` to configure your preferred provider. [See setup guide →](/cli/installation#authenticate)
...
```bash
cline --autoapprove true "Fix all ESLint errors in src/"
```
Finds and fixes linting violations throughout your source directory.

```bash
cline --autoapprove true "Update all deprecated React lifecycle methods"
```
Migrates legacy code patterns to modern equivalents (e.g., `componentWillMount` → `useEffect`).

```bash
cline --autoapprove true "Update dependencies with known vulnerabilities"
```
...
```bash
git diff origin/main | cline --autoapprove true "Review these changes for issues"
```
Pipes your PR diff to Cline for automated code review, catching bugs and style issues before merge.

```bash
git log --oneline v1.0..v1.1 | cline --autoapprove true "Write release notes"
```
Generates human-readable release notes from your commit history between two tags.

```bash
cline --autoapprove true "Run tests and fix failures" --timeout 600
```
...
```bash
gh pr diff 123 | cline --autoapprove true "Review this PR"
```
...
```bash
git diff | cline --autoapprove true "explain" | cline --autoapprove true "write a haiku about these changes"
```
...
| Auto-approve | ✓ (Shift+Tab) | `--autoapprove true` |
| Session summary | ✓ | - |
| JSON output | - | `--json` |
| Piped input | - | ✓ |
| [MCP servers](/cli/configuration#mcp-server-configuration) | ✓ | ✓ |
...
[Configure MCP servers for the CLI →](/cli/configuration#mcp-server-configuration)

## Learn More

<Columns cols={2}>
  <Card title="Installation & Setup" icon="download" href="/cli/installation">
  ...
  <Card title="Use in Other Editors" icon="code" href="/cli/acp-editor-integrations">
    Run Cline as an ACP agent in JetBrains, Neovim, Zed, and more.
  </Card>
  
  <Card title="CLI Samples" icon="flask" href="/cli/samples/overview">
