...
Webhook mode requires a Slack app, a bot token, a signing secret, and a public HTTPS URL that can forward requests to your local connector.
...
Socket mode requires a bot token and an app-level token with the `connections:write` scope. It does not need a public request URL and is single-workspace only.
...
Enable Socket Mode in the Slack app before starting the connector.
