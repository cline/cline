...
SUMMARY=$(echo "$ISSUE_CONTENT" | clite --auto-approve true --config ~/.cline-haiku \
  "summarize this issue in 2-3 sentences")

# Phase 2: Detailed plan with expensive model + thinking
PLAN=$(echo "$SUMMARY" | clite --auto-approve true --thinking high --config ~/.cline-opus \
  "create detailed implementation plan with edge cases")

# Phase 3: Execute with mid-tier model
echo "$PLAN" | clite --auto-approve true --config ~/.cline-sonnet \
  "implement the plan from above"
```

<Note>
Each `clite` invocation needs to complete before passing output to the next phase. Use shell variables to store intermediate results rather than piping `clite` commands directly.
</Note>
...
clite --config ~/.cline-haiku auth anthropic --modelid claude-haiku-4-20250514
clite --config ~/.cline-sonnet auth anthropic --modelid claude-sonnet-4-20250514  
clite --config ~/.cline-opus auth anthropic --modelid claude-opus-4-5-20251101

# Or use different providers entirely
clite --config ~/.cline-gemini auth gemini --modelid gemini-2.0-flash-exp
clite --config ~/.cline-codex auth openai-codex --modelid gpt-5-latest
```

Now you can switch models per-task with `--config`:

```bash
clite --config ~/.cline-haiku "quick task"
clite --config ~/.cline-opus "complex reasoning task"
```
...
git show | clite --auto-approve true --config ~/.cline-gemini "review and save to gemini-review.md" &
git show | clite --auto-approve true --config ~/.cline-codex "review and save to codex-review.md" &
git show | clite --auto-approve true --config ~/.cline-opus "review and save to opus-review.md" &

# Wait for all to complete
wait

# Synthesize
cat *-review.md | clite --auto-approve true "create consensus review"
```
...
clite --auto-approve true "refactor this codebase"

# With thinking: Slower but more thorough
clite --auto-approve true --thinking high \
...
gh pr view $PR | clite --auto-approve true --config ~/.cline-haiku \
    "list all issues to fix, output as JSON"

# Opus with thinking: Deep analysis only if issues found
if [ -s issues.json ]; then
    clite --auto-approve true --thinking high --config ~/.cline-opus \
...
git diff main | clite --auto-approve true --config ~/.cline-gemini "security review" > gemini-sec.md &
git diff main | clite --auto-approve true --config ~/.cline-opus "security review" > opus-sec.md &
git diff main | clite --auto-approve true --config ~/.cline-codex "security review" > codex-sec.md &
wait

# High-priority: Issues all 3 models found
cat *-sec.md | clite --auto-approve true "find security issues all 3 reviews mentioned"
```
