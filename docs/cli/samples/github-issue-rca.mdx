...
<Note>
**New to Cline CLI?** This sample assumes you have already completed the [Installation Guide](https://docs.cline.bot/getting-started/installing-cline) and authenticated with `clite auth`. If you haven't set up Cline CLI yet, please start there first.
</Note>
...
clite --auto-approve true --json "$PROMPT: $ISSUE_URL" | \
...
**1. `clite --auto-approve true --json "$PROMPT: $ISSUE_URL"`**
   - `clite` is the Cline CLI binary
