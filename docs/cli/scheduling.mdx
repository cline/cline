...
The CLI supports running agents on cron schedules through the RPC sidecar. Scheduled agents persist across process restarts and run independently of any terminal session.
...
Scheduling requires the RPC server. It starts automatically when you create a schedule.
