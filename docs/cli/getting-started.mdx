...
**When it activates:** Using the `--autoapprove true` flag, `--json` flag, piping input/output, or when stdin is not a TTY.
...
| `cline --autoapprove true "task"` | Headless | Auto-approval flag triggers headless |
...
- **[Interactive Mode](/cli/interactive-mode)** - Master the interactive CLI with shortcuts and slash commands
- **[Headless Mode](/cli/three-core-flows)** - Run Cline autonomously in scripts, CI/CD pipelines, and automated workflows
- **[Configuration](/cli/configuration)** - Configure settings, rules, skills, and environment variables
- **[CLI Reference](/cli/cli-reference)** - Complete command documentation with all flags and options
