...
View and manage [skills](/customization/skills#triggering-skills-with-slash-commands):

- List available skills
- View skill definitions
- Skills can be triggered as slash commands in interactive mode
...
- Hooks run at key points in Cline's task lifecycle
...
Cline stores configuration in `~/.cline/`:
...
Project-level configuration lives in `.cline/` at the workspace root:
...
Cline CLI supports [MCP (Model Context Protocol)](/mcp/mcp-overview) servers, giving you access to external tools and data sources directly from the terminal.

Run `cline mcp` to manage MCP servers interactively, or configure them directly in `.cline/mcp.json`. See [MCP Servers](/cli/mcp-servers) for details.
...
See [Installation & Setup](/cli/installation#option-3-import-from-existing-tools) for importing configurations from:
- Codex CLI
- OpenCode
...
<Columns cols={2}>
  <Card title="Headless Mode" icon="robot" href="/cli/three-core-flows">
    Run Cline autonomously in scripts, CI/CD pipelines, and automated tasks.
  </Card>
