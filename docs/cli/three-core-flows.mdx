...
For collaborative, conversational development, see [Interactive Mode](/cli/interactive-mode) instead.
...
If none of these apply (e.g., running `cline` or `cline "task"` in a terminal), Cline launches in [interactive mode](/cli/interactive-mode).
...
See [Configuration](/cli/configuration#environment-variables) for full documentation.
