...
| `CLINE_OTEL_EXPORTER_OTLP_ENDPOINT` | OTLP collector endpoint (applies to both metrics and logs) |
| `CLINE_OTEL_EXPORTER_OTLP_HEADERS` | Authentication headers (comma-separated `key=value` pairs) |
| `CLINE_OTEL_EXPORTER_OTLP_INSECURE` | Disable TLS for gRPC | `"true"` for local development |
...
export CLINE_OTEL_EXPORTER_OTLP_ENDPOINT=https://otlp.nr-data.net
