...
<Note>
Cline emits telemetry from two pipelines: the host extension pipeline (VS Code / JetBrains UI, browser tool, worktrees, hooks) and the shared SDK runtime pipeline (session lifecycle, provider requests, compaction, agents — used by the CLI, the hub daemon, and SDK-backed sessions inside the extensions). Which events you receive depends on which Cline surfaces your users run. Attribute naming also varies by pipeline: extension events mostly use `snake_case` attributes, while SDK runtime events mostly use `camelCase`.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="session.* / sdk.* / agent.*" icon="microchip">
    SDK session lifecycle, runtime diagnostics, agent runtime
  </Card>
...
| `user.auth_failed` | Authentication flow failed | provider, errorMessage, request_id |
| `user.auth_logged_out` | User logged out | reason, provider, status, errorCode, request_id |
| `user.auth_refresh_soft_failure` | Token refresh failed transiently (network error, timeout, 5xx) and stored credentials were kept — the session was not invalidated | provider, status, errorCode, errorName, tokenExpired, request_id |
| `user.auth_run_retry` | A run failed with an auth-like error, credentials were refreshed, and the run was retried once (`recovered: true` means the retry completed) | provider, recovered |
| `user.provider_configured` | User finished configuring an API-key ("bring your own key") provider; unlike the OAuth `auth_*` events there is no start/fail counterpart | provider |
...
Emitted by the SDK runtime when a session is created (one `session.started` per session, from every backend — CLI, hub daemon, or SDK-backed extension sessions).

| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `session.started` | New SDK session created | sessionId, source, providerId, modelId, enableTools, enableSpawnAgent, enableAgentTeams, clientName, runtimeAddress |
...
| `task.completed` | Task completed. SDK-runtime emissions include a `source` attribute: `submit_and_exit` (explicit completion tool) or `shutdown` (session finished without one). At most one per session. | task_id, duration_ms, model, provider, tokens_total, source |
| `task.cancelled` | Task run aborted/cancelled | sessionId, agentId, runId, iteration, provider, model, error_type, error_message |
...
| `task.legacy_task_migration` | Legacy VS Code task history migrated into SDK sessions | migration_type, outcome, reason, durationMs, legacyApiHistoryLength, convertedMessageCount |
| `task.retry_clicked` | User clicked retry on a failed action/request | task_id, action_type |

### Provider Request Lifecycle

Emitted by the SDK runtime around every model provider request. All carry the same correlation attributes (sessionId, ulid, agentId, conversationId, runId, iteration, provider, model) plus a `phase` and `durationMs` measured from the start of the request.

| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `task.provider_request_started` | Model provider request dispatched | phase, durationMs + correlation attributes |
| `task.provider_stream_started` | Provider accepted the request and began streaming | phase, durationMs + correlation attributes |
| `task.first_chunk_received` | First streamed chunk arrived (time-to-first-token) | phase, durationMs, eventType + correlation attributes |
| `task.provider_stream_failed` | Provider request or stream failed (non-abort errors) | phase, durationMs, error_type, error_message, error_code, error_status + correlation attributes |
...
| `task.agent_created` | Agent runtime created for a session (SDK runtime) | ulid, agentId, agentKind, provider, modelId |
| `task.agent_team_created` | Multi-agent team created (SDK runtime) | ulid, teamId, teamName, leadAgentId, restoredFromPersistence |
...
| `task.compaction_executed` | SDK context compaction ran and reduced the conversation. `strategy` is `basic` (token-budget truncation), `agentic` (LLM summarization), or `custom`; `mode` is `auto` or `manual`. | strategy, mode, messagesBefore, messagesAfter, messagesRemoved, tokensBefore, tokensAfter, tokensSaved, triggerTokens, maxInputTokens, thresholdRatio, durationMs, provider, modelId |
| `task.compaction_skipped` | SDK compaction attempt decided not to compact (e.g. `reason: no_result` when nothing was safe to remove) | strategy, mode, reason, tokensBefore, triggerTokens, maxInputTokens, thresholdRatio, durationMs |
| `task.compaction_budget_emergency` | Compaction budget projection triggered an emergency policy action | strategy, mode, policyIntent, actionCount, warningCount, liveTailHandling |
...
| `task.mistake_limit_reached` | Consecutive-mistake limit reached, right before the limit decision (host prompt / auto-stop) is resolved | ulid, model, provider, reason, consecutiveMistakes, maxConsecutiveMistakes |
...
| `ui.panel_opened` | Cline panel became visible | source |
| `ui.new_task_clicked` | User explicitly started a new task flow | source, hasActiveTask |
| `ui.prompt_submitted` | User submitted chat composer content | source, hasText, hasImages, hasFiles, hasActiveTask, textLength |
...
## SDK Diagnostic Events

Runtime diagnostics emitted by the SDK pipeline.

| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `sdk.error` | Handled SDK error captured for diagnostics. `component` identifies the emitting package or app (`shared`, `llms`, `agents`, `core`, `cli`, `vscode`, `desktop`). Error messages are sanitized (secrets and home paths redacted) and truncated. | component, operation, severity, handled, error_type, error_message, error_code, error_status |
| `sdk.tool_timeout` | The `run_commands` tool hit its execution timeout | tool_name, effective_timeout_ms, timeout_source (default_setting/configured_setting), command_count, duration_ms, session_id, agent_id, tool_call_id |
| `agent.reasoning.unexpected_tokens` | Model returned reasoning tokens even though extended thinking was not requested | agentId, runId, iteration, providerId, modelId, requestedThinking, reasoningTokenCount |
...
| `extension_version` | string | Cline extension version (on the SDK pipeline: the cline-core bundle version) |
| `host_plugin_version` | string | Version of the host-side Cline distribution: the JetBrains plugin version on JetBrains, the extension version on VS Code (where it matches `extension_version`). Absent when the host does not report one. |
| `host_type` | string | vscode, jetbrains, or cli |

### Agent Identity Attributes

SDK-runtime task events (`task.created`, `task.completed`, `task.tokens`, `task.tool_used`, compaction events, and others) can additionally carry agent identity attributes for correlating multi-agent work:

| Attribute | Description |
|-----------|-------------|
| `agentId` | Identifier of the emitting agent |
| `agentKind` | `root`, `subagent`, `team_lead`, or `team_teammate` |
| `conversationId` | Conversation the agent belongs to |
| `parentAgentId` / `createdByAgentId` | Spawning relationships for subagents |
| `isSubagent` | Whether the emitting agent is a subagent |
| `teamId` / `teamName` / `teamRole` / `teamAgentId` | Multi-agent team membership, when applicable |
