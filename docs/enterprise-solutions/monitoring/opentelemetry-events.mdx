...
| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `task.created` | New task/conversation started | task_id, mode, model_id, provider |
| `task.restarted` | Existing task restarted/reopened | task_id, time_since_last_message |
| `task.completed` | Task completed | task_id, duration_ms, model_id, provider, tokens_total |
...
| Event | Description | Key Attributes |
|-------|-------------|----------------|
| `ui.model_selected` | Model selected in UI | model_id, provider, previous_model |
