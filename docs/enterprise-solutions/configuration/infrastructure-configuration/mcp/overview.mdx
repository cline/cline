...
description: "Configure Model Context Protocol (MCP) servers and marketplace integrations for enterprise Beadsmith deployments"
---

Model Context Protocol (MCP) provides standardized communication between AI models and external data sources, tools, and services. Enterprise MCP integration allows you to securely connect Beadsmith to your organization's systems while maintaining governance and compliance.
...
```mermaid
graph TB
    A[Beadsmith Enterprise] --> B[MCP Hub]
    B --> C[MCP Marketplace]
...
mcp:
  hub:
    ...
    discovery:
      ...
      marketplace_url: "https://mcp.beadsmith.bot/marketplace"
...
apiVersion: mcp.beadsmith.bot/v1
kind: MCPServer
metadata:
  name: custom-api-server
  namespace: beadsmith-enterprise
spec:
...
mcp:
  hub:
    replicaCount: 3
    image:
      repository: beadsmith/mcp-hub-enterprise
      tag: "1.5.2"
...
import { MCPServer, Tool, Resource } from '@beadsmith/mcp-enterprise-sdk';
