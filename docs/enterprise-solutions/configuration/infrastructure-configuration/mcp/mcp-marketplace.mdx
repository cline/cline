...
description: "Deploy pre-built enterprise MCP servers from the Beadsmith marketplace with one-click configuration"
---
...
<CardGroup cols={2}>
  ...
  <Card title="Maintained & Updated" icon="sync">
    Regular security updates and feature enhancements managed by Beadsmith Enterprise team.
  </Card>
...
### Via Beadsmith Enterprise Dashboard
...
mcp:
  marketplace_servers:
    - name: "github-enterprise"
      package: "@beadsmith/mcp-github-enterprise"
      version: "2.1.0"
    ...
    - name: "jira-enterprise"
      package: "@beadsmith/mcp-jira-enterprise"
      version: "1.8.3"
      environment: "production"
...
Deploy using the Beadsmith Enterprise CLI:

```bash
# Install GitHub Enterprise integration
beadsmith-enterprise mcp install github-enterprise \
  --version 2.1.0 \
  --config-file github-config.yaml \
  --environment production

# Install Slack Enterprise Grid integration  
beadsmith-enterprise mcp install slack-enterprise-grid \
  --version 1.5.2 \
...
beadsmith-enterprise mcp list --environment production

# Check server status
beadsmith-enterprise mcp status github-enterprise --environment production
```
...
slack:
  ...
  channels:
    notifications:
      ...
      - name: "#ai-activity"
        types: ["beadsmith-tasks", "completions"]
  ...
  # Bot behavior
  bot:
    display_name: "Beadsmith Enterprise"
    default_channel: "#general"
    response_delay_ms: 1000
    
    commands:
      - command: "/beadsmith-status"
        description: "Check Beadsmith Enterprise status"
        permission: "all"
        
      - command: "/beadsmith-deploy"
...
beadsmith-enterprise mcp versions github-enterprise

# Upgrade to latest version
beadsmith-enterprise mcp upgrade github-enterprise --version 2.2.0 --environment staging

# Rollback to previous version
beadsmith-enterprise mcp rollback github-enterprise --version 2.1.0 --environment staging

# Pin to specific version (disable auto-updates)
beadsmith-enterprise mcp pin github-enterprise --version 2.1.0
```
...
beadsmith-enterprise mcp marketplace-status

# Verify authentication
beadsmith-enterprise auth verify --service marketplace

# Check installation logs
beadsmith-enterprise logs mcp-installer --lines 100
```

**Configuration Errors**:
```bash
# Validate configuration
beadsmith-enterprise mcp validate-config --file config.yaml

# Test connectivity 
beadsmith-enterprise mcp test-connection github-enterprise --environment staging

# Check server status
beadsmith-enterprise mcp status --all
```

**Performance Issues**:
```bash
# Check server metrics
beadsmith-enterprise mcp metrics github-enterprise --duration 1h

# View recent error logs
beadsmith-enterprise logs github-enterprise --level error --lines 50
```
...
- **Community**: Join the Beadsmith Enterprise community forum
