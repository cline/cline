...
description: "Configure LiteLLM proxy for your Beadsmith deployment"
---
...
Configure Beadsmith to use an existing LiteLLM proxy for unified access to multiple AI models through a single API endpoint.

## What is LiteLLM?

[LiteLLM](https://github.com/BerriAI/litellm) is an open-source proxy that provides a unified OpenAI-compatible API for accessing 100+ AI models from different providers. Beadsmith connects to your deployed LiteLLM instance.

<Note>
LiteLLM is a separate service you deploy and manage. This guide covers how to configure Beadsmith to connect to an existing LiteLLM deployment.
</Note>
...
Before configuring Beadsmith to use LiteLLM, you need:
...
4. **Network Access** from where Beadsmith is being used
...
Verify the model is configured in your LiteLLM deployment. Model IDs in Beadsmith's config must match the model names in LiteLLM's configuration.
