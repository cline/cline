...
This guide covers self-hosted infrastructure deployment with multi-account AWS, VPC endpoints, CloudTrail logging, YAML configs, and advanced security features. For simple web-based setup, see [AWS Bedrock SaaS Configuration](/enterprise-solutions/configuration/remote-configuration/aws-bedrock/admin-configuration).
</Info>

AWS Bedrock provides enterprise-grade access to foundation models through Amazon's cloud infrastructure. This guide covers integrating Bedrock with your self-hosted Cline Enterprise deployment for scalable, compliant AI operations with advanced security and cost management features.
