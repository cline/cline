...
**File:** `.beadsmithrules/workflows/deploy.md`
...
When invoked, Beadsmith adds the workflow instructions to its context for that specific task.

## Creating Workflows

<Tabs>
  <Tab title="Manual Creation">
    Create workflow files in the `.beadsmithrules/workflows/` directory:
    
    1. Create `.beadsmithrules/workflows/` in your repository root
  ...
  <Tab title="Slash Command">
    ...
    3. Beadsmith creates the workflow file in `.beadsmithrules/workflows/`
...
The workflow instructions are added to Beadsmith's context for the current task only.
...
<CardGroup cols={2}>
  <Card title="Workspace Workflows" icon="folder">
    **Location:** `.beadsmithrules/workflows/` in your repository
  ...
  <Card title="Global Workflows" icon="globe">
    **Location:** `Documents/Beadsmith/Workflows/` directory
