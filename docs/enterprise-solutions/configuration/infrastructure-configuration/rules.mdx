...
title: "Rules"
sidebarTitle: "Rules"
description: "Custom instruction files that guide Beadsmith's behavior in your enterprise deployment"
---

Rules are custom instruction files that provide Beadsmith with guidelines about your coding preferences, standards, and best practices. These instructions get added to Beadsmith's context when working on tasks.

## What are Rules?

Rules are simple markdown files stored in a `.beadsmithrules/` directory that contain your team's conventions, preferences, and guidelines. They help Beadsmith understand your:
...
<Tabs>
  <Tab title="Using /newrule Command">
    ...
    1. During a conversation with Beadsmith, type `/newrule`
    2. Beadsmith will analyze your conversation and preferences
    3. It creates an appropriately named `.md` file in `.beadsmithrules/`
  ...
  <Tab title="Manual Creation">
    ...
    1. Create a `.beadsmithrules/` directory in your repository root
...
<CardGroup cols={2}>
  <Card title="Workspace Rules" icon="folder">
    **Location:** `.beadsmithrules/` in your repository
  ...
  <Card title="Global Rules" icon="globe">
    **Location:** `Documents/Beadsmith/` directory
...
1. Click the rules icon in Beadsmith's interface
2. Toggle rules on/off as needed
3. Changes apply immediately to new tasks

<Note>
Disabling a rule removes it from Beadsmith's context, but keeps the file intact. You can re-enable it anytime.
</Note>
...
Beadsmith also respects rules from other AI coding tools:
...
<Note>
**AGENTS.md behavior:** Beadsmith only searches for nested `AGENTS.md` files recursively if a top-level `AGENTS.md` exists in your workspace root. If found, all `AGENTS.md` files are combined with their relative paths as headers.
</Note>

These files work the same way as `.beadsmithrules/` files and can be toggled on/off independently.
...
<AccordionGroup>
  ...
  <Accordion title="Don't Overdo It" icon="gauge-simple-high">
    Too many rules can overwhelm Beadsmith's context:
