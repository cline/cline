...
The VS Code extension exposes an MCP marketplace browse/install surface in the Customize view. For Enterprise administrators, the legacy remote configuration field names govern both the marketplace and MCP server access: marketplace availability, local server availability, local server allowlisting, organization-managed remote MCP servers, and blocking personal remote MCP servers.
...
| Setting | Purpose |
|---------|---------|
| `mcpMarketplaceEnabled` | Allow or block the MCP marketplace and locally configured MCP servers |
| `allowedMCPServers` | Restrict the MCP marketplace and locally configured MCP servers to approved server IDs |
...
- The MCP marketplace is hidden and MCP marketplace installs are blocked
...
- The MCP marketplace is available unless restricted by `allowedMCPServers`
...
- The MCP marketplace shows only entries matching an allowed `id`, and installs of other entries are blocked. Marketplace entries match when the `id` equals the entry's marketplace id, display name, installed server name, or source repository URL
...
- No local server or marketplace allowlist restriction is applied
