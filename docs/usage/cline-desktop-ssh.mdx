...
SSH does not use your system's HTTP(S) proxy, even when the rest of Cline Desktop does. If the host is only reachable through a proxy or a bastion, tell your SSH client how to get there and Cline follows along, because it runs your own `ssh` and reads your `~/.ssh/config`.

Define an alias with the route, then enter the alias in the **SSH host** field:
...
`%h` and `%p` expand to the host and port. On Windows, `nc` is not included; use `connect.exe` from Git for Windows instead, for example `ProxyCommand connect -H proxy.example.com:3128 %h %p`.

Test it from a terminal first: `ssh build-box` should log you in with no prompts. Then add `build-box` as the **SSH host** in Cline and leave **User** and **Port** blank so the alias supplies them.
...
| `Connection timed out during banner exchange` or `Connection to UNKNOWN port 65535 timed out` | The host sits behind a `ProxyJump` or `ProxyCommand`, and the hop through the bastion took longer than Cline's 30 second connection budget. | Check that `ssh bastion` logs in quickly with no prompts (a passphrase prompt on the bastion stalls the hop), and that the bastion can reach the host. |
