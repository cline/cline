...
| **Proxy command (optional)** | Only if the host is behind a proxy or jump host. See [Behind a proxy or jump host](#behind-a-proxy-or-jump-host). |
...
## Behind a proxy or jump host

SSH does not use your system's HTTP(S) proxy, even when the rest of Cline Desktop does. If the host is only reachable through a proxy or a bastion, tell your SSH client how to get there and Cline follows along, because it runs your own `ssh`.

The most flexible way is `~/.ssh/config`. Define an alias with the route, then enter the alias in the **SSH host** field:
...
Host build-box
  HostName 10.0.4.12
  User dev
  ProxyJump me@bastion.example.com

# Through an HTTP proxy (macOS / Linux, using netcat)
Host build-box-via-proxy
  HostName dev.example.com
  User dev
  ProxyCommand nc -X connect -x proxy.example.com:3128 %h %p

# Through a local SOCKS proxy such as Clash or v2ray
Host build-box-via-socks
  HostName dev.example.com
  User dev
  ProxyCommand nc -X 5 -x 127.0.0.1:7890 %h %p
```

Test it from a terminal first: `ssh build-box` should log you in with no prompts. Then add `build-box` as the **SSH host** in Cline and leave **User** and **Port** blank.

If you only need a `ProxyCommand` and would rather not edit SSH config, enter the same command in the **Proxy command** field when adding the host. `%h` and `%p` expand to the host and port. On Windows, `nc` is not included; use `connect.exe` from Git for Windows (`connect -H proxy.example.com:3128 %h %p`) or another helper that is on your `PATH`.
...
1. Cline runs your system `ssh` with `BatchMode=yes` and `StrictHostKeyChecking=yes`, plus `-i <identity file>`, `-p <port>`, and `-o ProxyCommand=<command>` if you filled those in. It never stores passwords or private-key contents; host settings live in `~/.cline/data/settings/remote-environments.json` with owner-only permissions.
