...
| Invocation | Reason |
|---|---|
| `cline --autoapprove true "task"` | Auto-approval mode for unattended execution |
...
```bash
# plan-first
cline --autoapprove true -p "design migration plan"

# act immediately
cline --autoapprove true -a "apply migration"
```
...
```bash
cline --autoapprove true --timeout 600 "run full test suite"
```
