...
<Tabs>
  <Tab title="VS Code / Cursor / Windsurf">
    ...
    <Tip>
    **Can't find the Cline icon?** Open the Command Palette with `Cmd+Shift+P` (Mac) or `Ctrl+Shift+P` (Windows/Linux), type **"Cline: Jump to Chat Input"**, and press Enter. This opens the Cline panel and puts your cursor in the chat box, even if the icon is currently hidden from the Activity Bar.

    If the icon is still missing afterwards, it was probably hidden: right-click the Activity Bar and make sure **Cline** is checked.
    </Tip>
  </Tab>
  <Tab title="JetBrains">
