...
- **Reasoning level** — control how much model-side thinking to request (off, low, medium, high, or extra high) from the client's session settings; thinking streams to the client as it happens. Preset it by adding `--thinking <level>` to the launch args, e.g. `["--acp", "--thinking", "medium"]`. Off by default.
