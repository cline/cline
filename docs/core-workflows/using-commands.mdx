...
Beyond the built-in slash commands, Cline still supports workflow files that can be invoked as `/your-workflow.md`. Use this compatibility path only when you specifically need file-based slash commands; for new reusable guidance, prefer [Skills](/customization/skills).

For migration guidance and the remaining compatibility behavior, see [Workflows](/customization/workflows).
