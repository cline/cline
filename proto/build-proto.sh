...
import { register${service_name^}Method } from "./index";
EOL
...
export function registerAll${service_name^}Methods(): void {
...
EOL
  ...
  for file in "$service_dir"/*.ts; do
    ...
    # Skip index.ts and methods.ts
    if [[ "$base_name" != "index" && "$base_name" != "methods" ]]; then
      echo "  register${service_name^}Method(\"$base_name\", $base_name);" >> "$service_dir/$registry_file"
    fi
  done
