...
- **Run it (dev host):** `DISPLAY=:1 code --no-sandbox --disable-dev-shm-usage --user-data-dir=/tmp/vscode-userdata --extensionDevelopmentPath=/workspace/apps/vscode <some-folder>`, then click the Cline icon in the Activity Bar to open the webview. (`--no-sandbox` is required in this container.)
- **`--disable-dev-shm-usage` is required in this container.** `/dev/shm` here is only 64 MB. VS Code webviews are Chromium renderers that put shared memory in `/dev/shm`; the Cline panel's webview exhausts 64 MB (opening it, and especially `Developer: Reload Window`), so the renderer dies with `renderer process gone (reason: crashed, code: 133)` and the "window terminated unexpectedly (code: '133')" dialog — an environment artifact, not an extension bug. `--disable-dev-shm-usage` routes Chromium shared memory to a temp file instead and eliminates the crash (verified: 0 crashes across panel-open + 3 reloads at 64 MB `/dev/shm` with the flag, vs a crash on essentially every open/reload without it). Alternatively `sudo mount -o remount,size=2G /dev/shm` also fixes it. The pre-installed `/usr/local/bin/google-chrome` wrapper already passes this flag for the same reason.
