...
- `.agents/.aidlc-rule-details/` (Cursor, Cline, Claude Code, GitHub Copilot)
- `.agents/aws-aidlc-rule-details/` (Kiro IDE and CLI)
...
- `.agents/.aidlc-rule-details/*` (27 files: common/, inception/, construction/, operations/, extensions/)
...
- `.agents/skills/*` — custom workflow skills (source of truth)
- `.claude/skills`, `.codex/skills` — IDE-specific runtime links to `.agents/skills`
...
- `.agents/workflow-bundle/*` — bundle metadata (manifest, compatibility, foundation version)
...
**Validation**: Run `.agents/skills/meta-knowledge/scripts/validate_layer1_integrity.py` to verify Layer1 remains unmodified.

## MANDATORY: Project-Scoped Workflow Metadata
When present, use `.agents/workflow-bundle/bundle-manifest.json`, `.agents/workflow-bundle/compatibility.md`, and `.agents/skills/skills.json` as project-local workflow metadata.
