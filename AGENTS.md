...
- Cloud VMs inject provider keys under **non-standard secret names** (`AnthropicKEy`, `OpenrouterAPIKey`, `openAIKey`) — the CLI does not auto-read these. To run a real end-to-end agent turn, authenticate non-interactively into a sandbox data dir and run a one-shot prompt, e.g. `bun run cli auth -p anthropic -k "$AnthropicKEy" -m claude-sonnet-5 --data-dir /tmp/cline-test` then `bun --conditions=development /workspace/apps/cli/src/index.ts "<prompt>" --data-dir /tmp/cline-test -y` (the `-y` flag auto-approves tools). Use `--data-dir` to avoid touching `~/.cline`.
