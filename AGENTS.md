...
- **Pre-installed on startup:** the startup script builds the extension (`bun run package` + `bunx vsce package --no-dependencies --allow-package-secrets sendgrid --out dist/cline.vsix`) and installs `apps/vscode/dist/cline.vsix` into **both** editors — VS Code stable (`code`) and **VS Code Insiders** (`code-insiders`, installed from the Microsoft apt repo). So `saoudrizwan.claude-dev` is already listed by `code --list-extensions` / `code-insiders --list-extensions` at session start. This packaged install is independent of the `--extensionDevelopmentPath` dev-host flow above; after editing extension source, rebuild + repackage + reinstall the vsix (or use the dev host) to pick up changes.
