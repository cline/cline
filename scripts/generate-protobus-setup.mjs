...
async function generateVscodeServiceTypes(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			...
			if (!rpc.responseStream) {
				servers.push(`     ${rpcName}: JsonUnaryHandler<${requestType}, ${responseType}>`)
			} else {
				servers.push(`     ${rpcName}: JsonStreamingHandler<${requestType}, ${responseType}>`)
			}
		}
		servers.push(`}\n`)
	}
...
// Generated by ${SCRIPT_NAME}
import * as proto from "@shared/proto/index"
import { JsonUnaryHandler, JsonStreamingHandler } from "@/core/controller/grpc-handler"
...
async function generateVscodeProtobusServers(protobusServices) {
	...
	for (const [serviceName, def] of Object.entries(protobusServices)) {
		...
		for (const [rpcName, rpc] of Object.entries(def.service)) {
			...
			servers.push(
				`    ${rpcName}: {handler: ${rpcName}, decodeRequest: ${requestType}.fromJSON, encodeResponse: ${responseType}.toJSON},`,
			)
		}
		servers.push(`} \n`)
