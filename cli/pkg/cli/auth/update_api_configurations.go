...
func AddProviderPartial(ctx context.Context, manager *task.Manager, provider cline.ApiProvider, modelID string, credentials *ProviderCredentials, modelInfo interface{}) error {
	...
	// Set API key field
	if credentials.APIKey != "" || fields.APIKeyField != "ollamaBaseUrl" {
		setAPIKeyField(apiConfig, fields.APIKeyField, proto.String(credentials.APIKey))
	}

	// Set base URL for OpenAI Native if provided
	if provider == cline.ApiProvider_OPENAI_NATIVE && credentials.BaseURL != "" {
		apiConfig.OpenAiBaseUrl = proto.String(credentials.BaseURL)
	}
	...
	// Add base URL to field mask for OpenAI Native if provided
	if provider == cline.ApiProvider_OPENAI_NATIVE && credentials.BaseURL != "" {
