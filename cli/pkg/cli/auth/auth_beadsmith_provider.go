...
// Beadsmith provider specific code

func HandleBeadsmithAuth(ctx context.Context) error {
	verboseLog("Authenticating with Beadsmith...")
	...
	// Configure default Beadsmith model after successful authentication
	if err := configureDefaultBeadsmithModel(ctx); err != nil {
		fmt.Printf("Warning: Could not configure default Beadsmith model: %v\n", err)
		fmt.Println("You can configure a model later with 'beadsmith auth' and selecting 'Change Beadsmith model'")
	}
...
func signOut(ctx context.Context) error {
	...
	if _, err = client.Account.AccountLogoutClicked(ctx, &beadsmith.EmptyRequest{}); err != nil {
		return err
	}

	isSessionAuthenticated = false
	fmt.Println("You have been signed out of Beadsmith.")
	return nil
}
...
func signOutDialog(ctx context.Context) error {
	...
	form := huh.NewForm(
		huh.NewGroup(
			huh.NewConfirm().
				Title("You are already signed in to Beadsmith.").
...
func signIn(ctx context.Context) error {
	...
	// Initiate login (opens browser with callback URL from beadsmith-core's AuthHandler)
	...
	response, err := client.Account.AccountLoginClicked(ctx, &beadsmith.EmptyRequest{})
	if err != nil {
	...
	if err := listener.WaitForAuthentication(5 * time.Minute); err != nil {
		...
		fmt.Println("  Please try again with 'beadsmith auth'")
		return err
	}
...
func IsAuthenticated(ctx context.Context) bool {
	...
	_, err = client.Account.GetUserCredits(ctx, &beadsmith.EmptyRequest{})
	if err == nil {
...
// HandleChangeBeadsmithModel allows Beadsmith-authenticated users to change their Beadsmith model selection. Hidden when not authenticated.
func HandleChangeBeadsmithModel(ctx context.Context) error {
	// Ensure user is authenticated
	if !IsAuthenticated(ctx) {
		return fmt.Errorf("you must be authenticated with Beadsmith to change models. Run 'beadsmith auth' to sign in")
	}
	...
	// Launch Beadsmith model selection
	return SelectBeadsmithModel(ctx, manager)
}

// configureDefaultBeadsmithModel configures the default Beadsmith model after authentication
func configureDefaultBeadsmithModel(ctx context.Context) error {
	verboseLog("Configuring default Beadsmith model...")
	...
	// Set default Beadsmith model
	return SetDefaultBeadsmithModel(ctx, manager)
}

// HandleSelectOrganization allows Beadsmith-authenticated users to select which organization to use
func HandleSelectOrganization(ctx context.Context) error {
	// Ensure user is authenticated
	if !IsAuthenticated(ctx) {
		return fmt.Errorf("you must be authenticated with Beadsmith to select an organization. Run 'beadsmith auth' to sign in")
	}
	...
	orgsResponse, err := client.Account.GetUserOrganizations(ctx, &beadsmith.EmptyRequest{})
	if err != nil {
	...
	if len(organizations) == 0 {
		...
		fmt.Println("Visit https://app.beadsmith.bot/dashboard to create an organization.")
		return HandleAuthMenuNoArgs(ctx)
	}
	...
	req := &beadsmith.UserOrganizationUpdateRequest{
