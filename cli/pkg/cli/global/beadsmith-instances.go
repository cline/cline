...
type BeadsmithInstances struct {
	registry *InstanceRegistry
}

// NewBeadsmithInstances creates a new BeadsmithInstances instance
func NewBeadsmithInstances(configPath string) *BeadsmithInstances {
	registry := NewInstanceRegistry(configPath)
	return &BeadsmithInstances{
		registry: registry,
	}
}

// Initialize performs cleanup of stale instances
func (c *BeadsmithInstances) Initialize(ctx context.Context) error {
...
// StartNewInstance starts a new Beadsmith instance and waits for beadsmith-core to self-register
// An "instance" is a pair of beadsmith-core and beadsmith-host processes
func (c *BeadsmithInstances) StartNewInstance(ctx context.Context, workspaces ...string) (*common.CoreInstanceInfo, error) {
	...
	if Config.Verbose {
		fmt.Printf("Starting new Beadsmith instance on ports %d (core) and %d (host bridge)\n", corePort, hostPort)
	}

	// Start beadsmith-host first
	hostCmd, err := startClineHost(hostPort, workspaces)
	if err != nil {
		return nil, fmt.Errorf("failed to start beadsmith-host: %w", err)
	}

	// Start beadsmith-core (it will register itself in SQLite locks database)
	...
	if err != nil {
		...
		return nil, fmt.Errorf("failed to start beadsmith-core: %w", err)
	}
...
// StartNewInstanceAtPort starts a new Beadsmith instance at the specified port and waits for self-registration
func (c *BeadsmithInstances) StartNewInstanceAtPort(ctx context.Context, corePort int, workspaces ...string) (*common.CoreInstanceInfo, error) {
	...
	if common.IsInstanceHealthy(ctx, coreAddress) {
		return nil, fmt.Errorf("port %d is already in use by another Beadsmith instance", corePort)
	}
...
func (c *BeadsmithInstances) GetRegistry() *InstanceRegistry {
	return c.registry
}

// EnsureInstanceAtAddress ensures an instance exists at the given address, starting one if needed
func (c *BeadsmithInstances) EnsureInstanceAtAddress(ctx context.Context, address string) error {
	// Expect host:port everywhere
	normalized := address
	if normalized == "" {
		normalized = fmt.Sprintf("localhost:%d", common.DEFAULT_BEADSMITH_CORE_PORT)
	}
...
func startClineHost(hostPort int, workspaces []string) (*exec.Cmd, error) {
	if Config.Verbose {
		fmt.Printf("Starting beadsmith-host on port %d\n", hostPort)
	}
	...
	clineHostPath := path.Join(binDir, "beadsmith-host")
	...
	// Start the beadsmith-host process
	cmd := exec.Command(clineHostPath, args...)

	// Create logs directory in ~/.beadsmith/logs
	...
	logFileName := fmt.Sprintf("beadsmith-host-%s-localhost-%d.log", timestamp, hostPort)
	...
	if Config.Verbose {
		fmt.Printf("Started beadsmith-host (PID: %d)\n", cmd.Process.Pid)
		fmt.Printf("Logging beadsmith-host output to: %s\n", logFilePath)
	}
	return cmd, nil
}

// KillInstanceByAddress kills a Beadsmith instance by its address
...
func startClineCore(corePort, hostPort int) (*exec.Cmd, error) {
	if Config.Verbose {
		fmt.Printf("Starting beadsmith-core on port %d (with hostbridge on %d)\n", corePort, hostPort)
	}
	...
	// /opt/homebr../bin/beadsmith -> /opt/homebrew/lib/node_modules/cli../bin/beadsmith
	...
	clineCorePath := path.Join(installDir, "beadsmith-core.js")
	...
	if Config.Verbose {
		...
		fmt.Printf("Looking for beadsmith-core.js at: %s\n", clineCorePath)
	}

	// Check if beadsmith-core.js exists at the primary location
	var finalClineCorePath string
	var finalInstallDir string
	if _, err := os.Stat(clineCorePath); os.IsNotExist(err) {
		// Development mode: Try ../../dist-standalone/beadsmith-core.js
		// This handles the case where we're running from c../bin/beadsmith
		devClineCorePath := path.Join(binDir, "..", "..", "dist-standalone", "beadsmith-core.js")
		...
		if _, err := os.Stat(devClineCorePath); os.IsNotExist(err) {
			return nil, fmt.Errorf("beadsmith-core.js not found at '%s' or '%s'. Please ensure you're running from the correct location or reinstall with 'npm install -g cline'", clineCorePath, devClineCorePath)
		}

		finalClineCorePath = devClineCorePath
		finalInstallDir = devInstallDir
		if Config.Verbose {
			fmt.Printf("Using development mode: beadsmith-core.js found at %s\n", finalClineCorePath)
		}
	} else {
		finalClineCorePath = clineCorePath
		finalInstallDir = installDir
		if Config.Verbose {
			fmt.Printf("Using production mode: beadsmith-core.js found at %s\n", finalClineCorePath)
		}
	}
	...
	// Create timestamped log file
	timestamp := time.Now().Format("2006-01-02-15-04-05")
	logFileName := fmt.Sprintf("beadsmith-core-%s-localhost-%d.log", timestamp, corePort)
	logFilePath := path.Join(logsDir, logFileName)
	logFile, err := os.Create(logFilePath)
	if err != nil {
		return nil, fmt.Errorf("failed to create log file: %w", err)
	...
	// Start the beadsmith-core process with --config flag using system node
	...
	if Config.Verbose {
		fmt.Printf("Started beadsmith-core (PID: %d)\n", cmd.Process.Pid)
		fmt.Printf("Logging beadsmith-core output to: %s\n", logFilePath)
	}
	return cmd, nil
}
