...
Global settings, secrets and workspace state are stored in **file-backed JSON stores** under `~/.bedrock-coder/data/`. This is the shared storage layer used by VSCode, CLI, and JetBrains.

## Key Abstractions

### `StorageContext` (src/shared/storage/storage-context.ts)
The entry point. Created via `createStorageContext()` and passed to `StateManager.initialize()`. Contains three `BedrockCoderFileStorage` instances:
- `globalState` → `~/.bedrock-coder/data/globalState.json`
- `secrets` → `~/.bedrock-coder/data/secrets.json` (mode 0o600)
- `workspaceState` → `~/.bedrock-coder/data/workspaces/<hash>/workspaceState.json`

### `BedrockCoderFileStorage` (src/shared/storage/BedrockCoderFileStorage.ts)
...
Remember that your data may be read by a different client than the one that wrote it. For example, a value written by BedrockCoder in JetBrains may be read by BedrockCoder CLI.
