# Bedrock Coder Hooks Documentation

## Overview

Bedrock Coder hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/.bedrock-coder/hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.bedrock-coder/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open Bedrock Coder settings in VSCode
...
- **Global Location**: `~/.bedrock-coder/hooks/TaskStart`
- **Workspace Location**: `.bedrock-coder/hooks/TaskStart`
...
- **Global Location**: `~/.bedrock-coder/hooks/TaskResume`
- **Workspace Location**: `.bedrock-coder/hooks/TaskResume`
...
- **Global Location**: `~/.bedrock-coder/hooks/TaskCancel`
- **Workspace Location**: `.bedrock-coder/hooks/TaskCancel`
...
- **Global Location**: `~/.bedrock-coder/hooks/TaskComplete`
- **Workspace Location**: `.bedrock-coder/hooks/TaskComplete`
...
- **Global Location**: `~/.bedrock-coder/hooks/UserPromptSubmit`
- **Workspace Location**: `.bedrock-coder/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/.bedrock-coder/hooks/PreToolUse`
- **Workspace Location**: `.bedrock-coder/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/.bedrock-coder/hooks/PostToolUse`
- **Workspace Location**: `.bedrock-coder/hooks/PostToolUse`
...
- **Global Location**: `~/.bedrock-coder/hooks/PreCompact`
- **Workspace Location**: `.bedrock-coder/hooks/PreCompact`

## Cross-Platform Hook Format

Bedrock Coder uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, Bedrock Coder executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/.bedrock-coder/hooks/PreToolUse

# Make executable
chmod +x ~/.bedrock-coder/hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.bedrock-coder/hook-logs/tool-usage.jsonl
...
Bedrock Coder supports two levels of hooks:

### Global Hooks
- **Location**: `~/.bedrock-coder/hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.bedrock-coder/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/.bedrock-coder/hooks/`
...
3. Enable hooks in Bedrock Coder settings
...
```bash
#!/usr/bin/env bash
# ~/.bedrock-coder/hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.bedrock-coder/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (Bedrock Coder channel)
