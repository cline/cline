...
The debugee runs with `BEDROCK_CODER_DIR=~/.bedrock-coder2` by default, separate from your real `~/.bedrock-coder`.
This prevents the debugee's logout from logging out the debugger, and vice versa.
Override with `--bedrock-coder-dir /tmp/test-dir`. Check with `status()` → `bedrockCoderDir`.

## Browser Capture & OAuth

The debugee runs with `BEDROCK_CODER_CAPTURE_BROWSER=1`, which intercepts `openExternal()` in
`src/utils/env.ts`. URLs are captured instead of opening a real browser:

- Logged to `$BEDROCK_CODER_DIR/data/debug-captured-urls.jsonl`
...
For **BedrockCoder OAuth** (SDK local callback): The SDK starts a local HTTP server, the auth URL
...
when `BEDROCK_CODER_CAPTURE_BROWSER` is set (the harness always sets it; never ships in prod).
...
| Command | View |
|---------|------|
| `bedrockCoder.accountButtonClicked` | Account / sign-in |
| `bedrockCoder.historyButtonClicked` | Task history |
| `bedrockCoder.settingsButtonClicked` | Settings |
| `bedrockCoder.mcpButtonClicked` | MCP servers |
| `bedrockCoder.plusButtonClicked` | New task (chat) |
| `bedrockCoder.worktreesButtonClicked` | Worktrees |
...
- **`ui.screenshot`** — screenshot to `/tmp/bedrock-coder-debug/`; returns `{path}` — **use `read_file` on the path to examine, do NOT `open` the file** (Preview.app covers the VSCode window)
...
- **⚠️ Dismiss promotional overlays FIRST**: On fresh launches, full-screen promo overlays block the sidebar. **Dismiss immediately after `ui.open_sidebar`**, before any other interaction or screenshot. May need to run twice:
  ...
  curl localhost:19229/api -d '{"method": "web.evaluate", "params": {"expression": "document.querySelectorAll(\".sr-only\").forEach(el => el.parentElement?.click())"}}'
  ```
- **Screenshots — don't open the file**: `ui.screenshot` and `ui.sidebar_screenshot` save PNGs to `/tmp/bedrock-coder-debug/` and return the `{path}`. Use `read_file` on that path to examine screenshots. Running `open <path>` launches Preview.app on macOS which covers the VSCode window.
