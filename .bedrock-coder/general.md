...
## Miscellaneous
- The whole repo (including `apps/vscode`) uses **bun** for package management and task running. Emit `bun run X` / `bun install` / `bunx <bin>` / `bun file.ts`, never npm/npx. Node remains the *runtime* (VS Code's extension host and the standalone bedrock-coder-core are Node), so Node-runtime tokens are legitimate and must not be "fixed" to bun — see @.bedrock-coder/bun-and-node.md for the keep-list vs rewrite-list.
...
- When reading a configuration files that users may edit, use `readFileStrippingUtf8Bom`, `readFileSyncStrippingUtf8Bom`, or `stripUtf8Bom` from `@bedrock-coder/shared/node`. DON'T strip byte order marks of user files handled by tools/passed to models.
- When creating PRs, contributors should not create changelog-entry files. Maintainers handle release versioning and changelog curation during the release process.
- When adding new feature flags, see this PR as a reference https://github.com/FFFalexgo/AWS_Bedrock_Coder/pull/7566
- Additional instructions about making requests: @.bedrock-coder/network.md
...
**Proto files live in `proto/`** (e.g., `proto/bedrock_coder/task.proto`, `proto/bedrock_coder/ui.proto`)
- Each feature domain has its own `.proto` file
- For simple data, use shared types in `proto/bedrock_coder/common.proto` (`StringRequest`, `Empty`, `Int64Request`)
...
**Adding new enum values** (like a new `BedrockCoderSay` type) requires updating conversion mappings in `src/shared/proto-conversions/bedrock-coder-message.ts`
...
- `proto/bedrock_coder/task.proto` - Added `ExplainChangesRequest` message and `explainChanges` RPC
- `proto/bedrock_coder/ui.proto` - Added `GENERATE_EXPLANATION = 29` to `BedrockCoderSay` enum
- `src/shared/ExtensionMessage.ts` - Added `BedrockCoderSayGenerateExplanation` type
- `src/shared/proto-conversions/bedrock-coder-message.ts` - Added mapping for new say type
...
- Add the field to `UpdateSettingsRequest` in `proto/bedrock_coder/state.proto` (for webview update requests), then run `bun run protos`
...
run the harness from a process that was itself spawned by VSCode (e.g. the BedrockCoder
