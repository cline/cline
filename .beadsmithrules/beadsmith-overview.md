# Beadsmith Extension Architecture & Development Guide

## Project Overview

Beadsmith is a VSCode extension that provides AI assistance through a combination of a core extension backend and a React-based webview frontend. The extension is built with TypeScript and follows a modular architecture pattern.
...
The WebviewProvider maintains a reference to the Controller and delegates message handling to it. It also handles the creation of both sidebar and tab panel webviews, allowing Beadsmith to be used in different contexts within VSCode.
...
Beadsmith supports multiple AI providers through a modular API provider system. Each provider is implemented as a separate module in the `src/api/providers/` directory and follows a common interface.
...
Beadsmith supports separate model configurations for Plan and Act modes:
...
class Task {
  async initiateTaskLoop(userContent: UserContent, isNewTask: boolean) {
    while (!this.abort) {
      ...
      const recDidEndLoop = await this.recursivelyMakeBeadsmithRequests(
...
class Task {
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const previousRequest = this.beadsmithMessages[previousApiReqIndex]
...
class Task {
  async resumeTaskFromHistory() {
    // 1. Load saved state
    this.beadsmithMessages = await getSavedBeadsmithMessages(this.getContext(), this.taskId)
  ...
  private async saveTaskState() {
    ...
    await saveBeadsmithMessages(this.getContext(), this.taskId, this.beadsmithMessages)
...
Beadsmith implements a dual-mode system that separates planning from execution:
...
Beadsmith supports two types of MCP server connections:
...
class Controller {
  ...
  async downloadMcp(mcpId: string) {
    ...
    const response = await axios.post<McpDownloadResponse>(
      "https://api.beadsmith.bot/v1/mcp/download",
      { mcpId },
      {
    ...
    await this.initBeadsmithWithTask(task)
  }
}
```
...
This guide provides a comprehensive overview of the Beadsmith extension architecture, with special focus on state management, data persistence, and code organization. Following these patterns ensures robust feature implementation with proper state handling across the extension's components.
...
Contributions to the Beadsmith extension are welcome! Please follow these guidelines:
...
The `.beadsmithignore` file allows users to specify files and directories that Beadsmith should not access. When implementing new features, respect the `.beadsmithignore` rules and ensure that your code does not attempt to read or modify ignored files.
