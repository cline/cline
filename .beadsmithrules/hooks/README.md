# Beadsmith Hooks Documentation

## Overview

Beadsmith hooks allow you to execute custom scripts at specific points in the agentic workflow. Hooks can be placed in either:
- **Global hooks directory**: `~/Documents/Beadsmith/Hooks/` (applies to all workspaces)
- **Workspace hooks directory**: `.beadsmithrules/hooks/` (applies to the workspace the repo is part of)

Hooks run automatically when enabled.

## Enabling Hooks

1. Open Beadsmith settings in VSCode
...
- **Global Location**: `~/Documents/Beadsmith/Hooks/TaskStart`
- **Workspace Location**: `.beadsmithrules/hooks/TaskStart`
...
- **Global Location**: `~/Documents/Beadsmith/Hooks/TaskResume`
- **Workspace Location**: `.beadsmithrules/hooks/TaskResume`
...
- **Global Location**: `~/Documents/Beadsmith/Hooks/TaskCancel`
- **Workspace Location**: `.beadsmithrules/hooks/TaskCancel`
...
- **Global Location**: `~/Documents/Beadsmith/Hooks/TaskComplete`
- **Workspace Location**: `.beadsmithrules/hooks/TaskComplete`
...
- **Global Location**: `~/Documents/Beadsmith/Hooks/UserPromptSubmit`
- **Workspace Location**: `.beadsmithrules/hooks/UserPromptSubmit`

### PreToolUse Hook
- **When**: Runs BEFORE a tool is executed
- **Purpose**: Validate parameters, block execution, or add context
- **Global Location**: `~/Documents/Beadsmith/Hooks/PreToolUse`
- **Workspace Location**: `.beadsmithrules/hooks/PreToolUse`

### PostToolUse Hook
- **When**: Runs AFTER a tool completes
- **Purpose**: Observe results, track patterns, or add context
- **Global Location**: `~/Documents/Beadsmith/Hooks/PostToolUse`
- **Workspace Location**: `.beadsmithrules/hooks/PostToolUse`
...
- **Global Location**: `~/Documents/Beadsmith/Hooks/PreCompact`
- **Workspace Location**: `.beadsmithrules/hooks/PreCompact`

## Cross-Platform Hook Format

Beadsmith uses a git-style approach for hooks that works consistently across all platforms:
...
Like git hooks, Beadsmith executes hook files through a shell that interprets the shebang line:
...
```bash
# Create hook file
nano ~/Documents/Beadsmith/Hooks/PreToolUse

# Make executable
chmod +x ~/Documents/Beadsmith/Hooks/PreToolUse
```
...
# Log to file
echo "$input" >> ~/.beadsmith/hook-logs/tool-usage.jsonl
...
Beadsmith supports two levels of hooks:

### Global Hooks
- **Location**: `~/Documents/Beadsmith/Hooks/` (macOS/Linux)
...
### Workspace Hooks
- **Location**: `.beadsmithrules/hooks/` in each workspace root
...
1. The global hooks directory is automatically created at:
   - macOS/Linux: `~/Documents/Beadsmith/Hooks/`
...
3. Enable hooks in Beadsmith settings
...
```bash
#!/usr/bin/env bash
# ~/Documents/Beadsmith/Hooks/PreToolUse
...
```bash
#!/usr/bin/env bash
# .beadsmithrules/hooks/PreToolUse
...
If you have multiple workspace roots, you can place hooks in each root's `.beadsmithrules/hooks/` directory. All hooks (global and workspace) may execute concurrently. Their results will be combined:
...
- Look for errors in VSCode's Output panel (Beadsmith channel)
