...
- When adding new feature flags, see this PR as a reference https://github.com/beadsmith/beadsmith/pull/7566
- Additional instructions about making requests: @.beadsmithrules/network.md

## gRPC/Protobuf Communication
The extension and webview communicate via gRPC-like protocol over VS Code message passing.

**Proto files live in `proto/`** (e.g., `proto/beadsmith/task.proto`, `proto/beadsmith/ui.proto`)
- Each feature domain has its own `.proto` file
- For simple data, use shared types in `proto/beadsmith/common.proto` (`StringRequest`, `Empty`, `Int64Request`)
...
**Adding new enum values** (like a new `BeadsmithSay` type) requires updating conversion mappings in `src/shared/proto-conversions/beadsmith-message.ts`
...
- `proto/beadsmith/task.proto` - Added `ExplainChangesRequest` message and `explainChanges` RPC
- `proto/beadsmith/ui.proto` - Added `GENERATE_EXPLANATION = 29` to `BeadsmithSay` enum
- `src/shared/ExtensionMessage.ts` - Added `BeadsmithSayGenerateExplanation` type
- `src/shared/proto-conversions/beadsmith-message.ts` - Added mapping for new say type
...
1. `proto/beadsmith/models.proto` - Add to the `ApiProvider` enum (e.g., `OPENAI_CODEX = 40;`)
...
1. **Add to `BeadsmithDefaultTool` enum** in `src/shared/tools.ts`
2. **Tool definition** in `src/core/prompts/system-prompt/tools/` (create file like `generate_explanation.ts`)
   - Define variants for each `ModelFamily` (generic, next-gen, xs, etc.)
   - Export variants array (e.g., `export const my_tool_variants = [GENERIC, NATIVE_NEXT_GEN, XS]`)
   - **Fallback behavior**: If a variant isn't defined for a model family, `BeadsmithToolSet.getToolByNameWithFallback()` automatically falls back to GENERIC. So you only need to export `[GENERIC]` unless the tool needs model-specific behavior.
...
8. **If tool has UI feedback**: add `BeadsmithSay` enum in proto, update `src/shared/ExtensionMessage.ts`, update `src/shared/proto-conversions/beadsmith-message.ts`, update `webview-ui/src/components/chat/ChatRow.tsx`
