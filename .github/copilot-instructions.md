# Copilot Instructions for BedrockCoder

This is a VS Code extension. Read `.bedrock-coder-rules/general.md` for tribal knowledge and nuanced patterns.
...
1. **Define** in `proto/bedrock_coder/*.proto`. Naming: `PascalCaseService`, `camelCase` RPCs, `PascalCase` Messages. Use `common.proto` shared types for simple data.
...
- Adding enums (e.g. `BedrockCoderSay`) → also update `src/shared/proto-conversions/bedrock-coder-message.ts`.
...
1. `proto/bedrock_coder/models.proto` — add to `ApiProvider` enum.
...
1. Add enum to `BedrockCoderDefaultTool` in `src/shared/tools.ts`.
...
6. If tool has UI: add `BedrockCoderSay` enum in proto → `ExtensionMessage.ts` → `bedrock-coder-message.ts` → `ChatRow.tsx`.
