...
particularly highlighting new features, bug fixes, and improvements while avoiding repetitive information.
...
def generate_release_notes(model_name):
    ...
    Uses GPT-4 to analyze change summary, commit log, and code diff
    to generate concise and informative release notes in markdown format. The notes
    focus on important changes and their impact, with sections for new features,
    ...
    openai_prompt = CUSTOM_PROMPT if CUSTOM_PROMPT else f"""Based on the following summary of changes, commit log and code diff, please generate concise and informative release notes for the Cline VSCode extension:
    Summary of changes:
    {change_summary}
    Commit log:
    {change_log}
