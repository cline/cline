...
def extract_important_section(pr_body):
    # Find content between ## Important and the next ## or end of text
    important_match = re.search(r'## Important\s*\n(.*?)(?=\n##|$)', pr_body, re.DOTALL)
    if important_match:
        return important_match.group(1).strip()
    return ""
...
important = extract_important_section(PR_BODY)
...
BASE_PROMPT = CUSTOM_PROMPT if CUSTOM_PROMPT else f"""Based on the following 'PR Information', please generate concise and informative release notes for the Cline VSCode extension.
