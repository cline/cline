...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "\n"
        "PLANNING RULE — whenever you build a multi-step plan (≥ 2 tool calls),\n"
        "the LAST item in your plan MUST be:\n"
        "  ✓ Update research.md (sync_research_context)\n"
        "Add it to the plan before you start executing. Do not add it as an\n"
        "afterthought — write it in the plan so you check it off like every other step.\n"
        "\n"
        "sync_research_context workflow (always two calls):\n"
        "  Call 1 — site_id only → returns all raw computed data from the session.\n"
        "  Call 2 — site_id + site_name + interpretation → writes to research.md.\n"
        "Write 3–6 sentences of scientific prose covering: what the data shows,\n"
        "cross-slot patterns, contradictions with researcher notes, and what to do next.\n\n"
