...
1. **Source of Truth**: `.agents/skills/<skill-name>/` at project root — where skills are developed and edited
2. **Inventory**: `.agents/skills/skills.json` — single source of truth for registration and bundle metadata
3. **Claude Runtime Link**: `.claude/skills -> ../.agents/skills`
4. **Codex Runtime Link**: `.codex/skills -> ../.agents/skills`

## Canonical Project Namespace

Project-owned workflow assets stay under `.agents/`:

1. `.agents/.aidlc-rule-details/` — vendored foundation rules
2. `.agents/skills/` — only editable skill source
3. `.agents/workflow-bundle/` — workflow metadata and bootstrap bundle
...
2. Prefer ownership-revealing locations under `.agents/skills/<skill-name>/...`
...
Add an entry to `.agents/skills/skills.json` under the `skills` array:
...
Both links should resolve to `../.agents/skills`.
...
root = Path('.').resolve()
skills = json.loads((root / '.agents/skills/skills.json').read_text())
errors = []
for skill in skills['skills']:
    entry = skill.get('entrypoint')
    if entry and not (root / '.agents/skills' / entry).exists():
    ...
    for key in ['scripts', 'references', 'templates']:
        ...
        if isinstance(value, str):
            value = [value]
        for path in value:
            if path and not (root / '.agents/skills' / path).exists():
...
1. Edit the source at `.agents/skills/<skill-name>/`
2. Keep `.agents/skills/skills.json` in sync
...
2. Create `.agents/skills/<name>/agents/openai.yaml` if needed
3. Register in `.agents/skills/skills.json`
4. Copy reusable scripts to `.agents/skills/<name>/scripts/`
