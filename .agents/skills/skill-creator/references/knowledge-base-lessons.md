...
- Editing `.agents/.aidlc-rule-details/*` for project-specific guidance
...
- Prefer **helper promotion** when reusable project logic lives outside `.agents/skills/`
- State the chosen mode explicitly before editing

## Script Separation Convention

1. **Reusable scripts** → `.agents/skills/<skill-name>/scripts/`
...
1. Source at `.agents/skills/<name>/`
2. Registered in `.agents/skills/skills.json`
3. `.claude/skills` must remain a symlink to `../.agents/skills`
4. `.codex/skills` must remain a symlink to `../.agents/skills`
5. Verify the new skill is reachable through both runtime links

## Canonical `.agents` Topology

- Project-owned workflow assets live under `.agents/`
