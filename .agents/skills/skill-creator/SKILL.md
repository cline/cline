...
It also enforces the repository topology contract: project-owned workflow assets belong under `.agents/`, while `.claude/` and `.codex/` stay thin runtime adapters rather than competing source locations.
...
- **New skill** — create a brand-new custom skill in `.agents/skills/<name>/`
...
3. Does the asset belong under the canonical `.agents/` namespace?
...
- **Helper promotion**
  ...
  - Re-home durable project logic into `.agents/skills/<skill-name>/`
...
1. Create or update the source at `.agents/skills/<skill-name>/`
2. Register or update `.agents/skills/skills.json`
...
- Canonical `.agents/` namespace respected
...
- Verify `.agents/skills/skills.json` entry is valid
...
- [ ] Registered in `.agents/skills/skills.json`
...
- [ ] Naming and placement follow the canonical `.agents/` topology rules
