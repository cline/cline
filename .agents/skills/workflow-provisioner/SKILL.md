...
description: Create and manage AI workflow projects as isolated git worktrees with project-scoped runtime setup. Use when starting a new workflow project that should run in its own worktree with dedicated CLAUDE.md, skills, and workflow definition files. Handles worktree creation, template scaffolding, .worktreeinclude file copying, canonical `.agents/` topology, and IDE workspace registration.
---
...
- `.agents/.aidlc-rule-details/` — vendored foundation rules
- `.agents/skills/` — only editable skill source
- `.agents/workflow-bundle/` — workflow metadata and bootstrap bundle
...
- The base repo already contains the canonical `.agents/` workflow roots
...
Canonical workflow assets to preserve:
  ...
  - `.agents/skills/` (only editable skill source)
  - `.agents/workflow-bundle/` (workflow metadata bundle)

Skills to expose through runtime links (worktree `.agents/skills/` → `.claude/skills`, `.codex/skills`):
...
8. **Skills** — Keep `.agents/skills/` as the only editable skill source in the worktree, then expose it through `.claude/skills` and `.codex/skills` runtime links:

   **Source of truth**: `.agents/skills/skills.json` in the base repo defines the available skill bundle.
   ...
   **Provisioning procedure**:
   1. Read `.agents/skills/skills.json` from the base repo
   ...
   3. Parse `worktreeSkillPolicy.optional` → present to user for selection:
      ...
      ```
   4. Copy `<base-repo>/.agents/skills/` to `<worktree>/.agents/skills/`
   5. Create runtime links:
      - `<worktree>/.claude/skills -> ../.agents/skills`
      - `<worktree>/.codex/skills -> ../.agents/skills`
   6. Verify the selected skills are reachable through both runtime links
   7. Verify `<worktree>/.claude/skills` and `<worktree>/.codex/skills` both resolve to `<worktree>/.agents/skills`
   8. Do not create copied per-IDE skill trees under `.claude/skills/<name>` or `.codex/skills/<name>`

   **Why shared runtime links**: `.agents/skills/` remains the only editable skill source, while Claude Code and Codex read the same bundle without copy drift or competing tool-specific sources of truth.
...
2. **Directory existence**: Confirm directories exist:
   - questions/, artifacts/, extensions/, `.agents/.aidlc-rule-details/`, `.agents/skills/`, `.agents/workflow-bundle/`, `.claude/`, `.codex/`
3. **Skills**: Confirm all skills are reachable through both runtime links:
   ...
   - Verify `.claude/skills` and `.codex/skills` are symlinks to `.agents/skills`
   - Verify no copied per-IDE skill trees were introduced under `.claude/` or `.codex/`
   - Verify `.agents/.aidlc-rule-details/`, `.agents/skills/`, and `.agents/workflow-bundle/` remain the only project-owned workflow roots
