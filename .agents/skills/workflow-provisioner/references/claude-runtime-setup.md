...
- `.agents/.aidlc-rule-details/` — project-owned foundation rules
- `.agents/skills/` — only editable skill source
- `.agents/workflow-bundle/` — workflow metadata and bootstrap assets

`.claude/skills` is only a runtime adapter to `.agents/skills`, not an independent source of truth. The same rule applies to `.codex/skills`.
...
5. **.claude/skills -> ../.agents/skills** — Runtime exposure of the project-local skill bundle

Claude reads the skill bundle through `.claude/skills`, but the editable source remains `.agents/skills`.
...
- Claude Code reads `.claude/skills -> ../.agents/skills`
- Codex reads `.codex/skills -> ../.agents/skills`
