...
const NO_API_KEY_NEEDED = new Set([
	"ollama",
	"lmstudio",
	"vscode-lm",
	"bedrock",
...
function hasCredential(id, entry) {
	...
	if (id === "bedrock")
		return Boolean(entry.awsAccessKey && entry.awsSecretKey);
...
if (listOnly) {
	...
	console.log(
		`not provided  (${notProvided.length}): ${notProvided.join(", ") || "(none)"}`,
	);
	for (const id of usable) {
		const e = keys[id];
		const masked = e.apiKey
			? `${String(e.apiKey).slice(0, 6)}…(${String(e.apiKey).length})`
			: "-";
		console.log(
			`  ${id.padEnd(20)} key=${masked.padEnd(16)} model=${e.model || "-"} baseUrl=${e.baseUrl || "-"}`,
		);
	}
	process.exit(0);
}
...
if (select && !providers[select]) {
	console.error(
		`apply-keys: --select ${select} has no usable credential in ${keysPath}`,
	);
	process.exit(2);
}
...
console.log(
	`apply-keys: wrote ${Object.keys(providers).length} provider(s) to ${target}`,
);
console.log(
	`apply-keys: lastUsedProvider=${stored.lastUsedProvider ?? "(unset)"}`,
);
