...
async function handleChatCompletions(req, res, body) {
	...
	switch (model) {
		...
		case "fault/401":
			return sendJson(res, 401, {
				error: {
					message: "Incorrect API key provided for fault/401.",
		...
		case "fault/500":
			return sendJson(res, 500, {
				error: {
					message: "The server had an error processing fault/500.",
					type: "server_error",
				},
			});
	...
	if (model === "fault/ok-drip") {
		...
		return streamText(
			...
			model,
			long,
			Number(process.env.QA_PROXY_DRIP_MS || 700),
		);
	}

	const text = `PONG [served-by:${model}]`;
	return stream
		? streamText(res, model, text)
		: nonStreamText(res, model, text);
}
...
const server = http.createServer(async (req, res) => {
	...
	return sendJson(res, 404, {
		error: {
			message: `qa-fault-proxy: no route for ${req.method} ${url}`,
			type: "invalid_request_error",
		},
	});
});
...
server.listen(PORT, HOST, () => {
	process.stdout.write(
		`qa-fault-proxy listening on http://${HOST}:${PORT}/v1 (log: ${LOG_FILE})\n`,
	);
});
