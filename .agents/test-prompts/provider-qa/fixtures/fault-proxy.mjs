...
const MODELS = {
	"fault/ok":
		"Streams a short reply plus usage with cache read/write token counts.",
	"fault/ok-no-cache":
		"Streams a short reply with usage but no cache token fields.",
	"fault/big-usage":
		"Streams a short reply reporting 1.2M input / 90k output tokens (cost sanity check).",
	"fault/zero-usage":
		"Streams a short reply with every usage counter set to zero.",
	"fault/no-usage": "Streams a short reply and omits the usage block entirely.",
	"fault/tool-edit":
		"Emits one `editor` tool call that rewrites qa.txt, then finishes.",
	"fault/tool-edit-and-run":
		"Emits an `editor` call and a `run_commands` call in the same response.",
	"fault/tool-duplicate":
		"Emits the identical `editor` tool call twice with different ids.",
	"fault/tool-mangled-args":
		"Emits an `editor` tool call whose arguments are invalid JSON.",
	"fault/tool-split-args":
		"Emits an `editor` call with arguments streamed one character per delta.",
	"fault/tool-unicode-args":
		"Emits an `editor` call whose args contain quotes, newlines and emoji.",
	"fault/401": "Rejects with HTTP 401 invalid_api_key.",
	"fault/402": "Rejects with HTTP 402 insufficient_credits.",
	"fault/429":
		"Rejects with HTTP 429 rate_limit_exceeded and a Retry-After header.",
	"fault/context-overflow": "Rejects with HTTP 400 context_length_exceeded.",
	"fault/500": "Rejects with HTTP 500 and a non-JSON body.",
	"fault/hang":
		"Accepts the request and never responds (open-ended silent hang).",
	"fault/truncated-stream":
		"Starts streaming text and then destroys the socket mid-stream.",
	"fault/slow-stream":
		"Streams one token every 3s for 10 tokens (cancellation testing).",
};
	...
	switch (model) {
		...
		case "fault/402":
			return sendJson(res, 402, {
				error: {
					message:
						"Insufficient credits. Add credits to your account to continue.",
		...
		case "fault/truncated-stream": {
			...
			write(
				res,
				chunk(model, { content: "This response will be cut off mid-" }),
			);
			await sleep(150);
			return res.destroy();
		}
		...
		case "fault/tool-edit": {
			...
			write(
				res,
				chunk(model, { content: "Replacing john with cline in qa.txt." }),
			);
			write(
				res,
				chunk(
					...
					toolCallDelta(
						...
						"call_fault_edit_1",
						"editor",
						JSON.stringify(EDITOR_ARGS),
					),
				),
			);
			write(
				res,
				chunk(model, {}, "tool_calls", { usage: usageBlock("cache") }),
			);
			res.write("data: [DONE]\n\n");
			return res.end();
		}
		...
		case "fault/tool-edit-and-run": {
			...
			write(
				res,
				chunk(model, { content: "Editing the file and then printing it." }),
			);
			write(
				res,
				chunk(
			...
			write(
				...
				chunk(
					...
					toolCallDelta(
						...
						"call_fault_run_2",
						"run_commands",
						JSON.stringify(RUN_COMMAND_ARGS),
					),
				),
			);
			write(
		...
		case "fault/tool-duplicate": {
			...
			write(
				res,
				chunk(model, {}, "tool_calls", { usage: usageBlock("no-cache") }),
			);
			res.write("data: [DONE]\n\n");
			return res.end();
		}
		...
		case "fault/tool-split-args": {
			...
			write(
				res,
				chunk(model, toolCallDelta(0, "call_fault_split", "editor", "")),
			);
		...
		case "fault/big-usage":
			return streamText(
				res,
				model,
				"Reporting an implausibly large usage block.",
				"big",
			);
		...
		case "fault/zero-usage":
			return streamText(
				res,
				model,
				"Reporting an all-zero usage block.",
				"zero",
			);
				...
				output: [
					{
						...
						role: "assistant",
						content: [
							{ type: "output_text", text: "PONG from the fault proxy." },
						],
					},
				],
				usage: {
...
const server = createServer(async (req, res) => {
	...
	// Control endpoints (/__*) are excluded from the count: `proxy count` is
	// the oracle for retry volume, and polling it must not inflate the answer.
	const isControl = url.pathname.startsWith("/__");
	if (!isControl) {
		requestSeq += 1;
	}
	const entry = {
	...
	if (!isControl) {
		log(entry);
		console.log(
			`[${entry.seq}] ${req.method} ${url.pathname} model=${body?.model ?? "-"}`,
		);
	}
	...
	return sendJson(res, 404, {
		error: {
			message: `No fault-proxy route for ${req.method} ${url.pathname}`,
		},
	});
});
...
server.listen(PORT, "127.0.0.1", () => {
	writeFileSync(LOG_PATH, "");
	console.log(
		`fault-proxy listening on http://127.0.0.1:${PORT}/v1  (request log: ${LOG_PATH})`,
	);
	console.log("Models:");
