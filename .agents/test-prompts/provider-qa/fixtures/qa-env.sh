...
cmd_start() {
	...
	local qa
	qa="$(qa_dir "$slug")"
	# Reuse an existing data dir so restart flows (stop, then start on the same
	# slug) keep the state under test — providers.json, seeded legacy configs.
	# Only a slug that has never run (or was removed) starts from scratch.
	if [ -d "$qa/data" ]; then
		yellow "Reusing existing data dir $qa/data. For a clean install: rm -rf $qa"
	else
		rm -rf "$qa"
	fi
	mkdir -p "$qa/workspace" "$qa/data/settings"
	[ -f "$qa/workspace/qa.txt" ] ||
