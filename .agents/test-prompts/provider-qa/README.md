...
`start` creates an isolated data directory and VS Code profile — reusing the slug's data directory when it
already exists, so state under test survives stop/start — optionally applies a keys file, launches, and
