...
2. Seed, then launch against the seeded directory:
   ...
   ```
3. Launch with `bash $QA/qa-env.sh start $SLUG`. `start` reuses an existing data directory, so the seeded files
   survive the launch — but always confirm with `state` that the seeded files are the ones in play.
