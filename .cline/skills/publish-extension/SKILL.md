...
**Do not use `ext-vscode-publish-stable.yml`.** It lets a bare `vsce publish` re-run `vscode:prepublish` and repack the source tree, and that upload timed out (`Request timeout: /_apis/gallery`, 180s after "Publishing ...") on all five attempts for v4.1.18 (2026-09-15/17) while the prebuilt-artifact upload of the same code line went through in seconds as 4.1.19. It worked through June; the cause was never bisected. It is kept only until the new workflow has shipped once, then deleted (retirement list).
...
What the run does, in order: `preflight` (version regex, refuses `publish=true` off `main`, Marketplace monotonicity) and the test gate → `build` checks out the dispatch SHA, asserts `package.json` version == input and (publish only) the changelog heading → `bun install --frozen-lockfile` → `bun run build:sdk` → asserts the `better-sqlite3` native binary → swaps in `README.marketplace.md` and runs `vsce package --no-dependencies` (this runs `vscode:prepublish` = `bun run package`; it is the only build) → asserts the packaged manifest is `saoudrizwan.claude-dev@<VERSION>` and that `dist/extension.js` has no `process.env.TELEMETRY_SERVICE_API_KEY` literal → uploads the artifact → `publish` **waits for `publish` environment approval**, then re-checks monotonicity and both PATs → `vsce publish --packagePath` → `ovsx publish --packagePath` → tag `v<VERSION>` at the built SHA → GitHub release with the `.vsix` → Slack post. Check what a run is waiting on:
...
- **Publish the prebuilt artifact (`vsce publish --packagePath`), never a bare `vsce publish` from the source tree.** The bare form re-runs `vscode:prepublish` and repacks, and that upload hit vsce's fixed 180s socket timeout (`Request timeout: /_apis/gallery`) 5/5 times on 4.1.18 (Sep 2026) while `--packagePath` uploads of the same code line took 3–15s (4.1.16, 4.1.17, 4.1.19). Same `@vscode/vsce` 4.0.0 on both paths on Sep 17; the same bare path had worked in June (extension 4.0.0, vsce 3.9.2) and July (legacy hotfix 4.0.12). Root cause unbisected — `@vscode/vsce` 4.0.0 (2026-09-14) was investigated and is not it (its diff touches only the signed-upload path). vsce has no timeout knob; a `vsce publish` timeout does not necessarily mean the upload failed server-side — check the live version before retrying.
