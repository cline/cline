...
- The workflow creates the tag's GitHub release (universal DMG + macOS updater artifact + Windows NSIS installer with its updater signature + Linux AppImage with its updater signature + `.deb` + `.rpm` + `latest.json`; marked prerelease for beta) and refreshes the channel's rolling feed release, which is the static auto-update feed every installed app on that channel polls. Never delete the `desktop-latest` or `desktop-beta` release or tag.
...
The macOS and Windows bundles can only be built on their own OS; the workflow's build jobs are the real verification. For extra local confidence on a Mac checkout, `bun run package:desktop:mac --allow-unsigned-mac` from the app directory; on a Linux checkout, `bun run package:desktop:linux`.
...
The `version` field must be the new release; both `darwin-aarch64` and `darwin-x86_64` entries must point at the same new universal `.app.tar.gz` asset under the release tag (each slice of the fat binary requests its own arch key at runtime, so both keys serve the one artifact), the `windows-x86_64` entry must point at the new `*_x64-setup.exe` asset, and the `linux-x86_64` entry at the new `*_amd64.AppImage` asset. Installed apps on that channel — including older per-arch installs — pick the update up on next launch or within 2 hours. Linux `.deb`/`.rpm` installs have no updater path by design: they are refreshed by their package manager.
