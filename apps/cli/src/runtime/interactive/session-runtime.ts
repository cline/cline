...
export function createInteractiveSessionRuntime(input: {
	...
	// Reserve the source before the async metadata check, and keep it reserved
	// until both the config change and its runtime refresh have completed.
	const withLocalMutation = async <T>(
		mutate: () => T | Promise<T>,
	): Promise<T> => {
		if (handoffLocked) throw new Error("Wait for the cloud handoff to finish.");
		activeLocalMutations++;
		try {
			await assertHandoffMutationAllowed();
			return await mutate();
		} finally {
			activeLocalMutations--;
		}
	...
	const getHandoffSource = (): CloudHandoffSource | undefined => {
		const manager = sessionManager;
		const sessionId = activeSessionId;
		if (!manager || !sessionId) return undefined;
		const assertSource = () => {
			if (shutdownRequested || sessionId !== activeSessionId)
		...
		return {
			lock: () => {
				...
				if (
					...
					manualCompactionAbortController
				)
					throw new Error(
						"Wait for the local operation to finish before handing off.",
					);
				handoffLocked = true;
				return () => {
					handoffLocked = false;
			...
			read: async () => {
				assertSource();
				const [row, messages, queued] = await Promise.all([
					manager.get(sessionId),
					manager.readLiveMessages(sessionId),
					manager.pendingPrompts.list({ sessionId }),
				]);
				assertSource();
				if (!row) throw new Error("The local conversation no longer exists.");
				return {
					sessionId,
					cwd: input.config.cwd,
					...
					config: {
						autoApproveTools: input.chatCommandState.autoApproveTools === true,
						thinking: input.config.thinking,
						...(input.config.reasoningEffort === "low" ||
						input.config.reasoningEffort === "medium" ||
						input.config.reasoningEffort === "high" ||
						input.config.reasoningEffort === "xhigh"
					...
					busy:
						activeLocalSends > 0 ||
						Boolean(activeLocalMutations || manualCompactionAbortController) ||
						row.status === "running" ||
						row.status === "pending",
			...
			updateMetadata: async (id, metadata) => {
				assertSource();
				if (id !== sessionId)
					throw new Error("The handoff source does not match.");
				const result = await manager.update(id, { metadata });
				if (!result.updated)
					throw new Error(
						"Could not save cloud handoff recovery information. The local conversation remains available.",
					);
			},
		};
	};
