...
describe("createInteractiveSessionRuntime", () => {
	...
	it("requests core-owned compaction without reading or replacing history", async () => {
		...
		manager.compactSession.mockResolvedValue(result);
		const runtime = await makeRuntime(manager);
		await runtime.ensureReady();
		expect(await runtime.compactCurrentSession()).toEqual(result);
		expect(manager.compactSession).toHaveBeenCalledWith(
	...
	it("surfaces core compaction failures", async () => {
		const manager = makeManager();
		manager.compactSession.mockRejectedValue(
			new Error("Session compaction is running."),
		);
		const runtime = await makeRuntime(manager);
		await runtime.ensureReady();
	...
	it("rejects manual compact when compaction is disabled", async () => {
		...
		expect(manager.compactSession).not.toHaveBeenCalled();
		expect(manager.updateSessionCompactionState).not.toHaveBeenCalled();
	});
	...
	it("carries compacted working context across mode-switch restarts", async () => {
		...
		const compactionState = createSessionCompactionState({
			source: ConversationSnapshot.capture([prefixMessage]),
