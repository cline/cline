...
describe("CLI cloud isolation and creation lifecycle", () => {
	...
	it("waits for a provisioning receipt and reattaches before reading messages", async () => {
		...
		f.controller.attach.mockResolvedValueOnce({ status: "provisioning" });
		f.controller.waitUntilReady.mockReturnValueOnce(ready.promise);
		const attaching = f.runtime.attach("ses-one");
		await until(() => f.controller.waitUntilReady.mock.calls.length === 1);
		expect(f.controller.attach).toHaveBeenCalledTimes(1);
		expect(f.controller.readMessages).not.toHaveBeenCalled();

		ready.resolve();
		await attaching;
		expect(f.controller.attach).toHaveBeenCalledTimes(2);
		expect(f.controller.attach).toHaveBeenLastCalledWith("ses-one", {
			autoApproveTools: false,
		});
		expect(f.controller.readMessages).toHaveBeenCalledWith("ses-one");
	});

	it.each([
		"navigation",
		"account",
	])("does not reattach after %s changes while provisioning", async (change) => {
		const f = await fixture();
		const ready = deferred<void>();
		f.controller.attach.mockResolvedValueOnce({ status: "provisioning" });
		f.controller.waitUntilReady.mockReturnValueOnce(ready.promise);
		const attaching = f.runtime.attach("ses-one");
		await until(() => f.controller.waitUntilReady.mock.calls.length === 1);
		if (change === "navigation") f.runtime.detach();
		else {
			f.setAccount("b");
			await f.runtime.refreshIdentity();
		}
