...
describe("getMcpDescription", () => {
	it("discloses the default initialize timeout for unconfigured stdio servers", () => {
		...
		).toBe("stdio, local, request timeout 60s, initialize timeout 5s");
	});

	it("discloses the larger initialize timeout for package-runner commands", () => {
		expect(
			getMcpDescription({
				name: "local",
				transport: { type: "stdio", command: "npx", args: ["-y", "pkg"] },
			}),
		).toBe("stdio, local, request timeout 60s, initialize timeout 10s");
	});
