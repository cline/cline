...
### macOS code signing
Bun's cross-compile code-signing path produces a Mach-O whose `CodeDirectory` hashes the last code page over a full zero-padded page instead of the `[pageStart, codeLimit)` partial bytes ([oven-sh/bun#32159](https://github.com/oven-sh/bun/issues/32159), fixed upstream in Bun 1.4.2). macOS rejects the result at exec (`cs_invalid_page`, `Killed: 9`), so the build re-signs `bun-darwin-*` outputs ad-hoc: `codesign --force --sign -` on macOS hosts, `rcodesign sign` ([apple-codesign](https://github.com/indygreg/apple-platform-rs), checksum-pinned prebuilt in `cli-publish.yml`) elsewhere. Set `RCODESIGN` to override the binary path. The re-sign runs inside `buildCompiledBinary`, so it also covers the local `bun release cli` publish path.
