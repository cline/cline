...
export function resolveMenubarHubDaemonLaunchSpec(
	options: {
		execPath?: string;
		argv?: string[];
		execArgv?: string[];
	...
	return {
		launcher: join(dirname(execPath), `menubar-hub${extension}`),
		argsPrefix: [],
		cwd,
		version: HUB_VERSION,
	};
...
export function configureMenubarHubDaemonLaunch(): void {
	setHubDaemonLaunchSpec(resolveMenubarHubDaemonLaunchSpec());
}
