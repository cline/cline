...
- Cline now runs on Linux, published as an AppImage, a deb, and an rpm alongside the macOS and Windows releases. The tray menu, session notifications, workspace folder picker, and opening a file with the system default application all work as they do elsewhere, and the app-icon picker now takes effect on Linux by setting the GTK window icon (taskbar, Alt-Tab, and window list). Auto-update is format-aware because installing is only silent for some of them: an AppImage replaces itself in place, while deb and rpm updates download in the background and install when you restart, since handing the package to dpkg or rpm needs root and the app should never prompt for a password from a background check. A binary that is none of those — a release build run out of a build directory — no longer runs the update loop at all, because the AppImage path replaces the running executable and would have overwritten it rather than updated it. The tray icon needs a StatusNotifier/AppIndicator host (standard on KDE and XFCE, an extension on GNOME); without one the app runs without the tray menu
