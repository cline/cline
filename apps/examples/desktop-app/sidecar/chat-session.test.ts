...
describe("session forks", () => {
	it("forks the remote transcript even when the same session has local cached messages", async () => {
		const sessionId = "remote-fork-source";
		const remoteMessages = [
			{ role: "user" as const, content: "remote prompt" },
			{ role: "assistant" as const, content: "remote answer" },
		];
		const start = vi.fn(async () => ({ sessionId: "remote-fork" }));
		const manager = {
			get: vi.fn(async () => ({
				sessionId,
				provider: "ssh-test-provider",
				model: "test-model",
				cwd: "/remote/project",
				workspaceRoot: "/remote/project",
			})),
			readMessages: vi.fn(async () => remoteMessages),
			start,
			pendingPrompts: { list: vi.fn(async () => []) },
		};
		const ctx = {
			...localRuntimeContext({}),
			activeEnvironmentId: "ssh-test",
			liveSessions: new Map([
				[
					sessionId,
					{
						messages: [{ role: "user", content: "local cached prompt" }],
						config: {},
					},
				],
			]),
		...
		await handleChatSessionCommand(ctx, {
			action: "fork",
			sessionId,
			config: { environmentId: "ssh-test" },
		});
		expect(start).toHaveBeenCalledWith(
			expect.objectContaining({ initialMessages: remoteMessages }),
		);
		expect(ctx.sessionEnvironmentIds.get("remote-fork")).toBe("ssh-test");
	});
