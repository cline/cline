...
async function getCommandSessionBinding(
	...
	return knownEnvironmentId
		? getRuntimeBinding(ctx, knownEnvironmentId)
...
export async function handleCommand(
	...
	if (command === "proceed_while_running") {
		...
		if (!binding) throw new Error("Session environment is not connected");
	...
	if (command === "search_sessions") {
		...
		const workspaceRoot =
			typeof args?.workspaceRoot === "string"
				...
				: undefined;
		const searchBinding = getCommandRuntimeBinding(ctx, args);
		if (searchBinding.hubClient) {
			try {
				const reply = await withSearchDeadline(
					searchBinding.hubClient.command("session.search", {
						query,
						limit,
						workspaceRoot,
					}),
				...
				if (
					...
					return reply.payload.hits.slice(0, limit).map((hit) => ({
						...hit,
						environmentId: searchBinding.environmentId,
		...
		return metadataSessionSearchHits(
			(Array.isArray(sessions) ? sessions : []).filter(
				(session) => session.environmentId === searchBinding.environmentId,
			),
			query,
		).slice(0, limit);
	}
