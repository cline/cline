...
export function syncSidecarApprovalReadiness(
	ctx: SidecarContext,
	connectingClient?: NodeHubClient,
): Promise<void> {
	const previous = approvalReadinessUpdates.get(ctx) ?? Promise.resolve();
	const update = previous
		.catch(() => undefined)
		.then(async () => {
			for (const hubClient of connectingClient
				? [connectingClient]
				: [...ctx.runtimeBindings.values()].map(
						(binding) => binding.hubClient,
					)) {
...
export function handleHubLiveEvent(
	...
	if (event.event === "approval.requested") {
		...
		void handleHubApprovalRequest(ctx, event, environmentId).catch((error) => {
...
async function handleHubApprovalRequest(
	...
	const client = ctx.runtimeBindings.get(environmentId)?.hubClient;
	if (!client)
...
export async function connectRemoteSessionRuntime(
	...
	try {
		...
		await syncSidecarApprovalReadiness(ctx, hubClient);
		hubClient.subscribe((event) =>
			handleHubLiveEvent(ctx, event, environmentId),
		);
	} catch (error) {
		try {
			unsubscribe?.();
...
export async function ensureSharedHubClient(
	...
	const initialization = (async () => {
		...
		try {
			...
			await syncSidecarApprovalReadiness(ctx, client);
