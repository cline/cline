...
The original artwork lives in `src-tauri/icon-sources/`, with one PNG for
Classic, Midnight, Hologram, and Chip. Preserve the artwork's transparent
corners and baked-in borders when exporting; do not apply another corner mask.

The committed exports are:

| Location | Export |
| --- | --- |
| `src-tauri/app-icon.png` | Unpadded 1024x1024 Midnight artwork, used by `bun tauri icon` for Windows and Linux |
| `src-tauri/icons/app/<id>.png` | Unpadded 1024x1024 runtime icons for Windows |
| `src-tauri/icons/app/macos/<id>.png` | Artwork resized directly from its original to 824x824, centered on a transparent 1024x1024 canvas (100px margins) |
| `src-tauri/icons/icon.icns` | Generated by `bun tauri icon` from the padded macOS Midnight PNG |
| `webview/public/app-icons/<id>.png` | Unpadded 256x256 picker and favicon images |

When updating artwork, regenerate each corresponding export from its original.
For Midnight, also regenerate the Windows `.ico`, desktop PNGs in
`src-tauri/icons/`, and `webview/public/32x32.png` and `webview/public/icon.png`.
Generate platform outputs in a temporary directory, then copy the desktop
outputs into place. Generate `icon.icns` separately from
`src-tauri/icons/app/macos/midnight.png` so the bundled and selectable default
icons use the same artwork and margins.
