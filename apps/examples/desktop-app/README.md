...
random local loopback port. Linux and macOS x64 and arm64 helpers are bundled by
`bun run build:sidecar:bin`; 32-bit Raspberry Pi operating systems are not
supported. The helper includes its own runtime. It is copied once per matching desktop build and cached, with no
`apt`, `npm`, root access,
