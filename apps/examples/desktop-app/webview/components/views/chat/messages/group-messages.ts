...
import type { ChatMessage, ChatMessageMedia } from "@/lib/chat-schema";
...
export type ChatRenderItem =
	...
	| {
			...
			/**
			 * Media generated by a tool call folded into this span (e.g.
			 * generate_media). Collapsing hides the row that produced it, so the
			 * caller renders this outside the collapsible content — otherwise
			 * the result is invisible until the row is expanded at least once.
			 */
...
/**
 * Media produced by tool calls in a collapsed span. Only `"tools"` items can
 * carry it here — `isCollapsibleWorkItem` already excludes a `"message"` item
 * that has media, treating it as a deliverable/span boundary instead.
 */
function collectMediaFromCollapsedItems(
	items: ChatRenderItem[],
): ChatMessageMedia[] {
	...
	for (const item of items) {
		...
		for (const message of item.messages) {
			if (message.media?.length) media.push(...message.media);
		}
	}
	return media;
}
	...
	const flushSpan = (nextIndex: number) => {
		...
		if (!complete || toolCallCount === 0 || firstCollapsed === undefined) {
			...
			out.push({
				...
				toolCallCount,
				media: collectMediaFromCollapsedItems(collapsed),
			});
			if (answer) {
				out.push(answer);
			}
