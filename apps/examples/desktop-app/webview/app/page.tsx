...
import { ImagePlus, Loader2, LoaderCircle } from "lucide-react";
...
function ChatThreadPane({
	...
	const accountScopeKey = `${accountUserId ?? "signed-out"}:${activeOrganization?.organizationId ?? "personal"}`;
	const previousAccountScopeRef = useRef<string | null>(null);
	useEffect(() => {
		void accountUserId;
	...
	useEffect(() => {
		const previousScope = previousAccountScopeRef.current;
		previousAccountScopeRef.current = accountScopeKey;
		if (
			!previousScope ||
			previousScope === accountScopeKey ||
			!isCloudSession ||
			sessionId ||
		...
		setConfig((previous) =>
			previous.repoUrl || previous.branch
				? { ...previous, repoUrl: undefined, branch: undefined }
				: previous,
		);
	}, [
		accountScopeKey,
	...
	const headerDiff = useMemo(
		...
		[isCloudSession, summary.additions, summary.deletions],
	);
	...
	return (
		<WorkspaceProvider value={workspaceContextValue}>
			...
			<div
				...
				onDragOver={isCloudSession ? undefined : handleDragOver}
				onDrop={isCloudSession ? undefined : handleDrop}
			>
				{isDraggingFiles && !isCloudSession ? (
