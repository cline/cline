...
describe("useChatSession", () => {
	...
	it("settles a queued turn from the session status when its chat_done never arrives", async () => {
		// A turn drained from the prompt queue can lose its chat_done (the hub
		// runtime host suppresses a second done until the next run.started,
		// which drained turns never publish). The hub still reports the session
		// idle afterwards, and that status must settle the turn: clear the
		// streaming state and reconcile the live rows against persisted history.
		let canonicalMessages: unknown[] = [];
		invokeMock.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				if (command === "get_process_context") {
		...
		expect(current.status).toBe("running");
		expect(current.activeAssistantMessageId).not.toBeNull();
		...
		// No chat_done: only the hub's session status reports the turn over.
		await act(async () => {
			statusHandler({ sessionId: current.sessionId, status: "idle" });
		});
	...
	it("leaves a turn the stream already settled alone when the idle status trails it", async () => {
		invokeMock.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				if (command === "get_process_context") {
					return { cwd: "/workspace/cline", workspaceRoot: "/workspace/cline" };
		...
		await act(async () => {
			chatEventHandler({
				sessionId: current.sessionId,
				stream: "chat_queued_prompt_start",
				chunk: JSON.stringify({ promptId: "q1", prompt: "First prompt" }),
				ts: Date.now(),
				index: 1,
			});
			chatEventHandler({
		...
		// Let chat_done's own reconcile fire before counting.
		await act(async () => {
			await new Promise((resolve) => setTimeout(resolve, 400));
		});
		invokeMock.mockClear();
		...
		// chat_done already settled the turn; the trailing status must not
		// schedule a second reconcile.
		expect(
			invokeMock.mock.calls.filter(
				([command]) => command === "read_session_messages",
			),
		).toHaveLength(0);
	});
