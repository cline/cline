...
describe("useChatSession", () => {
	...
	it("steers the first server entry after enqueue acknowledgement without waiting for the active response", async () => {
		...
		await act(async () => current.start(current.config));
		let activeTask!: ReturnType<typeof current.sendPrompt>;
		await act(async () => {
			activeTask = current.sendPrompt("active");
		});
		let queuedTask!: ReturnType<typeof current.sendPrompt>;
		await act(async () => {
	...
	it.each([
		...
		await act(async () => {
			taken = Boolean(await current.sendPrompt("Rebase the branch"));
		});
	...
	it("appends live artifact-backed generated videos with the owning session id", async () => {
		invokeMock.mockImplementation(
			async (command: string, args?: Record<string, unknown>) => {
				if (command === "get_process_context") {
					return { cwd: "/workspace/cline", workspaceRoot: "/workspace/cline" };
	...
	it("projects generated media from a completed tool call onto its tool message", async () => {
		...
		const toolMessage = current.messages.find(
			(message) => message.meta?.toolName === "generate_media",
		);
		expect(toolMessage).toMatchObject({
			role: "tool",
			media: [media],
			meta: { hookEventName: "tool_call_end" },
		});
