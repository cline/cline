	...
	return (
		<div ref={containerRef} style={{ position: "relative", marginTop: "-1px", display: "flex" }}>
			<VSCodeButton
				...
				title={t("browser:settingsMenu.connectionInfoTitle")}>
				<i
			...
			{showInfoPopover && (
				...
				<div
					...
					<h4 style={{ margin: "0 0 8px 0" }}>{t("browser:settingsMenu.title")}</h4>
					{/* InfoRow - Status row container */}
					<div className="flex flex-wrap whitespace-nowrap mb-1">
						{/* InfoLabel - Fixed-width label */}
						<div className="flex-none w-[90px] font-medium">{t("browser:settingsMenu.statusLabel")}</div>
						...
						<div
							...
							{connectionInfo.isConnected
								? t("browser:settingsMenu.connected")
								: t("browser:settingsMenu.disconnected")}
						</div>
					</div>
					{connectionInfo.isConnected && (
						...
						<div className="flex flex-wrap whitespace-nowrap mb-1">
							{/* InfoLabel - Fixed-width label */}
							<div className="flex-none w-[90px] font-medium">{t("browser:settingsMenu.typeLabel")}</div>
							{/* InfoValue - Flexible value container */}
							<div className="flex-1 break-words">
								{connectionInfo.isRemote ? t("browser:settingsMenu.remote") : t("browser:settingsMenu.local")}
							</div>
						</div>
					)}
					{connectionInfo.isConnected && connectionInfo.isRemote && connectionInfo.host && (
						...
						<div className="flex flex-wrap whitespace-nowrap mb-1">
							{/* InfoLabel - Fixed-width label */}
							<div className="flex-none w-[90px] font-medium">{t("browser:settingsMenu.remoteHostLabel")}</div>
							{/* InfoValue - Flexible value container */}
