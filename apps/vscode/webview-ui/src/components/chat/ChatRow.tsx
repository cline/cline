...
export const ChatRowContent = memo(
	({
		...
		const { mcpServers, vscodeTerminalExecutionMode, bedrockCoderMessages, showFeatureTips } = useExtensionState()
		...
		const [cost, apiReqCancelReason, apiReqStreamingFailedMessage] = useMemo(() => {
			if (message.text != null && message.say === "api_req_started") {
				const info: BedrockCoderApiReqInfo = JSON.parse(message.text)
			...
			switch (type) {
				...
				case "mistake_limit_reached":
					return [
						...
						<span className="text-error font-bold">Bedrock Coder is having trouble...</span>,
					]
				case "command":
					return [
						<TerminalIcon className="text-foreground size-2" />,
						<span className="font-bold text-foreground">Bedrock Coder wants to execute this command:</span>,
					]
				case "use_mcp_server":
					const mcpServerUse = JSON.parse(message.text || "{}") as BedrockCoderAskUseMcpServer
					return [
						...
						<span className="ph-no-capture font-bold text-foreground break-words">
							Bedrock Coder wants to {mcpServerUse.type === "use_mcp_tool" ? "use a tool" : "access a resource"} on
				...
				case "followup":
					return [
						...
						<span className="font-bold text-foreground">Bedrock Coder has a question:</span>,
					]
				default:
					return [null, null]
			}
		...
		const tool = useMemo(() => {
			if (message.ask === "tool" || message.say === "tool") {
				return JSON.parse(message.text || "{}") as BedrockCoderSayTool
			}
			return null
			...
			switch (tool.tool) {
				case "editedExistingFile":
					...
					const editToolTitle = isApplyingPatch
						? "Bedrock Coder is creating patches to edit this file:"
						: "Bedrock Coder wants to edit this file:"
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span style={{ fontWeight: "bold" }}>Bedrock Coder wants to delete this file:</span>
							</div>
							<CodeAccordian
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Bedrock Coder wants to create a new file:</span>
							</div>
							<CodeAccordian
								code={tool.content!}
								isExpanded={isExpanded}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								{tool.operationIsLocatedInWorkspace === false &&
									toolIcon("sign-out", "yellow", -90, "This file is outside of your workspace")}
								<span className="font-bold">Bedrock Coder wants to read this file:</span>
							</div>
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Bedrock Coder wants to view the top level files in this directory:"
										: "Bedrock Coder viewed the top level files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Bedrock Coder wants to recursively view all files in this directory:"
										: "Bedrock Coder recursively viewed all files in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span style={{ fontWeight: "bold" }}>
									{message.type === "ask"
										? "Bedrock Coder wants to view source code definition names used in this directory:"
										: "Bedrock Coder viewed source code definition names used in this directory:"}
								</span>
							</div>
							<CodeAccordian
								code={tool.content!}
						...
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									Bedrock Coder wants to search this directory for{" "}
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Bedrock Coder is condensing the conversation:</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									{message.type === "ask"
										? "Bedrock Coder wants to fetch content from this URL:"
										: "Bedrock Coder fetched content from this URL:"}
								</span>
							</div>
							<div
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">
									{message.type === "ask"
										? "Bedrock Coder wants to search the web for:"
										: "Bedrock Coder searched the web for:"}
								</span>
							</div>
					...
					return (
						<div>
							<div className={HEADER_CLASSNAMES}>
								...
								<span className="font-bold">Bedrock Coder loaded the skill:</span>
							</div>
		...
		if (message.ask === "use_mcp_server" || message.say === "use_mcp_server") {
			const useMcpServer = JSON.parse(message.text || "{}") as BedrockCoderAskUseMcpServer
			...
			case "say":
				switch (message.say) {
					case "api_req_started":
						return (
							<RequestStartRow
								...
								bedrockCoderMessages={bedrockCoderMessages}
								cost={cost}
					...
					case "user_feedback":
						return (
							<UserMessage
								canRestoreWorkspace={canRestoreWorkspaceFromMessage(bedrockCoderMessages, message.ts)}
								files={message.files}
					...
					case "bedrockCoderignore_error":
						return <ErrorRow errorType="bedrockCoderignore_error" message={message} />
						...
						return (
							<div className="flex flex-col bg-warning/20 p-2 rounded-xs border border-error">
								...
								<div className="text-foreground opacity-80">
									Bedrock Coder may have trouble viewing the command's output. Please update VSCode (
									<code>CMD/CTRL + Shift + P</code> → "Update") and make sure you're using a supported shell:
									zsh, bash, fish, or PowerShell (<code>CMD/CTRL + Shift + P</code> → "Terminal: Select Default
									Profile").
									<a
										className="px-1"
										href="https://github.com/FFFalexgo/AWS_Bedrock_Coder/wiki/Troubleshooting-%E2%80%90-Shell-Integration-Unavailable">
			...
			case "ask":
				switch (message.ask) {
					...
					case "followup":
						...
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as BedrockCoderAskQuestion
					...
					case "new_task":
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">Bedrock Coder wants to start a new task:</span>
								</div>
							...
							<div>
								<div className={HEADER_CLASSNAMES}>
									...
									<span className="text-foreground font-bold">
										Bedrock Coder wants to condense your conversation:
									</span>
								</div>
								<NewTaskPreview context={message.text || ""} />
							</div>
						...
						return (
							<div>
								<div className={HEADER_CLASSNAMES}>
									<FilePlus2Icon className="size-2" />
									<span className="text-foreground font-bold">
										Bedrock Coder wants to create a Github issue:
									</span>
								</div>
					...
					case "plan_mode_respond": {
						...
						let selected: string | undefined
						try {
							const parsedMessage = JSON.parse(message.text || "{}") as BedrockCoderPlanModeResponse
