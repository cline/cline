		...
		if (this.state.hasError) {
			return (
				<div
					...
					<h3 style={{ margin: "0 0 8px 0" }}>{errorTitle || i18n.t("chat:errorBoundary.title")}</h3>
					<p style={{ margin: "0" }}>
						{errorBody ||
							i18n.t("chat:errorBoundary.errorPrefix", {
								message: this.state.error?.message || i18n.t("chat:errorBoundary.unknownError"),
							})}
					</p>
				</div>
			)
