	...
	return (
		<div className="p-2 border-none rounded-md mb-2 bg-(--vscode-textBlockQuote-background)">
			<div className="mb-3 font-geist-mono">
				<div className="text-error mb-2">{message || t("chat:errors.creditLimit.defaultMessage")}</div>
				<div className="mb-3">
					{currentBalance ? (
						<div className="text-foreground">
							{t("chat:errors.creditLimit.currentBalance")}{" "}
							<span className="font-bold">{currentBalance.toFixed(2)}</span>
						</div>
					) : null}
					{totalSpent ? (
						<div className="text-foreground">
							{t("chat:errors.creditLimit.totalSpent")} {totalSpent.toFixed(2)}
						</div>
					) : null}
					{totalPromotions ? (
						<div className="text-foreground">
							{t("chat:errors.creditLimit.totalPromotions")} {totalPromotions.toFixed(2)}
						</div>
					) : null}
				</div>
			</div>
			...
			<VSCodeButtonLink className="w-full mb-2" href={fullBuyCreditsUrl}>
				...
				{t("chat:errors.creditLimit.buyCredits")}
			</VSCodeButtonLink>
