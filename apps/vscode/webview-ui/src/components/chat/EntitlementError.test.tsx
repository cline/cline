...
const getSubscribeHref = () =>
	screen.getByRole("link", { name: /get cline pass/i }).getAttribute("href");
const querySubscribeLink = () =>
	screen.queryByRole("link", { name: /get cline pass/i });
...
describe("EntitlementError", () => {
	...
	it("shows friendly copy with the backend detail as muted support text", () => {
		...
		expect(
			screen.getByText("This model requires a Cline Pass subscription."),
		).toBeInTheDocument();
		expect(
			screen.getByText(
	...
	it("builds the subscribe link from the authenticated user's app base URL", () => {
		...
		expect(getSubscribeHref()).toBe(
			"https://staging-app.cline.bot/dashboard/subscription",
		);
		unmount();
		...
		render(<EntitlementError />);
		expect(getSubscribeHref()).toBe(
			"https://proxy.enterprise.com/cline/app/dashboard/subscription",
		);
	});
