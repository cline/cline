...
function toolResultTextParts(content: ContentBlock & { type: "tool_result" }): vscode.LanguageModelTextPart[] {
	...
	// ToolOperationResult `{ query, result, success, error? }` — NOT typed
	// text/image/file content blocks. Those items have no `.type`/`.text`, so a
	// naive per-item `part.text ?? ""` produced EMPTY LanguageModelTextParts and
	// the model saw no tool output. If none of the items look like typed content
	// blocks, hand the whole array to extractToolOutputText (which extracts
	// `.result`/`.error` from ToolOperationResult[], else JSON.stringify).
