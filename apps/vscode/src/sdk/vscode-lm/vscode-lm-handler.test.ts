...
const { parseVsCodeLmSelector, VsCodeLmHandler } = await import(
	"./vscode-lm-handler"
);
...
describe("parseVsCodeLmSelector", () => {
	...
	it("parses encoded family values containing slashes", () => {
		expect(parseVsCodeLmSelector("copilot/openai%2Fgpt-4o")).toEqual({
			vendor: "copilot",
...
function fakeModel(parts: unknown[]) {
	...
	const model = {
		...
		sendRequest: vi.fn(
			(_messages: unknown, options: Record<string, unknown>) => {
				calls.push({ options });
...
describe("VsCodeLmHandler.createMessage tool calling", () => {
	it("passes tools + Auto toolMode to sendRequest when tools are provided", async () => {
		...
		await collect(
			handler.createMessage("sys", [{ role: "user", content: "hi" }], tools),
		);

		expect(calls).toHaveLength(1);
		expect(calls[0].options.toolMode).toBe(
	...
	it("emits a native tool_calls chunk for a LanguageModelToolCallPart", async () => {
		...
		expect(chunks.some((c) => c.type === "text" && c.text === "thinking")).toBe(
			true,
		);
