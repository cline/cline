...
import { parseVsCodeLmModelSelector, SELECTOR_SEPARATOR } from "@/shared/vsCodeSelectorUtils"
 ...
 * into a `LanguageModelChatSelector`. The model picker commonly supplies just
 * `vendor` and `family`; any further segments map to version/id positionally,
 * mirroring `stringifyVsCodeLmModelSelector`.
 */
export function parseVsCodeLmSelector(modelId: string | undefined): vscode.LanguageModelChatSelector {
...
export class VsCodeLmHandler implements ApiHandler {
	...
	getMessages(systemPrompt: string, messages: Message[]): vscode.LanguageModelChatMessage[] {
		return [vscode.LanguageModelChatMessage.Assistant(systemPrompt), ...convertToVsCodeLmMessages(messages)]
	}

	// VS Code LM does not expose a tokenizer cheaply, so approximate as chars/4.
	// Good enough for budgeting and usage display.
	private approxTokens(text: string): number {
		return Math.ceil((text || "").length / 4)
	}

	async *createMessage(systemPrompt: string, messages: Message[], tools?: ToolDefinition[]): AsyncGenerator<ApiStreamChunk> {
		this.ensureCleanState()
		const client = await this.getClient()
		const id = nanoid()

		const vsCodeLmMessages = this.getMessages(systemPrompt, messages)
		const inputTokens = vsCodeLmMessages.reduce((sum, msg) => sum + this.approxTokens(extractText(msg)), 0)
		...
		if (this.abortSignal) {
			if (this.abortSignal.aborted) {
				...
				this.abortSignal.addEventListener("abort", () => this.currentRequestCancellation?.cancel(), { once: true })
			}
		}
	...
	getModel(): HandlerModelInfo {
		...
		const fallbackId =
			[this.selector.vendor, this.selector.family, this.selector.version, this.selector.id]
				.filter(Boolean)
