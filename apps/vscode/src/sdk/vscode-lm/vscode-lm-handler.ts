...
export function parseVsCodeLmSelector(modelId: string | undefined): vscode.LanguageModelChatSelector {
	...
	const [vendor, family, version, id] = modelId.split(SELECTOR_SEPARATOR)
		...
		try {
			...
			for await (const chunk of response.stream) {
				if (chunk instanceof vscode.LanguageModelTextPart) {
					...
					const textChunk: ApiStreamChunk = { type: "text", text: chunk.value, id }
					yield textChunk
					...
					accumulatedText += JSON.stringify({ name: chunk.name, input: chunk.input, callId: chunk.callId })
					yield toolCallChunk
				}
			}
