...
import {
	SdkMessageCoordinator,
	type SessionEventListener,
} from "./sdk-message-coordinator";
...
function metadataNumber(
	metadata: SessionHistoryRecord["metadata"] | undefined,
	key: string,
): number | undefined {
...
export class Controller {
	...
	constructor(readonly context: ClineExtensionContext) {
		...
		this.providerConfigStoreSubscription = this.providerConfigStore.subscribe(
			(event) => {
		...
		this.mcpHub = new McpHub(
			...
			async () => {
				const clineDir =
					process.env.CLINE_DIR || path.join(os.homedir(), ".cline");
		...
		this.interactions = new SdkInteractionCoordinator({
			...
			recordApprovedToolMessage: (toolCallId, messageTs) =>
				this.messageTranslatorState.recordApprovedToolMessageTs(
					toolCallId,
					messageTs,
				),
		...
		this.sessions = new SdkSessionLifecycle({
			...
			requestToolApproval: (request) =>
				this.interactions.handleRequestToolApproval(request),
			askQuestion: (question, options, context) =>
				this.interactions.handleAskQuestion(question, options, context),
			...
			onSendError: async (error, sessionId) => {
				...
				if (isClineAuthError) {
					...
					this.isClineProviderActive() &&
					this.isClineBalanceError(errorMessage)
				) {
		...
		this.followups = new SdkFollowupCoordinator({
			...
			createTempSessionHost: () =>
				VscodeSessionHost.create({ mcpHub: this.mcpHub }),
			getWorkspaceRoot: () => this.getWorkspaceRoot(),
			loadInitialMessages: (sessionHost, taskId) =>
				this.sessionHistory.loadInitialMessages(sessionHost, taskId),
			buildStartSessionInput,
		...
		this.taskControl = new SdkTaskControlCoordinator({
			...
			onAskResponse: (text, images, files) =>
				this.askResponse(text, images, files),
			resetMessageTranslator: () => this.resetMessageTranslatorAndFence(),
		...
		this.taskStart = new SdkTaskStartCoordinator({
			...
			createTempSessionHost: () =>
				VscodeSessionHost.create({ mcpHub: this.mcpHub }),
			loadInitialMessages: (reader, taskId) =>
				this.sessionHistory.loadInitialMessages(reader, taskId),
			resolveContextMentions: (text) => this.resolveContextMentions(text),
			isClineProviderActive: () => this.isClineProviderActive(),
		...
		this.mcpHub.setToolListChangeCallback(() =>
		...
		Logger.log(
			"[SdkController] Initialized with SDK adapter layer + gRPC bridge + auth services",
		);
	}
	...
	private handleProviderConfigChange(event: ProviderConfigChange): void {
		...
		if (
			...
			this.sessions
				...
				.catch((error) =>
					Logger.error(
						"[SdkController] Failed to update active session model:",
						error,
					),
				);
		}
	...
	private isSelectionForActiveModeProvider(
		event: Extract<ProviderConfigChange, { kind: "selection" }>,
	): boolean {
		try {
	...
	private scheduleProviderConfigStatePost(): void {
		...
		queueMicrotask(() => {
			...
			this.postStateToWebview().catch((error) => {
				Logger.error(
					"[SdkController] Failed to post state after provider config change:",
					error,
				);
			});
		});
	...
	private startRemoteConfigTimer(): void {
		...
		this.refreshRemoteConfig().catch((err) =>
			Logger.error(
				"[SdkController] Initial remote config refresh failed:",
				err,
			),
		);
		...
		this.remoteConfigTimer = setInterval(() => {
			this.refreshRemoteConfig().catch((err) =>
				Logger.error("[SdkController] Remote config timer failed:", err),
			);
		}, 3600000); // 1 hour
	}
	...
	async setRemoteConfigCoreIntegration(
		integration: PreparedRemoteConfigCoreIntegration | undefined,
	): Promise<void> {
		...
		if (previous && previous !== integration) {
			try {
				...
				Logger.error(
					"[SdkController] Failed to dispose previous remote config integration:",
					error,
				);
			}
		}
	...
	private async resolveContextMentions(text: string): Promise<string> {
		...
		try {
			...
			const resolved = await parseMentions(
				text,
				cwd,
				urlContentFetcher,
				undefined,
			...
			Logger.log(
				`[SdkController] Resolved context mentions (${text.length} → ${resolved.length} chars)`,
			);
			return resolved;
		} catch (error) {
			Logger.error(
				"[SdkController] Failed to resolve context mentions, using raw text:",
				error,
			);
			return text;
		}
	...
	private async getWorkspaceRoot(): Promise<string> {
		...
		try {
			...
			Logger.warn(
				"[SdkController] Failed to get workspace paths, falling back to Desktop:",
				error,
			);
		}
	...
	private async getRemoteConfigWorkspacePath(): Promise<string | undefined> {
		try {
			...
			Logger.warn(
				"[SdkController] Failed to get workspace paths for remote config, using global fallback:",
				error,
			);
			return undefined;
		}
	...
	private isClineBalanceError(errorMessage: string): boolean {
		try {
			const shaped = JSON.parse(
				reshapeErrorForWebview({ message: errorMessage }),
			);
	...
	async initTask(
		...
		this.refreshRemoteConfig().catch((err) =>
			Logger.error(
				"[SdkController] Remote config refresh before task failed:",
				err,
			),
		);
		...
		return this.taskStart.initTask(
			prompt,
			images,
			files,
			historyItem,
	...
	async askResponse(
		...
		if (
			this.pendingClineAuthRetryPrompt !== undefined &&
			this.task?.taskState?.askResponse === "yesButtonClicked"
		) {
	...
	async getTaskHistory(
		...
		let filteredTasks = sessionHistory.filter((item) => {
			const ts = dateStringToTimestamp(
				item.updatedAt ?? item.endedAt ?? item.startedAt,
			);
			...
			if (currentWorkspaceOnly && workspacePath) {
				...
				if (
					...
					!arePathsEqual(sessionWorkspacePath, workspacePath)
				) {
					return false;
				}
			}
		...
		filteredTasks.sort((a, b) => {
			switch (sortBy) {
				...
				case "mostExpensive":
					return (
						(metadataNumber(b.metadata, "totalCost") ?? 0) -
						(metadataNumber(a.metadata, "totalCost") ?? 0)
					);
				case "mostTokens":
					return (
		...
		const tasks = filteredTasks.slice(0, limit).map((item) => {
			...
			return {
				id: item.sessionId,
				task: formatDisplayUserInput(
					metadataString(metadata, "title") ?? item.prompt ?? "",
				),
				ts: dateStringToTimestamp(
					item.updatedAt ?? item.endedAt ?? item.startedAt,
				),
				isFavorited:
					metadataBoolean(metadata, "isFavorited") ??
					metadataBoolean(metadata, "is_favorited") ??
					false,
	...
	async deleteAllTaskHistory(): Promise<DeleteAllTaskHistoryCount> {
		...
		if (userChoice === "Delete All Except Favorites") {
			const hasFavoritedTasks = taskHistory.some(
				(task) =>
					metadataBoolean(task.metadata, "isFavorited") ??
					metadataBoolean(task.metadata, "is_favorited") ??
					false,
			);
	...
	async postStateToWebview(): Promise<void> {
		...
		const { sendStateUpdate } = await import(
	...
	async getStateToPostToWebview(): Promise<ExtensionState> {
		...
		// task history on top.
		try {
			const { getStateToPostToWebview: buildBaseState } = await import(
				"@core/controller/state/getStateToPostToWebview"
			);
			...
			const sdkTaskHistory = (
				await this.taskHistory.listHistory({ limit: 100, hydrate: false })
			)
			...
			if (this.task?.taskId && !mergedTaskHistoryById.has(this.task.taskId)) {
				const taskMessage = this.task.messageStateHandler
					...
					.find(
						(message) =>
							message.type === "say" && message.say === "task" && message.text,
					);
				if (taskMessage?.text) {
	...
	applyTerminalSettings(terminalManager: ITerminalManager): void {
		...
		if (terminalOutputLineLimit !== undefined) {
			terminalManager.setTerminalOutputLineLimit(
