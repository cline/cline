...
describe("buildSessionConfig", () => {
	...
	it("budgets LM Studio sessions against the live server context window", async () => {
		...
		expect(config.knownModels?.["qwen3-27b"]).toMatchObject({ contextWindow: 200_000 })
		expect((config.providerConfig as any).knownModels["qwen3-27b"]).toMatchObject({ contextWindow: 200_000 })
	...
	it("keeps LM Studio session creation non-fatal when the live model lookup fails", async () => {
		...
		expect(Logger.warn).toHaveBeenCalledWith("[SessionFactory] Failed to resolve live LM Studio model info:", lookupError)
		resolveSpy.mockRestore()
	})
