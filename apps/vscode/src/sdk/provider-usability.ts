...
// session: if we can resolve a credential (BYOK key or Cline OAuth token) for
// the active-mode provider, the user can chat. Keyless providers (Cline/OAuth
// providers, SDK "local-auth" providers, and the local-inference providers
...
function isKeylessViaModelProvider(providerId: string): boolean {
	...
	try {
		const fields = getProviderConfigFields(toSdkProviderId(providerId))
		return fields.authMethod === "local"
	} catch {
		return false
	}
}
...
export function hasUsableProvider(apiConfig: ApiConfiguration, mode: Mode): boolean {
	...
	// A resolvable credential (BYOK key, Bedrock key, or Cline OAuth token)
