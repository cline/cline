...
describe("TelemetryService metrics", () => {
	...
	it("captureConversationTurnEvent emits counters with cache and cost", () => {
		...
		const costEntry = provider.counters.find(
			(entry) => entry.name === "cline.cost.total",
		);
		assert.ok(costEntry);
	...
	it("captureDiffView lifecycle methods emit counters with edit metadata", () => {
		...
		service.captureDiffViewOpened({
			editType: "modify",
			editSurface: "vscode_diff",
			isNotebook: false,
		});
		service.captureDiffViewAccepted({
			editType: "modify",
			editSurface: "background",
			isNotebook: false,
		});
		service.captureDiffViewRejected({
			editType: "delete",
			editSurface: "external",
			isNotebook: true,
		});
