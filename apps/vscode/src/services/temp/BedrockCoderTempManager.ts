/**
 * BedrockCoderTempManager - Manages temporary files for BedrockCoder with automatic cleanup.
 *
 * Simple approach:
 * - Uses a "bedrockCoder" subdirectory inside the system temp dir (falls back to system temp if creation fails)
...
/**
 * Singleton manager for BedrockCoder's temporary files.
 */
class BedrockCoderTempManagerImpl {
	private readonly tempDir: string
	private cleanupIntervalId: NodeJS.Timeout | null = null

	constructor() {
		// Uses system temp directory with a dedicated "bedrockCoder" subdirectory when possible:
		// macOS: /var/folders/xx/.../T/bedrockCoder
		// Windows: C:\Users\{user}\AppData\Local\Temp\bedrockCoder
		// Linux: /tmp/bedrockCoder
		const baseTempDir = os.tmpdir()
		const bedrockCoderTempDir = path.join(baseTempDir, "bedrockCoder")

		try {
			fs.mkdirSync(bedrockCoderTempDir, { recursive: true })
			this.tempDir = bedrockCoderTempDir
		} catch {
	...
	/**
	 * Clean up old BedrockCoder temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the BedrockCoder temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			if (deletedCount > 0) {
				Logger.info(
					`BedrockCoder temp cleanup: deleted ${deletedCount} files, freed ${Math.round(freedBytes / 1024 / 1024)}MB`,
				)
			}
		} catch (error) {
			Logger.error("Error during Bedrock Coder temp cleanup", error)
		}
...
export const BedrockCoderTempManager = new BedrockCoderTempManagerImpl()
