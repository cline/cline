...
const vscodeMock = require("../../../test/vscode-mock");
...
describe("rollout stand-down at the token rotation site", () => {
	...
	beforeEach(() => {
		...
		sinon
			.stub(ClineAuthProvider.prototype, "config")
			.get(() => ({ apiBaseUrl: "http://127.0.0.1:1" }));
		refreshSpy = sinon
			.stub(ClineAuthProvider.prototype, "refreshToken")
			.rejects(new Error("network call attempted"));
		warnSpy = sinon
			.stub(vscodeMock.window, "showWarningMessage")
			.resolves(undefined);
	});
	...
	it("keeps serving the current access token without rotating it", async () => {
		...
		initializeRolloutStanddown(fakeExtensionContext("next"));
		const { controller, secrets } = fakeController(authBlob(120, "rt-shared"));

		const result = await new ClineAuthProvider().retrieveClineAuthInfo(
			controller,
		);
		...
		assert.strictEqual(
			warnSpy.callCount,
			0,
			"must not nag while the token is still usable",
		);
		assert.ok(
			secrets.get(SECRET_KEY),
			"the stored blob must stay intact for next's migration",
		);
	});
	...
	it("surfaces the reload notice once the access token expires, without clearing the blob", async () => {
		...
		assert.match(
			...
			/upgraded to the new version of Cline/,
		);
		assert.strictEqual(warnSpy.firstCall.args[1], "Reload Window");
		assert.ok(
			secrets.get(SECRET_KEY),
			"stand-down must never destroy the credentials",
		);
	...
	it("refreshes normally when the machine is still assigned to legacy", async () => {
		giveNextTheCredentials();
		initializeRolloutStanddown(fakeExtensionContext("legacy"));
		...
		assert.strictEqual(
			refreshSpy.callCount,
			1,
			"a non-promoted machine must keep refreshing",
		);
	});
	...
	it("refreshes normally when next holds no Cline credentials yet", async () => {
		...
		assert.strictEqual(
			refreshSpy.callCount,
			1,
			"sole owner must keep refreshing",
		);
	});
	...
	it("refreshes normally in a standalone (non-rollout) build", async () => {
		...
		assert.strictEqual(
			refreshSpy.callCount,
			1,
			"a leftover memento must not disable auth outside the rollout",
		);
	});
});

function restoreEnv(
	key: "CLINE_ROLLOUT_VARIANT" | "CLINE_DATA_DIR",
