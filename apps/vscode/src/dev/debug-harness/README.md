...
An HTTP-controlled debug server for the BedrockCoder VSCode extension. Provides
...
- **Data isolation**: separate `~/.bedrock-coder2` profile so debugee doesn't interfere with debugger
...
Options:
  ...
  --workspace PATH    Workspace directory to open (default: /tmp/bedrock-coder-debug-workspace)
  --port PORT         Server port (default: 19229)
  --bedrock-coder-dir PATH    Override the debugee's BEDROCK_CODER_DIR (default: ~/.bedrock-coder2)
```
...
The debugee runs with `BEDROCK_CODER_DIR=~/.bedrock-coder2` by default, keeping its data
separate from your real `~/.bedrock-coder`. This prevents:
...
The isolated BEDROCK_CODER_DIR is reported in `status()` and `launch()` responses:

```bash
curl localhost:19229/api -d '{"method":"status"}'
# → { "bedrockCoderDir": "/Users/you/.bedrock-coder2", ... }
```

To use a different directory: `--bedrock-coder-dir /tmp/test-bedrock-coder-dir`

## Browser Capture & OAuth Testing

When the debug harness launches VSCode, it sets `BEDROCK_CODER_CAPTURE_BROWSER=1`
which intercepts all `openExternal()` calls in the debugee. Instead of
opening a real browser, URLs are:

1. **Logged to disk** at `$BEDROCK_CODER_DIR/data/debug-captured-urls.jsonl`
...
### Testing BedrockCoder OAuth (login flow)

The BedrockCoder OAuth flow uses the SDK's local callback server. When the user
...
curl localhost:19229/api -d '{"method":"ui.locator","params":{"text":"Login to BedrockCoder","frame":"sidebar","action":"click"}}'

# 2. Check captured URLs to find the authorization URL
curl localhost:19229/api -d '{"method":"oauth.captured_urls"}'
# → { "urls": [{ "url": "https://api.bedrock-coder.bot/auth/authorize?callback_url=http://127.0.0.1:PORT/..." }] }
...
# → { "found": true, "hasAccountId": true, "keys": ["bedrockCoder:bedrockCoderAccountId"] }
...
curl localhost:19229/api -d '{"method":"oauth.captured_urls"}'
#    The authorize URL contains redirect_uri=vscode://fffalexgo.bedrock-coder/mcp-auth/callback/HASH
...
#    handler module. Instead, call the __bedrockCoderHandleUri hook (see below):
curl localhost:19229/api -d '{
  "method": "ext.evaluate",
  "params": {
    "awaitPromise": true,
    "expression": "globalThis.__bedrockCoderHandleUri(\"vscode://fffalexgo.bedrock-coder/mcp-auth/callback/HASH?code=REAL_CODE&state=SAVED_STATE\")"
  }
}'
...
> **`globalThis.__bedrockCoderHandleUri(url)` — debug-only URI delivery hook.**
> Registered in `src/extension.ts` during activation, **only** when
> `BEDROCK_CODER_CAPTURE_BROWSER` is set (which the harness always sets), so it never
...
|---------|--------------|
| `bedrockCoder.accountButtonClicked` | Account / sign-in view |
| `bedrockCoder.historyButtonClicked` | Task history view |
| `bedrockCoder.settingsButtonClicked` | Settings view |
| `bedrockCoder.mcpButtonClicked` | MCP servers view |
| `bedrockCoder.plusButtonClicked` | New task (chat view) |
| `bedrockCoder.worktreesButtonClicked` | Worktrees view |

```bash
# Navigate to account view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"bedrockCoder.accountButtonClicked"}}'

# Navigate to history view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"bedrockCoder.historyButtonClicked"}}'

# Navigate to settings view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"bedrockCoder.settingsButtonClicked"}}'

# Navigate to MCP view
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"bedrockCoder.mcpButtonClicked"}}'

# Start a new task (return to chat view)
curl localhost:19229/api -d '{"method":"ui.command_palette","params":{"command":"bedrockCoder.plusButtonClicked"}}'
```
...
# 3. Check status (verify BEDROCK_CODER_DIR, browser capture, etc.)
curl localhost:19229/api -d '{"method":"status"}'
...
| `ui.open_sidebar` | | Open the BedrockCoder sidebar |
...
`globalThis.__bedrockCoderHandleUri("vscode://fffalexgo.bedrock-coder/...?code=...&state=...")`.
It invokes the same `SharedUriHandler.handleUri` as VSCode's real URI handler
and is registered only when `BEDROCK_CODER_CAPTURE_BROWSER` is set (never in prod). See
...
3. **Data Isolation**: Sets `BEDROCK_CODER_DIR=~/.bedrock-coder2` in the debugee's environment, ensuring
   the debugee uses a completely separate data directory from the user's real `~/.bedrock-coder`.
...
4. **Browser Capture**: Sets `BEDROCK_CODER_CAPTURE_BROWSER=1` and `BEDROCK_CODER_DEBUG_HARNESS_PORT=19229`
   in the debugee's environment. When `openExternal()` is called in `src/utils/env.ts`, it
   checks for `BEDROCK_CODER_CAPTURE_BROWSER` and, if set, logs the URL to a JSONL file and POSTs
...
**⚠️ Data Isolation**: The debugee uses `~/.bedrock-coder2` by default. If you need to test with
existing data from your real `~/.bedrock-coder`, copy it: `cp -r ~/.bedrock-coder ~/.bedrock-coder2`. Be aware that
secrets (API keys, auth tokens) will be shared if you do this.

**⚠️ "Introducing BedrockCoder Kanban" overlay**: On fresh launches, a full-screen promo overlay may
...
**Screenshots**: `ui.screenshot` and `ui.sidebar_screenshot` save PNG files to `/tmp/bedrock-coder-debug/`
...
to open. For BedrockCoder OAuth, the SDK's local callback server is still running and will accept
...
**BedrockCoder OAuth with invalid codes**: If you simulate the OAuth callback with a fake code, the
...
**"Sidebar frame not found"**: The BedrockCoder sidebar isn't open. Use `ui.open_sidebar` first.
...
**Screenshots directory**: Saved to `/tmp/bedrock-coder-debug/` (configurable via SCREENSHOT_DIR).

**Debugee still uses ~/.bedrock-coder**: Check that `BEDROCK_CODER_DIR` appears in the `status()` response.
