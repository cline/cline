...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	/**
	 * User-facing copy when consecutiveMistakeCount hits the configured limit.
	 * Distinguishes narration-only streaks (no tool_use) from real tool failures
	 * so users aren't sent debugging MCP install/transport when the model simply
	 * forgot to invoke a tool (#12431).
	 */
	mistakeLimitReached: (input: {
		consecutiveMistakes: number;
		fromNoToolTurns: boolean;
	}) =>
		input.fromNoToolTurns
			? `Cline responded without calling a tool ${input.consecutiveMistakes} times in a row. Every turn needs a tool call (or attempt_completion when the task is done). Try guiding it with a new prompt.`
