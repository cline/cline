...
function createDiffViewProviderStub() {
	...
	provider.open = sinon.stub().callsFake(async (absolutePath: string) => {
		...
		provider.originalContent = await fs.readFile(absolutePath, "utf8").catch(() => "")
	})
...
function createConfig() {
	...
	const config = {
		...
		services: {
			diffViewProvider,
			fileContextTracker: { trackFileContext: sinon.stub().resolves(), markFileAsEditedByCline: sinon.stub() },
			stateManager: {
...
function makeBlock(options: { name?: string; partial: boolean; path?: string; diff?: string; content?: string }): ToolUse {
...
describe("WriteToFileToolHandler path guards (issue #12863)", () => {
	...
	describe("directory paths", () => {
		it("skips a directory path during streaming without opening or erroring", async () => {
			const { config, callbacks, taskState, validator, diffViewProvider } = createConfig()
			...
			const block = makeBlock({ partial: true, path: "tests", diff: SIMPLE_DIFF })
			const result = await handler.validateAndPrepareFileOperation(config, block, "tests", SIMPLE_DIFF)

			assert.equal(result, undefined)
			assert.equal(diffViewProvider.open.called, false, "must not open a directory")
			assert.equal(diffViewProvider.editType, undefined, "must not lock in an edit type for a streaming path")
		...
		it("fails once with a clear tool error when the complete block targets a directory", async () => {
			...
			const block = makeBlock({ partial: false, path: "tests", diff: SIMPLE_DIFF })
			const result = await handler.execute(config, block)

			assert.equal(result, "")
			assert.equal(diffViewProvider.open.called, false, "must not open a directory")
			assert.equal(taskState.consecutiveMistakeCount, 1)
			assert.ok(callbacks.say.calledWith("error"), "should surface a single clear error")

			const toolResultText = JSON.stringify(config.taskState.userMessageContent)
			assert.ok(toolResultText.includes("is a directory, not a file"), `unexpected tool result: ${toolResultText}`)
		})
		...
		it("write_to_file also rejects directory paths on the complete block", async () => {
			...
			const block = makeBlock({ name: "write_to_file", partial: false, path: "src", content: "hello" })
			const result = await handler.execute(config, block)
	...
	describe("replace_in_file on non-existent paths during streaming", () => {
		it("does not create a file at a possibly-incomplete path mid-stream", async () => {
			...
			const block = makeBlock({ partial: true, path: truncated, diff: SIMPLE_DIFF })
			const result = await handler.validateAndPrepareFileOperation(config, block, truncated, SIMPLE_DIFF)
			...
			assert.equal(exists, false, "must not create a junk file at the truncated path")
		})
		...
		it("still streams edits into an existing file", async () => {
			...
			const block = makeBlock({ partial: true, path: "file.cpp", diff: SIMPLE_DIFF })
			await handler.validateAndPrepareFileOperation(config, block, "file.cpp", SIMPLE_DIFF)

			assert.equal(diffViewProvider.open.calledOnce, true)
			assert.equal(diffViewProvider.openedPaths[0], path.join(tmpDir, "file.cpp"))
	...
	describe("handlePartialBlock path sanitization", () => {
		it("strips a partial closing tag from the streamed path before opening", async () => {
			...
			const block = makeBlock({ partial: true, path: "SortingTests.cpp</pat", diff: SIMPLE_DIFF })
			await handler.handlePartialBlock(block, createUIHelpers(config))

			assert.equal(diffViewProvider.open.calledOnce, true)
			assert.equal(diffViewProvider.openedPaths[0], path.join(tmpDir, "SortingTests.cpp"))
		})
		...
		it("strips a trailing '<' from the streamed path before opening", async () => {
			...
			const block = makeBlock({ partial: true, path: "SortingTests.cpp<", diff: SIMPLE_DIFF })
			await handler.handlePartialBlock(block, createUIHelpers(config))
