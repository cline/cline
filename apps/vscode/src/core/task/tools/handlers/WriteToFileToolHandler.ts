...
export class WriteToFileToolHandler implements IFullyManagedTool {
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		// While streaming, the path param may carry a partial closing tag (e.g. `foo.cpp</pat` or a
		// trailing `<`) when the model emits params out of order. Strip it before touching the
		// filesystem, otherwise we'd open/create a file at a malformed path.
		const sanitizedRelPath = uiHelpers.removeClosingTag(block, block.params.path ? "path" : "absolutePath", rawRelPath)
		if (!sanitizedRelPath) {
			return
		}
		...
		const result = await this.validateAndPrepareFileOperation(config, block, sanitizedRelPath, rawDiff, rawContent)
		if (!result) {
			return
		}
	...
	async validateAndPrepareFileOperation(config: TaskConfig, block: ToolUse, relPath: string, diff?: string, content?: string) {
		...
		if (config.services.diffViewProvider.editType !== undefined) {
			...
			// Never treat a directory as an editable file. A streaming path can be truncated
			// mid-path (e.g. `tests` while the model is still emitting `tests/JsonOutputTests.cpp`),
			// and reading a directory would throw EISDIR and poison the diff view state.
			if (await isDirectory(absolutePath)) {
				if (block.partial) {
					// The path may still be streaming in; wait for the complete block.
					return
				}
				config.taskState.consecutiveMistakeCount++
				await config.callbacks.say(
					"error",
					`Cline tried to use ${block.name} for '${resolvedPath}', which is a directory, not a file.`,
				)
				const errorResponse = formatResponse.toolError(
					`The specified path '${resolvedPath}' is a directory, not a file. ${block.name} can only operate on files. Please provide the path to the exact file you want to ${block.name === "replace_in_file" ? "edit" : "write"}.`,
				)
				ToolResultUtils.pushToolResult(
					errorResponse,
					block,
			...
			// For replace_in_file during streaming, don't lock in a "create" edit for a file that
			// doesn't exist: the streamed path may still be incomplete or malformed, and opening
			// now would create a junk file at the wrong path. The complete block in execute()
			// still handles the (rare) legitimate case of replace_in_file on a new file.
			if (!fileExists && block.partial && block.name === "replace_in_file") {
				return
			}
