import { type ClineDefaultTool, getToolUseNames } from "@shared/tools";
...
export function parseAssistantMessageV2(
	...
	// Only possible if a tool use wasn't open at the very end
		...
		let textContent = assistantMessage
			...
			.trim();
		let hadTruncatedOpenTag = false;

		const trailingText = assistantMessage.trimEnd();
		const lastLt = trailingText.lastIndexOf("<");
		if (lastLt !== -1 && lastLt >= currentTextContentStart) {
			const candidate = trailingText.slice(lastLt + 1);
			const isIncompleteOpenTag =
				!candidate.includes(">") &&
				!candidate.startsWith("/") &&
				/^[a-zA-Z_]*$/.test(candidate) &&
				candidate.length > 0;

			if (isIncompleteOpenTag) {
				const hasMatchingTool = [...toolUseOpenTags.keys()].some((tag) =>
					tag.startsWith(`<${candidate}`),
				);
				if (hasMatchingTool) {
					textContent = assistantMessage
						.slice(currentTextContentStart, lastLt)
						.trim();
					hadTruncatedOpenTag = true;
				}
			}
		}

		currentTextContent.content = textContent;
		// Text is partial because the loop finished
		if (currentTextContent.content.length > 0) {
			contentBlocks.push(currentTextContent);
		}

		if (hadTruncatedOpenTag) {
			contentBlocks.push({
				type: "tool_use",
				name: "" as ClineDefaultTool,
				params: {},
				partial: true,
				call_id: nanoid(8),
				isNativeToolCall: false,
