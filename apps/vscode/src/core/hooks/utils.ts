...
/**
 * All valid hook types that can be created and executed by BedrockCoder.
...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "Bedrock Coder", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".bedrock-coder", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".bedrock-coder", "hooks")
}
