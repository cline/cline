...
// If a user modifies a file outside of BedrockCoder, the context may become stale and need to be updated.
// We do not want BedrockCoder to reload the context every time a file is modified, so we use this class merely
// to inform BedrockCoder that the change has occurred, and tell BedrockCoder to reload the file before making
// any changes to it. This fixes an issue with diff editing, where BedrockCoder was unable to complete a diff edit.
// a diff edit because the file was modified since BedrockCoder last read it.

// FileContextTracker
/**
This class is responsible for tracking file operations.
If the full contents of a file are passed to BedrockCoder via a tool, mention, or edit, the file is marked as active.
If a file is modified outside of BedrockCoder, we detect and track this change to prevent stale context.
...
export class FileContextTracker {
	...
	private recentlyEditedByBedrockCoder = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.on("change", () => {
			if (this.recentlyEditedByBedrockCoder.has(filePath)) {
				this.recentlyEditedByBedrockCoder.delete(filePath) // This was an edit by BedrockCoder, no need to inform BedrockCoder
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform BedrockCoder
	 ...
	 * This is the main entry point for FileContextTracker and is called when a file is passed to BedrockCoder via a tool, mention, or edit.
	 */
	async trackFileContext(filePath: string, operation: "read_tool" | "user_edited" | "bedrockCoder_edited" | "file_mentioned") {
		try {
			const cwd = await getCwd()
			if (!cwd) {
				Logger.info("No workspace folder available - cannot determine current working directory")
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: FileMetadataEntry["record_source"]) {
		try {
			...
			const newEntry: FileMetadataEntry = {
				...
				bedrockCoder_read_date: getLatestDateForField(filePath, "bedrockCoder_read_date"),
				bedrockCoder_edit_date: getLatestDateForField(filePath, "bedrockCoder_edit_date"),
			...
			switch (source) {
				...
				// bedrockCoder_edited: BedrockCoder has edited the file
				case "bedrockCoder_edited":
					newEntry.bedrockCoder_read_date = now
					newEntry.bedrockCoder_edit_date = now
					break

				// read_tool/file_mentioned: BedrockCoder has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.bedrockCoder_read_date = now
					break
	...
	/**
	 * Marks a file as edited by BedrockCoder to prevent false positives in file watchers
	 */
	markFileAsEditedByBedrockCoder(filePath: string): void {
		this.recentlyEditedByBedrockCoder.add(filePath)
	}
	...
	/**
	 * Detects files that were edited by BedrockCoder or users after a specific message timestamp
	 * This is used when restoring checkpoints to warn about potential file content mismatches
	 */
	async detectFilesEditedAfterMessage(messageTs: number, deletedMessages: BedrockCoderMessage[]): Promise<string[]> {
		const editedFiles: string[] = []

		try {
			// Check task metadata for files that were edited by BedrockCoder or users after the message timestamp
			...
			if (taskMetadata?.files_in_context) {
				for (const fileEntry of taskMetadata.files_in_context) {
					const bedrockCoderEditedAfter =
						fileEntry.bedrockCoder_edit_date && fileEntry.bedrockCoder_edit_date > messageTs
					const userEditedAfter = fileEntry.user_edit_date && fileEntry.user_edit_date > messageTs

					if (bedrockCoderEditedAfter || userEditedAfter) {
