...
describe("VertexHandler", () => {
	...
	it("supports Gemini 3.6 Flash model metadata", () => {
		const handler = new VertexHandler({
			vertexProjectId: "test-project",
			vertexRegion: "global",
		...
		model.info.supportsGlobalEndpoint!.should.equal(true)
		model.info.supportsReasoning!.should.equal(true)
		vertexGlobalModels.should.have.property("gemini-3.6-flash")
	})

	it("supports Gemini 3.5 Flash Lite model metadata", () => {
		const handler = new VertexHandler({
			vertexProjectId: "test-project",
			vertexRegion: "global",
		...
		model.info.supportsGlobalEndpoint!.should.equal(true)
		model.info.supportsReasoning!.should.equal(true)
		vertexGlobalModels.should.have.property("gemini-3.5-flash-lite")
	})
})
