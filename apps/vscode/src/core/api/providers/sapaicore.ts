...
export class SapAiCoreHandler implements ApiHandler {
	...
	private async *createMessageWithDeployments(
		...
		if (!deploymentId) {
			...
			Logger.log(
				`No pre-configured deployment ID found for model ${model.id}, falling back to runtime fetching`,
			);
		...
		if (anthropicModels.includes(model.id) || novaModels.includes(model.id)) {
			...
			if (converseStreamModels.includes(model.id)) {
				...
				const messagesForPayload = enableCaching
					...
					: formattedMessages;
				const systemMessages = Bedrock.prepareSystemMessages(
					systemPrompt,
					enableCaching,
				);
						...
						try {
							...
							if (data.metadata?.usage) {
								...
								const cacheReadInputTokens =
									data.metadata.usage.cacheReadInputTokens || 0;
								const cacheWriteInputTokens =
									data.metadata.usage.cacheWriteInputTokens || 0;
								inputTokens =
							...
							if (data.contentBlockDelta) {
								...
								if (data.contentBlockDelta?.delta?.reasoningContent?.text) {
									yield {
										type: "reasoning",
										reasoning:
											data.contentBlockDelta.delta.reasoningContent.text,
									};
								}
							}
						} catch (error) {
				...
				for (const line of lines) {
					...
					try {
						...
						if (data.type === "message-end") {
							const usage =
								data.delta?.usage?.tokens ?? data.delta?.usage?.billed_units;
							if (usage) {
					...
					if (line.startsWith("data: ")) {
						...
						try {
							...
							if (processed.usageMetadata) {
								promptTokens =
									processed.usageMetadata.promptTokenCount ?? promptTokens;
								outputTokens =
									processed.usageMetadata.candidatesTokenCount ?? outputTokens;
								thoughtsTokenCount =
									processed.usageMetadata.thoughtsTokenCount ??
									thoughtsTokenCount;
								cacheReadTokens =
									processed.usageMetadata.cachedContentTokenCount ??
									cacheReadTokens;
