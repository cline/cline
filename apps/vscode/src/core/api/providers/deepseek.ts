...
/** DeepSeek accepts high/max; map UI efforts (low/medium → high, xhigh → max). */
function toDeepSeekReasoningEffort(
	effort: string,
): ChatCompletionReasoningEffort {
	if (effort === "low" || effort === "medium") {
		return "high" as ChatCompletionReasoningEffort;
	}
	if (effort === "xhigh") {
		return "max" as ChatCompletionReasoningEffort;
	}
