import { refreshBedrockCoderRulesToggles } from "@core/context/instructions/user-instructions/bedrock-coder-rules"
import { createRuleFile as createRuleFileImpl } from "@core/context/instructions/user-instructions/rule-helpers"
import { getWorkspaceBasename } from "@core/workspace"
import { RuleFile, RuleFileRequest } from "@shared/proto/bedrock_coder/file"
...
export async function createRuleFile(controller: Controller, request: RuleFileRequest): Promise<RuleFile> {
	...
	if (fileExists) {
		...
		if (request.type === "workflow") {
			...
			await refreshBedrockCoderRulesToggles(controller, cwd)
		}
