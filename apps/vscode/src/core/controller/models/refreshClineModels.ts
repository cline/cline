...
function preferCanonicalModelIds<T>(
...
function preferClineCanonicalModelIds(
...
export async function refreshClineModels(
	...
	if (!shouldUseClineEndpointSource) {
		return preferClineCanonicalModelIds(
			await refreshOpenRouterModels(controller),
		);
	}
...
async function fetchAndCacheClineModels(): Promise<Record<string, ModelInfo>> {
	...
	try {
		...
		for (const rawModel of rawModels) {
			...
			if (
				...
				const isSonnet5 =
					rawModel.id === "anthropic/claude-sonnet-5" ||
					rawModel.id === "anthropic/claude-5-sonnet";
				claudeSonnet1mModelInfo.tiers = isSonnet5
					? CLAUDE_SONNET_5_1M_TIERS
					: CLAUDE_SONNET_1M_TIERS;
			...
			if (
				rawModel.id === "anthropic/claude-opus-5" ||
				rawModel.id === "anthropic/claude-5-opus"
			) {
		...
		await fs.writeFile(
			clineModelsFilePath,
			JSON.stringify(preferClineCanonicalModelIds(models)),
		);
