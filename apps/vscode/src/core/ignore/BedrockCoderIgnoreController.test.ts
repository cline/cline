...
import { BedrockCoderIgnoreController } from "./BedrockCoderIgnoreController"
import "should"

describe("BedrockCoderIgnoreController", () => {
	let tempDir: string
	let controller: BedrockCoderIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .bedrock-coderignore file
		await fs.writeFile(
			path.join(tempDir, ".bedrock-coderignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new BedrockCoderIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .bedrock-coderignore file", async () => {
			const result = controller.validateAccess(".bedrock-coderignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .bedrock-coderignore", async () => {
			// Create a new .bedrock-coderignore with comments
			await fs.writeFile(
				path.join(tempDir, ".bedrock-coderignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .bedrock-coderignore gracefully", async () => {
			// Create a new controller in a directory without .bedrock-coderignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new BedrockCoderIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .bedrock-coderignore", async () => {
			await fs.writeFile(path.join(tempDir, ".bedrock-coderignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .bedrock-coderignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".bedrock-coderignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .bedrock-coderignore
			controller = new BedrockCoderIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .bedrock-coderignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .bedrock-coderignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".bedrock-coderignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".bedrock-coderignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
