 ...
 * Designed to be instantiated once in BedrockCoder.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .bedrock-coderignore files.
 */
export class BedrockCoderIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	bedrockCoderIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.bedrockCoderIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .bedrock-coderignore
		this.setupFileWatcher()
		await this.loadBedrockCoderIgnore()
	}

	/**
	 * Set up the file watcher for .bedrock-coderignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".bedrock-coderignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .bedrock-coderignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .bedrock-coderignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadBedrockCoderIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.bedrockCoderIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".bedrock-coderignore")
			} else {
				this.bedrockCoderIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .bedrock-coderignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processBedrockCoderIgnoreIncludes(content)
	 ...
	 */
	private async processBedrockCoderIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[BedrockCoderIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .bedrock-coderignore does not exist
		if (!this.bedrockCoderIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .bedrock-coderignore exists
		if (!this.bedrockCoderIgnoreContent) {
			return undefined
		}
