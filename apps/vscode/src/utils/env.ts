 ...
 * The host enforces the corporate external-navigation setting. There is no
 * package/shell fallback because that would bypass the host policy.
 *
 * @param url The URL to open
 * @returns Promise that resolves when the operation is complete
 */
export async function openExternal(url: string): Promise<void> {
	Logger.log("External browser navigation requested")
	try {
		await HostProvider.env.openExternal(StringRequest.create({ value: url }))
	} catch (error) {
		Logger.warn("Host blocked or failed external browser navigation", error)
		throw error
	}
}
