...
describe("Filesystem Utilities", () => {
	const tmpDir = path.join(os.tmpdir(), "bedrock-coder-test-" + Math.random().toString(36).slice(2))
	...
	it("should exclude .bedrock-coder/workflows directory specifically", async () => {
		// Create a test directory structure
		const bedrockCoderRulesDirTest = path.join(tmpDir, "bedrock-coder-rules-test")
		const bedrockCoderRulesDirPath = path.join(bedrockCoderRulesDirTest, ".bedrock-coder")

		// Create .bedrock-coder directory and root files
		await fs.mkdir(bedrockCoderRulesDirPath, { recursive: true })
		await fs.writeFile(path.join(bedrockCoderRulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(bedrockCoderRulesDirPath, "settings.js"), "// settings")

		// Create .bedrock-coder/other directory and files
		const otherDirPath = path.join(bedrockCoderRulesDirPath, "other")
		...
		// Create .bedrock-coder/workflows directory and files
		const workflowsDirPath = path.join(bedrockCoderRulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(bedrockCoderRulesDirPath)
		...
		const filteredFiles = await readDirectory(bedrockCoderRulesDirPath, [[".bedrock-coder", "workflows"]])

		// Verify workflows files are excluded but others remain
		filteredFiles.length.should.equal(4) // 2 in root + 2 in other

		const expectedFiles = [
			path.resolve(bedrockCoderRulesDirPath, "config.json"),
			path.resolve(bedrockCoderRulesDirPath, "settings.js"),
		...
		const multiExcludeFiles = await readDirectory(bedrockCoderRulesDirPath, [
	...
	it("should exclude .bedrock-coder/hooks directory specifically", async () => {
		// Create a test directory structure
		const bedrockCoderRulesDirTest = path.join(tmpDir, "bedrock-coder-rules-hooks-test")
		const bedrockCoderRulesDirPath = path.join(bedrockCoderRulesDirTest, ".bedrock-coder")
		...
		// Create .bedrock-coder/hooks directory and files
		const hooksDirPath = path.join(bedrockCoderRulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(bedrockCoderRulesDirPath, [[".bedrock-coder", "hooks"]])
