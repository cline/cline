...
export function getReadablePath(cwd: string, relPath?: string): string {
	...
	const absolutePath =
		typeof absolutePathResult === "string"
			? absolutePathResult
...
export function getDesktopDir() {
	...
	return typeof desktopResult === "string"
		? desktopResult
		: desktopResult.absolutePath;
}
...
export async function getWorkspacePath(defaultCwd = ""): Promise<string> {
	const currentFilePath = (await HostProvider.window.getActiveEditor({}))
		.filePath;
	if (!currentFilePath) {
...
export async function isLocatedInWorkspace(pathToCheck = ""): Promise<boolean> {
	...
	for (const workspacePath of workspacePaths) {
		...
		const resolvedPath =
			typeof resolvedPathResult === "string"
				? resolvedPathResult
				: resolvedPathResult.absolutePath;
...
export function isLocatedInPath(dirPath: string, pathToCheck: string): boolean {
	...
	const resolvedDir =
		typeof resolvedDirResult === "string"
			? resolvedDirResult
			: resolvedDirResult.absolutePath;
