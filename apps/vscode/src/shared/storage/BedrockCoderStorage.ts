...
interface BedrockCoderStorageChangeEvent {
	readonly key: string
}

export type StorageEventListener = (event: BedrockCoderStorageChangeEvent) => Promise<void>
 ...
 * VSCode's Memento and BedrockCoderSyncStorage both satisfy this interface.
 */
export interface BedrockCoderMemento {
 ...
 */
export abstract class BedrockCoderStorage {
	/**
	 * The name of the storage, used for logging purposes.
	 */
	protected name = "BedrockCoderStorage"
	/**
 ...
 * Unlike BedrockCoderStorage (string key-value, async), this stores any JSON-serializable
 * values and provides synchronous access - required for VSCode Memento compatibility.
 */

export type SyncStorageEventListener = (event: BedrockCoderStorageChangeEvent) => void

export abstract class BedrockCoderSyncStorage<T = any> {
