...
export abstract class DiffViewProvider {
	...
	public async open(relPath: string, options?: { displayPath?: string }): Promise<void> {
		...
		try {
			...
			this.absolutePath =
				typeof absolutePathResolved === "string" ? absolutePathResolved : absolutePathResolved.absolutePath
			...
			// A failed open (e.g. EISDIR from a malformed path, or a diff editor that never came up)
			// must not leave the provider in "editing" state. Otherwise every subsequent edit attempt
			// in the task skips open() and fails with "User closed text editor..." until the task is
			// restarted.
			if (this.editType === "create" && this.absolutePath) {
				// Remove the empty file/directories we may have created before the failure.
				await fs.rm(this.absolutePath, { force: true }).catch(() => {})
				for (let i = this.createdDirs.length - 1; i >= 0; i--) {
					await fs.rmdir(this.createdDirs[i]).catch(() => {})
				}
			}
			await this.reset()
			throw error
