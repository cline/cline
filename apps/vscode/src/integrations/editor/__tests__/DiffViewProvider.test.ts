...
describe("DiffViewProvider open() failure recovery", () => {
	...
	it("resets editing state when reading the target throws (EISDIR on a directory path)", async () => {
		...
		assert.strictEqual(provider.isEditing, false, "isEditing must be cleared after a failed open")
		assert.strictEqual(provider.editType, undefined, "editType must be cleared after a failed open")
	})

	it("allows a subsequent open() to succeed after a failed one", async () => {
		await fs.mkdir(path.join(tmpDir, "tests"))
		await fs.writeFile(path.join(tmpDir, "tests", "file.cpp"), "int main() {}\n")
	...
	it("removes the created file and directories when opening the diff editor fails", async () => {
		...
		await assert.rejects(provider.open("newdir/newfile.txt"), /Failed to open diff editor/)
		...
		assert.strictEqual(existsSync(path.join(tmpDir, "newdir")), false, "created directories must be removed")
	})
})
