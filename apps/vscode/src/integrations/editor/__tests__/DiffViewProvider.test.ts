...
describe("DiffViewProvider open() failure recovery", () => {
	// Regression tests for https://github.com/cline/cline/issues/12863:
	// a failed open() (e.g. EISDIR when a malformed/truncated streaming path resolves to a
	// directory) used to leave isEditing=true, so every subsequent edit attempt skipped open()
	// and failed with "User closed text editor, unable to edit file..." until task restart.

	class FailingEditorDiffViewProvider extends TestBoundaryDiffViewProvider {
		public failOpenDiffEditor = false;

		override async openDiffEditor(): Promise<void> {
			if (this.failOpenDiffEditor) {
	...
	beforeEach(async () => {
		tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "cline-diffview-open-"));
		setVscodeHostProviderMock({
			hostBridgeClient: {
				workspaceClient: {
					getWorkspacePaths: async () => ({ paths: [tmpDir] }),
	...
	it("resets editing state when reading the target throws (EISDIR on a directory path)", async () => {
		await fs.mkdir(path.join(tmpDir, "tests"));

		const provider = new TestBoundaryDiffViewProvider();
		// "modify" is what WriteToFileToolHandler sets when fs.access succeeds on the path,
		// which is also true for directories.
		provider.editType = "modify";

		await assert.rejects(provider.open("tests"), /EISDIR/);

		assert.strictEqual(
			provider.isEditing,
			false,
			"isEditing must be cleared after a failed open",
		);
		assert.strictEqual(
			provider.editType,
			undefined,
			"editType must be cleared after a failed open",
		);
	});

	it("allows a subsequent open() to succeed after a failed one", async () => {
		await fs.mkdir(path.join(tmpDir, "tests"));
		await fs.writeFile(
			path.join(tmpDir, "tests", "file.cpp"),
			"int main() {}\n",
		);
		...
		// Retry with the correct (complete) path must work instead of erroring with
		// "User closed text editor, unable to edit file..."
		provider.editType = "modify";
		await provider.open("tests/file.cpp");

		assert.strictEqual(provider.isEditing, true);
		assert.strictEqual(provider.originalContent, "int main() {}\n");
	});

	it("removes the created file and directories when opening the diff editor fails", async () => {
		const provider = new FailingEditorDiffViewProvider();
		provider.failOpenDiffEditor = true;
		provider.editType = "create";

		await assert.rejects(
			provider.open("newdir/newfile.txt"),
			/Failed to open diff editor/,
		);
		...
		assert.strictEqual(
			existsSync(path.join(tmpDir, "newdir", "newfile.txt")),
			false,
			"the empty placeholder file must be removed",
		);
		assert.strictEqual(
			existsSync(path.join(tmpDir, "newdir")),
			false,
			"created directories must be removed",
		);
	});
});
...
describe("DiffViewProvider Boundary Validation", () => {
	...
	it("safelyTruncateDocument should no-op when lineNumber >= lineCount", async () => {
		...
		assert.strictEqual(
			provider.truncatedAt,
			undefined,
			"Should not have called truncateDocument",
		);
	});
...
describe("DiffViewProvider Update Throttling", () => {
	...
	it("should skip empty content during streaming", async () => {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			0,
			"Should not call replaceText for empty content",
		);
	});
	...
	it("should skip unchanged content length during streaming", async () => {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			1,
			"Should skip update with same content length",
		);
	});
	...
	it("should always process final updates regardless of throttling", async () => {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			2,
			"Final update should bypass throttling",
		);
	});
	...
	it("should process final update even with empty content", async () => {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			1,
			"Empty final update should be processed",
		);
	});
	...
	it("should process final update even with same content length", async () => {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			2,
			"Final update should bypass length check",
		);
	});
	...
	it("should reset throttle state on reset()", async function () {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			1,
			"Update after reset should go through immediately",
		);
	});
	...
	it("should handle streaming simulation with many rapid updates", async function () {
		...
		for (let i = 1; i <= 100; i++) {
			const content =
				Array.from({ length: i }, (_, j) => `line${j + 1}`).join("\n") + "\n";
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			1,
			"Should throttle rapid streaming updates",
		);
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			2,
			"Update after throttle should go through",
		);
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			3,
			"Final update should go through",
		);
	});
	...
	it("should throttle by time regardless of content length changes", async function () {
		...
		assert.strictEqual(
			provider.replaceTextCallCount,
			1,
			"Should still throttle by time even with different length",
		);
