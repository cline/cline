...
import type { DiffEditSurface } from "../DiffViewProvider";
import {
	DiffViewProvider,
	setDiffViewTelemetryForTesting,
...
function stubDiffTelemetry(): {
	events: DiffTelemetryEvent[];
	restore: () => void;
} {
	const events: DiffTelemetryEvent[] = [];
	const restore = setDiffViewTelemetryForTesting({
...
class TestBoundaryDiffViewProvider extends DiffViewProvider {
	...
	public setupOpenState(
		absolutePath: string,
		editType: "create" | "modify" | "delete" = "modify",
	) {
		this.isEditing = true;
...
describe("DiffViewProvider telemetry lifecycle", () => {
	it("open() emits opened telemetry with edit surface metadata", async () => {
		const tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "cline-diff-telemetry-open-"),
		);
		const filePath = path.join(tempDir, "file.txt");
		await fs.writeFile(filePath, "original");
		setupHostProviderForDiffTests(tempDir);
		const { events, restore } = stubDiffTelemetry();
		try {
			const provider = new TestBoundaryDiffViewProvider();
	...
	it("saveChanges() emits accepted once and reset re-arms accepted telemetry", async () => {
		const tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "cline-diff-telemetry-accept-"),
		);
		setupHostProviderForDiffTests(tempDir);
		const { events, restore } = stubDiffTelemetry();
		try {
	...
	it("revert after accept records previous accepted outcome", async () => {
		const tempDir = await fs.mkdtemp(
			path.join(os.tmpdir(), "cline-diff-telemetry-revert-"),
		);
		setupHostProviderForDiffTests(tempDir);
		const { events, restore } = stubDiffTelemetry();
		try {
