...
import { fetchLiveModelsDevProviderModels } from "./shared/models/models-dev-catalog";
...
export async function activate(context: vscode.ExtensionContext) {
	...
	void refreshModelsDevCatalogOnActivation(webview);
	...
	HookDiscoveryCache.getInstance().initialize(
		context as { subscriptions: Array<{ dispose(): void }> },
		(dir: string) => {
			try {
...
async function refreshModelsDevCatalogOnActivation(
	webview: VscodeWebviewProvider,
): Promise<void> {
	try {
		const models = await fetchLiveModelsDevProviderModels();
		if (Object.keys(models).length === 0) {
			return;
		}

		webview.controller.modelsDevProviderModels = models;
		await webview.controller.postStateToWebview();
		Logger.log("[ModelsDev] Refreshed live models.dev catalog");
	} catch (error) {
		Logger.warn(
			"[ModelsDev] Failed to refresh live models.dev catalog; using generated fallback",
			error,
		);
	}
}
