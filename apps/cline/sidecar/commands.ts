...
import {
	broadcastEvent,
	claimSidecarMessageBot,
	ensureSharedHubClient,
	resolveSidecarAskQuestion,
	resolveSidecarMessageBot,
} from "./context";
import {
...
// Matches main.rs's own SYSTEM_PROMPT_FILE_NAME constant - kept in sync
// manually, since that's Rust and this is TypeScript, the same way
// DEFAULT_BOT_ID is already kept in sync across the two elsewhere.
const SYSTEM_PROMPT_FILE_NAME = "system-prompt.md";
...
async function listUserInstructionConfigs(
	...
	const loadUserInstructionSnapshot = (
		...
		for (const record of userInstructionService.listRecords(type)) {
			...
			// The bot's system prompt is a plain rules file so it's picked up by
			// the same mechanism that loads every other rule into the session's
			// system prompt (see main.rs's write_bot_system_prompt) - but it has
			// its own dedicated Settings section, so it's excluded here to avoid
			// listing it twice.
			if (
				type === "rule" &&
				basename(record.filePath) === SYSTEM_PROMPT_FILE_NAME
			) {
				continue;
			}
			items.push({
...
export async function handleCommand(
	...
	if (command === "claim_message_bot") {
		const requestId = String(args?.requestId ?? "").trim();
		if (!requestId) {
			throw new Error("requestId is required");
		}
		if (!options?.connection) {
			throw new Error("claim_message_bot requires a websocket connection");
		}
		return claimSidecarMessageBot(ctx, requestId, options.connection);
	}
	if (command === "respond_message_bot") {
		const requestId = String(args?.requestId ?? "").trim();
		if (!requestId) {
			throw new Error("requestId is required");
		}
		...
		if (
			pending?.claimedBy &&
			(!options?.connection || pending.claimedBy !== options.connection)
		) {
			throw new Error(`message_bot request is claimed by another connection`);
		}
		const rawResult = (args?.result ?? {}) as Partial<{
			delivered: boolean;
			botId: string;
			botName: string;
			sessionId: string;
		...
		const target = {
			...(typeof rawResult.botId === "string"
				? { botId: rawResult.botId }
				: {}),
			...(typeof rawResult.botName === "string"
				? { botName: rawResult.botName }
				: {}),
			...(typeof rawResult.sessionId === "string"
				? { sessionId: rawResult.sessionId }
				: {}),
		};
		const result: MessageBotResult = rawResult.delivered
			? {
					delivered: true,
					...target,
					...(typeof rawResult.reply === "string"
			...
			: {
					...
					...target,
					error:
						typeof rawResult.error === "string" && rawResult.error.trim()
							? rawResult.error.trim()
							: "message_bot failed without an error detail",
				};
		const resolved = resolveSidecarMessageBot(ctx, requestId, result);
		if (!resolved) {
			// Unlike respond_ask_question (one human, one answer), more than one
			// webview client can end up subscribed to the same "cline" sidecar
			// (e.g. two connected windows, or a stale subscription surviving a
			// dev-mode Fast Refresh) and each independently relay + respond to
			// the same requestId. The first response already resolved the
			// pending tool call; a duplicate/late one here is benign, not an
			// error worth surfacing as an unhandled rejection to the caller.
			return false;
		}
		broadcastEvent(ctx, "message_bot_answered", { requestId });
		return true;
	}
	...
	// Not a native command (see desktop-client.ts's NATIVE_COMMANDS) - it
	// must reach whichever bot's sidecar is currently connected over the WS
	// transport, since a brand-new bot (the common caller) has no provider
	// config of its own yet to generate with; this bot's own already-running
	// provider settings are used instead.
	if (command === "generate_bot_system_prompt") {
		const description = String(args?.description ?? "").trim();
		if (!description) {
			throw new Error("description is required");
		}
		const manager = new ProviderSettingsManager();
		const providerConfig = manager.getLastUsedProviderConfig();
		if (!providerConfig) {
			throw new Error("No model provider is configured yet.");
		}
		return await generateSystemPromptFromDescription({
			providerConfig,
			description,
			logger: ctx.logger,
		});
