...
function text(parts: readonly AgentMessagePart[]): string {
	return parts.map((part) => {
		...
		if (part.type === "tool-result") return JSON.stringify({ toolName: part.toolName, output: part.output, error: part.isError });
		return "";
	}).filter(Boolean).join("\n");
}
...
async function setGatewayToolsDisabled(
	...
	const ids = catalog.entries
		...
		.filter((id) => requested.has(id) || requested.has(id.slice(id.indexOf(":") + 1)));
	if (ids.length === 0) throw new Error(`Unknown Gateway tool: ${names.join(", ")}`);
...
async function sessionMessages(ctx: SidecarContext, sessionId: string) {
	const snapshot = await ctx.client.getSession({ sessionId: sessionId as SessionId });
	return snapshot.messages.map(({ message }) => toChatMessage(sessionId, message));
}
...
async function chatCommand(ctx: SidecarContext, request: RecordValue) {
	...
	if (action === "attach") {
		...
		const snapshot = await ctx.client.getSession({ sessionId: sessionId as SessionId });
		const running = [...snapshot.runs].reverse().find((run) => run.state === "running" || run.state === "queued");
	...
	if (action === "start" || action === "send") {
		...
		if (!acceptedSessionId) throw new Error("Gateway accepted a run without a session");
		ctx.activeRuns.set(acceptedSessionId, accepted.runId);
		return { sessionId: acceptedSessionId, workspaceRoot: ctx.workspaceRoot, cwd: ctx.workspaceRoot, ok: true, queued: true };
	}
	if (action === "stop" || action === "abort") {
		const runId = sessionId ? ctx.activeRuns.get(sessionId) : undefined;
		if (runId) await ctx.client.interruptRun({ runId: runId as RunId, reason: action });
	...
	if (action === "steer_prompt") {
		...
		await ctx.client.steerRun({ runId: runId as RunId, text: String(request.prompt ?? "") });
		return { sessionId, ok: true };
	}
...
export async function handleCommand(ctx: SidecarContext, command: string, args: RecordValue = {}): Promise<unknown> {
	...
	if (command === "get_process_context") {
		...
		return { workspaceRoot: ctx.workspaceRoot, cwd: ctx.workspaceRoot, homeDir: homedir(), platform: process.platform, appVersion: "0.0.1", runningSessionCount: status.counts.runningRuns, gateway: { status: "connected", gatewayId: status.gatewayId, namespace: status.namespace } };
	}
	if (command === "list_bots") {
		const { bots } = await ctx.client.listBots();
		return { bots: bots.map((bot) => ({ id: bot.identity.botId, botId: bot.identity.botId, name: bot.identity.name, role: bot.identity.role, active: bot.status === "active" })) };
	}
	if (command === "poll_tool_approvals") return [...ctx.pendingApprovals.entries()].map(([requestId, value]) => ({ requestId, sessionId: value.sessionId, ...(value.request as object) }));
	if (command === "respond_tool_approval") {
		ctx.client.resolveApproval(String(args.requestId), { approved: Boolean(args.approved), reason: typeof args.reason === "string" ? args.reason : undefined });
	...
	if (["set_tray_status", "set_app_icon", "update_chat_session_title", "update_chat_session_metadata", "respond_ask_question", "respond_message_bot", "claim_message_bot"].includes(command)) return true;
