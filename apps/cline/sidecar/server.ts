...
import {
	encodeSidecarEvent,
	releaseSidecarMessageBotClaims,
	sendEvent,
} from "./context";
...
export function createWebSocketHandler(ctx: SidecarContext) {
	return {
		...
		open(ws: SidecarWebSocketClient) {
			...
			// A renderer may reconnect after the connection that owned a relay
			// claim disappeared. Re-offer every unclaimed request to this client;
			// claim_message_bot still elects exactly one winner across windows.
			for (const pending of ctx.pendingMessageBots.values()) {
				if (!pending.claimedBy) {
					ws.send(encodeSidecarEvent("message_bot_requested", pending.item));
				}
			}
		},
		...
		close(ws: SidecarWebSocketClient) {
			...
			releaseSidecarMessageBotClaims(ctx, ws);
