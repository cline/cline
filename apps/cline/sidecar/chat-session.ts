...
import { DEFAULT_BOT_ID, resolveActiveBotId } from "../sandbox/bot-config";
import {
...
import { buildMessageBotTool, emitChunk, nowMs, sendEvent } from "./context";
...
/**
 * `message_bot` is only ever offered to the default "cline" bot - see
 * apps/cline/sandbox/bot-config.ts's own "a bot's tree is never seeded"
 * isolation principle; other bots stay simple workers with no cross-bot
 * capability of their own. `buildCoreSessionConfig`'s return type is a fixed
 * shape (no passthrough of unknown keys), so `extraTools` can't flow through
 * it - spread this in *alongside* its output at each call site instead.
 */
function messageBotExtraTools(ctx: SidecarContext): JsonRecord {
	return resolveActiveBotId() === DEFAULT_BOT_ID
		? { extraTools: [buildMessageBotTool(ctx) as unknown as AgentTool] }
		: {};
}
...
async function handleSend(
	...
	const clientTurnId = request.clientTurnId?.trim();
	if (clientTurnId && clientTurnId.length > 200) {
		throw new Error("clientTurnId must be at most 200 characters");
	}
