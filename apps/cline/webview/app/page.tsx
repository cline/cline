...
import type { OnCreateBot } from "@/components/views/chat/messages/tool-message-block";
...
export default function Home() {
	...
	const handleSwitchBot = useCallback(
		async (botId: string) => {
			if (botId === botRegistry.activeBotId) {
				return;
			}
			await botRegistry.switchBot(botId);
			// A fresh thread remounts ChatThreadPane (key={activeThread.id}),
	...
	const handleCreateBot = useCallback(
		async (
			...
			const created = await botRegistry.createBot(
				name,
				initialProjectPath,
				icon,
				systemPrompt,
	...
	const handleInitialWorkspacePathConsumed = useCallback((threadId: string) => {
		dispatchApp({ type: "consume-initial-workspace-path", threadId });
	}, []);
						...
						{view === "sessions" ? (
							...
							>
								<ChatThreadPane
									...
									onInitialWorkspacePathConsumed={
										handleInitialWorkspacePathConsumed
									}
									onUpdateSessionMetadata={handleUpdateSessionMetadata}
...
function ChatThreadPane({
	...
	onInitialWorkspacePathConsumed?: (threadId: string) => void;
	onUpdateSessionMetadata?: (
		sessionId: string,
		metadata: SessionMetadata,
	) => void;
	onDeleteSession?: (sessionId: string, threadId?: string) => void;
	onNewThread?: (initialWorkspacePath?: string) => void;
	onOpenSession?: (
	...
	const [hasConnectedProvider, setHasConnectedProvider] = useState<
		boolean | null
	>(null);
	// In the sandboxed Tauri app, the offered project list comes solely from
	// this bot's assigned-projects registry (see list_assigned_projects) -
	// starts empty and is populated by the mount effect below. Outside Tauri
	// (plain web dev mode, no per-bot registry) this falls back to the
	// previous session-history + localStorage merge. History paths lead each
	// merge: they are ordered by session recency, so stored or stale entries
	// only append after them.
	...
	// Historical-session metadata already names the backend project before the
	// chat hook has hydrated that metadata into its local config. Use it for the
	// first render so transport selection does not briefly connect to the empty
	// project and then switch again while hydration requests are in flight.
	const backendWorkspaceCwd = historySession
	...
	useEffect(() => {
		...
		// construction, so merging them back in here would just reintroduce
		// the untrusted "recent paths" catalog this whole change removes.
		if (isTauriAvailable()) {
			return;
		}
	...
	useEffect(() => {
		// The registry (host-owned, per bot) is authoritative in the
		// sandboxed app; no need to also mirror the list into localStorage,
		// which isn't namespaced per bot at all.
		if (isTauriAvailable()) {
			return;
		}
	...
	useLayoutEffect(() => {
		...
		const projectPath = isChatWorkspacePath(backendWorkspaceCwd)
			? ""
			: backendWorkspaceCwd;
		desktopClient.setActiveProject(activeBotId, projectPath);
	}, [activeBotId, backendWorkspaceCwd]);
	...
	const refreshWorkspaces = useCallback(
		async (preferredWorkspace?: string) => {
			try {
				// In the sandboxed app, the offered list is exactly this bot's
				// assigned-projects registry - never session history or
				// localStorage, which could show a project the bot was never
				// actually granted. `preferredWorkspace` isn't merged in here:
				// switchWorkspace already adds it to the registry (via
				// assign_project) before calling this, so it's already present.
				const results = isTauriAvailable()
					? await desktopClient.invoke<string[]>("list_assigned_projects", {
							botId: activeBotId,
						})
					: await listWorkspaces(preferredWorkspace);
				setWorkspaces((current) => {
	...
	const switchWorkspace = useCallback(
		async (workspacePath: string): Promise<boolean> => {
			...
			if (isTauriAvailable()) {
				// assign_project validates unsandboxed and registers the path
				// (idempotent if already assigned) before anything mounts it -
				// see main.rs. Always called, even for an already-listed path,
				// rather than trusting local `workspaces` state to be in sync
				// with the real registry.
				try {
					resolvedWorkspace = await desktopClient.invoke<string>(
						"assign_project",
						{ botId: activeBotId, path: nextWorkspace },
					);
				} catch {
					return false;
				}
		...
		[activeBotId, invalidateGitBranch, refreshWorkspaces, setWorkspacePath],
	);

	// A freshly created bot handed an initial project (see Home()'s
	// handleCreateBot) opens straight into it, once, the same way a forked
	// session hands off its initialPromptDraft exactly once.
	const initialWorkspaceConsumedRef = useRef(false);
	useEffect(() => {
		if (!initialWorkspacePath || initialWorkspaceConsumedRef.current) {
			return;
		}
		initialWorkspaceConsumedRef.current = true;
		void switchWorkspace(initialWorkspacePath);
		onInitialWorkspacePathConsumed?.(threadId);
	}, [
		initialWorkspacePath,
		onInitialWorkspacePathConsumed,
		switchWorkspace,
