...
export function useChatSession() {
	...
	const setErrorState = useCallback(
		(msg: string, sid: string | null = null) => {
			// A project/bot switch deliberately drops in-flight requests (see
			// DesktopClient.setActiveProject) - that's not a failure worth
			// surfacing as an error bubble, and reconnecting already shows its
			// own "Connecting chat..." banner. The turn that was in flight
			// re-resolves normally once the new connection completes.
			if (msg === PROJECT_SWITCH_ERROR_MESSAGE) {
				return;
			}
	...
	const applyProcessContext = useCallback(async () => {
		...
		try {
			...
			// In the desktop app, which project is open is governed entirely by
			// the host-owned per-bot project registry (assign_project /
