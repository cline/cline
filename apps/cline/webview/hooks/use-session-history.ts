...
export type UseSessionHistoryOptions = {
	...
	/** Which bot's history this is. Switching bots means switching to a
	 * completely different backend process with its own session history -
	 * the previous bot's sessions are meaningless once this changes, so they
	 * get cleared immediately instead of lingering on screen (stale-looking)
	 * until the new bot's sidecar comes up and its own sessions arrive. */
	activeBotId?: string;
...
export function useSessionHistory({
	...
	// A bot switch points desktopClient at a completely different sidecar
	// process with its own, unrelated session history - the previous bot's
	// sessions/threads are dropped immediately (rather than left on screen
	// looking stale) and isLoadingHistory is forced back on so the sidebar's
	// "Loading session history..." state shows for however long the new
	// bot's sidecar takes to come up, instead of silently doing nothing
	// visible until its sessions happen to arrive. Skipped on the initial
	// mount (the ref already matches on first render) - that case is already
	// handled by the mount effect below.
	useEffect(() => {
		if (activeBotIdRef.current === activeBotId) {
			return;
		}
		...
		fetchLimitRef.current = INITIAL_HISTORY_FETCH_LIMIT;
		mayHaveMoreSessionsRef.current = false;
		setMayHaveMoreSessions(false);
		setIsLoadingHistory(true);
		scheduleRefresh(0, { force: true });
	}, [activeBotId, scheduleRefresh]);
