...
const { invokeMock, subscribeMock, isTauriAvailableMock } = vi.hoisted(() => ({
	invokeMock: vi.fn(),
	subscribeMock: vi.fn(
...
vi.mock("@/lib/desktop-client", () => ({
	...
	PROJECT_SWITCH_ERROR_MESSAGE: "Switching to a different project",
}));
...
beforeEach(async () => {
	...
	isTauriAvailableMock.mockReset().mockReturnValue(false);
...
describe("useChatSession", () => {
	...
	it("does not show an error bubble when a request is dropped by a deliberate project switch", async () => {
		invokeMock.mockImplementation(async (command: string) => {
			...
			if (command === "chat_session_command") {
				throw new Error("Switching to a different project");
			}
			return [];
		});
	...
	it("in Tauri mode, never seeds the workspace from remembered localStorage or the backend's ambient cwd", async () => {
		isTauriAvailableMock.mockReturnValue(true);
		await act(async () => root.unmount());
		window.localStorage.setItem(
			"cline.code.workspace-selection.v1",
			JSON.stringify({
				lastWorkspace: "/Users/beatrix/dev/cline",
				workspaces: ["/Users/beatrix/dev/cline"],
			}),
		);
		invokeMock.mockImplementation(async (command: string) => {
			if (command === "get_process_context") {
				return {
					cwd: "/Users/beatrix/dev/clinee",
					workspaceRoot: "/Users/beatrix/dev/clinee",
				};
			}
			if (command === "validate_workspace_directory") {
				return { valid: true };
