...
describe("desktopAppReducer", () => {
	...
	it("hands a new thread its initial workspace path exactly once", () => {
		let state = createDesktopAppState("welcome", settingsSection);
		state = desktopAppReducer(state, {
			type: "new-thread",
			threadId: "thread-with-project",
			initialWorkspacePath: "/Users/example/projects/marketing",
		});

		expect(
			state.threads.find((thread) => thread.id === "thread-with-project")
				?.initialWorkspacePath,
		).toBe("/Users/example/projects/marketing");

		state = desktopAppReducer(state, {
			type: "consume-initial-workspace-path",
			threadId: "thread-with-project",
		});
