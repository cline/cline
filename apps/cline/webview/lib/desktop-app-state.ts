...
export type DesktopAppAction<SettingsSection extends string> =
	...
	| { type: "back" }
	| { type: "forward" }
	| { type: "new-thread"; threadId: string; initialWorkspacePath?: string }
	| {
			type: "open-session";
	...
	| { type: "consume-initial-workspace-path"; threadId: string }
	| {
			type: "delete-session";
...
export function desktopAppReducer<SettingsSection extends string>(
	...
	switch (action.type) {
		...
		case "consume-initial-workspace-path":
			return {
				...state,
				threads: state.threads.map((thread) =>
					thread.id === action.threadId &&
					thread.initialWorkspacePath !== undefined
						? { ...thread, initialWorkspacePath: undefined }
						: thread,
				),
			};
