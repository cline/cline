# [experimental] @bedrock-coder/agents

`@bedrock-coder/agents` is the runtime-agnostic agent loop package in the BedrockCoder SDK.
...
`@bedrock-coder/agents` does not ship a full application runtime by itself.

- Default host tools like filesystem access, shell execution, or web fetching live in `@bedrock-coder/core`
- Session persistence and stateful orchestration live in `@bedrock-coder/core`
- Shared hub runtime/session transport lives in `@bedrock-coder/core` (see `@bedrock-coder/core/hub`)
- Sub-agent and team coordination primitives live in `@bedrock-coder/core`
...
```bash
npm install @bedrock-coder/agents @bedrock-coder/shared @bedrock-coder/llms
```
...
when the host already owns gateway construction (this is what `@bedrock-coder/core`
uses internally):

```ts
import { createGateway } from "@bedrock-coder/llms";
...
`@bedrock-coder/shared`. Each tool has a JSON Schema `inputSchema` and an
...
`AgentRuntimeEvent` in `@bedrock-coder/shared` for the full union.
...
subprocess-backed hooks, MCP extensions), use `@bedrock-coder/core`.
...
```ts
import type { AgentRuntimePlugin } from "@bedrock-coder/shared";
...
For multi-agent workflows, use `@bedrock-coder/core`:
...
- `@bedrock-coder/agents` — the single package entrypoint. The `package.json`
  `exports` map automatically serves a browser-safe bundle when bundlers
  resolve the `browser` condition.

## Related Packages

- `@bedrock-coder/shared`: shared types (`AgentTool`, `AgentMessage`,
  `AgentRuntimeEvent`, `AgentRuntimeHooks`, etc.)
- `@bedrock-coder/llms`: provider settings, model catalogs, and gateway/handler
  creation
- `@bedrock-coder/core`: stateful runtime assembly, storage, default tools,
  subprocess hooks, hub transport, and MCP integration

## More Examples

- Repo examples:
  [examples/plugins](https://github.com/bedrock-coder/sdk/tree/main/examples/plugins),
  [examples/hooks](https://github.com/bedrock-coder/sdk/tree/main/examples/hooks),
  [examples/cron](https://github.com/bedrock-coder/sdk/tree/main/examples/cron)
- Workspace overview: [README.md](https://github.com/bedrock-coder/sdk/blob/main/README.md)
- API and architecture references:
  [DOC.md](https://github.com/bedrock-coder/sdk/blob/main/DOC.md),
  [ARCHITECTURE.md](https://github.com/bedrock-coder/sdk/blob/main/ARCHITECTURE.md)
