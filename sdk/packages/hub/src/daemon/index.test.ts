...
describe("ensureDetachedHubServer", () => {
	...
	beforeEach(() => {
		...
		readHubDaemonLaunchSpec.mockReset();
		readHubDaemonLaunchSpec.mockReturnValue({
			launcher: "/usr/local/bin/cline-hub",
			argsPrefix: [],
			cwd: "/workspace",
			version: "current-build",
	...
	it("does not use port 0 for default production startup", async () => {
		...
		expect(spawnCalls[0]?.[0]).toBe("/usr/local/bin/cline-hub");
		expect(spawnArgs).toContain("--port");
		expect(spawnArgs).toContain("25463");
		expect(spawnArgs).not.toContain("0");
		expect(spawnArgs).not.toContain("--cline-hub-daemon");
	...
	it("reuses a protocol-compatible healthy hub from a newer release", async () => {
		const kill = vi.spyOn(process, "kill").mockImplementation(() => true);
		try {
			readHubDaemonLaunchSpec.mockReturnValue({
				launcher: "/usr/local/bin/cline-hub",
...
describe("isManagedHubVersionCompatible", () => {
	it.each([
		["1.2.3", "1.2.3", true],
		["1.3.0", "1.2.9", true],
		["2.0.0", "1.99.99", true],
		["1.2.2", "1.2.3", false],
		["0.9.9", "1.0.0", false],
		["development-a", "development-a", true],
		["development-b", "development-a", false],
		[undefined, "1.0.0", false],
	])("compares running %s against bundled %s", async (running, bundled, expected) => {
		const { isManagedHubVersionCompatible } = await import(".");
		expect(isManagedHubVersionCompatible(running, bundled)).toBe(expected);
	});
});
