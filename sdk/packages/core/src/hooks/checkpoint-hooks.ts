 ...
 * checkpoint restore path runs `git clean -fd` before `git stash apply`, so
 * without the untracked parent any file the agent created would be deleted on
 * restore instead of being brought back. Returns `undefined` when there is
 * nothing to snapshot (clean working tree with no untracked files), letting
 * the caller fall back to a HEAD commit checkpoint.
 */
