...
describe("HubServerTransport boundaries", () => {
	...
	it("returns the current session compaction state", async () => {
		...
		const readSessionCompactionState = vi
			.fn()
			.mockResolvedValue(compactionState);
		const transport = createTransport({
			sessionHost: {
				subscribe: vi.fn(),
				startSession: vi.fn(),
		...
		expect(reply).toMatchObject({
			...
			requestId: "req-compaction-get",
			ok: true,
			payload: { sessionId: "session-1", state: compactionState },
		});
	});
	...
	it("passes explicit compaction state from session.create to the runtime host", async () => {
		let capturedStartInput: StartSessionInput | undefined;
		const sourceMessages = [
			{ id: "u1", role: "user" as const, content: "original" },
		];
		...
		const reply = await transport.handleCommand({
			version: "v1",
			requestId: "req-create-compaction",
			command: "session.create",
			clientId: "client-1",
			payload: {
				workspaceRoot: "/tmp/project",
		...
		expect(reply.ok).toBe(true);
		expect(capturedStartInput?.initialMessages).toEqual(sourceMessages);
		expect(capturedStartInput?.initialCompactionState).toEqual(compactionState);
	});

	it("passes active compaction state updates to the runtime host", async () => {
		const transport = createTransport();
		const ctx = getContext(transport);
		const updateSessionCompactionState = vi.mocked(
			ctx.sessionHost.updateSessionCompactionState,
		);
		updateSessionCompactionState.mockResolvedValue({ updated: true });
		const sourceMessages = [
			{ id: "u1", role: "user" as const, content: "original" },
		];
		const compactionState = createSessionCompactionState({
		...
		const reply = await transport.handleCommand({
			version: "v1",
			requestId: "req-update-compaction",
	...
	it("does not publish session.updated when compaction state update is rejected", async () => {
		const updateSessionCompactionState = vi.fn().mockResolvedValue({
			updated: false,
		});
		const transport = createTransport({
			sessionHost: {
		...
		const reply = await transport.handleCommand({
			version: "v1",
			requestId: "req-update-compaction-rejected",
			command: "session.compaction.update",
			clientId: "client-1",
			sessionId: "session-1",
			payload: {
	...
	it("rejects compaction state updates from non-owner clients", async () => {
		...
		const reply = await transport.handleCommand({
			version: "v1",
			requestId: "req-update-compaction-wrong-client",
			command: "session.compaction.update",
			clientId: "viewer-client",
			sessionId: "session-1",
			payload: {
