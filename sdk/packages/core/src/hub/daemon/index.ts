...
const COMPILED_BUN_HUB_DAEMON_ARG = "--bedrock-coder-hub-daemon";
...
function openDetachedHubLogFile(): { fd: number; logPath: string } | undefined {
	try {
		const logPath = join(resolveBedrockCoderDataDir(), "logs", "hub-daemon.log");
...
async function retireLegacySharedHub(owner: HubOwnerContext): Promise<void> {
	if (resolveBedrockCoderBuildEnv() !== "production") {
		return;
	}
...
function resolveLaunchCommand(
	...
	return {
		...
		cwd: workspaceRoot,
		env: {
			...withResolvedBedrockCoderBuildEnv(process.env),
			BEDROCK_CODER_NO_INTERACTIVE: "1",
	...
	if (expected?.url) {
		...
		if (isCompatibleHubRecord(expected)) {
			const upgradeHint = retiredUnusableDiscovery
				? " This can happen immediately after upgrading from a build that wrote an empty hub auth token; run 'bedrockCoder doctor fix' to stop the old daemon and repair local hub discovery."
				: "";
			throw new Error(
				`A compatible BedrockCoder Hub is already running at ${expectedUrl}, but its discovery record is missing or unreadable. Run 'bedrockCoder doctor fix' to repair local hub discovery.${upgradeHint}`,
			);
		}
		const retiredExpected = await retireIncompatibleHub(
		...
		if (
			...
			throw new Error(
				`An incompatible BedrockCoder Hub is already running at ${expectedUrl} and could not be retired automatically. Run 'bedrockCoder doctor fix' to stop stale hub daemons before starting a new hub.`,
			);
		}
	}
	const shouldUseFallbackPort =
	...
	while (Date.now() < deadline) {
		...
		if (nextExpected?.url && !isCompatibleHubRecord(nextExpected)) {
			...
			if (
				...
				throw new Error(
					`An incompatible BedrockCoder Hub is still running at ${expectedUrl} and could not be retired automatically. Run 'bedrockCoder doctor fix' to stop stale hub daemons before starting a new hub.`,
				);
			}
		}
