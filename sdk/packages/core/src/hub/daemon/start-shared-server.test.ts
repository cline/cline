...
const {
	...
	mockResolveBedrockCoderBuildEnv,
	...
	mockResolveBedrockCoderBuildEnv: vi.fn(() => "production"),
	mockResolveProductionHubOwnerContext: vi.fn(() => ({
		ownerId: "production",
		discoveryPath: "/tmp/bedrock-coder-data/locks/hub/production.json",
	})),
	mockResolveSharedHubOwnerContext: vi.fn(() => ({
		ownerId: "shared",
		discoveryPath: "/tmp/bedrock-coder-data/locks/hub/owners/shared.json",
	})),
	mockStartHubWebSocketServer: vi.fn(),
}));

vi.mock("@bedrock-coder/shared", () => ({
	resolveBedrockCoderBuildEnv: mockResolveBedrockCoderBuildEnv,
}));
...
const originalHubPort = process.env.BEDROCK_CODER_HUB_PORT;

describe("ensureHubServer", () => {
	afterEach(() => {
		mockEnsureHubWebSocketServer.mockClear();
		mockResolveHubEndpointOptions.mockClear();
		mockResolveBedrockCoderBuildEnv.mockClear();
		mockResolveBedrockCoderBuildEnv.mockReturnValue("production");
		...
		if (originalHubPort === undefined) {
			delete process.env.BEDROCK_CODER_HUB_PORT;
		} else {
			process.env.BEDROCK_CODER_HUB_PORT = originalHubPort;
		}
	});
	...
	it("allows port fallback by default in development when no port is explicit", async () => {
		delete process.env.BEDROCK_CODER_HUB_PORT;
		mockResolveBedrockCoderBuildEnv.mockReturnValue("development");
		const { ensureHubServer } = await import("./start-shared-server");
	...
	it("does not default port fallback when BEDROCK_CODER_HUB_PORT is explicit", async () => {
		process.env.BEDROCK_CODER_HUB_PORT = "30001";
		const { ensureHubServer } = await import("./start-shared-server");
