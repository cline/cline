...
describe("HubRuntimeHost", () => {
	...
	it("passes hub cancellation to mistake-limit decisions", async () => {
		let onEvent: ((event: HubEventEnvelope) => void) | undefined;
		subscribeMock.mockImplementation((listener) => {
			onEvent = listener;
			return () => {};
		...
		let receivedSignal: AbortSignal | undefined;
		let resolveExecutor: ((value: { action: "stop" }) => void) | undefined;
		const onConsecutiveMistakeLimitReached = vi.fn(
			async (context: { signal?: AbortSignal }) => {
				receivedSignal = context.signal;
				return await new Promise<{ action: "stop" }>((resolve) => {
					resolveExecutor = resolve;
				});
			},
		);
		...
		await host.startSession({
			config: createConfig(),
			source: SessionSource.CLI,
			localRuntime: { onConsecutiveMistakeLimitReached },
		});
		...
		onEvent?.({
			...
			payload: {
				requestId: "capreq-1",
				targetClientId: "client-1",
				capabilityName: "mistake_limit.decide",
				payload: {
					context: {
						iteration: 5,
