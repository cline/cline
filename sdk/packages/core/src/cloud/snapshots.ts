...
export function reconcileBufferedCloudEvents(
	...
	const queueEvents = queueSnapshotApplied
		...
		: events;
	const lastQueueEvent = [...queueEvents]
		.reverse()
		.find((event) => event.event === "session.pending_prompts");
	...
	const flush = (terminal: boolean) => {
		...
		const contentEnd = (kind: "assistant" | "reasoning") => {
			...
			const reversedIndex = [...snapshotSegment]
				.reverse()
				.findIndex((event) => event.event === `${kind}.finished`);
			const finished =
				reversedIndex < 0 ? -1 : snapshotSegment.length - 1 - reversedIndex;
