...
/**
 * Goal completion guard shared by interactive Cline surfaces (the CLI TUI and
 ...
 * with an active goal, the host sends a follow-up verification turn asking
...
export function createInteractiveGoalGuard() {
	...
	return {
		 ...
		 * undefined when no goal needs verification. Called by the host
 ...
 * Used by hosts that await turns inline (the CLI); hosts with fire-and-forget
 * sends drive the same guard from their turn-settled callbacks instead.
 */
