...
describe("runOverflowTruncation", () => {
	...
	const totalTokens = (messages: MessageWithMetadata[]): number =>
		messages.reduce((total, message) => total + estimateJsonTokens(message), 0);

	const makeOverflowContext = (
	...
	const noticeText = (messages: MessageWithMetadata[] | undefined): string => {
	...
	it("drops the oldest messages and replaces them with a notice", () => {
		...
		const result = runOverflowTruncation({
			context: makeOverflowContext(messages, 600),
			estimateMessageTokens: estimateJsonTokens,
		});
	...
	it("always drops at least the oldest message, even when the estimate says everything fits", () => {
		...
		const result = runOverflowTruncation({
			context: makeOverflowContext(messages, 1_000_000),
			estimateMessageTokens: estimateJsonTokens,
		});
	...
	it("never splits a tool_use/tool_result pair at the cut", () => {
		...
		const target =
			estimateJsonTokens(messages[3]) + estimateJsonTokens(messages[2]) + 550;
		const result = runOverflowTruncation({
			context: makeOverflowContext(messages, target),
			estimateMessageTokens: estimateJsonTokens,
		});
	...
	it("truncates oversized text when even the newest message exceeds the budget", () => {
		...
		const result = runOverflowTruncation({
			context: makeOverflowContext(messages, 2_000),
			estimateMessageTokens: estimateJsonTokens,
		});
	...
	it("records the dropped user runs on the notice for checkpoint alignment", () => {
		...
		const result = runOverflowTruncation({
			context: makeOverflowContext(messages, 700),
			estimateMessageTokens: estimateJsonTokens,
		});
