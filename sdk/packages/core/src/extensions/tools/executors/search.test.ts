...
describe("createSearchExecutor", () => {
	it("middle-truncates oversized search output with recovery guidance", async () => {
		...
		// Many independently matching lines so the joined output exceeds the
		// cap even though each individual line stays under MAX_LINE_CHARS.
		...
		try {
			...
			// Lookahead is unsupported by ripgrep's default engine, forcing
			// the fallback regex scan.
	...
	it("returns bounded output when a match lands in a giant single-line file", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agents-search-"));
		// Simulates a serialized trace dump: one JSON file whose only line is
		// tens of MB. Buffering ripgrep's --json events for such files
		// unbounded previously crashed the host process (uncaught RangeError
		// once accumulated stdout passed the engine's max string length).
		await fs.writeFile(
	...
	it("truncates long matched lines in fallback scan context output", async () => {
		const dir = await fs.mkdtemp(path.join(os.tmpdir(), "agents-search-"));
		await fs.writeFile(
			path.join(dir, "minified.js"),
			`needle${"x".repeat(10_000)}`,
			"utf-8",
		);

		try {
			const search = createSearchExecutor();
			// Lookahead forces the fallback regex scan.
			const result = await search("(?=needle)", dir, ctx);
