...
describe("run_commands plan-mode command guard", () => {
	const context = {
		agentId: "agent-1",
		conversationId: "conv-1",
		iteration: 1,
	...
	it("blocks file-editing commands without executing them when enabled", async () => {
		const execute = vi.fn(async () => "ran");
		const tool = createShellTool(execute, { blockFileEditingCommands: true });

		const results = (await tool.execute(
			{ commands: ["rm -rf build"] } as never,
			context,
		...
		expect(results[0].error).toContain("`rm`");
	});

	it("executes read-only commands and blocks only offending entries", async () => {
		const execute = vi.fn(async (command: string | { command: string }) =>
			typeof command === "string" ? `ran:${command}` : `ran:${command.command}`,
		);
		const tool = createShellTool(execute, { blockFileEditingCommands: true });
		...
		const results = (await tool.execute(
			{ commands: ["git status", "echo hi > out.txt", "ls -la"] } as never,
			context,
		)) as Array<{ query: string; success: boolean; error?: string }>;

		expect(results).toHaveLength(3);
		expect(results[0]).toMatchObject({ query: "git status", success: true });
		expect(results[1].success).toBe(false);
		expect(results[1].error).toContain("output redirection");
		expect(results[2]).toMatchObject({ query: "ls -la", success: true });
		expect(execute).toHaveBeenCalledTimes(2);
		expect(execute).toHaveBeenCalledWith(
			"git status",
			process.cwd(),
	...
	it("guards structured commands too", async () => {
		const execute = vi.fn(async () => "ran");
		const tool = createShellTool(execute, { blockFileEditingCommands: true });

		const results = (await tool.execute(
			{ commands: [{ command: "git", args: ["checkout", "main"] }] } as never,
			context,
		)) as Array<{ success: boolean; error?: string }>;

		expect(execute).not.toHaveBeenCalled();
		expect(results[0].success).toBe(false);
		expect(results[0].error).toContain("`git checkout`");
	});

	it("does not guard commands when the flag is off (act mode default)", async () => {
		const execute = vi.fn(async () => "ran");
		const tool = createShellTool(execute);
	...
	it("announces the read-only restriction in the tool description", () => {
		const guarded = createShellTool(async () => "ok", {
			blockFileEditingCommands: true,
			shell: "/bin/bash",
		});
		const unguarded = createShellTool(async () => "ok", {
			shell: "/bin/bash",
		});

		expect(guarded.description).toContain("plan mode");
		expect(guarded.description).toContain("read-only");
		expect(unguarded.description).not.toContain("plan mode");
	});
});
