...
describe("mcp client request timeout", () => {
	...
	it("spawns stdio servers from the home directory when no cwd is configured", async () => {
		// The hub daemon's cwd is whatever workspace first started it; npx walks
		// that project tree before launching, which made marketplace servers
		// miss the initialize budget from large repos.
...
describe("default connect budget", () => {
	it("keeps the two-attempt initialize budget well under the hub command timeout", () => {
		...
		// spends a budget twice (newline then Content-Length framing). If the
		...
		expect(
			DEFAULT_MCP_CONNECT_TIMEOUT_MS + DEFAULT_MCP_FRAMED_RETRY_TIMEOUT_MS,
