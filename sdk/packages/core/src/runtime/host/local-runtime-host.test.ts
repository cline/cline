...
describe("LocalRuntimeHost", () => {
	...
	it("does not unmap a same-id replacement started while the old session stop is in flight", async () => {
		const sessionId = "sess-same-id-replacement";
		const manifest = createManifest(sessionId);
		const sessionService = {
			ensureSessionsDir: vi.fn().mockReturnValue("/tmp/sessions"),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
		...
		await manager.startSession(
			normalizeStartInput({
				config: createConfig({ sessionId }),
				interactive: true,
				initialMessages: [{ role: "user", content: "plan it" }],
			}),
		);
		const sessions = Reflect.get(manager as object, "sessions") as Map<
			string,
			unknown
		>;
		const firstSession = sessions.get(sessionId);
		expect(firstSession).toBeDefined();

		// Stop parks in cleanup awaiting agent.shutdown; the same-id replacement
		// starts while that cleanup is still in flight.
		const events: Array<{ type: string }> = [];
		manager.subscribe((event) => {
			events.push(event);
		});
		const stopPromise = manager.stopSession(sessionId);
		await new Promise((resolve) => setTimeout(resolve, 0));
		await manager.startSession(
			normalizeStartInput({
				config: createConfig({ sessionId }),
				interactive: true,
				initialMessages: [{ role: "user", content: "act on it" }],
			}),
		);
		...
		// The old session's cleanup must not delete the live replacement, and it
		// must not broadcast "ended" for an id that now belongs to a running
		// session (id-keyed listeners would tear down the successor's state).
		expect(sessions.get(sessionId)).toBe(replacement);
		expect(events.some((event) => event.type === "ended")).toBe(false);
	});

	it("drops agent events from a superseded session's agent after a same-id replacement", async () => {
		const sessionId = "sess-stale-agent-events";
		const manifest = createManifest(sessionId);
		const sessionService = {
			ensureSessionsDir: vi.fn().mockReturnValue("/tmp/sessions"),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
		...
		let resolveShutdown: () => void = () => {};
		const makeAgent = (deferShutdown: boolean) => ({
			run: vi.fn().mockResolvedValue(createResult()),
			continue: vi.fn().mockResolvedValue(createResult()),
			getMessages: vi.fn().mockReturnValue([]),
			getAgentId: vi.fn().mockReturnValue("agent-root-1"),
		...
		const agentHooks: Array<{
			onEvent?: (event: { type: string }) => Promise<void>;
		}> = [];
		const manager = new RuntimeHostUnderTest({
			distinctId,
			sessionService: sessionService as never,
			runtimeBuilder: runtimeBuilder as never,
			createAgent: (agentConfig: unknown) => {
				agentCount += 1;
				agentHooks.push(
					(
						agentConfig as {
				...
				return makeAgent(agentCount === 1) as never;
			},
		});
		...
		// Sanity: while the first session is current, its agent events flow.
		agentListeners[0]?.(staleEvent);
		expect(events.some((event) => event.type === "agent_event")).toBe(true);
		events.length = 0;
		...
		// The old agent's late event must not be attributed to the successor.
		agentListeners[0]?.(staleEvent);
		expect(events.some((event) => event.type === "agent_event")).toBe(false);

		// Same for the persistence hook: a stale assistant-message from the old
		// agent must not rewrite the successor's persisted messages under the
		// old session's config.
		const persistCallsAfterReplacement =
			sessionService.persistSessionMessages.mock.calls.length;
		await agentHooks[0]?.onEvent?.({ type: "assistant-message" });
		expect(sessionService.persistSessionMessages.mock.calls.length).toBe(
			persistCallsAfterReplacement,
		);
		// The successor's own hook still persists.
		await agentHooks[1]?.onEvent?.({ type: "assistant-message" });
		expect(sessionService.persistSessionMessages.mock.calls.length).toBe(
			persistCallsAfterReplacement + 1,
		);
	...
	it("does not write or emit a terminal status onto a same-id replacement when cleanup stalls in updateStatus", async () => {
		const sessionId = "sess-stale-status";
		const manifest = createManifest(sessionId);
		let statusUpdateCalls = 0;
		let resolveStatusUpdate: (result: {
			updated: boolean;
			endedAt?: string;
		}) => void = () => {};
		const updateSessionStatus = vi.fn(() => {
			...
			if (statusUpdateCalls === 2) {
				// The stop's terminal-status write parks here while the same-id
				// replacement registers.
				return new Promise<{ updated: boolean; endedAt?: string }>(
					(resolve) => {
		...
		resolveStatusUpdate({ updated: true, endedAt: "2026-01-01T00:00:05.000Z" });
		await stopPromise;

		// The stale terminal status must not be written to the shared manifest
		// path, emitted as a status event, or applied to the replacement.
		expect(sessionService.writeSessionManifest).not.toHaveBeenCalled();
		expect(sessions.get(sessionId)).toBe(replacement);
		expect((replacement as { status: string }).status).toBe("running");
		expect(events.some((event) => event.type === "ended")).toBe(false);
	});
