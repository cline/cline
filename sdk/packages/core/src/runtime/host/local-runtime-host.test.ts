...
describe("LocalRuntimeHost", () => {
	...
	it("persists canonical messages separately from auto compaction state", async () => {
		const sessionId = "sess-compaction-persistence";
		const manifest = createManifest(sessionId);
		const canonicalMessages: MessageWithMetadata[] = [
			{ id: "u-old", role: "user", content: "old full detail" },
			{ id: "a-old", role: "assistant", content: "prior answer" },
			{ id: "u-new", role: "user", content: "new prompt" },
		];
		...
		const compactedMessages: MessageWithMetadata[] = [
			{ id: "summary", role: "user", content: "compacted summary" },
		];
		const sessionService = {
			ensureSessionsDir: vi.fn().mockReturnValue("/tmp/sessions"),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
				manifestPath: "/tmp/manifest-compaction-persistence.json",
				messagesPath: "/tmp/messages-compaction-persistence.json",
				compactionPath: "/tmp/compaction.json",
				manifest,
			}),
			persistSessionMessages: vi.fn(),
		...
		const compact = vi.fn(() => ({ messages: compactedMessages }));
			...
			run: vi.fn(async () => {
				await config.prepareTurn?.({
					...
					model: {
						id: "mock-model",
						provider: "mock-provider",
						info: { id: "mock-model", contextWindow: 1 },
					},
				});
				return createResult({
					messages: [...canonicalMessages, assistantMessage],
				});
			}),
			continue: vi.fn(),
			abort: vi.fn(),
		...
		const persistedMessages =
			sessionService.persistSessionMessages.mock.calls.at(-1)?.[1];
	...
	it("resumes with persisted compaction state and projects later canonical messages", async () => {
		const sessionId = "sess-compaction-resume";
		const manifest = createManifest(sessionId);
		...
		const compactedMessages: MessageWithMetadata[] = [
			{ id: "summary", role: "user", content: "summary" },
		];
		...
		const sessionService = {
			ensureSessionsDir: vi.fn().mockReturnValue("/tmp/sessions"),
			readSessionManifest: vi.fn().mockReturnValue(manifest),
			readSessionCompactionState: vi.fn().mockReturnValue(state),
			createRootSessionWithArtifacts: vi.fn(),
			persistSessionMessages: vi.fn(),
			updateSessionStatus: vi.fn().mockResolvedValue({ updated: true }),
			writeSessionManifest: vi.fn(),
	...
	it("updates active compaction state without replacing the session", async () => {
		const sessionId = "sess-compaction-update";
		const manifest = createManifest(sessionId);
		const sourceMessages: MessageWithMetadata[] = [
			{ id: "u1", role: "user", content: "large original" },
			{ id: "a1", role: "assistant", content: "original answer" },
		...
		const sessionService = {
			...
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
				manifestPath: "/tmp/manifest-compaction-update.json",
				messagesPath: "/tmp/messages-compaction-update.json",
				compactionPath: "/tmp/compaction-update.json",
				manifest,
			}),
			persistSessionMessages: vi.fn(),
			persistSessionCompactionState: vi.fn(),
		...
		sessionService.createRootSessionWithArtifacts.mockClear();
	...
	it("serializes active compaction writes and refuses stale prefixes", async () => {
		...
		const newerState = createSessionCompactionState({
			...
			compactedMessages: [
				{ id: "newer-summary", role: "user", content: "newer summary" },
			],
			conversationId: "conv-root-1",
		...
		const staleState = createSessionCompactionState({
			sourceMessages,
			compactedMessages: [
				{ id: "older-summary", role: "user", content: "older summary" },
			],
			conversationId: "conv-root-1",
		...
		const sessionService = {
			...
			readSessionManifest: vi.fn().mockReturnValue(undefined),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
				manifestPath: "/tmp/manifest-compaction-write-order.json",
				messagesPath: "/tmp/messages-compaction-write-order.json",
				compactionPath: "/tmp/compaction-write-order.json",
				manifest,
			}),
			persistSessionMessages: vi.fn(),
		...
		const newerWrite = manager.updateSessionCompactionState(
			sessionId,
			newerState,
		);
		await Promise.resolve();
		const staleWrite = manager.updateSessionCompactionState(
			sessionId,
			staleState,
		);
		...
		releaseFirstPersist();
		await expect(newerWrite).resolves.toEqual({ updated: true });
		await expect(staleWrite).resolves.toEqual({ updated: false });
		expect(persistSessionCompactionState).toHaveBeenCalledTimes(1);
	});
	...
	it("rejects active compaction state for a different conversation", async () => {
		...
		const sessionService = {
			...
			readSessionManifest: vi.fn().mockReturnValue(undefined),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
				manifestPath: "/tmp/manifest-wrong-conversation.json",
				messagesPath: "/tmp/messages-wrong-conversation.json",
				compactionPath: "/tmp/compaction-wrong-conversation.json",
				manifest,
			}),
			persistSessionMessages: vi.fn(),
			persistSessionCompactionState: vi.fn(),
		...
		await expect(
			manager.updateSessionCompactionState(sessionId, state),
		).resolves.toEqual({ updated: false });
		expect(sessionService.persistSessionCompactionState).not.toHaveBeenCalled();
	});

	it("ignores initial compaction state for a different conversation before model projection", async () => {
		const sessionId = "sess-compaction-bad-initial-state";
		const manifest = createManifest(sessionId);
		const sourceMessages: MessageWithMetadata[] = [
			{ id: "u1", role: "user", content: "large original" },
		];
		...
		const sessionService = {
			...
			readSessionManifest: vi.fn().mockReturnValue(undefined),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
				manifestPath: "/tmp/manifest-bad-initial-state.json",
				messagesPath: "/tmp/messages-bad-initial-state.json",
				compactionPath: "/tmp/compaction-bad-initial-state.json",
				manifest,
			}),
			persistSessionMessages: vi.fn(),
			persistSessionCompactionState: vi.fn(),
	...
	it("fails active compaction updates when the backend cannot persist sidecars", async () => {
		const sessionId = "sess-compaction-missing-backend";
		const manifest = createManifest(sessionId);
		const sourceMessages: MessageWithMetadata[] = [
			{ id: "u1", role: "user", content: "large original" },
		];
		...
		const sessionService = {
			...
			readSessionManifest: vi.fn().mockReturnValue(undefined),
			createRootSessionWithArtifacts: vi.fn().mockResolvedValue({
				manifestPath: "/tmp/manifest-missing-backend.json",
				messagesPath: "/tmp/messages-missing-backend.json",
				compactionPath: "/tmp/compaction-missing-backend.json",
				manifest,
			}),
			persistSessionMessages: vi.fn(),
			updateSessionStatus: vi.fn().mockResolvedValue({ updated: true }),
		...
		await expect(
			...
			"session service method not available: persistSessionCompactionState",
		);
	});

	it("deletes stale persisted compaction state when projection validation fails", async () => {
		const sessionId = "sess-compaction-stale-sidecar";
		const manifest = createManifest(sessionId);
		const originalMessages: MessageWithMetadata[] = [
			{ id: "u1", role: "user", content: "original detail" },
			{ id: "a1", role: "assistant", content: "original answer" },
		];
		const changedMessages: MessageWithMetadata[] = [
			{ id: "u1", role: "user", content: "redacted detail" },
			originalMessages[1]!,
		];
		const staleState = createSessionCompactionState({
		...
		const sessionService = {
			ensureSessionsDir: vi.fn().mockReturnValue("/tmp/sessions"),
			readSessionManifest: vi.fn().mockReturnValue(manifest),
			readSessionCompactionState: vi.fn().mockReturnValue(staleState),
			createRootSessionWithArtifacts: vi.fn(),
			persistSessionMessages: vi.fn(),
			persistSessionCompactionState: vi.fn(),
			deleteSessionCompactionState: vi.fn(),
			updateSessionStatus: vi.fn().mockResolvedValue({ updated: true }),
			writeSessionManifest: vi.fn(),
			listSessions: vi.fn().mockResolvedValue([]),
			deleteSession: vi.fn().mockResolvedValue({ deleted: true }),
		...
		const createAgent = vi.fn((config) => {
			...
			return {
				...
				restore: vi.fn(),
				shutdown: vi.fn().mockResolvedValue(undefined),
				getMessages: vi.fn().mockReturnValue(changedMessages),
		...
		await manager.startSession(
			normalizeStartInput({
				...
				interactive: true,
				initialMessages: changedMessages,
			}),
		);
		...
		expect(result).toBeUndefined();
		expect(sessionService.deleteSessionCompactionState).toHaveBeenCalledWith(
			sessionId,
		);
		expect(sessionService.persistSessionCompactionState).not.toHaveBeenCalled();
	});
	...
	it("does not preload persisted compaction state when compaction is disabled", async () => {
		...
		let capturedPrepareTurn: AgentConfig["prepareTurn"] | undefined;
		const createAgent = vi.fn((config) => {
			capturedPrepareTurn = config.prepareTurn;
			return {
				run: vi.fn(),
	...
	it("resumes without compaction state by running the configured compaction path", async () => {
		const sessionId = "sess-compaction-resume-no-state";
		const manifest = createManifest(sessionId);
		const sourceMessages: MessageWithMetadata[] = [
			{ id: "u1", role: "user", content: "large original" },
		];
		...
		const sessionService = {
			ensureSessionsDir: vi.fn().mockReturnValue("/tmp/sessions"),
			readSessionManifest: vi.fn().mockReturnValue(manifest),
			readSessionCompactionState: vi.fn().mockReturnValue(undefined),
			createRootSessionWithArtifacts: vi.fn(),
			persistSessionCompactionState: vi.fn(),
			persistSessionMessages: vi.fn(),
			updateSessionStatus: vi.fn().mockResolvedValue({ updated: true }),
		...
		expect(compact).toHaveBeenCalledWith(
			expect.objectContaining({ messages: sourceMessages }),
		);
