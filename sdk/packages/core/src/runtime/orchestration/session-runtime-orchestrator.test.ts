...
describe("SessionRuntime.addTools / updateConnection / clearHistory / restore", () => {
	...
	it("retains display-only history while deferring a provider change", async () => {
		let releaseRun: () => void = () => {};
		const release = new Promise<void>((resolve) => {
			releaseRun = resolve;
		});
		...
		session.restore([
			{
				id: "saved-error",
				role: "assistant",
				content: [{ type: "text", text: "previous failure" }],
				metadata: { displayOnly: true, displayRole: "error" },
			},
		]);

		const activeRun = session.continue("try again");
		await vi.waitFor(() => expect(calls.continue).toHaveLength(1));
		session.updateConnection({ providerId: "openai" });
		releaseRun();

		const result = await activeRun;
		expect(result.model.provider).toBe("anthropic");
		expect(result.messages).toEqual(session.getMessages());
		expect(
			result.messages.filter((message) => message.metadata?.displayOnly),
		).toEqual([expect.objectContaining({ id: "saved-error" })]);
	});
