...
describe("migrateLegacyProviderSettings", () => {
	...
	it("does not re-import a legacy provider after the user removes it", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
		...
		// Constructing the manager runs the import (as every host does on startup).
		const manager = new ProviderSettingsManager({ filePath: providersPath });
		expect(manager.getProviderSettings("openai-codex")).toBeDefined();
		expect(manager.read().migratedLegacyProviders).toEqual(["openai-codex"]);

		// Sign out: the desktop app and CLI remove the provider entry.
		saveLocalProviderSettings(manager, {
			providerId: "openai-codex",
			enabled: false,
		});

		// Next startup, with the legacy secrets still on disk.
		const relaunched = new ProviderSettingsManager({ filePath: providersPath });
		expect(relaunched.getProviderSettings("openai-codex")).toBeUndefined();
		expect(
			migrateLegacyProviderSettings({
				providerSettingsManager: relaunched,
				dataDir: tempDir,
			}),
	...
	it("does not resurrect a pre-existing provider that also has legacy credentials once it is removed", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
	...
	it("still imports legacy providers that appear after an earlier import", () => {
		const tempDir = mkdtempSync(
			path.join(os.tmpdir(), "core-legacy-provider-"),
		);
		tempDirs.push(tempDir);
		...
		writeFileSync(
			path.join(tempDir, "secrets.json"),
			JSON.stringify({ apiKey: "legacy-anthropic-key" }),
		);
		migrateLegacyProviderSettings({
			providerSettingsManager: manager,
			dataDir: tempDir,
		...
		// The classic extension is still in use and the user adds another key.
		writeFileSync(
			path.join(tempDir, "secrets.json"),
			JSON.stringify({
				apiKey: "legacy-anthropic-key",
		...
		expect(result).toMatchObject({ migrated: true, providerCount: 2 });
		expect(manager.getProviderSettings("openrouter")?.apiKey).toBe(
			"legacy-openrouter-key",
		);
		expect(manager.read().migratedLegacyProviders).toEqual([
			"anthropic",
			"openrouter",
		]);
	});
