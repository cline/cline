...
export function migrateLegacyProviderSettings(
	...
	// Every legacy provider is imported at most once. The import runs on every
	// ProviderSettingsManager construction, so without this a provider the user
	// removed (sign out / disconnect) is resurrected from the still-present
	// legacy files the next time any surface builds a manager. Providers that
	// already have an entry are recorded too, so removing them later sticks as
	// well.
