...
function removeProviderFromSettingsState(
	...
	mutated = clearMediaGenerationSelections(state.modes, providerId) || mutated;
	if (mutated) manager.write(state);
	LlmsModels.unregisterProvider(providerId);
}

function clearMediaGenerationSelections(
	modes: ReturnType<ProviderSettingsManager["read"]>["modes"],
	providerId: string,
): boolean {
	let mutated = false;
	const mediaGeneration = modes.mediaGeneration;
...
export async function createConfiguredStreamingTranscriptionSession(
	...
	return createConfiguredModeSession(manager, {
		mode: "voiceInput",
		expiresAfterSeconds: request.expiresAfterSeconds,
		abortSignal: request.abortSignal,
	});
...
export function saveLocalProviderSettings(
	...
	if (request.enabled === false) {
		...
		clearMediaGenerationSelections(state.modes, providerId);
		manager.write(state);
