/**
 * Pure XML tool-calling primitives: prompt generation, assistant-text
 * parsing, and provider-bound serialization.
 *
 * The parser is a port of the legacy Cline extension's
 * `parseAssistantMessageV2` (apps/vscode/src/core/assistant-message/ on the
 * legacy branch), generalized from a fixed tool list to schema-derived tool
 * and parameter names.
 */
...
export function toXmlToolSpec(tool: AgentToolDefinition): XmlToolSpec {
...
/**
 * The full XML tool-calling section appended to the system prompt: static
 * "TOOL USE" instructions (adapted from the legacy extension's XML tool-use
 * prompt) followed by per-turn "TOOL DOCUMENTATION" generated from the tool
 * schemas in the request.
 */
export function buildXmlToolCallingPrompt(
	...
	return `${XML_TOOL_USE_INSTRUCTIONS}

# TOOL DOCUMENTATION

These are the tools currently available to you. Invoke them with XML tags as described above.
