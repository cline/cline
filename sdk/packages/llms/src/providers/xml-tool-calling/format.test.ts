...
describe("buildXmlToolCallingPrompt", () => {
	const prompt = buildXmlToolCallingPrompt(specs);

	it("carries the static tool-use instructions", () => {
		expect(prompt).toContain("TOOL USE");
		expect(prompt).toContain("<tool_name>");
		expect(prompt).toContain("exactly ONE tool per message");
	});

	it("documents every tool with usage skeletons", () => {
		expect(prompt).toContain("TOOL DOCUMENTATION");
		for (const tool of TOOLS) {
			expect(prompt).toContain(`## ${tool.name}`);
			expect(prompt).toContain(`<${tool.name}>`);
			expect(prompt).toContain(`</${tool.name}>`);
		}
		expect(prompt).toContain("- path: (required, text)");
		expect(prompt).toContain("- commands: (required, JSON array)");
		expect(prompt).toContain("- background: (optional, true or false)");
	});
	...
	it("falls back to plain-text completion guidance without completion tools", () => {
		const withoutCompletion = buildXmlToolCallingPrompt(
