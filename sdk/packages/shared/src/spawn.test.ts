...
describe("resolveNpmInvocation", () => {
	...
	it("passes through unchanged on non-Windows platforms", () => {
		expect(
			resolveNpmInvocation(["install", "left-pad@1.3.0"], {
				platform: "linux",
			}),
		).toEqual({ command: "npm", args: ["install", "left-pad@1.3.0"] });
	});

	it("runs npm's CLI through node.exe on Windows", () => {
		const existingFiles = new Set([NODE, NPM_CLI]);
		expect(
			resolveNpmInvocation(["install", "left-pad@1.3.0"], {
				platform: "win32",
			...
			command: NODE,
			args: [NPM_CLI, "install", "left-pad@1.3.0"],
		});
	});

	// The regression this whole change exists for: under `shell: true`,
	// cmd.exe reads `>=` as redirection and `<` as input redirection, so these
	// specs are either mangled or silently written to a file on disk.
	it.each([
		"mongodb-mcp-server@<3",
		"@toolbox-sdk/server@>=1.1.0",
	])("preserves the semver range %s as one argv element", (spec) => {
		const existingFiles = new Set([NODE, NPM_CLI]);
		const invocation = resolveNpmInvocation(["install", spec], {
			platform: "win32",
			env: { Path: WINDOWS_PATH },
			execPath: "C:\\Apps\\cline.exe",
			fileExists: (path) => existingFiles.has(path),
		...
		expect(invocation?.args).toEqual([NPM_CLI, "install", spec]);
	});

	it("uses a native npm.exe shim when a version manager provides one", () => {
		const voltaNpm = "C:\\Tools\\npm.exe";
		expect(
			resolveNpmInvocation(["--version"], {
				platform: "win32",
				env: { Path: "C:\\Tools" },
				execPath: "C:\\Apps\\cline.exe",
				fileExists: (path) => path === voltaNpm,
			}),
		).toEqual({ command: voltaNpm, args: ["--version"] });
	});

	it("pairs npm-cli.js with the node.exe of its own install", () => {
		const oldNode = "C:\\Node14\\node.exe";
		const installNode = "C:\\Node22\\node.exe";
		const installCli = "C:\\Node22\\node_modules\\npm\\bin\\npm-cli.js";
		const existingFiles = new Set([oldNode, installNode, installCli]);
		...
		).toEqual({ command: installNode, args: [installCli, "install"] });
	});

	it("resolves npm-cli.js from npm_execpath when npm invoked us", () => {
		const existingFiles = new Set([NODE, NPM_CLI]);
		expect(
			resolveNpmInvocation(["install"], {
				platform: "win32",
				env: { Path: "C:\\Tools", npm_execpath: NPM_CLI },
				execPath: "C:\\Apps\\cline.exe",
				fileExists: (path) => existingFiles.has(path),
			}),
		).toEqual({ command: NODE, args: [NPM_CLI, "install"] });
	});

	it("returns undefined when only an unparseable npm.cmd shim exists", () => {
		// The official npm.cmd computes its CLI path at run time with a FOR /F
		// loop, so shim rewriting cannot recover it. Callers must surface an
		// error rather than silently fall back to a shell.
		...
		expect(
			resolveNpmInvocation(["install"], {
				...
				env: { Path: "C:\\Program Files\\nodejs" },
				execPath: "C:\\Apps\\cline.exe",
				fileExists: (path) => path === npmCmd,
			}),
		).toBeUndefined();
	});
});

describe("resolveShellFreeInvocation with npm", () => {
	it("routes a bare npm command to the npm CLI resolver", () => {
		const node = "C:\\Program Files\\nodejs\\node.exe";
		const npmCli =
			"C:\\Program Files\\nodejs\\node_modules\\npm\\bin\\npm-cli.js";
		const existingFiles = new Set([node, npmCli]);

		expect(
			resolveShellFreeInvocation("npm", ["install", "pkg@>=1.1.0"], {
				platform: "win32",
				env: { Path: "C:\\Program Files\\nodejs" },
				execPath: "C:\\Apps\\cline.exe",
				fileExists: (path) => existingFiles.has(path),
			...
			command: node,
			args: [npmCli, "install", "pkg@>=1.1.0"],
		});
	});

	it("leaves a custom npm command path untouched on non-Windows", () => {
		expect(
			resolveShellFreeInvocation("/tmp/fake-npm.sh", ["install"], {
				platform: "linux",
			}),
		).toEqual({ command: "/tmp/fake-npm.sh", args: ["install"] });
	});
});
