 ...
 *
 * Policy (single source of truth for every request path): a model receives
 * image content only when it *explicitly* declares the `images` capability.
 * Missing or unknown capabilities resolve to `false` — a model whose catalog
 * entry omits `capabilities` is treated as text-only and never sent images,
 * since text-only endpoints (e.g. Z.AI GLM) reject any request containing a
 * non-text content block (`messages.content.type is invalid`). Keeping this in
 * the model layer prevents each caller from interpreting an incomplete catalog
 * differently.
 */
export function modelSupportsImages(
	model: Pick<GatewayModelDefinition, "capabilities">,
): boolean {
