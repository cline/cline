...
/**
 * Resolve a shell-free invocation of one of npm's own launchers.
 *
 * `npx` and `npm` ship the same way on Windows — a `.cmd` batch shim beside a
 * Node CLI script in the install's bundled npm — so both resolve identically
 * apart from which CLI script and native shim name to look for.
 *
 * Prefers a native `.exe` shim when one exists (a version manager such as Volta
 * installs real executables), otherwise runs the CLI script through `node.exe`
 * so every forwarded argument remains a distinct argv item. Returns `undefined`
 * when Windows only exposes an unsafe `.cmd` shim (which would require a
 * shell) — callers should surface a clear error rather than fall back to one.
 */
function resolveNpmLauncherInvocation(
	launcher: { executableName: string; cliScriptName: string },
	...
	if (
		...
		const cliPath = win32.join(
			win32.dirname(npmExecPath),
			launcher.cliScriptName,
		);
	...
	for (const directory of directories) {
		const executable = win32.join(directory, launcher.executableName);
		if (fileExists(executable)) {
			return { command: executable, args: [...launcherArgs] };
		}
		const cliPath = win32.join(
			directory,
			"node_modules",
...
export function resolveNpxInvocation(
	...
	return resolveNpmLauncherInvocation(
		{ executableName: "npx.exe", cliScriptName: "npx-cli.js" },
		npxArgs,
		options,
	);
}
 ...
 *
 * `npm` cannot go through `resolveShellFreeInvocation`'s generic `.cmd`-shim
 * rewriting: the official `npm.cmd` computes its CLI path at run time with a
 * `FOR /F` loop rather than naming it literally, so the shim parser cannot
 * recover it. Locating `npm-cli.js` in the install layout is the only reliable
 * way to launch npm without a shell.
 */
export function resolveNpmInvocation(
	...
	return resolveNpmLauncherInvocation(
		{ executableName: "npm.exe", cliScriptName: "npm-cli.js" },
		npmArgs,
		options,
	);
}
 ...
 *
 * `npx` and `npm` are special-cased to npm's Node CLI resolution, because their
 * official `.cmd` shims are not statically parseable. Other commands resolve to
