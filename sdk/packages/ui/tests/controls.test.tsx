...
import { Button, SessionStatus } from "../components/index.js";

afterEach(cleanup);

function verifyButtonTypes() {
	return [
		<Button aria-label="Create session" iconOnly key="named">
			+
		</Button>,
		// @ts-expect-error icon-only buttons require an accessible name
		<Button iconOnly key="unnamed">
			+
		</Button>,
	];
}
void verifyButtonTypes;
