...
describe("Switch", () => {
	...
	it("hides the whole switch without discarding its state or form value", async () => {
		const preference = (hidden: boolean) => (
			<form>
				<Switch hidden={hidden} name="notifications" />
			</form>
		);
		await render(preference(false));
		await act(async () => input().click());
