...
describe("runInteractiveTerminalUi", () => {
	...
	it("destroys the renderer when root creation fails", async () => {
		const { runInteractiveTerminalUi } = await import(
		...
		await expect(
			runInteractiveTerminalUi({} as InteractiveTerminalUiProps),
	...
	it("defers explicit shutdown until the current input dispatch unwinds", async () => {
		...
		const tui = await runInteractiveTerminalUi(
			{} as InteractiveTerminalUiProps,
		);
