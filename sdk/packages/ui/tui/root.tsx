...
import type {
	...
	HostSurfaceContext,
	HostSurfaces,
	HubBuildMismatchEvent,
	InteractiveTerminalUiProps,
	RepoStatus,
	TuiStartupTarget,
} from "./types";
import { isSameRepoStatus } from "./utils/repo-status";
...
function App(props: InteractiveTerminalUiProps) {
	...
	const [appView, setAppView] = useState<AppView>(() => {
		if (props.startInOnboarding) return "onboarding";
	...
	const autocomplete = useAutocomplete({
		...
		systemCommands,
		skillCommands,
		searchFilesForMention: props.searchFilesForMention,
	});

	const repoStatusInFlightRef = useRef(false);
	const readRepoStatus = props.refreshRepoStatus;
	const refreshRepoStatus = useCallback(() => {
		if (!readRepoStatus) return;
	...
	// Poll so branch switches made outside the host (another terminal, an
	...
	const createHostSurfaces = props.createHostSurfaces;
	const hostSurfaces = useMemo<HostSurfaces>(() => {
		if (!createHostSurfaces) return {};
		const ctx: HostSurfaceContext = {
			dialog,
			termWidth,
			termHeight,
			config: props.config,
			...
			populateInput: (value: string) => populateInputRef.current(value),
	...
	const openModelSelector = useCallback(
		async (options?: { startWithProviderChange?: boolean }) => {
			if (!hostSurfaces.openModelSelector) {
				showToast("Model selection is not available in this host", "info");
				return;
			}
			await hostSurfaces.openModelSelector(options);
		},
	...
	const openConfig = useConfigPanel({
		...
		openMcpManager,
		openThemePicker,
		autoUpdateSetting: props.autoUpdateSetting,
		refocusTextarea: () => refocusTextareaRef.current(),
	});

	const openAccount = useCallback(async () => {
		if (!hostSurfaces.openAccountDialog) {
			showToast("Account management is not available in this host", "info");
			return;
		}
		await hostSurfaces.openAccountDialog();
	}, [hostSurfaces, showToast]);
	...
	const openCheckpointRestore = useCallback(async () => {
		...
		try {
			const loadCheckpointItems = props.loadCheckpointItems;
			const restoreCheckpoint = props.onRestoreCheckpoint;
			if (!loadCheckpointItems || !restoreCheckpoint) {
				showToast("Checkpoint restore is not available in this host", "info");
				return;
			}
			const items = await loadCheckpointItems();
			if (!items) {
			...
			const result = await restoreCheckpoint(picked.runCount, restoreWorkspace);
			if (!result) {
		...
		async (invocation?: LocalSlashCommandInvocation) => {
			...
			if (selected === SKILLS_MARKETPLACE_ACTION) {
				if (props.openExternal) {
					await props
						.openExternal(SKILLS_MARKETPLACE_URL)
						.catch(() => showToast(`Visit ${SKILLS_MARKETPLACE_URL}`, "info"));
				} else {
	...
	const startupNotice = props.startupNotice;
	...
	useEffect(() => {
		...
		if (!startupNotice.shouldShowForProvider(currentProviderId)) {
		...
		const timeout = setTimeout(() => {
			void dialog
				.choice<boolean>({
					content: (ctx: ChoiceContext<boolean>) => startupNotice.content(ctx),
				})
				.finally(() => {
					Promise.resolve(startupNotice.onShown?.()).catch(() => {});
					refocusTextareaRef.current();
				});
		}, 0);
		return () => clearTimeout(timeout);
	}, [appView, currentProviderId, dialog, startupNotice]);

	const [hubBuildMismatch, setHubBuildMismatch] =
		useState<HubBuildMismatchEvent | null>(null);
	const watchHubBuildMismatch = props.watchHubBuildMismatch;
	const hubBuildWatchEnabled =
		watchHubBuildMismatch !== undefined &&
		shouldWatchManagedHubBuild(props.config);
	useEffect(() => {
		if (!hubBuildWatchEnabled || !watchHubBuildMismatch) return;
		return watchHubBuildMismatch({
			onMismatch: (mismatch) => setHubBuildMismatch(mismatch),
		});
	}, [hubBuildWatchEnabled, watchHubBuildMismatch]);
	...
	useEffect(() => {
		const loadDeferredInitialEntries = props.loadDeferredInitialEntries;
		if (!loadDeferredInitialEntries) {
			return;
		}
		let cancelled = false;
		...
		props.loadDeferredInitialEntries,
		appendSessionEntry,
	...
	const openHistory = useCallback(async () => {
		if (!hostSurfaces.openHistory) {
			showToast("Session history is not available in this host", "info");
			return;
		}
		await hostSurfaces.openHistory();
	}, [hostSurfaces, showToast]);
	...
	const promptInput = usePromptInputController({
		...
		shouldExpandSkillCommands: props.shouldExpandSkillCommands,
	...
	if (appView === "onboarding" && hostSurfaces.renderOnboarding) {
		content = hostSurfaces.renderOnboarding({
			onComplete: (result) => {
...
export function Root(
	props: InteractiveTerminalUiProps & {
	...
	const initialUsage = props.initialUsage;
	...
	return (
		<TerminalColorsContext value={terminalColors}>
			<ThemeProvider
				initialThemeId={props.initialThemeId}
