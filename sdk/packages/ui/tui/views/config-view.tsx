...
export interface ConfigPanelProps extends ChoiceContext<ConfigAction> {
	...
	/** Host-owned auto-update preference; the row is hidden when absent. */
	autoUpdateSetting?: {
		load(): boolean;
...
export function ConfigPanelContent(props: ConfigPanelProps) {
	...
	const [autoUpdateEnabled, setAutoUpdateEnabled] = useState(
		() => props.autoUpdateSetting?.load() ?? true,
	);
				...
				switch (row.id) {
					...
					case "auto-update":
						setAutoUpdateEnabled((previous) => {
							const next = !previous;
							props.autoUpdateSetting?.save(next);
							return next;
						});
						break;
					case "compaction": {
