...
export function usePromptInputController(input: {
	...
	onSubmit: InteractiveTerminalUiProps["onSubmit"];
	...
	shouldExpandSkillCommands?: InteractiveTerminalUiProps["shouldExpandSkillCommands"];
	inputHistory?: InteractiveTerminalUiProps["inputHistory"];
}) {
	const {
		autocomplete,
	...
	const inputHistory = useInputHistory(textareaRef, input.inputHistory);
	...
	const submitPrompt = useCallback(
		async (delivery?: "queue" | "steer") => {
			...
			const promptForSubmit = expandUserCommandPrompt(
				...
				{
					expandSkillCommands:
						shouldExpandSkillCommands?.(session.uiMode) ?? true,
				},
			);
