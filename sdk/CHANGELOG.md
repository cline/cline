...
- The empty-turn retry now covers every provider, not just Ollama — production telemetry showed hosted backends (OpenRouter, Cline, OpenAI-compatible endpoints) intermittently returning all-empty turns that hard-failed tasks with "Model returned empty response"; the same middleware is applied at the shared AI SDK composition point, with per-vendor opt-out and tuning available, and tool-call-only turns are still never retried
