...
- A nested PowerShell wrapper that names a bare executable of the configured shell's edition now runs the host's configured shell path instead of the bare name from the command text. Hosts resolve pwsh by probing its install locations because it is often not on PATH, so the bare name could fail as not found on zip and portable installs, or run a different pwsh than the one the tool description names. An explicit path in the wrapper is honored as written and a cross-edition wrapper still takes the requested name
