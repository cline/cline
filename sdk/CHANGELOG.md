...
- On Windows, a program planted in the workspace can no longer stand in for one Cline spawns by bare name. libuv resolves a bare program name by searching the child's working directory before PATH, which PowerShell itself never does, so a same-named `powershell.exe`, `rg.exe` or `git.exe` in a workspace could run in place of the real shell, ripgrep or git — for the SDK's bare `powershell` default shell, for the nested-wrapper unwrapping below, for the file index and search, and for workspace metadata. The runtime bootstrap and each host entry point now set `NoDefaultCurrentDirectoryInExePath` for their own process, which turns that step off for every spawn; the shell executor removes it again from the shell's own environment so cmd.exe keeps the user's console behavior; a same-edition nested wrapper keeps the host's configured shell path instead of the bare name it was written with; and the executor's own `taskkill.exe` and `powershell.exe` probe use their fixed install paths
