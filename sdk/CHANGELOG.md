...
- Plugin slash commands are owned by the runtime and exposed through `ClineCore.pluginCommands` and hub RPC. CLI, desktop, and connectors reuse existing hub connections instead of loading command sandboxes in each client. Workspace discovery watches plugin/settings changes and retries failed loads in the background; session execution uses the session's existing contribution registry and plugin context. This replaces `createPluginCommandService`. Discovery works before a turn and without Git; command arguments preserve internal whitespace.
