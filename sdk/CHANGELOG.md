...
- On Windows, a program committed to a repository can no longer stand in for one the SDK spawns by bare name. libuv's `search_path` probes the child's working directory before every PATH entry unless `NoDefaultCurrentDirectoryInExePath` is defined in the spawning process, and the SDK spawns `rg` for the file index and the search tool, `git` for workspace metadata and checkpoints, and the configured shell, all with the user's repository as the working directory. The file index runs as soon as a workspace opens, ahead of any approval, so an `rg.exe` carried by a repository executed with the user's privileges just for opening it. The new `excludeCurrentDirectoryFromExecutableSearch()` defines that variable, and every Cline entry point (CLI and hub daemon, VS Code extension, standalone cline-core, desktop sidecar) calls it before anything can spawn. One call covers every spawn the process makes, including those inside dependencies such as `simple-git` and the MCP SDK, and children inherit it, so a cmd.exe the agent starts applies the same rule to its own lookups. Embedders that build a runtime without going through those entry points should call it themselves. Bun 1.3 resolves bare names through PATH only, so the CLI and desktop app were never exposed; Bun 1.4 adopts the same working-directory search and they are covered ahead of it
