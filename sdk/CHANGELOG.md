...
- `run_commands` rejects with `CommandSpawnError` when the shell process itself cannot be started, carrying the operating system error code (`ENOENT` when the shell is not on PATH); the message is unchanged, so hosts can tell a missing shell from a failed command without parsing it
