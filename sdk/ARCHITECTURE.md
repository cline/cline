...
6. `@cline/core` hub services broker sessions, events, approvals, schedules, and client-owned runtime capabilities such as session-local tool executors.
   Abrupt client disconnects retain pending `askQuestion` capability requests for 30 seconds. Recovery requires an explicit `session.claim_client_contributions` call after reconnect; registration alone does not reclaim questions. A registered client can claim its own contribution or a disconnected owner's contribution, but cannot take it from another connected client. Unanswered requests expire after the disconnect grace; participant-less ownership state is also evicted after 30 seconds. Session deletion and Hub shutdown clear retained state. Explicit `session.detach` remains a deliberate abandonment and cancels pending requests immediately.
