...
- `session.abort` remains the cancellation boundary for a root session in both
  ...
  teammate work plus running or queued async runs. Teammate definitions and
	...
	runtimes are not stopped. One-off `spawn_agent` delegations receive the
	parent's abort signal through their `SessionRuntime`; cancellation is retained
	through asynchronous agent initialization and forwarded once the delegated
	`AgentRuntime` starts. The team runtime marks intentional abort task-end events
	as cancelled so persistence does not record them as failures.
