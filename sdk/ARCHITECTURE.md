...
- Provider message preparation may shorten external tool results only after saving their full content to `<sessions-directory>/<session-id>/tools/<tool-call-id>.result.txt`. String results are stored verbatim; structured content is stored as JSON. Recovery notices are appended after all provider budget passes and identify the saved-file line range containing omitted content. They provide a bounded `read_files` example and explain that whole-file reads or very long lines may be truncated again. Without session storage, or if saving fails, external result text remains intact. Default tools retain their existing truncation behavior.
