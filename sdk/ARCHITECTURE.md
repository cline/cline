...
## Hub-Owned Tasks Tool

- Hub-hosted agent sessions receive one snake-case `tasks` tool that routes to
  `HubScheduleService` and manages the same records shown by the desktop
  Routine view. Scheduled operations inherit the current session workspace,
  cwd, and model, filter reads and mutations to that workspace, and allow
  mutations only from an interactive session. One-time schedules accept an
  exact future ISO timestamp; recurring schedules accept a five-field cron
  expression and optional IANA timezone.
- The Hub contributes one tool-conditional system-prompt rule for `tasks`. It
  restricts the tool to explicitly requested scheduled work, prevents
  proactive schedule creation, and requires self-contained prompts because a
  schedule runs in a new unattended session.
