...
6. Agentic compaction uses the provider/model captured by its turn context.
   ...
   Connection resolution uses the main request's live top-level credential
   precedence; imported-history policy overrides keep the live config reference.
   Callers without a turn settings snapshot use the current session connection.
...
store retains their transcript positions when replacing agent snapshots. Run
results use this stored transcript, including display-only entries, while the
result model metadata remains tied to the active turn when a connection
selection changes mid-run.
