...
- session history lives in the session messages artifact at full fidelity; `SessionHistoryEntry` distinguishes conversation messages from `role: "error"` session failures. Errors are never assistant output or model input. Compaction state lives separately in `${sessionId}.compaction.json`
- core captures `ConversationSnapshot` before compaction. The snapshot excludes session errors, normalizes the runtime codec representation (including split tool results), and owns an isolated source copy. Sidecar creation and projection require this snapshot rather than an arbitrary message array
- source fingerprints cover conversation content and durable conversation metadata, excluding transport identity, accounting, and model labels. Prefix counts and appended tails use the same normalized conversation; projection reports `source_changed` or `source_truncated` when it cannot reuse state
- `ClineCore.compactSession(sessionId)` / hub `session.compact` own manual source capture, provider configuration, summarization and persistence. CLI and VS Code request the operation rather than building sidecars. The host excludes concurrent turn preparation and manual compaction; abort/stop cancel summarization and prevent queued writes. Automatic compaction saves against its captured mid-turn snapshot, never a separately re-read disk transcript
