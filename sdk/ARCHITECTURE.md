...
- Provider message preparation may shorten external tool results only after saving their full content to `<sessions-directory>/<session-id>/tools/<tool-call-id>.result.txt`. The host forwards its persistence backend’s session history root to root and delegated runtimes. Child results are namespaced under the root session’s `tools/sessions/<child-session-id>/` directory and removed on child deletion or parent deletion. Session artifact paths accept persistence-compatible IDs; containment checks run when saving, so invalid storage paths cannot prevent session startup. String results are stored verbatim; structured content is stored as JSON. A short recovery notice is appended as a separate text block after all provider budget passes, only in the provider copy (not persisted history or UI events). Truncation markers identify the saved-file line range containing omitted content; retrieval instructions remain in tool descriptions. Without session storage, or if saving fails, external result text remains intact. Default tools retain their existing truncation behavior.
