...
> (`AGENDA_TODO_TOOL_ENABLED` in `hub-server-transport.ts`). The backend
> described below — the manager, storage, `task.*` Hub commands, and desktop
> plumbing — stays fully wired, and the schedule kind remains active.
