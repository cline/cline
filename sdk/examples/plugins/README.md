...
| [atr-detector.ts](./atr-detector.ts) | Threat detection via `beforeTool`, inspired by the open Agent Threat Rules pack | Scans tool name and input against a small high-precision regex set (destructive shells, curl-pipe-sh, prompt injection, leaked AWS/OpenAI/Bearer credentials) and stops the run on a match. No network calls, no new core deps; swap the inline list for the full `agent-threat-rules` pack when wanted. |
