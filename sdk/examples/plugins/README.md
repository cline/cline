...
| [xml-tool-calling/](./xml-tool-calling/) | Legacy-style XML tool calling via `beforeModel`/`afterModel` | Replaces native function calling with the XML tag format the legacy Cline extension used — for local/weak models that fumble native tool schemas. Strips tool schemas from provider requests, documents every registered tool as XML in the system prompt, and parses XML tool uses from assistant text back into native tool calls. |
