...
| ------- | ------------- | ------------ |
| [weather-metrics.ts](./weather-metrics.ts) | Tool registration + lifecycle metrics hooks. Best starting point. | Adds a mock `get_weather` tool and logs run/tool metrics, workspace git context, token usage, and cost. It also demonstrates blocking `git push` on protected branches from a `beforeTool` hook, and emitting plugin telemetry (`ctx.telemetry.capture` / `recordCounter`) into the host telemetry service. |
| [telemetry.ts](./telemetry.ts) | Plugin observability: `ctx.telemetry` + `ctx.logger`, with the full contract in one place. | Adds a `roll_die` demo tool that emits telemetry events, counters, histograms, and gauges plus structured logs, and documents the rules: feature-detect both channels, pass JSON-only data, automatic `plugin.` namespacing, host-side opt-out (do not gate on `isEnabled()` in the sandbox), and keeping telemetry low-cardinality while details go to the logger. |
...
| [clineignore-read-files-guard.ts](./clineignore-read-files-guard.ts) | Enforced `.clineignore`: block reads/edits/shell access to user-listed files | Uses `beforeTool` to skip `read_files`, `editor`, `apply_patch`, and `run_commands` calls whose paths match gitignore-style patterns in a workspace `.clineignore` file. Scoped to `.clineignore` alone (no `.gitignore` rules), works outside git repositories, and covers straightforward shell access like `cat .env` via a conservative token check. |
