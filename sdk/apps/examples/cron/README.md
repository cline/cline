...
cp apps/examples/cron/events/pr-review.event.md ~/.cline/cron/events/
...
- Plugin-emitted events (see `plugins/cline-plugin/automation-events.ts`)
- Connector adapters
...
Test spec for plugin-emitted events. Pairs with `plugins/cline-plugin/automation-events.ts`.
...
```bash
mkdir -p ~/.cline/cron/events
cp apps/examples/cron/events/local-plugin-event.event.md ~/.cline/cron/events/

# Load the plugin that emits these events
mkdir -p ~/.cline/plugins
cp apps/examples/plugins/cline-plugin/automation-events.ts ~/.cline/plugins/
...
- [Automation Documentation](../../DOC.md#automation) — Full spec details
- [`plugins/cline-plugin/automation-events.ts`](../plugins/cline-plugin/automation-events.ts) — Plugin event emission
