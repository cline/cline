...
export const AVALAI_MODEL_PICKER_Z_INDEX = 1_000
...
export interface AvalaiModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
}
...
const AvalaiModelPicker: React.FC<AvalaiModelPickerProps> = ({ isPopup: _isPopup, currentMode }) => {
	...
	const [searchTerm, setSearchTerm] = useState(modeFields.avalaiModelId || "anthropic.claude-haiku-4-5-20251001-v1:0")
	...
	const handleModelChange = (newModelId: string) => {
		...
		handleModeFieldsChange(
			{
				avalaiModelId: { plan: "planModeAvalaiModelId", act: "actModeAvalaiModelId" },
				avalaiModelInfo: { plan: "planModeAvalaiModelInfo", act: "actModeAvalaiModelInfo" },
			},
			{
				avalaiModelId: newModelId,
				avalaiModelInfo: avalaiModels[newModelId] || {},
			},
			currentMode,
		)
	}

	useMount(refreshAvalaiModels)

	// Sync external changes when the modelId changes
	useEffect(() => {
		const currentModelId = modeFields.avalaiModelId || "anthropic.claude-haiku-4-5-20251001-v1:0"
		setSearchTerm(currentModelId)
	}, [modeFields.avalaiModelId])
	...
	const modelIds = useMemo(() => {
		const unfilteredModelIds = Object.keys(avalaiModels).sort((a, b) => a.localeCompare(b))
			...
			<div className="flex flex-col">
				...
				<div className="relative w-full" ref={dropdownRef}>
					<VSCodeTextField
						...
						style={{ zIndex: AVALAI_MODEL_PICKER_Z_INDEX }}
						value={searchTerm}>
						{searchTerm && (
							<div
					...
					{isDropdownVisible && (
						<div
							...
							rounded-b-[3px]"
							ref={dropdownListRef}
							style={{ zIndex: AVALAI_MODEL_PICKER_Z_INDEX - 1 }}>
				...
				<p
					...
					Search from 100+ models across 20+ providers
				</p>
			</div>
		</div>
	)
...
export default AvalaiModelPicker
