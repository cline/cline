	...
	return (
		<div className="flex flex-col gap-1.5">
			<DebouncedTextField
				...
				placeholder={t("api_provider.sapaicore.client_id_placeholder")}
				style={{ width: "100%" }}
				type="password">
				<span className="font-medium">{t("api_provider.sapaicore.client_id_label")}</span>
			</DebouncedTextField>
			{apiConfiguration?.sapAiCoreClientId && (
				<p className="text-xs text-(--vscode-descriptionForeground)">{t("api_provider.sapaicore.client_id_set")}</p>
			)}

			<DebouncedTextField
				initialValue={apiConfiguration?.sapAiCoreClientSecret || ""}
				onChange={(value) => handleFieldChange("sapAiCoreClientSecret", value)}
				placeholder={t("api_provider.sapaicore.client_secret_placeholder")}
				style={{ width: "100%" }}
				type="password">
				<span className="font-medium">{t("api_provider.sapaicore.client_secret_label")}</span>
			</DebouncedTextField>
			{apiConfiguration?.sapAiCoreClientSecret && (
				<p className="text-xs text-(--vscode-descriptionForeground)">{t("api_provider.sapaicore.client_secret_set")}</p>
			)}

			<DebouncedTextField
				initialValue={apiConfiguration?.sapAiCoreBaseUrl || ""}
				onChange={(value) => handleFieldChange("sapAiCoreBaseUrl", value)}
				placeholder={t("api_provider.sapaicore.base_url_placeholder")}
				style={{ width: "100%" }}>
				<span className="font-medium">{t("api_provider.sapaicore.base_url_label")}</span>
			</DebouncedTextField>

			<DebouncedTextField
				initialValue={apiConfiguration?.sapAiCoreTokenUrl || ""}
				onChange={(value) => handleFieldChange("sapAiCoreTokenUrl", value)}
				placeholder={t("api_provider.sapaicore.token_url_placeholder")}
				style={{ width: "100%" }}>
				<span className="font-medium">{t("api_provider.sapaicore.token_url_label")}</span>
			</DebouncedTextField>

			<DebouncedTextField
				initialValue={apiConfiguration?.sapAiResourceGroup || ""}
				onChange={(value) => handleFieldChange("sapAiResourceGroup", value)}
				placeholder={t("api_provider.sapaicore.resource_group_placeholder")}
				style={{ width: "100%" }}>
				<span className="font-medium">{t("api_provider.sapaicore.resource_group_label")}</span>
			</DebouncedTextField>

			<p className="text-xs mt-1.5 text-(--vscode-descriptionForeground)">
				{t("api_provider.sapaicore.credentials_info")}{" "}
				<VSCodeLink
					className="inline"
					href="https://help.sap.com/docs/sap-ai-core/sap-ai-core-service-guide/access-sap-ai-core-via-api">
					{t("api_provider.sapaicore.more_info_link")}
				</VSCodeLink>
			</p>
			...
			{orchestrationAvailable && (
				<div className="flex flex-col gap-2.5 mt-[15px]">
					<div className="flex items-center gap-2">
						<VSCodeCheckbox
							aria-label={t("api_provider.sapaicore.orchestration_mode_label")}
						...
						/>
						<span className="font-medium">{t("api_provider.sapaicore.orchestration_mode_label")}</span>
					</div>

					<p className="text-xs text-(--vscode-descriptionForeground)">
						{t("api_provider.sapaicore.orchestration_mode_description")}
					</p>
				</div>
			)}
					...
					<div className="flex flex-col gap-1.5">
						{isLoadingModels ? (
							<div className="text-xs text-(--vscode-descriptionForeground)">
								{t("api_provider.sapaicore.loading_models")}
							</div>
						) : modelError ? (
							<div className="text-xs text-(--vscode-errorForeground)">
								...
								<button
									...
									{t("api_provider.sapaicore.retry")}
								</button>
							</div>
							...
							<>
								{sapAiCoreModelDeployments.length === 0 && (
									<div className="text-xs text-(--vscode-errorForeground) mb-2">
										{t("api_provider.sapaicore.no_models_error")}
									</div>
								)}
								<SapAiCoreModelPicker
									onModelChange={handleModelChange}
									placeholder={t("api_provider.sapaicore.model_picker_placeholder")}
							...
							<div className="text-xs text-(--vscode-errorForeground)">
								{t("api_provider.sapaicore.configure_credentials")}
							</div>
						)}
					</div>
