...
const experimentalFeatures: FeatureToggle[] = [
	{
		id: "skills",
		label: "Skills",
		description: "On-demand agent instructions from .cline/skills/",
		stateKey: "skillsEnabled",
		settingKey: "skillsEnabled",
		isExperimental: true,
	},
	{
		id: "multi-root-workspaces",
		label: "Multi-Root Workspace",
		description: "Allows cline to work across multiple workspaces.",
		stateKey: "multiRootSetting",
	...
	},
	{
		id: "yolo",
		label: "Yolo Mode",
		description: "Execute tasks without user's confirmation. Use with extreme caution.",
	...
	},
	{
		...
		label: "Dictation",
		description:
			"Speech-to-text with Aqua Voice's Avalon model via Cline provider. $0.0065 credits per minute of audio processed. 5 minutes max per message.",
		stateKey: "dictationSettings",
		settingKey: "dictationSettings",
		isExperimental: true,
		nestedKey: "dictationEnabled",
	},
	{
		id: "focus-chain",
		label: "Focus Chain",
...
const FeatureSettingsSection = ({ renderSectionHeader }: FeatureSettingsSectionProps) => {
	...
	const [sectionClosed, setSectionClosed] = useState(new Set("advance"))
	...
	return (
		<div className="mb-2">
			...
			<Section>
				<div className="mb-5">
					{/* Core features */}
					<Collapsible onOpenChange={() => handleSectionToggle("agent")} open={!sectionClosed.has("agent")}>
						<CollapsibleTrigger className="flex items-center gap-2 text-xs font-medium text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors w-full">
							<ChevronRightIcon
								className={cn("w-3.5 h-3.5 transition-transform", !sectionClosed.has("agent") && "rotate-90")}
							/>
							Agent
						</CollapsibleTrigger>
					...
					{/* Optional features */}
					<Collapsible onOpenChange={() => handleSectionToggle("editor")} open={!sectionClosed.has("editor")}>
						<CollapsibleTrigger className="flex items-center gap-2 text-xs font-medium text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors w-full">
							<ChevronRightIcon
								className={cn("w-3.5 h-3.5 transition-transform", !sectionClosed.has("editor") && "rotate-90")}
							/>
							Editor
						</CollapsibleTrigger>
						<CollapsibleContent className="mt-3">
					...
					<Collapsible
						onOpenChange={() => handleSectionToggle("experimental")}
						open={!sectionClosed.has("experimental")}>
						<CollapsibleTrigger className="flex items-center gap-2 text-xs font-medium text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors w-full">
							<ChevronRightIcon
								className={cn(
									"w-3.5 h-3.5 transition-transform",
									!sectionClosed.has("experimental") && "rotate-90",
								)}
							/>
							Experimental
						</CollapsibleTrigger>
						<CollapsibleContent className="mt-3">
							...
							{showSubagents && (
								<div
									...
									<div className="absolute -top-2 -right-2 px-2 py-0.5 rounded text-xs font-semibold bg-button-secondary-background text-button-secondary-foreground">
										NEW
									</div>
							...
							<div
								...
								{focusChainSettings?.enabled && (
									<div className="ml-3 pl-3 border-l border-border/50 pb-3">
				...
				{/* Advanced */}
				<Collapsible onOpenChange={() => handleSectionToggle("advance")} open={sectionClosed.has("advance")}>
					<CollapsibleTrigger className="flex items-center gap-2 text-xs font-medium text-muted-foreground uppercase tracking-wider hover:text-foreground transition-colors w-full">
						<ChevronRightIcon
							className={cn("w-3.5 h-3.5 transition-transform", sectionClosed.has("advance") && "rotate-90")}
						/>
						Advanced
					</CollapsibleTrigger>
					<CollapsibleContent className="mt-3">
