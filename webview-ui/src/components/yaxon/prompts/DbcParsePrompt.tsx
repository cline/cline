export const DbcFileParsePrompt=()=>
`
<explicit_instructions type="matrix_file_parse">
# DBC解析交互式工作流

## 目标
处理 从CAN-DBC文件根据用户偏好生成 C/Java 代码。
...
## 步骤 1: 调用Mcp将DBC文件转换为C/Java代码
  ...
  然后调用 MCP 服务,对传入的DBC文件url $TASK_FILES 进行处理生成对应代码：
...
## 步骤 2: 将生成的代码保存到本地项目的指定目录
  ...
  -文件保存到本地时，请简化本地文件名，去掉文件名下划线后面的Md5。
   <download_file>
    <fileUrl>(header_file_url and source_file_url) or (signal_file_url and parser_file_url) </fileUrl>
    <savePath>$code_file_paths</savePath>
   </download_file>
