						...
						<div className="flex flex-col gap-3">
							{model.score && (
								<div className="inline-flex gap-1 [&_svg]:stroke-warning [&_svg]:size-3 items-center text-sm">
									<StarIcon />
									<span>{t("onboarding.model_overview")} </span>
		...
		<div className="flex flex-col w-full items-center px-2">
			...
			<div className="flex w-full max-w-lg flex-col gap-6 my-4 border-t border-muted-foreground">
				<div className="flex flex-col gap-3 mt-6" key="search-results">
					<h4 className="text-sm font-bold text-foreground/70 uppercase mb-2">{t("onboarding.other_options")}</h4>
					<Input
						autoFocus={false}
						...
						placeholder={t("onboarding.search_model")}
						type="search"
						value={searchTerm}
					/>
					<div className="w-full flex flex-col gap-3">
						...
						{searchTerm.length > 0 && searchedModels.length === 0 && (
							<p className="px-1 mt-1 text-sm text-foreground/70">
								{t("onboarding.no_result_found", { searchTerm })}
							</p>
						)}
					</div>
				</div>
...
const UserTypeSelectionStep = ({ userType, onSelectUserType }: UserTypeSelectionProps) => {
	...
	return (
		<div className="flex flex-col w-full items-center">
			<div className="flex w-full max-w-lg flex-col gap-6 my-4">
				<h3 className="text-base text-left self-start font-semibold">{t("onboarding.lets_get_started")}</h3>
					...
					return (
						<Item
							...
							<ItemContent className="w-full">
								...
								<ItemDescription>{t(option.description)}</ItemDescription>
							</ItemContent>
						</Item>
					)
				})}
...
const OnboardingView = () => {
	...
	const stepDisplayInfo = useMemo(() => {
		...
		const description = step ? t(step.description || "") : null
				...
				<footer className="flex w-full max-w-lg flex-col gap-3 my-2 px-2 overflow-hidden">
					...
					{stepNumber !== 2 && (
						<div className="items-center justify-center flex text-sm text-foreground gap-2 mb-3 text-pretty">
							<AlertCircleIcon className="shrink-0 size-2" /> {t("onboarding.change_later_settings")}
						</div>
					)}
				</footer>
			</div>
