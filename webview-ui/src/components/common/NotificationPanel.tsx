...
export const NotificationPanel: React.FC<NotificationPanelProps> = ({
		...
		return (
			<div className="notification-panel" ref={panelRef}>
				<div className="notification-header">
					<div>
						<h3>Notifications</h3>
						{taskId && <div className="task-id-display">Task ID: {taskId}</div>}
					</div>
					<button className="close-button" onClick={onClose} title="Close notifications">
	...
	const renderNotification = (notification: ExternalAdvice, isDismissed: boolean = false) => {
		...
		const hasSuggestions =
			hasMachineData && notification.machineData?.suggestions && notification.machineData.suggestions.length > 0
			...
			<div
				...
				<div className="notification-content">
					...
					<div className="notification-content-text">
						{notification.content.length > 200 ? (
							<>
								...
								<div className="content-toggle-button" onClick={() => toggleExpandContent(notification.id)}>
							...
							{notification.machineData?.suggestions?.map((suggestion, index) => (
								<div key={index} className="suggestion-item">
									<div className="suggestion-header">
										...
										<span
											className="suggestion-file"
											onClick={() => onOpenRelatedFile && onOpenRelatedFile(suggestion.file)}>
					...
					<div className="notification-meta">
						<span className="notification-time">{formatDistanceToNow(notification.timestamp)} ago</span>
						<span className={`notification-type ${notification.type}`}>{notification.type}</span>
						<span className="notification-action-type">{hasMachineData && notification.machineData?.actionType}</span>
					</div>
				</div>
