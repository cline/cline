...
const TimelineItemWrapper = styled.div`
	...
	&:hover ${CheckpointControls}, &:has(${CheckpointControls}) ${CheckpointControls} {
		opacity: 1;
	}
`
...
const Timeline: React.FC<TimelineProps> = ({ messages }) => {
	...
	return (
		<TimelineContainer isCollapsed={isCollapsed}>
			<TimelineHeader>
				<TimelineTitle>
					<CollapseToggle className={isCollapsed ? "collapsed" : ""} onClick={() => setIsCollapsed(!isCollapsed)}>
								...
								return (
									<TimelineItemWrapper key={item.ts}>
										<TimelineItem
											...
											<i
												className={`codicon codicon-${getItemIcon(itemType)}`}
												style={{ fontSize: "14px" }}
											/>
										</TimelineItem>
