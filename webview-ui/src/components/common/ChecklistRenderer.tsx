		...
		<div
			...
			{items.map((item, index) => (
				...
				<div className="flex items-start gap-1.5 p-0.5" key={`checklist-item-${index}`}>
					<span className={cn("text-xs shrink-0 mt-0.5", item.checked ? "text-success" : "text-badge-foreground")}>
						{item.checked ? <CheckIcon size={10} /> : <CircleIcon size={10} />}
					</span>
					<span
						className={cn("text-xs break-words flex-1", item.checked ? "text-description" : "text-badge-foreground")}
						style={{
