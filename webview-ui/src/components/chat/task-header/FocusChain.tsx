...
const getStaticStrings = (t: any) => ({
	COMPLETED_MESSAGE: t("taskHeader.allTasksCompleted"),
	TODO_LIST_LABEL: t("taskHeader.todoList"),
	NEW_STEPS_MESSAGE: t("taskHeader.newStepsGenerated"),
	CLICK_TO_EDIT_TITLE: t("taskHeader.clickToEditTodoList"),
})
...
const ToDoListHeader = memo<{
	...
	const strings = getStaticStrings(t)
	// Pre-compute display text
	const displayText = isCompleted ? strings.COMPLETED_MESSAGE : currentTodo?.text || strings.TODO_LIST_LABEL
...
export const FocusChain: React.FC<FocusChainProps> = memo(
	({ currentTaskItemId, lastProgressMessageText }) => {
		...
		const strings = useMemo(() => getStaticStrings(t), [t])
		...
		return (
			<div
				aria-label={isExpanded ? t("taskHeader.collapseFocusChain") : t("taskHeader.expandFocusChain")}
				...
				tabIndex={0}
				title={strings.CLICK_TO_EDIT_TITLE}>
				...
				{isExpanded && (
					<div className="mx-1 pb-2 px-1 relative" onClick={handleEditClick}>
						...
						{isCompleted && (
							<div className="mt-2 text-xs font-semibold text-muted-foreground">{strings.NEW_STEPS_MESSAGE}</div>
						)}
					</div>
				)}
			</div>
