...
const BrowserSessionRow = memo((props: BrowserSessionRowProps) => {
    ...
    const pages = useMemo(() => {
        const result: {
            currentState: {
                ...
                consoleLogs?: string
                messages: PostHogMessage[] // messages up to and including the result
            }
            nextAction?: {
                messages: PostHogMessage[] // messages leading to next result
            }
        }[] = []

        let currentStateMessages: PostHogMessage[] = []
        let nextActionMessages: PostHogMessage[] = []
    ...
    // Calculate maxWidth
    const maxWidth =
        browserSettings.viewport.width < BROWSER_VIEWPORT_PRESETS['Small Desktop (900x600)'].width ? 200 : undefined
        ...
        <BrowserSessionRowContainer style={{ marginBottom: -10 }}>
            ...
            >
                ...
                <span style={{ fontWeight: 'bold' }}>
                    <>{isAutoApproved ? 'PostHog is using the browser:' : 'PostHog wants to use the browser:'}</>
                </span>
            </div>
            <div
                style={{
