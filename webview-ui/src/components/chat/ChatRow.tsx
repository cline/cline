...
interface ChatRowProps {
    message: PostHogMessage
    isExpanded: boolean
    onToggleExpand: () => void
    lastModifiedMessage?: PostHogMessage
    isLast: boolean
...
export const ChatRowContent = ({
    ...
    const tool = useMemo(() => {
        if (message.ask === 'tool' || message.say === 'tool') {
            return JSON.parse(message.text || '{}') as PostHogSayTool
        }
        return null
    ...
    if (message.ask === 'use_mcp_server' || message.say === 'use_mcp_server') {
        const useMcpServer = JSON.parse(message.text || '{}') as PostHogAskUseMcpServer
        ...
        case 'say':
            switch (message.say) {
                ...
                case 'user_feedback_diff':
                    const tool = JSON.parse(message.text || '{}') as PostHogSayTool
                    return (
                        <div
                            style={{
                                marginTop: -10,
        ...
        case 'ask':
            switch (message.ask) {
                ...
                case 'followup':
                    ...
                    try {
                        const parsedMessage = JSON.parse(message.text || '{}') as PostHogAskQuestion
                ...
                case 'plan_mode_respond': {
                    ...
                    let selected: string | undefined
                    try {
                        const parsedMessage = JSON.parse(message.text || '{}') as PostHogPlanModeResponse
