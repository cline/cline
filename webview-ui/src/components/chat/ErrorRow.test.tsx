...
vi.mock("@/context/AiHydroAuthContext", () => ({
	useAiHydroAuth: () => ({
		aihydroUser: null,
	}),
	handleSignIn: vi.fn(),
...
// Mock AiHydroError
vi.mock("../../../../src/services/error/AiHydroError", () => ({
	AiHydroError: {
		parse: vi.fn(),
	},
	AiHydroErrorType: {
...
describe("ErrorRow", () => {
	const mockMessage: AiHydroMessage = {
		ts: 123456789,
		type: "say",
		say: "error",
	...
	it("renders aihydroignore error", () => {
		const aihydroignoreMessage = { ...mockMessage, text: "/path/to/file.txt" }
		render(<ErrorRow errorType="aihydroignore_error" message={aihydroignoreMessage} />)

		expect(screen.getByText(/AI-Hydro tried to access/)).toBeInTheDocument()
		expect(screen.getByText("/path/to/file.txt")).toBeInTheDocument()
	})

	describe("API error handling", () => {
		it("renders credit limit error when balance error is detected", async () => {
			const mockAiHydroError = {
			...
			const { AiHydroError } = await import("../../../../src/services/error/AiHydroError")
			vi.mocked(AiHydroError.parse).mockReturnValue(mockAiHydroError as any)
		...
		it("renders auth error with sign in button when user is not signed in", async () => {
			...
			expect(screen.getByText("Check your provider credentials, then click Retry.")).toBeInTheDocument()
		})
		...
		it("falls back to regular error message when AiHydroError.parse returns null", async () => {
			const { AiHydroError } = await import("../../../../src/services/error/AiHydroError")
			vi.mocked(AiHydroError.parse).mockReturnValue(undefined)

			render(<ErrorRow apiRequestFailedMessage="Some API error" errorType="error" message={mockMessage} />)

			// When AiHydroError.parse returns null, we display the raw error message for non-AI-Hydro providers
			// Since parsedError is undefined, isAiHydroProvider is false, so we show the raw apiRequestFailedMessage
