...
const ChatView = ({ isHidden, showHistoryView }: ChatViewProps) => {
    ...
    const [posthogAsk, setPostHogAsk] = useState<PostHogAsk | undefined>(undefined)
                ...
                case 'ask':
                    ...
                    switch (lastMessage.ask) {
                        ...
                        case 'mistake_limit_reached':
                            ...
                            setPostHogAsk('mistake_limit_reached')
                            setEnableButtons(true)
                        ...
                        case 'auto_approval_max_req_reached':
                            setTextAreaDisabled(true)
                            setPostHogAsk('auto_approval_max_req_reached')
                            setEnableButtons(true)
                        ...
                        case 'plan_mode_respond':
                            setTextAreaDisabled(isPartial)
                            setPostHogAsk('plan_mode_respond')
                            setEnableButtons(false)
                            // setPrimaryButtonText(undefined)
                            // setSecondaryButtonText(undefined)
                            break
                        case 'tool':
                            ...
                            const tool = JSON.parse(lastMessage.text || '{}') as PostHogSayTool
                            switch (tool.tool) {
                        ...
                        case 'browser_action_launch':
                            setTextAreaDisabled(isPartial)
                            setPostHogAsk('browser_action_launch')
                            setEnableButtons(!isPartial)
                            setPrimaryButtonText('Approve')
                            setSecondaryButtonText('Reject')
                            break
                        ...
                        case 'completion_result':
                            ...
                            setTextAreaDisabled(isPartial)
                            setPostHogAsk('completion_result')
                            setEnableButtons(!isPartial)
                        ...
                        case 'resume_completed_task':
                            setTextAreaDisabled(false)
                            setPostHogAsk('resume_completed_task')
                            setEnableButtons(true)
                            setPrimaryButtonText('Start New Task')
                            setSecondaryButtonText(undefined)
    ...
    const isStreaming = useMemo(() => {
        const isLastAsk = !!modifiedMessages.at(-1)?.ask // checking posthogAsk isn't enough since messages effect may be called again for a tool for example, set posthogAsk to its value, and if the next message is not an ask then it doesn't reset. This is likely due to how much more often we're updating messages as compared to before, and should be resolved with optimizations as it's likely a rendering bug. but as a final guard for now, the cancel button will show if the last message is not an ask
        const isToolCurrentlyAsking =
            isLastAsk && posthogAsk !== undefined && enableButtons && primaryButtonText !== undefined
        ...
        return false
    }, [modifiedMessages, posthogAsk, enableButtons, primaryButtonText])
    ...
    */
    ...
    const handleSecondaryButtonClick = useCallback(
        ...
        [posthogAsk, startNewTask, isStreaming]
    )
    ...
    const isBrowserSessionMessage = (message: PostHogMessage): boolean => {
    ...
    const groupedMessages = useMemo(() => {
        const result: (PostHogMessage | PostHogMessage[])[] = []
        let currentGroup: PostHogMessage[] = []
            ...
            if (message.ask === 'browser_action_launch' || message.say === 'browser_action_launch') {
                ...
                if (isBrowserSessionMessage(message)) {
                    ...
                    if (message.say === 'browser_action') {
                        const browserAction = JSON.parse(message.text || '{}') as PostHogSayBrowserAction
    ...
    const itemContent = useCallback(
        (index: number, messageOrGroup: PostHogMessage | PostHogMessage[]) => {
