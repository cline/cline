...
const AiHydroRulesToggleModal: React.FC = () => {
	const {
		...
		setGlobalAiHydroRulesToggles,
		setLocalAiHydroRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalAiHydroRulesToggles?.toggles) {
						setGlobalAiHydroRulesToggles(response.globalAiHydroRulesToggles.toggles)
					}
					if (response.localAiHydroRulesToggles?.toggles) {
						setLocalAiHydroRulesToggles(response.localAiHydroRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalAiHydroRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localAiHydroRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleAiHydroRule(
			...
			.catch((error) => {
				console.error("Error toggling AI-Hydro rule:", error)
			})
	}
	...
	return (
		<div ref={modalRef}>
			<div className="inline-flex min-w-0 max-w-full" ref={buttonRef}>
				<Tooltip tipText="Manage AI-Hydro Rules & Workflows" visible={isVisible ? false : undefined}>
					<VSCodeButton
						appearance="icon"
						aria-label={isVisible ? "Hide AI-Hydro Rules & Workflows" : "Show AI-Hydro Rules & Workflows"}
			...
			{isVisible && (
				<div
					...
					<div className="text-xs text-[var(--vscode-descriptionForeground)] mb-4">
						{currentView === "rules" ? (
							<p>
								Rules allow you to provide AI-Hydro with system-level guidance. Think of them as a persistent way
							...
							<p>
								Workflows allow you to define a series of steps to guide AI-Hydro through a repetitive set of
...
export default AiHydroRulesToggleModal
