...
import {
	...
	ToggleBeadsmithRuleRequest,
	ToggleCursorRuleRequest,
	ToggleSkillRequest,
	ToggleWindsurfRuleRequest,
	ToggleWorkflowRequest,
} from "@shared/proto/beadsmith/file"
...
const BeadsmithRulesToggleModal: React.FC = () => {
	const {
		...
		hooksEnabled,
		skillsEnabled,
		setGlobalBeadsmithRulesToggles,
		setLocalBeadsmithRulesToggles,
		...
		if (isVisible) {
			FileServiceClient.refreshRules({} as EmptyRequest)
				.then((response: RefreshedRules) => {
					...
					if (response.globalBeadsmithRulesToggles?.toggles) {
						setGlobalBeadsmithRulesToggles(response.globalBeadsmithRulesToggles.toggles)
					}
					if (response.localBeadsmithRulesToggles?.toggles) {
						setLocalBeadsmithRulesToggles(response.localBeadsmithRulesToggles.toggles)
					}
	...
	const globalRules = Object.entries(globalBeadsmithRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))

	// Format local rules for display with proper typing
	const localRules = Object.entries(localBeadsmithRulesToggles || {})
		.map(([path, enabled]): [string, boolean] => [path, enabled as boolean])
		.sort(([a], [b]) => a.localeCompare(b))
	...
	const toggleRule = (isGlobal: boolean, rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleBeadsmithRule(
			ToggleBeadsmithRuleRequest.create({
			...
			.catch((error) => {
				console.error("Error toggling Beadsmith rule:", error)
			})
	}
	...
	const toggleWindsurfRule = (rulePath: string, enabled: boolean) => {
		FileServiceClient.toggleWindsurfRule(
			...
			.then((response: BeadsmithRulesToggles) => {
				if (response.toggles) {
	...
	return (
		<div className="inline-flex min-w-0 max-w-full items-center" ref={modalRef}>
			<div className="inline-flex w-full items-center" ref={buttonRef}>
				<Tooltip>
					{!isVisible && <TooltipContent>Manage Beadsmith Rules & Workflows</TooltipContent>}
					<TooltipTrigger>
						<VSCodeButton
							appearance="icon"
							aria-label={isVisible ? "Hide Beadsmith Rules & Workflows" : "Show Beadsmith Rules & Workflows"}
					...
					<div className="flex-shrink-0 px-2 pt-0">
						...
						<div className="text-xs text-description mb-4">
							{currentView === "rules" ? (
								...
								<p>
									Workflows allow you to define a series of steps to guide Beadsmith through a repetitive set of
								...
								{workspaceHooks.map((workspace, index) => (
									<div
										...
										<div className="text-sm font-normal mb-2">
											{workspace.workspaceName}/.beadsmithrules/hooks/
										</div>
										<div className="flex flex-col gap-0">
											{workspace.hooks
												.sort((a, b) => a.name.localeCompare(b.name))
...
export default BeadsmithRulesToggleModal
