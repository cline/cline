...
const ClineRulesToggleModal = forwardRef<ClineRulesToggleModalHandle, ClineRulesToggleModalProps>(
					...
					<PopupModalContainer $arrowPosition={arrowPosition} $menuPosition={menuPosition} ref={popupContainerRef}>
						...
						<div className="flex-shrink-0 px-2 pt-0">
							...
							<div className="text-xs text-description mb-4">
								{currentView === "rules" ? (
									<p>
										Rules allow you to provide Cline with system-level guidance. Think of them as a persistent
										way to include context and preferences for your projects or globally for every
										conversation.{" "}
										<VSCodeLink
											className="text-xs"
									...
									<p>
										Skills are reusable instruction sets that Cline can activate on-demand. When a task
										matches a skill's description, Cline uses the <span className="font-bold">use_skill</span>{" "}
										tool to load the full instructions.
									</p>
								) : (
									<p>
										Hooks allow you to execute custom scripts at specific points in Cline's execution
								...
								<>
									...
									{isWindows && (
										<div className="flex items-center gap-2 px-5 py-3 mb-4 bg-vscode-inputValidation-warningBackground border-l-[3px] border-vscode-inputValidation-warningBorder">
											...
											<span className="text-base">
												Hook toggling is not supported on Windows. Hooks can be created, edited, and
												deleted, but cannot be enabled/disabled and will not execute.
											</span>
										</div>
									)}
