...
export const ExtensionStateContextProvider: React.FC<{
	...
	const hideSkills = useCallback(() => setShowSkills(false), [setShowSkills])
	...
	const navigateToMap = useCallback(() => {
		...
		setShowSkills(false)
		setShowMap(true)
	}, [setShowSettings, closeMcpView, setShowHistory, setShowAccount, setShowConnectors, setShowSkills, setShowMap])
	...
	}, [setShowSettings, closeMcpView, setShowHistory, setShowAccount, setShowMap, setShowSkills, setShowConnectors])
	...
	const navigateToSkills = useCallback(() => {
		...
		setShowConnectors(false)
		setShowSkills(true)
	}, [setShowSettings, closeMcpView, setShowHistory, setShowAccount, setShowMap, setShowConnectors, setShowSkills])
	...
	const skillsButtonClickedSubscriptionRef = useRef<(() => void) | null>(null)
	...
	useEffect(() => {
		...
		// Set up skills button clicked subscription
		skillsButtonClickedSubscriptionRef.current = UiServiceClient.subscribeToSkillsButtonClicked(EmptyRequest.create(), {
			onResponse: () => {
				navigateToSkills()
			},
			onError: (error) => {
				console.error("Error in skills button clicked subscription:", error)
			},
			onComplete: () => {
				console.log("Skills button clicked subscription completed")
			},
		})
		...
		return () => {
			...
			if (skillsButtonClickedSubscriptionRef.current) {
				skillsButtonClickedSubscriptionRef.current()
				skillsButtonClickedSubscriptionRef.current = null
			}
