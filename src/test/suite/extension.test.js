...
describe('Extension Tests', function () {
    ...
    it('should activate extension successfully', async () => {
        // Get the extension
        const extension = vscode.extensions.getExtension('posthog.posthog-extension')
    ...
    it('should handle basic commands', async () => {
        ...
        await vscode.commands.executeCommand('posthog.historyButtonClicked')
        // Success if no error thrown
    })

    it('should handle advanced settings configuration', async () => {
        // Test browser session setting
        await vscode.workspace.getConfiguration().update('posthog.disableBrowserTool', true, true)
        const updatedConfig = vscode.workspace.getConfiguration('posthog')
        expect(updatedConfig.get('disableBrowserTool')).to.be.true

        // Reset settings
        await vscode.workspace.getConfiguration().update('posthog.disableBrowserTool', undefined, true)
    })
})
