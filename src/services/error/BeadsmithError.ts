...
export enum BeadsmithErrorType {
	Auth = "auth",
	Network = "network",
	RateLimit = "rateLimit",
	Balance = "balance",
...
export class BeadsmithError extends Error {
	readonly title = "BeadsmithError"
	...
	/**
	 * Parses a stringified error into a BeadsmithError instance.
	 */
	static parse(errorStr?: string, modelId?: string): BeadsmithError | undefined {
		if (!errorStr || typeof errorStr !== "string") {
			return undefined
		}
		return BeadsmithError.transform(errorStr, modelId)
	}

	/**
	 * Transforms any object into a BeadsmithError instance.
	 * Always returns a BeadsmithError, even if the input is not a valid error object.
	 */
	static transform(error: any, modelId?: string, providerId?: string): BeadsmithError {
		try {
			// If already a BeadsmithError, return it directly to prevent infinite recursion
			if (error instanceof BeadsmithError) {
				return error
			}
			return new BeadsmithError(JSON.parse(error), modelId, providerId)
		} catch {
			return new BeadsmithError(error, modelId, providerId)
		}
	}

	public isErrorType(type: BeadsmithErrorType): boolean {
		return BeadsmithError.getErrorType(this) === type
	}
	 ...
	 */
	static getErrorType(err: BeadsmithError): BeadsmithErrorType | undefined {
		...
		if (code === "insufficient_credits" && typeof details?.current_balance === "number") {
			return BeadsmithErrorType.Balance
		}
		...
		if (code === "ERR_BAD_REQUEST" || err instanceof AuthInvalidTokenError || isAuthStatus) {
			return BeadsmithErrorType.Auth
		}
		...
		if (message) {
			...
			if (message?.includes(BEADSMITH_ACCOUNT_AUTH_ERROR_MESSAGE) || authErrorRegex.some((regex) => regex.test(message))) {
				return BeadsmithErrorType.Auth
			}
			...
			if (RATE_LIMIT_PATTERNS.some((pattern) => pattern.test(lowerMessage))) {
				return BeadsmithErrorType.RateLimit
			}
		}
...
export class AuthNetworkError extends Error {
	constructor(
		...
		super(message)
		this.name = BeadsmithErrorType.Network
	}
}
...
export class AuthInvalidTokenError extends Error {
	constructor(message: string) {
		super(message)
		this.name = BeadsmithErrorType.Auth
	}
}
