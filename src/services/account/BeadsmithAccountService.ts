...
export class BeadsmithAccountService {
	private static instance: BeadsmithAccountService
	...
	/**
	 * Returns the singleton instance of BeadsmithAccountService
	 * @returns Singleton instance of BeadsmithAccountService
	 */
	public static getInstance(): BeadsmithAccountService {
		if (!BeadsmithAccountService.instance) {
			BeadsmithAccountService.instance = new BeadsmithAccountService()
		}
		return BeadsmithAccountService.instance
	}

	/**
	 * Returns the base URL for the Beadsmith API
	...
	async fetchMe(): Promise<UserResponse | undefined> {
		try {
			const data = await this.authenticatedRequest<UserResponse>(BEADSMITH_API_ENDPOINT.USER_INFO)
			return data
		} catch (error) {
	...
	async switchAccount(organizationId?: string): Promise<void> {
		...
		try {
			...
			const _response = await this.authenticatedRequest<string>(BEADSMITH_API_ENDPOINT.ACTIVE_ACCOUNT, {
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				},
	...
	/**
	 * Transcribes audio using the Beadsmith transcription service
