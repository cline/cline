...
export class McpOAuthManager {
	...
	/**
	 * Opens the browser to the OAuth authorization URL when user clicks "Authenticate"
	 *
	 * Flow:
	 ...
	 * 4. initiateOAuth() calls auth() which generates the auth URL
	 * 5. McpHub calls THIS METHOD
	 * 6. We open browser to the stored auth URL
	...
	async startOAuthFlow(serverName: string, serverUrl: string): Promise<void> {
		...
		if (storedAuthUrl) {
			...
			throw new Error(`No auth URL found for ${serverName}. Please try again.`)
		}
	}
	...
	/**
	 * Clears only tokens for a server (used when refresh token is invalid/expired)
	 * Preserves client_info so re-authentication can reuse the registered client
	 */
	async clearServerTokens(serverName: string, serverUrl: string): Promise<void> {
		const serverHash = getServerAuthHash(serverName, serverUrl)
		const secrets = getMcpOAuthSecrets()

		if (secrets[serverHash]) {
			delete secrets[serverHash].tokens
			delete secrets[serverHash].tokens_saved_at
			saveMcpOAuthSecrets(secrets)
			console.log(`[McpOAuth] Cleared expired tokens for ${serverName}`)
		}
	}
}
