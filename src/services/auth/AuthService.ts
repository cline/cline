...
export class AuthService {
	...
	private _activeAuthCallbackSubscriptions = new Set<StreamingResponseHandler>()
	private _activeAuthStatusUpdateSubscriptions = new Set<StreamingResponseHandler>()
	private _controllerRef: WeakRef<Controller> | null = null
	 ...
	 */
	private constructor(config: ServiceConfig, authProvider?: any, controller?: Controller) {
		...
		if (controller) {
			this._controllerRef = new WeakRef(controller)
		}
	}
	 ...
	 */
	public static getInstance(config?: ServiceConfig, authProvider?: any, controller?: Controller): AuthService {
		if (!AuthService.instance) {
			AuthService.instance = new AuthService(config || {}, authProvider, controller)
		} else if (controller) {
			// Update controller reference if provided
			AuthService.instance._controllerRef = new WeakRef(controller)
		}
	...
	async createAuthRequest(): Promise<String> {
		...
		if (!this._authenticated) {
			...
			// If already authenticated, no need to create a new auth request
			console.log("Already authenticated, no need to create auth request")
	...
	private async _autoRefreshAuth(): Promise<void> {
		...
		await this._user.getIdToken(true) // Force refresh the token
		const timeoutDuration = Math.floor(this._user.stsTokenManager.expirationTime - 5 * 60000 - Date.now()) // Milliseconds until 5 minutes before expiration
		setTimeout(() => this._autoRefreshAuth(), timeoutDuration)
		this.sendAuthStatusUpdate()
		// Notify all subscribers about the authentication status update
		await this.sendAuthCallbackEvent(this._user ? this._user.token : "")
	}
	...
	async subscribeToAuthCallback(
		...
		this._activeAuthCallbackSubscriptions.add(responseStream)

		// Register cleanup when the connection is closed
		const cleanup = () => {
			this._activeAuthCallbackSubscriptions.delete(responseStream)
		}
	 ...
	 */
	async sendAuthCallbackEvent(customToken: string): Promise<void> {
		// Send the event to all active subscribers
		const promises = Array.from(this._activeAuthCallbackSubscriptions).map(async (responseStream) => {
			try {
				const event: String = String.create({
					value: customToken,
				})
	...
	async subscribeToAuthStatusUpdate(
		...
		// Add this subscription to the active subscriptions
		this._activeAuthStatusUpdateSubscriptions.add(responseStream)
		// Register cleanup when the connection is closed
		const cleanup = () => {
			this._activeAuthStatusUpdateSubscriptions.delete(responseStream)
		}
		// Register the cleanup function with the request registry if we have a requestId
		if (requestId) {
	...
	async sendAuthStatusUpdate(): Promise<void> {
		// Send the event to all active subscribers
		const promises = Array.from(this._activeAuthStatusUpdateSubscriptions).map(async (responseStream) => {
			try {
				...
				const controller = this._controllerRef?.deref()
				if (controller) {
