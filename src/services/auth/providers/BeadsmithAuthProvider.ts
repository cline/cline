...
import { BeadsmithEnv, EnvironmentConfig } from "@/config"
import { Controller } from "@/core/controller"
import { HostProvider } from "@/hosts/host-provider"
import { buildBasicBeadsmithHeaders } from "@/services/EnvUtils"
import { AuthInvalidTokenError, AuthNetworkError } from "@/services/error/BeadsmithError"
import { telemetryService } from "@/services/telemetry"
import { BEADSMITH_API_ENDPOINT } from "@/shared/beadsmith/api"
import { fetch, getAxiosSettings } from "@/shared/net"
import { Logger } from "@/shared/services/Logger"
import { type BeadsmithAccountUserInfo, type BeadsmithAuthInfo } from "../AuthService"
import { parseJwtPayload } from "../oca/utils/utils"

interface BeadsmithAuthApiUser {
	subject: string | null
	email: string
	name: string
	beadsmithUserId: string | null
	accounts: string[] | null
}

// Unified API response data shape for token exchange/refresh
interface BeadsmithAuthResponseData {
	/**
...
export interface BeadsmithAuthApiTokenExchangeResponse {
	success: boolean
	data: BeadsmithAuthResponseData
}

export interface BeadsmithAuthApiTokenRefreshResponse {
	success: boolean
	data: BeadsmithAuthResponseData
}
...
export class BeadsmithAuthProvider {
	...
	private clearSession(controller: Controller, reason: string, storedAuthData?: BeadsmithAuthInfo) {
		Logger.error(reason)
		...
		controller.stateManager.setSecret("cline:beadsmithAccountId", undefined)
		this.refreshRetryCount = 0
		this.lastRefreshAttempt = 0
		return null
	}

	private logFailedRefreshAttempt(response: Response, storedAuthData?: BeadsmithAuthInfo) {
		const startedAt = storedAuthData?.startedAt
		const timeSinceStarted = Date.now() - (startedAt || 0)
	...
	/**
	 * Retrieves Beadsmith auth info using the stored access token.
	 * @param controller - The controller instance to access stored secrets.
	 * @returns {Promise<BeadsmithAuthInfo | null>} A promise that resolves with the auth info or null.
	 */
	async retrieveBeadsmithAuthInfo(controller: Controller): Promise<BeadsmithAuthInfo | null> {
		try {
			// Get the stored auth data from secure storage
			const storedAuthDataString = controller.stateManager.getSecretKey("cline:beadsmithAccountId")
			...
			let storedAuthData: BeadsmithAuthInfo
			try {
			...
			if (await this.shouldRefreshIdToken(storedAuthData.refreshToken, storedAuthData.expiresAt)) {
				...
				try {
					...
					if (newAuthInfoString !== storedAuthDataString) {
						controller.stateManager.setSecret("beadsmithAccountId", undefined) // cleanup old key
						controller.stateManager.setSecret("cline:beadsmithAccountId", newAuthInfoString)
					}
					...
					// when the user actually tries to use Beadsmith, not at startup
					return storedAuthData
				}
			}
	 ...
	 * @returns {Promise<BeadsmithAuthInfo>} The new access token and user info.
	 */
	async refreshToken(refreshToken: string, storedData: BeadsmithAuthInfo): Promise<BeadsmithAuthInfo> {
		try {
			const endpoint = new URL(BEADSMITH_API_ENDPOINT.REFRESH_TOKEN, this.config.apiBaseUrl)
			...
			const data: BeadsmithAuthApiTokenExchangeResponse = await response.json()
	...
	async getAuthRequest(callbackUrl: string): Promise<string> {
		const authUrl = new URL(BEADSMITH_API_ENDPOINT.AUTH, this.config.apiBaseUrl)
	...
	async signIn(controller: Controller, authorizationCode: string, provider: string): Promise<BeadsmithAuthInfo | null> {
		try {
			...
			const tokenUrl = new URL(BEADSMITH_API_ENDPOINT.TOKEN_EXCHANGE, this.config.apiBaseUrl)
			...
			const responseType: BeadsmithAuthApiTokenExchangeResponse = responseJSON
			...
			controller.stateManager.setSecret("cline:beadsmithAccountId", JSON.stringify(beadsmithAuthInfo))

			return beadsmithAuthInfo
		} catch (error) {
	...
	private async fetchRemoteUserInfo(
		...
		try {
			...
			return {
				id: tokenData.userInfo.beadsmithUserId || "",
