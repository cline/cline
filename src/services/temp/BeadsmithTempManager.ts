/**
 * BeadsmithTempManager - Manages temporary files for Beadsmith with automatic cleanup.
 *
 * Simple approach:
...
/**
 * Singleton manager for Beadsmith's temporary files.
 */
class BeadsmithTempManagerImpl {
	...
	constructor() {
		...
		const beadsmithTempDir = path.join(baseTempDir, "cline")

		try {
			fs.mkdirSync(beadsmithTempDir, { recursive: true })
			this.tempDir = beadsmithTempDir
		} catch {
	...
	/**
	 * Clean up old Beadsmith temp files based on age and total size constraints.
	 * Called on extension activation.
	 *
	 * Strategy:
	 * 1. Scan the Beadsmith temp directory
	...
	async cleanup(): Promise<{ deletedCount: number; freedBytes: number }> {
		...
		try {
			...
			Logger.error("Error during Beadsmith temp cleanup", error)
		}
...
export const BeadsmithTempManager = new BeadsmithTempManagerImpl()
