		...
		while (queue.length > 0 && results.size < limit) {
			...
			for (const file of filesAtLevel) {
				...
				if (file.endsWith("/")) {
					...
					// are checked against relative entry paths, not absolute ones.
					const cwd = options?.cwd?.toString() ?? ""
					const relativeDir = path.relative(cwd, file)
					// Escape backslashes and parentheses to prevent glob pattern interpretation.
					// Crucial for NextJS folder naming conventions which use parentheses like (auth), (dashboard).
