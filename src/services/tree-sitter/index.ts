...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = beadsmithIgnoreController ? beadsmithIgnoreController.filterPaths(filesToParse) : filesToParse

	for (const filePath of allowedFilesToParse) {
		const definitions = await parseFile(filePath, languageParsers, beadsmithIgnoreController)
		if (definitions) {
...
async function parseFile(
	...
	if (beadsmithIgnoreController && !beadsmithIgnoreController.validateAccess(filePath)) {
		return null
	}
