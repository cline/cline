 ...
 */
export function registerTaskCommands(context: vscode.ExtensionContext, provider: PostHogProvider): vscode.Disposable[] {
    return [
        vscode.commands.registerCommand('posthog.dev.createTestTasks', async () => {
...
function createRealisticMessageSequence(
    ...
    const messages: PostHogMessage[] = [
        // Initial task message - uses "say" with "text" which is the format used in PostHog.ts
        {
            ts: baseTimestamp,
            type: 'say',
            say: 'text',
