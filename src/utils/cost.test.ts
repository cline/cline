...
const modelInfo: ModelInfo = {
  ...
  supportsPromptCache: true, // or false, depending on the expected behavior
    ...
    const expectedCost = (5 / 1_000_000) * 500 + (3 / 1_000_000) * 300 + (10 / 1_000_000) * 1000 + (20 / 1_000_000) * 2000;
    expect(calculateApiCost(modelInfo, inputTokens, outputTokens, cacheCreationInputTokens, cacheReadInputTokens)).toBe(expectedCost);
  });
...
const modelInfo: ModelInfo = {
  ...
  supportsPromptCache: true, // or false, depending on the expected behavior
    ...
    expect(calculateApiCost(modelInfo, inputTokens, outputTokens, undefined, cacheReadInputTokens)).toBe(expectedCost);
  });
...
const modelInfo: ModelInfo = {
  ...
  supportsPromptCache: true, // or false, depending on the expected behavior
    ...
    expect(calculateApiCost(modelInfo, inputTokens, outputTokens, cacheCreationInputTokens, undefined)).toBe(expectedCost);
  });
