...
describe("Filesystem Utilities", () => {
	...
	it("should exclude .beadsmithrules/workflows directory specifically", async () => {
		// Create a test directory structure
		const beadsmithrulesDirTest = path.join(tmpDir, "beadsmithrules-test")
		const beadsmithrulesDirPath = path.join(beadsmithrulesDirTest, ".beadsmithrules")

		// Create .beadsmithrules directory and root files
		await fs.mkdir(beadsmithrulesDirPath, { recursive: true })
		await fs.writeFile(path.join(beadsmithrulesDirPath, "config.json"), "{}")
		await fs.writeFile(path.join(beadsmithrulesDirPath, "settings.js"), "// settings")

		// Create .beadsmithrules/other directory and files
		const otherDirPath = path.join(beadsmithrulesDirPath, "other")
		...
		// Create .beadsmithrules/workflows directory and files
		const workflowsDirPath = path.join(beadsmithrulesDirPath, "workflows")
		...
		const allFiles = await readDirectory(beadsmithrulesDirPath)
		...
		const filteredFiles = await readDirectory(beadsmithrulesDirPath, [[".beadsmithrules", "workflows"]])
		...
		const multiExcludeFiles = await readDirectory(beadsmithrulesDirPath, [
			[".beadsmithrules", "workflows"],
			[".beadsmithrules", "other"],
		])
	...
	it("should exclude .beadsmithrules/hooks directory specifically", async () => {
		// Create a test directory structure
		const beadsmithrulesDirTest = path.join(tmpDir, "beadsmithrules-hooks-test")
		const beadsmithrulesDirPath = path.join(beadsmithrulesDirTest, ".beadsmithrules")
		...
		// Create .beadsmithrules/hooks directory and files
		const hooksDirPath = path.join(beadsmithrulesDirPath, "hooks")
		...
		const filteredFiles = await readDirectory(beadsmithrulesDirPath, [[".beadsmithrules", "hooks"]])
