...
/**
 * S3/R2 blob storage implementation of BeadsmithStorage.
 * Uses AWS S3 or Cloudflare R2 as the backend storage.
 */
export class BeadsmithBlobStorage extends BeadsmithStorage {
	override name = "BeadsmithBlobStorage"

	private static store: BeadsmithBlobStorage | null = null
	static get instance(): BeadsmithBlobStorage {
		if (!BeadsmithBlobStorage.store) {
			BeadsmithBlobStorage.store = new BeadsmithBlobStorage()
		}
		return BeadsmithBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!BeadsmithBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[BeadsmithBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[BeadsmithBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = BeadsmithBlobStorage.instance
