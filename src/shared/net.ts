...
export function isPublicNvidiaNimBaseUrl(baseUrl: string | undefined): boolean {
	...
	try {
		return new URL(baseUrl.trim()).hostname.toLowerCase() === "integrate.api.nvidia.com"
	} catch {
		return false
	}
}

export function addNvidiaBillingOriginHeader(headers?: Record<string, string>): Record<string, string> {
