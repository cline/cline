...
public setAutoAccept(enabled: boolean, threadId?: number) {
    this.autoAcceptEnabled = enabled;
    this.autoAcceptThreadId = threadId;
}
...
// Communicate with webview
	...
	async handleAskResponse(response: { type: string; askResponse: string }) {
		if (response.type === "askResponse") {
			this.askResponse = response.askResponse as ClineAskResponse;
		}
	}
					...
					const askApproval = async (type: ClineAsk, partialMessage?: string, relPath?: string): Promise<boolean> => {
						...
						if (this.autoAcceptEnabled && (!this.autoAcceptThreadId || this.lastMessageTs === this.autoAcceptThreadId)) {
							...
							if (type === 'tool' && ['read_file', 'list_files', 'list_code_definition_names', 'search_files'].includes(block.name)) {
								return this.alwaysAllowReadOnly;
							} else {
								// For write operations, check for errors but don't ask for approval
								if (type === 'tool' && ['write_to_file', 'execute_command', 'browser_action'].includes(block.name)) {
									//return this.autoAcceptEnabled;
									/*
									// Get current diagnostics for the file being edited
									const diagnostics = vscode.languages.getDiagnostics();
									const hasErrors = diagnostics.some(([uri, diags]) => {
										const currentFilePath = block.params.path ? path.resolve(cwd, block.params.path) : '';
										const diagnosticFilePath = uri.fsPath;
										return arePathsEqual(currentFilePath, diagnosticFilePath) && 
											diags.some(d => d.severity === vscode.DiagnosticSeverity.Error);
									});

									// If there are errors, return false without asking
									if (hasErrors) {
										return false;
									}

									// Check DiffViewProvider's diagnostics
									if (this.diffViewProvider.isEditing) {
										const { newProblemsMessage } = await this.diffViewProvider.saveChanges();
								...
								// For other operations like execute_command or browser_action
							return true;
						}
					}
						...
						case "write_to_file": {
							...
							try {
								if (block.partial) {
									...
									const didApprove = await askApproval("tool", completeMessage, relPath)
									if (!didApprove) {
