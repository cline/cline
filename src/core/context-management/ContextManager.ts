...
export class ContextManager {
    getNewContextMessagesAndMetadata(
        ...
        if (previousApiReqIndex >= 0) {
            const previousRequest = posthogMessages[previousApiReqIndex]
            if (previousRequest && previousRequest.text) {
                const { tokensIn, tokensOut, cacheWrites, cacheReads }: PostHogApiReqInfo = JSON.parse(
    ...
    public getNextTruncationRange(
        ...
        // NOTE: anthropic format messages are always user-assistant-user-assistant, while openai format messages can have multiple user messages in a row (we use anthropic format throughout posthog)
