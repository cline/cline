...
describe("TaskStart Hook", () => {
	...
	describe("Hook Input Format", () => {
		it("should receive task metadata from startTask", async () => {
			const hookPath = path.join(tempDir, ".beadsmithrules", "hooks", "TaskStart")
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
		...
		it("should receive all common hook input fields", async () => {
			...
			const hookScript = `#!/usr/bin/env node
const input = JSON.parse(require('fs').readFileSync(0, 'utf-8'));
const hasAllFields = input.beadsmithVersion && input.hookName === 'TaskStart' && 
...
console.log(JSON.stringify({
  ...
  errorMessage: ""
			...
			const workspaceHookPath = path.join(tempDir, ".beadsmithrules", "hooks", "TaskStart")
