...
+++++++ REPLACE`,
		...
		it("should preserve tool_result blocks when tool_use is converted", () => {
			const input: BeadsmithStorageMessage[] = [
				{
					role: "assistant",
					content: [
						{
...
EOF`,
			...
			// Native tools are FILE_EDIT/FILE_NEW, messages use apply_patch → convert FROM apply_patch
			const result = transformToolCallMessages(input, [BeadsmithDefaultTool.FILE_EDIT, BeadsmithDefaultTool.FILE_NEW])
...
EOF`,
		...
		testCases.forEach((testCase) => {
			it(testCase.name, () => {
				...
				const result = transformToolCallMessages(testCase.input, [BeadsmithDefaultTool.APPLY_PATCH])
		...
		it("should use final_file_content to generate apply_patch for write_to_file", () => {
			...
			// Native tools are APPLY_PATCH, messages use write_to_file/replace_in_file → convert TO apply_patch
			const result = transformToolCallMessages(input, [BeadsmithDefaultTool.APPLY_PATCH])
