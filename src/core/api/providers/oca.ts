...
export class OcaHandler implements ApiHandler {
	...
	protected ocaAccountService: OcaAuthService | undefined
	protected controller: Controller

	constructor(options: OcaHandlerOptions) {
		this.options = options
		this.client = this.initializeClient(options)
		this.ocaAccountService = OcaAuthService.getInstance()
		...
		return new OpenAI({
			...
			fetch: async (url, init) => {
				try {
					...
					const token = await this.ocaAccountService?.getAuthToken(this.controller)
					if (!token) {
						throw new Error("Oracle Code Assist (OCA) access token is not available. Please try logging in again")
					}
