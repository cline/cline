...
export class DifyHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: BeadsmithStorageMessage[]): ApiStream {
		...
		const requestBody = {
			...
			user: "beadsmith-user", // A unique user identifier
			files: [],
		}
	...
	private convertMessagesToQuery(systemPrompt: string, messages: BeadsmithStorageMessage[]): string {
	 ...
	 * @param user User identifier (defaults to "beadsmith-user")
	 * @returns Promise with file upload response
	 */
	async uploadFile(file: Buffer, filename: string, user: string = "beadsmith-user"): Promise<DifyFileResponse> {
	 ...
	 */
	async stopGeneration(taskId: string, user: string = "beadsmith-user"): Promise<void> {
	 ...
	 */
	async deleteConversation(conversationId: string, user: string = "beadsmith-user"): Promise<void> {
