...
describe("AwsBedrockHandler", () => {
	...
	describe("executeConverseStream", () => {
		...
		describe("thinking response handling (new API structure)", () => {
			it("should handle thinking response in additionalModelResponseFields", async () => {
				...
				const originalGetBedrockClient = (handler as any).getBedrockClient
				;(handler as any).getBedrockClient = async () => mockClient as any

				const generator = (handler as any).executeConverseStream(command, mockModelInfo)
				const results = await collectGeneratorResults(generator)

				// Restore original method
				;(handler as any).getBedrockClient = originalGetBedrockClient
	...
	describe("tool config mapping", () => {
		it("should map Anthropic tools to Bedrock toolConfig", () => {
			...
			const toolConfig = (handler as any).mapClineToolsToBedrockToolConfig([
				{
					name: "read_file",
		...
		it("should return undefined when tools is undefined or empty", () => {
			const handler = new AwsBedrockHandler(mockOptions)
			should.not.exist((handler as any).mapClineToolsToBedrockToolConfig(undefined))
			should.not.exist((handler as any).mapClineToolsToBedrockToolConfig([]))
		})
	...
	describe("formatMessagesForConverseAPI", () => {
		it("should format tool_use and tool_result blocks", () => {
			...
			const formatted = (handler as any).formatMessagesForConverseAPI(messages)
	...
	describe("native tool calling integration", () => {
		...
		it("should pass toolConfig to ConverseStreamCommand when tools are provided", async () => {
			...
			const _originalExecuteConverseStream = (handler as any).executeConverseStream.bind(handler)
			// biome-ignore lint/correctness/useYield: Yield nothing — we just want to capture the command
			;(handler as any).executeConverseStream = async function* (command: any, _modelInfo: any) {
			...
			const gen = (handler as any).createAnthropicMessage(
				"system prompt",
				[],
				"test-model",
				handler.getModel(),
		...
		it("should format a complete tool call round-trip correctly", () => {
			...
			const formatted = (handler as any).formatMessagesForConverseAPI(conversation)
