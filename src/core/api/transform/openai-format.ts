 ...
 *
 * @param toolId - The original tool ID from Beadsmith/Anthropic format
...
/**
 * Converts an array of BeadsmithStorageMessage objects to OpenAI's Completions API format.
 *
 * Handles conversion of Beadsmith-specific content types (tool uses, tool results, images, reasoning details)
 * into OpenAI's expected message structure, including tool_calls and tool_call_id fields.
 *
 * @param anthropicMessages - Array of BeadsmithStorageMessage objects to be converted
 * @returns Array of OpenAI.Chat.ChatCompletionMessageParam objects
 */
export function convertToOpenAiMessages(
	anthropicMessages: Omit<BeadsmithStorageMessage, "modelInfo">[],
         ...
         */
			if (anthropicMessage.role === "user") {
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					nonToolMessages: (BeadsmithTextContentBlock | BeadsmithImageContentBlock)[]
					toolMessages: BeadsmithUserToolResultContentBlock[]
				}>(
					(acc, part) => {
				...
				const toolResultImages: BeadsmithImageContentBlock[] = []
				...
				const { nonToolMessages, toolMessages } = anthropicMessage.content.reduce<{
					...
					toolMessages: BeadsmithAssistantToolUseBlock[]
				}>(
					(acc, part) => {
...
const UNIQUE_ERROR_TOOL_NAME = "_beadsmith_error_unknown_function_"
