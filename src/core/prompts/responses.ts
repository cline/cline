...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		const aihydroIgnoreParsed = aihydroIgnoreController
			? sorted.map((filePath) => {
					// path is relative to absolute path, not cwd
					// validateAccess expects either path relative to cwd or absolute path
					// otherwise, for validating against ignore patterns like "assets/icons", we would end up with just "icons", which would result in the path not being ignored.
					const absoluteFilePath = path.resolve(absolutePath, filePath)
					const isIgnored = !aihydroIgnoreController.validateAccess(absoluteFilePath)
					if (isIgnored) {
		...
		if (didHitLimit) {
			return `${aihydroIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		} else if (aihydroIgnoreParsed.length === 0 || (aihydroIgnoreParsed.length === 1 && aihydroIgnoreParsed[0] === "")) {
...
(Remember: If it seems the user wants you to use tools only available in Act Mode, you should ask the user to "toggle to Act mode" (use those words) - they will have to manually do this themselves with the Plan/Act toggle button below. You do not have the ability to switch to Act Mode yourself, and must wait for the user to do it themselves once they are satisfied with the plan. You also cannot present an option to toggle to Act mode, as this will be something you need to direct the user to do manually themselves.)`
	...
	aihydroIgnoreInstructions: (content: string) =>
		`# .aihydroignore\n\n(The following is provided by a root-level .aihydroignore file where the user has specified files and directories that should not be accessed. When using list_files, you'll notice a ${LOCK_TEXT_SYMBOL} next to files that are blocked. Attempting to access the file's contents e.g. through read_file will result in an error.)\n\n${content}\n.aihydroignore`,

	aihydroRulesGlobalDirectoryInstructions: (globalAiHydroRulesFilePath: string, content: string) =>
		`# .aihydrorules/\n\nThe following is provided by a global .aihydrorules/ directory, located at ${globalAiHydroRulesFilePath.toPosix()}, where the user has specified instructions for all working directories:\n\n${content}`,

	aihydroRulesLocalDirectoryInstructions: (cwd: string, content: string) =>
		`# .aihydrorules/\n\nThe following is provided by a root-level .aihydrorules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,

	aihydroRulesLocalFileInstructions: (cwd: string, content: string) =>
		`# .aihydrorules\n\nThe following is provided by a root-level .aihydrorules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${content}`,
