...
${context.enableParallelToolCalling ? "- When multiple operations are independent (for example reading several files or searching in multiple directories), call multiple tools in a single response rather than one at a time. Use sequential tool calls only when later steps depend on earlier results." : ""}
