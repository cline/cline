...
const MISTRAL_TOOL_USE_TEMPLATE = `Begin every task by exploring the codebase (e.g., list_files, search_files, read_file) and outlining the required changes. Do not implement until exploration yields enough context to state objectives, approach, affected files, and risks. Briefly summarize the plan, then proceed with implementation.
...
const MISTRAL_OBJECTIVE_TEMPLATE = `OBJECTIVE
...
const MISTRAL_MCP_TEMPLATE = `MCP SERVERS
...
const MISTRAL_RULES_TEMPLATE = (context: SystemPromptContext) => `RULES
...
export const mistralComponentOverrides = {
	[SystemPromptSection.OBJECTIVE]: {
		template: MISTRAL_OBJECTIVE_TEMPLATE,
	},
	[SystemPromptSection.TOOL_USE]: {
		template: MISTRAL_TOOL_USE_TEMPLATE,
	},
	[SystemPromptSection.RULES]: {
		template: MISTRAL_RULES_TEMPLATE,
	},
	[SystemPromptSection.MCP]: {
		template: MISTRAL_MCP_TEMPLATE,
	},
}
