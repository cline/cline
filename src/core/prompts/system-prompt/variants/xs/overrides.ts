...
const XS_AI_HYDRO_TOOLS = `AI-HYDRO MCP TOOLS
AI-Hydro tools are registered on the **ai-hydro MCP server** — always call them via use_mcp_tool, never via Python imports or execute_command.

Core tools (server: "ai-hydro"):
- delineate_watershed       — NHDPlus watershed delineation from USGS gauge
- fetch_streamflow_data     — USGS NWIS daily discharge
- extract_hydrological_signatures — 15+ flow signatures (BFI, FDC, runoff ratio)
- extract_geomorphic_parameters   — 28 basin morphometry metrics
- compute_twi               — Topographic Wetness Index from 3DEP DEM
- create_cn_grid            — NRCS curve number grid (NLCD + Polaris)
- fetch_forcing_data        — GridMET basin-averaged climate forcing
- fetch_camels_us           — CAMELS-US catchment attributes (671 CONUS gauges)
- train_hydro_model         — Differentiable HBV-light or LSTM calibration
- start_session / sync_research_context / export_session — session management

**Do not** invent tool names or Python module paths not listed above — use list_available_tools to discover what is installed.`
