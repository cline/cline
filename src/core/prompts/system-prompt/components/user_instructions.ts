...
export async function getUserInstructions(variant: PromptVariant, context: SystemPromptContext): Promise<string | undefined> {
	...
	const template = variant.componentOverrides?.user_instructions?.template || USER_CUSTOM_INSTRUCTIONS_TEMPLATE_TEXT
...
const USER_CUSTOM_INSTRUCTIONS_TEMPLATE_TEXT = `====
