...
3. If multiple independent actions are needed, batch them into a single message so they can be executed in parallel. If one action depends on another action's result, use tools sequentially. Do not assume the outcome of any tool use. Each dependent step must be informed by the previous step's result.
