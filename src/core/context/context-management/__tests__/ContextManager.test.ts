...
describe("ContextManager", () => {
	...
	describe("getNextTruncationRange", () => {
		...
		it("sequential truncation with quarter keep", () => {
			...
			expect(secondRange[0][0]).to.equal(2)
			expect(secondRange[0][1]).to.be.greaterThan(firstRange[0][1])
		})
		...
		it("preserves the message structure when truncating", () => {
			...
			const effectiveMessages = [...messages.slice(0, result[0][0]), ...messages.slice(result[0][1] + 1)]
