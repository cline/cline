...
describe("FileContextTracker", () => {
	...
	it("should add a record when a file is read by a tool", async () => {
		...
		expect(fileEntry.beadsmith_read_date).to.be.a("number")
		expect(fileEntry.beadsmith_edit_date).to.be.null
	})

	it("should add a record when a file is edited by Beadsmith", async () => {
		await tracker.trackFileContext(filePath, "beadsmith_edited")
		...
		expect(activeEntry.record_source).to.equal("beadsmith_edited")
		expect(activeEntry.beadsmith_read_date).to.be.a("number")
		expect(activeEntry.beadsmith_edit_date).to.be.a("number")
	})
	...
	it("should mark existing entries as stale when adding a new entry for the same file", async () => {
		...
		mockTaskMetadata.files_in_context = [
			{
				...
				beadsmith_read_date: Date.now() - 1000, // 1 second ago
				beadsmith_edit_date: null,
				user_edit_date: null,
			},
		]
		...
		expect(newEntry.record_source).to.equal("beadsmith_edited")
	})
	...
	it("should not track Beadsmith edits as user edits", async () => {
		// First track the file to set up the watcher
		await tracker.trackFileContext(filePath, "read_tool")

		// Mark the file as edited by Beadsmith
		tracker.markFileAsEditedByBeadsmith(filePath)
