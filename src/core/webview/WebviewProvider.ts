		...
		try {
			...
			if (process.env.IS_DEV) {
				HostProvider.window.showMessage({
					...
					message:
						"AI-Hydro: Local webview dev server is not running, HMR will not work. Please run 'npm run dev:webview' before launching the extension to enable HMR. Using bundled assets.",
				})
			}
