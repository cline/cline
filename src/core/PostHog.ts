...
export class PostHog {
    ...
    private posthogIgnoreController: PostHogIgnoreController
    private askResponse?: PostHogAskResponse
    ...
    private async getSavedPostHogMessages(): Promise<PostHogMessage[]> {
    ...
    private async addToPostHogMessages(message: PostHogMessage) {
        // these values allow us to reconstruct the conversation history at the time this posthog message was created
        // it's important that apiConversationHistory is initialized before we add posthog messages
        message.conversationHistoryIndex = this.apiConversationHistory.length - 1 // NOTE: this is the index of the last added message which is the user message, and once the posthogmessages have been presented we update the apiconversationhistory with the completed assistant message. This means when resetting to a message, we need to +1 this index to get the correct assistant message that this tool use corresponds to
        message.conversationHistoryDeletedRange = this.conversationHistoryDeletedRange
        this.posthogMessages.push(message)
        await this.savePostHogMessages()
    }

    private async overwritePostHogMessages(newMessages: PostHogMessage[]) {
        this.posthogMessages = newMessages
        await this.savePostHogMessages()
    }

    private async savePostHogMessages() {
        try {
            const taskDir = await this.ensureTaskDirectoryExists()
            const filePath = path.join(taskDir, GlobalFileNames.uiMessages)
            await fs.writeFile(filePath, JSON.stringify(this.posthogMessages))
            // combined as they are in ChatView
            const apiMetrics = getApiMetrics(combineApiRequests(combineCommandSequences(this.posthogMessages.slice(1))))
            ...
            console.error('Failed to save posthog messages:', error)
        }
    }

    async restoreCheckpoint(messageTs: number, restoreType: PostHogCheckpointRestore) {
        const messageIndex = this.posthogMessages.findIndex((m) => m.ts === messageTs)
        const message = this.posthogMessages[messageIndex]
        if (!message) {
            console.error('Message not found', this.posthogMessages)
            return
        }
        ...
        if (!didWorkspaceRestoreFail) {
            switch (restoreType) {
                ...
                case 'taskAndWorkspace':
                    ...
                    const deletedMessages = this.posthogMessages.slice(messageIndex + 1)
                    const deletedApiReqsMetrics = getApiMetrics(
                        combineApiRequests(combineCommandSequences(deletedMessages))
                    )

                    const newPostHogMessages = this.posthogMessages.slice(0, messageIndex + 1)
                    await this.overwritePostHogMessages(newPostHogMessages) // calls savePostHogMessages which saves historyItem
            ...
            if (restoreType !== 'task') {
                ...
                const checkpointMessages = this.posthogMessages.filter((m) => m.say === 'checkpoint_created')
        ...
        try {
            if (seeNewChangesSinceLastTaskCompletion) {
                ...
                const lastTaskCompletedMessageCheckpointHash = findLast(
                    this.posthogMessages.slice(0, messageIndex),
                ...
                const firstCheckpointMessageCheckpointHash = this.posthogMessages.find(
    ...
    async doesLatestTaskCompletionHaveNewChanges() {
        const messageIndex = findLastIndex(this.posthogMessages, (m) => m.say === 'completion_result')
        const message = this.posthogMessages[messageIndex]
        if (!message) {
    ...
    async ask(
        ...
        response: PostHogAskResponse
        text?: string
        images?: string[]
    }> {
        // If this PostHog instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of PostHog now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set PostHog = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
        if (this.abort) {
        ...
        if (partial !== undefined) {
            ...
            if (partial) {
                if (isUpdatingPreviousPartial) {
                    ...
                    await this.addToPostHogMessages({
                        ts: askTs,
                        type: 'ask',
                        ask: type,
                        text,
    ...
    async handleWebviewAskResponse(askResponse: PostHogAskResponse, text?: string, images?: string[]) {
        this.askResponse = askResponse
        this.askResponseText = text
        this.askResponseImages = images
    }

    async say(type: PostHogSay, text?: string, images?: string[], partial?: boolean): Promise<undefined> {
        if (this.abort) {
            throw new Error('PostHog instance aborted')
        }
    ...
    async removeLastPartialMessageIfExistsWithType(type: 'ask' | 'say', askOrSay: PostHogAsk | PostHogSay) {
        const lastMessage = this.posthogMessages.at(-1)
        if (
            lastMessage?.partial &&
    ...
    private async startTask(task?: string, images?: string[]): Promise<void> {
        // conversationHistory (for API) and posthogMessages (for webview) need to be in sync
        // if the extension process were killed, then on restart the posthogMessages might not be empty, so we need to set it to [] when we create a new PostHog client (otherwise webview would show stale messages from previous session)
        this.posthogMessages = []
        this.apiConversationHistory = []
    ...
    private async resumeTaskFromHistory() {
        ...
        const modifiedPostHogMessages = await this.getSavedPostHogMessages()
        ...
        await this.overwritePostHogMessages(modifiedPostHogMessages)
        this.posthogMessages = await this.getSavedPostHogMessages()

        // Now present the posthog messages to the user and ask if they want to resume (NOTE: we ran into a bug before where the apiconversationhistory wouldnt be initialized when opening a old task, and it was because we were waiting for resume)
        // This is important in case the user deletes messages without resuming the task first
        this.apiConversationHistory = await this.getSavedApiConversationHistory()

        const lastPostHogMessage = this.posthogMessages
            .slice()
            .reverse()
            .find((m) => !(m.ask === 'resume_task' || m.ask === 'resume_completed_task')) // could be multiple resume tasks
        // const lastPostHogMessage = this.posthogMessages[lastPostHogMessageIndex]
        // could be a completion result with a command
        // const secondLastPostHogMessage = this.posthogMessages
        // 	.slice()
        // 	.reverse()
        // 	.find(
        // 		(m, index) =>
        // 			index !== lastPostHogMessageIndex && !(m.ask === "resume_task" || m.ask === "resume_completed_task")
        // 	)
        // (lastPostHogMessage?.ask === "command" && secondLastPostHogMessage?.ask === "completion_result")

        let askType: PostHogAsk
        if (lastPostHogMessage?.ask === 'completion_result') {
        ...
        // need to make sure that the api conversation history can be resumed by the api, even if it goes out of sync with posthog messages
        ...
        const agoText = (() => {
            const timestamp = lastPostHogMessage?.ts ?? Date.now()
            const now = Date.now()
        ...
        const wasRecent = lastPostHogMessage?.ts && Date.now() - lastPostHogMessage.ts < 30_000
    ...
    private async initiateTaskLoop(userContent: UserContent, isNewTask: boolean): Promise<void> {
        ...
        while (!this.abort) {
            const didEndLoop = await this.recursivelyMakePostHogRequests(nextUserContent, includeFileDetails, isNewTask)
            ...
            if (didEndLoop) {
                ...
                // this.say(
                // 	"tool",
                // 	"PostHog responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
                // )
                nextUserContent = [
                    {
                        type: 'text',
    ...
    async saveCheckpoint(isAttemptCompletionMessage: boolean = false) {
        ...
        this.posthogMessages.forEach((message) => {
        ...
        if (!isAttemptCompletionMessage) {
            ...
            this.checkpointTracker?.commit().then(async (commitHash) => {
                const lastCheckpointMessage = findLast(this.posthogMessages, (m) => m.say === 'checkpoint_created')
        ...
        // for (let i = this.posthogMessages.length - 1; i >= 0; i--) {
        // 	const message = this.posthogMessages[i]
    ...
    async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
        ...
        const disableBrowserTool =
            vscode.workspace.getConfiguration('posthog').get<boolean>('disableBrowserTool') ?? false
        ...
        const preferredLanguageInstructions =
            preferredLanguage && preferredLanguage !== DEFAULT_LANGUAGE_SETTINGS
                ...
                : ''
        const posthogRulesFilePath = path.resolve(cwd, GlobalFileNames.posthogRules)
        let posthogRulesFileInstructions: string | undefined
        if (await fileExistsAtPath(posthogRulesFilePath)) {
            if (await isDirectory(posthogRulesFilePath)) {
                try {
                    // Read all files in the .posthogrules/ directory.
                    const ruleFiles = await fs
                        .readdir(posthogRulesFilePath, { withFileTypes: true, recursive: true })
                        .then((files) => files.filter((file) => file.isFile()))
                        .then((files) => files.map((file) => path.resolve(file.parentPath, file.name)))
                    const ruleFileContent = await Promise.all(
                        ruleFiles.map(async (file) => {
                            const ruleFilePath = path.resolve(posthogRulesFilePath, file)
                            const ruleFilePathRelative = path.relative(cwd, ruleFilePath)
                            return `${ruleFilePathRelative}\n` + (await fs.readFile(ruleFilePath, 'utf8')).trim()
                        })
                    ).then((contents) => contents.join('\n\n'))
                    posthogRulesFileInstructions = `# .posthogrules/\n\nThe following is provided by a root-level .posthogrules/ directory where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${ruleFileContent}`
                } catch {
                    console.error(`Failed to read .posthogrules directory at ${posthogRulesFilePath}`)
                }
            } else {
                try {
                    const ruleFileContent = (await fs.readFile(posthogRulesFilePath, 'utf8')).trim()
                    if (ruleFileContent) {
                        posthogRulesFileInstructions = `# .posthogrules\n\nThe following is provided by a root-level .posthogrules file where the user has specified instructions for this working directory (${cwd.toPosix()})\n\n${ruleFileContent}`
                    }
                } catch {
                    console.error(`Failed to read .posthogrules file at ${posthogRulesFilePath}`)
                }
            }
        }
    ...
    async presentAssistantMessage() {
        ...
        switch (block.type) {
            ...
            case 'tool_use':
                ...
                const askApproval = async (type: PostHogAsk, partialMessage?: string) => {
                ...
                switch (block.name) {
                    ...
                    case 'replace_in_file': {
                        ...
                        if (!accessAllowed) {
                            await this.say('posthogignore_error', relPath)
                            pushToolResult(formatResponse.toolError(formatResponse.posthogIgnoreError(relPath)))
                        ...
                        try {
                            ...
                            if (diff) {
                                ...
                                // open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but PostHog throws error
                            ...
                            const sharedMessageProps: PostHogSayTool = {
                            ...
                            if (block.partial) {
                                if (action === 'launch') {
                                    ...
                                    await this.say(
                                        ...
                                        } satisfies PostHogSayBrowserAction),
                                        undefined,
                                        block.partial
                                    )
                                }
                    ...
                    case 'use_mcp_tool': {
                        ...
                        try {
                            if (block.partial) {
                                ...
                                } satisfies PostHogAskUseMcpServer)
                                ...
                                if (mcp_arguments) {
                                    try {
                                        ...
                                        await this.say(
                                            'error',
                                            `PostHog tried to use ${tool_name} with an invalid JSON argument. Retrying...`
                                        )
                                        pushToolResult(
                                            formatResponse.toolError(
                    ...
                    case 'plan_mode_respond': {
                        ...
                        const sharedMessage = {
                            ...
                            options: parsePartialArrayString(removeClosingTag('options', optionsRaw)),
                        } satisfies PostHogPlanModeResponse
                        try {
                            if (block.partial) {
                        ...
                        try {
                            ...
                            if (block.partial) {
                                if (command) {
                                    ...
                                    // const secondLastMessage = this.posthogMessages.at(-2)
    ...
    async recursivelyMakePostHogRequests(
        ...
        if (this.consecutiveMistakeCount >= 3) {
            ...
            const { response, text, images } = await this.ask(
                ...
                this.api.getModel().id.includes('claude')
                    ...
                    : "PostHog uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities."
            )
            if (response === 'messageResponse') {
                userContent.push(
                    ...[
        ...
        if (
            ...
            if (this.autoApprovalSettings.enableNotifications) {
                showSystemNotification({
                    ...
                    message: `PostHog has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
                })
            }
            await this.ask(
                'auto_approval_max_req_reached',
                `PostHog has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`
            )
        ...
        const previousApiReqIndex = findLastIndex(this.posthogMessages, (m) => m.say === 'api_req_started')

        // Save checkpoint if this is the first API request
        const isFirstRequest = this.posthogMessages.filter((m) => m.say === 'api_req_started').length === 0
        if (isFirstRequest) {
            ...
            try {
                this.checkpointTracker = await pTimeout(
                    CheckpointTracker.create(this.taskId, this.providerRef.deref()?.context.globalStorageUri.fsPath),
                    {
                        milliseconds: 15_000,
                        message:
                            'Checkpoints taking too long to initialize. Consider re-opening PostHog in a project that uses git, or disabling checkpoints.',
                    }
                )
            } catch (error) {
                const errorMessage = error instanceof Error ? error.message : 'Unknown error'
                console.error('Failed to initialize checkpoint tracker:', errorMessage)
                this.checkpointTrackerErrorMessage = errorMessage // will be displayed right away since we savePostHogMessages next which posts state to webview
            }
        }
        ...
        try {
            ...
            if (assistantMessage.length > 0) {
                ...
                const recDidEndLoop = await this.recursivelyMakePostHogRequests(this.userMessageContent)
    ...
    async getEnvironmentDetails(includeFileDetails: boolean = false) {
        ...
        // It could be useful for posthog to know if the user went from one or no file to another between messages, so we always include this context
        ...
        // Filter paths through posthogIgnoreController
        const allowedVisibleFiles = this.posthogIgnoreController
        ...
        // Filter paths through posthogIgnoreController
        const allowedOpenTabs = this.posthogIgnoreController
        ...
        const diagnostics = await this.diagnosticsMonitor.getCurrentDiagnostics(this.didEditFile || terminalWasBusy) // if posthog ran a command (ie npm install) or edited the workspace then wait a bit for updated diagnostics
        ...
        */
        ...
        if (includeFileDetails) {
            ...
            if (isDesktop) {
                ...
                const result = formatResponse.formatFilesList(cwd, files, didHitLimit, this.posthogIgnoreController)
                details += result
            }
        }
