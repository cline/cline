const descriptionForAgent = (cwd: string) => `Use this tool to propose an edit to an existing file or create a new file.
...
Usage:
- The target_file parameter must be a relative path to the current working directory: ${cwd.toPosix()}
- This tool uses Morph's fast apply API to modify files efficiently`

export const fastApplyEditToolDefinition = (cwd: string) => ({
	name: "edit_file",
