 ...
 * Designed to be instantiated once in Beadsmith.ts and passed to file manipulation services.
 * Uses the 'ignore' library to support standard .gitignore syntax in .beadsmithignore files.
 */
export class BeadsmithIgnoreController {
	private cwd: string
	private ignoreInstance: Ignore
	private fileWatcher?: FSWatcher
	beadsmithIgnoreContent: string | undefined

	constructor(cwd: string) {
		this.cwd = cwd
		this.ignoreInstance = ignore()
		this.beadsmithIgnoreContent = undefined
	}
	...
	async initialize(): Promise<void> {
		// Set up file watcher for .beadsmithignore
		this.setupFileWatcher()
		await this.loadBeadsmithIgnore()
	}

	/**
	 * Set up the file watcher for .beadsmithignore changes
	 */
	private setupFileWatcher(): void {
		const ignorePath = path.join(this.cwd, ".beadsmithignore")
		...
		this.fileWatcher.on("error", (error) => {
			Logger.error("Error watching .beadsmithignore file:", error)
		})
	}

	/**
	 * Load custom patterns from .beadsmithignore if it exists.
	 * Supports "!include <filename>" to load additional ignore patterns from other files.
	 */
	private async loadBeadsmithIgnore(): Promise<void> {
		try {
			...
			if (await fileExistsAtPath(ignorePath)) {
				...
				this.beadsmithIgnoreContent = content
				await this.processIgnoreContent(content)
				this.ignoreInstance.add(".beadsmithignore")
			} else {
				this.beadsmithIgnoreContent = undefined
			}
		} catch (error) {
			// Should never happen: reading file failed even though it exists
			Logger.error("Unexpected error loading .beadsmithignore:", error)
		}
	}
	...
	private async processIgnoreContent(content: string): Promise<void> {
		...
		const combinedContent = await this.processBeadsmithIgnoreIncludes(content)
	 ...
	 */
	private async processBeadsmithIgnoreIncludes(content: string): Promise<string> {
		let combinedContent = ""
	...
	private async readIncludedFile(includeLine: string): Promise<string | null> {
		...
		if (!(await fileExistsAtPath(resolvedIncludePath))) {
			Logger.debug(`[BeadsmithIgnore] Included file not found: ${resolvedIncludePath}`)
			return null
		}
	...
	validateAccess(filePath: string): boolean {
		// Always allow access if .beadsmithignore does not exist
		if (!this.beadsmithIgnoreContent) {
			return true
		}
		try {
	...
	validateCommand(command: string): string | undefined {
		// Always allow if no .beadsmithignore exists
		if (!this.beadsmithIgnoreContent) {
			return undefined
		}
