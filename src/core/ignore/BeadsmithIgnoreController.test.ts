...
import { BeadsmithIgnoreController } from "./BeadsmithIgnoreController"
import "should"

describe("BeadsmithIgnoreController", () => {
	let tempDir: string
	let controller: BeadsmithIgnoreController
	...
	beforeEach(async () => {
		...
		// Create default .beadsmithignore file
		await fs.writeFile(
			path.join(tempDir, ".beadsmithignore"),
			[".env", "*.secret", "private/", "# This is a comment", "", "temp.*", "file-with-space-at-end.* ", "**/.git/**"].join(
				"\n",
			),
		)

		controller = new BeadsmithIgnoreController(tempDir)
	...
	describe("Default Patterns", () => {
		...
		it("should block access to .beadsmithignore file", async () => {
			const result = controller.validateAccess(".beadsmithignore")
	...
	describe("Custom Patterns", () => {
		...
		it("should handle comments in .beadsmithignore", async () => {
			// Create a new .beadsmithignore with comments
			await fs.writeFile(
				path.join(tempDir, ".beadsmithignore"),
	...
	describe("Error Handling", () => {
		...
		it("should handle missing .beadsmithignore gracefully", async () => {
			// Create a new controller in a directory without .beadsmithignore
			const emptyDir = path.join(os.tmpdir(), `llm-test-empty-${Date.now()}`)
			await fs.mkdir(emptyDir)

			try {
				const controller = new BeadsmithIgnoreController(emptyDir)
				await controller.initialize()
		...
		it("should handle empty .beadsmithignore", async () => {
			await fs.writeFile(path.join(tempDir, ".beadsmithignore"), "")
	...
	describe("Include Directive", () => {
		it("should load patterns from an included file", async () => {
			...
			// Create a .beadsmithignore file that includes .gitignore and adds an extra pattern "secret.txt"
			await fs.writeFile(path.join(tempDir, ".beadsmithignore"), ["!include .gitignore", "secret.txt"].join("\n"))

			// Initialize the controller to load the updated .beadsmithignore
			controller = new BeadsmithIgnoreController(tempDir)
			await controller.initialize()
			...
			// "secret.txt" should be ignored as specified directly in .beadsmithignore
		...
		it("should handle non-existent included file gracefully", async () => {
			// Create a .beadsmithignore file that includes a non-existent file
			await fs.writeFile(path.join(tempDir, ".beadsmithignore"), ["!include missing-file.txt"].join("\n"))
		...
		it("should handle non-existent included file gracefully alongside a valid pattern", async () => {
			...
			await fs.writeFile(path.join(tempDir, ".beadsmithignore"), ["!include non-existent.txt", "*.tmp"].join("\n"))
