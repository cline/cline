...
export async function refreshGroqModels(controller: Controller, request: EmptyRequest): Promise<OpenRouterCompatibleModelInfo> {
	...
	try {
		if (!groqApiKey) {
			throw new Error("Groq API key is required to refresh models")
		}
		...
		if (response.data?.data) {
			...
			for (const rawModel of rawModels) {
				...
				const modelInfo: Partial<OpenRouterModelInfo> = {
					...
					inputPrice: staticModelInfo?.inputPrice || estimateInputPrice(rawModel),
					outputPrice: staticModelInfo?.outputPrice || estimateOutputPrice(rawModel),
...
/**
 * Estimates input price based on model characteristics
 */
function estimateInputPrice(rawModel: any): number {
	const modelId = rawModel.id.toLowerCase()

	// Estimate based on model size/type
	if (modelId.includes("70b") || modelId.includes("32b")) {
		return 0.59
		...
		// New compound models - estimate based on similar complexity
		return 0.3
	} else {
...
/**
 * Estimates output price based on model characteristics
 */
function estimateOutputPrice(rawModel: any): number {
	const modelId = rawModel.id.toLowerCase()
