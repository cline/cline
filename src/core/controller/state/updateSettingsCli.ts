...
export async function updateSettingsCli(controller: Controller, request: UpdateSettingsRequestCli): Promise<Empty> {
	...
	try {
		...
		if (request.settings) {
			...
			if (mode !== undefined) {
				controller.stateManager.setGlobalState("mode", convertPlanActMode(mode))
			}
			...
			if (planModeApiProvider !== undefined) {
				controller.stateManager.setGlobalState("planModeApiProvider", convertProtoToApiProvider(planModeApiProvider))
			}

			if (actModeApiProvider !== undefined) {
				controller.stateManager.setGlobalState("actModeApiProvider", convertProtoToApiProvider(actModeApiProvider))
			}
