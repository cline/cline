...
export async function resetState(controller: Controller, request: ResetStateRequest): Promise<Empty> {
	try {
		if (request.global) {
			await showInformationMessage("Resetting global state...")
			await resetGlobalState(controller.context)
		} else {
			await showInformationMessage("Resetting workspace state...")
		...
		await showInformationMessage("State reset")
		...
		await showErrorMessage(`Failed to reset state: ${error instanceof Error ? error.message : String(error)}`)
		throw error
	}
}
