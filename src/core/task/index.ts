		...
		if (this.taskState.consecutiveMistakeCount >= this.stateManager.getGlobalSettingsKey("maxConsecutiveMistakes")) {
			...
			const { response, text, images, files } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Cline uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 5 Sonnet for its advanced agentic coding capabilities.",
			)
