...
export class Task {
	...
	/**
	 * Backwards-compat accessor used by unit tests that call Task.prototype.say with a POJO.
	 * In production, use Task#messagePayloadFingerprint.
	 */
	private getMessagePayloadFingerprint(text?: string, images?: string[], files?: string[]): string {
		return this.messagePayloadFingerprint(text, images, files)
	}
	...
	constructor(params: TaskParams) {
		...
		// Reasoning effort is stored per-mode as part of ApiHandler option settings.
		// Keep these values in sync with the current mode before building the handler.
		if (currentProvider === "openai" || currentProvider === "openai-native" || currentProvider === "sapaicore") {
			if (mode === "plan") {
				effectiveApiConfiguration.planModeReasoningEffort = apiConfiguration.planModeReasoningEffort
			} else {
				effectiveApiConfiguration.actModeReasoningEffort = apiConfiguration.actModeReasoningEffort
			}
		}
	...
	async say(
		...
		if (partial !== undefined) {
			...
			if (
				...
				this.getMessagePayloadFingerprint(text, images, files) ===
					this.getMessagePayloadFingerprint(lastMessage.text, lastMessage.images, lastMessage.files)
			) {
				return undefined
			}
			...
			if (partial) {
				if (isUpdatingPreviousPartial) {
					...
					const isDuplicate =
						this.getMessagePayloadFingerprint(text, images, files) ===
						this.getMessagePayloadFingerprint(previousMessage.text, previousMessage.images, previousMessage.files)
					if (isDuplicate) {
						return undefined
					}
