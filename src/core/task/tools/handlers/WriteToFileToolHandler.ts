...
import { recordFileCreated, recordFileModified } from "../utils/BeadIntegrationUtils"
import { applyModelContentFixes } from "../utils/ModelContentProcessor"
import { ToolDisplayUtils } from "../utils/ToolDisplayUtils"
import { ToolResultUtils } from "../utils/ToolResultUtils"

export class WriteToFileToolHandler implements IFullyManagedTool {
	readonly name = BeadsmithDefaultTool.FILE_NEW // This handler supports write_to_file, replace_in_file, and new_rule
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		try {
			...
			if (await config.callbacks.shouldAutoApproveToolWithPath(block.name, relPath)) {
				...
				const notificationMessage = `Beadsmith wants to ${fileExists ? "edit" : "create"} ${getWorkspaceBasename(relPath, "WriteToFile.notification")}`
			...
			// Mark the file as edited by Beadsmith
			config.services.fileContextTracker.markFileAsEditedByBeadsmith(relPath)
			...
			await config.services.fileContextTracker.trackFileContext(relPath, "beadsmith_edited")

			// Record file change for bead tracking (if bead mode is active)
			if (fileExists) {
				recordFileModified(config, relPath, { diff })
			} else {
				recordFileCreated(config, relPath)
			}
	...
	async validateAndPrepareFileOperation(config: TaskConfig, block: ToolUse, relPath: string, diff?: string, content?: string) {
		...
		// Check beadsmithignore access first
		...
		if (!accessValidation.ok) {
			...
			const errorResponse = formatResponse.toolError(formatResponse.beadsmithIgnoreError(resolvedPath))
		...
		if (diff) {
			...
			// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but Beadsmith throws error
