...
import { BeadsmithPlanModeResponse } from "@/shared/ExtensionMessage"
...
export class PlanModeRespondHandler implements IToolHandler, IPartialBlockHandler {
	readonly name = BeadsmithDefaultTool.PLAN_MODE
	...
	async handlePartialBlock(block: ToolUse, uiHelpers: StronglyTypedUIHelpers): Promise<void> {
		...
		} satisfies BeadsmithPlanModeResponse
	...
	async execute(config: TaskConfig, block: ToolUse): Promise<ToolResponse> {
		...
		if (config.mode === "plan" && config.yoloModeToggled && !needsMoreExploration) {
			...
			if (switchSuccessful) {
				...
				const lastPlanMessage = findLast(config.messageState.getBeadsmithMessages(), (m: any) => m.ask === this.name)
				if (lastPlanMessage) {
