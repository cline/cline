 ...
 * @param modelId The model ID to check if fixes are needed
...
export function applyModelContentFixes(text: string, modelId: string, filePath?: string): string {
	...
	const usesEscapedCharacters = filePath && ESCAPED_CHARACTER_EXTENSIONS.some((ext) => filePath.toLowerCase().endsWith(ext))
