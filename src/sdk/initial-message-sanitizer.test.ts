...
describe("sanitizeInitialMessagesForSessionStart", () => {
	...
	it("consolidates parallel tool results spread across separate user messages (ENG-1885)", () => {
		...
		const input = [
			{ role: "user", content: [{ type: "text", text: "list /usr" }] },
			{
				...
				content: [
					...
					{ type: "tool_use", id: "toolu_A", name: "execute_command", input: { command: "ls /usr/bin" } },
					{ type: "tool_use", id: "toolu_B", name: "execute_command", input: { command: "ls /usr/lib" } },
					{ type: "tool_use", id: "toolu_C", name: "execute_command", input: { command: "ls /usr/share" } },
				],
			},
			// SDK persists each tool result as a separate user message
			{ role: "user", content: [{ type: "tool_result", tool_use_id: "toolu_A", content: "bin contents" }] },
			{ role: "user", content: [{ type: "tool_result", tool_use_id: "toolu_B", content: "lib contents" }] },
			{ role: "user", content: [{ type: "tool_result", tool_use_id: "toolu_C", content: "share contents" }] },
		...
		const toolResultMsg = result[2] as { content: Array<Record<string, unknown>> }
		expect(toolResultMsg.content).toHaveLength(3)
		expect(toolResultMsg.content[0]).toMatchObject({ type: "tool_result", tool_use_id: "toolu_A", content: "bin contents" })
		expect(toolResultMsg.content[1]).toMatchObject({ type: "tool_result", tool_use_id: "toolu_B", content: "lib contents" })
		expect(toolResultMsg.content[2]).toMatchObject({ type: "tool_result", tool_use_id: "toolu_C", content: "share contents" })

		// Verify no duplicates: count tool_result blocks across all messages
		const allToolResultIds = (result as Array<{ role: string; content: unknown }>).flatMap((msg) => {
