...
export class OpenAIHandler implements ApiHandler {
	...
	async createMessage(
		...
		const createParams: OpenAI.Chat.Completions.ChatCompletionCreateParamsNonStreaming = {
			model: this.options.apiModelId || this.getModel().id,
		...
		const errorMessage = (completion as any).error?.message // openAI returns an error object instead of the openai sdk throwing an error
		if (errorMessage) {
	...
	/*
	Streaming the completion is a fallback behavior for when a normal request responds with an invalid JSON object ("Unexpected end of JSON input"). This would usually happen in cases where the model makes tool calls with large arguments. After talking with OpenAI folks, streaming mitigates this issue for now until they fix the underlying problem ("some weird data from anthropic got decoded wrongly and crashed the buffer")
	*/
	async streamCompletion(
		...
		const completion: OpenAI.Chat.Completions.ChatCompletion = {
			...
			id: `openAI-${Date.now()}-${Math.random().toString(36).slice(2, 11)}`, // this ID won't be traceable back to OpenAI's systems if you need to debug issues
			choices: [
				{
					message: {
						role: "assistant",
