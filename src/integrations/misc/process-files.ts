		...
		if (isImage) {
			...
			try {
				...
				if (dimensions.width! > 7500 || dimensions.height! > 7500) {
					...
					await showErrorMessage(`Image too large: ${path.basename(filePath)} was skipped (dimensions exceed 7500px).`)
					return null
				}
			} catch (error) {
				console.error(`Error reading file or getting dimensions for ${filePath}:`, error)
				await showErrorMessage(`Could not read dimensions for ${path.basename(filePath)}, skipping.`)
				return null
			}
			...
			try {
				...
				if (stats.size > 20 * 1000 * 1024) {
					...
					await showErrorMessage(`File too large: ${path.basename(filePath)} was skipped (size exceeds 20MB).`)
					return null
				}
			} catch (error) {
				console.error(`Error checking file size for ${filePath}:`, error)
				await showErrorMessage(`Could not check file size for ${path.basename(filePath)}, skipping.`)
				return null
			}
