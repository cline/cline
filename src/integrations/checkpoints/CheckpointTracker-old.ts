...
class CheckpointTracker {
    ...
    private constructor(provider: PostHogProvider, taskId: string, cwd: string) {
        this.providerRef = new WeakRef(provider)
        this.taskId = taskId
        this.cwd = cwd
    }

    public static async create(taskId: string, provider?: PostHogProvider): Promise<CheckpointTracker | undefined> {
        try {
            if (!provider) {
            ...
            const enableCheckpoints =
                vscode.workspace.getConfiguration('posthog').get<boolean>('enableCheckpoints') ?? true
    ...
    private static async getWorkingDirectory(): Promise<string> {
        ...
        if (!cwd) {
            throw new Error('No workspace detected. Please open PostHog in a workspace to use checkpoints.')
        }
    ...
    public static async doesShadowGitExist(taskId: string, provider?: PostHogProvider): Promise<boolean> {
    ...
    public async initShadowGit(): Promise<string> {
        ...
        if (await fileExistsAtPath(gitPath)) {
            ...
            await git.addConfig('user.name', 'PostHog Checkpoint')
