...
export const DIFF_VIEW_URI_SCHEME = 'posthog-diff'
...
export class DiffViewProvider {
    ...
    async open(relPath: string): Promise<void> {
        ...
        // get diagnostics before editing the file, we'll compare to diagnostics after editing to see if posthog needs to fix anything
    ...
    async saveChanges(): Promise<{
		...
		Since these are new problems resulting from PostHog's edit, we know they're
		directly related to the work he's doing. This eliminates the risk of PostHog
		going off-task or getting distracted by unrelated issues, which was a problem
		with the previous auto-debug approach. Some users' machines may be slow to
		update diagnostics, so this approach provides a good balance between automation
		and avoiding potential issues where PostHog might get stuck in loops due to
		outdated problem information. If no new problems show up by the time the user
		accepts the changes, they can always debug later using the '@problems' mention.
		This way, PostHog only becomes aware of new problems resulting from his edits
		and can address them accordingly. If problems don't change immediately after
		applying a fix, PostHog won't be notified, which is generally fine since the
    ...
    private async openDiffEditor(): Promise<vscode.TextEditor> {
        ...
        return new Promise<vscode.TextEditor>((resolve, reject) => {
            ...
            vscode.commands.executeCommand(
                ...
                uri,
                `${fileName}: ${fileExists ? "Original ↔ PostHog's Changes" : 'New File'} (Editable)`
            )
