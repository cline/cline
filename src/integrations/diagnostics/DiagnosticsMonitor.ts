...
- this works great for us since when posthog edits/creates files its through vscode's textedit api's and we get those diagnostics for free
...
class DiagnosticsMonitor {
	...
	public async getCurrentDiagnostics(shouldWaitForChanges: boolean): Promise<FileDiagnostics> {
		...
		// if diagnostics contain existing errors (since the check above didn't trigger) then it's likely posthog just did something that should have fixed the error, so we'll give a longer grace period for diagnostics to catch up
