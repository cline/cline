...
export class VscodeTerminalProcess extends EventEmitter<TerminalProcessEvents> implements ITerminalProcess {
	...
	async run(terminal: vscode.Terminal, command: string) {
		...
		if (terminal.shellIntegration && terminal.shellIntegration.executeCommand) {
			...
			let isOfficiallyCompleted = false
			const endEventDisposable = (vscode.window as any).onDidEndTerminalShellExecution?.((event: any) => {
				if (event.execution === execution) {
					this.exitCode = event.exitCode
					isOfficiallyCompleted = true
				}
			})
			...
			try {
				...
				if (error?.message === "TERMINAL_IDLE_TIMEOUT") {
					// We hit the idle timeout, let's treat it as if the stream finished
					Logger.warn("[VscodeTerminalProcess] Breaking out of stream read loop due to idle timeout")
				} else {
					throw error
				}
			} finally {
