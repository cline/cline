...
async function main() {
	log("\n\n\nStarting beadsmith-core service...\n\n\n")
...
function setupHostProvider(extensionContext: any, extensionDir: string, dataDir: string) {
	...
	// beadsmith-core expects the binaries to be unpacked in the directory where it is running.
...
/**
 * Gracefully shutdown the beadsmith-core process by:
...
function showHelp() {
	Logger.log(`
Beadsmith Core - Standalone Server

Usage: node beadsmith-core.js [options]
...
Options:
  ...
  -c, --config <path>            Directory for Beadsmith data storage (default: ~/.beadsmith)
